---
description: Виконати наступний невиконаний таск з ініціативи у docs/work/specs/initiatives/
argument-hint: "[NNNN-slug | NNNN | --list]"
---

You are executing **one atomic task** from a Sergeant initiative in `docs/work/specs/initiatives/`. Initiatives are multi-PR program-of-work plans; each phase has a table of PR rows + a `## Критерії DONE` checklist. Your job: pick the next undone unit, route it to the right specialist skill + playbook, and ship it through the canonical CI gates.
...
1. Discover active initiatives via `glob docs/work/specs/initiatives/[0-9]*.md` (filter out `_`-prefixed completed files) + `glob docs/work/specs/initiatives/*/` for directory-form series like `stack-pulse-2026-05/`. Cross-check against the active-initiatives table in `docs/work/specs/initiatives/README.md`; warn if drift exists between file system and README.
...
1. **Resolve file.** Glob `docs/work/specs/initiatives/{_,}NNNN-*.md`. If 0 hits → report missing and offer `--list`. If multiple (active + archived) → prefer active (no `_` prefix).
2. **Parse the file.**
...
3. **Route to specialist skill** by inspecting the `Файли` column:
   ...
   - `.github/workflows/**` (non-docs), `tools/**` (non-openclaw), `packages/config/**`, root `eslint.config.js` / `pnpm-workspace.yaml` → no specialist exists; load `sergeant-start-here` + relevant playbook from `docs/start/instructions/` (e.g. `fix-failing-ci.md`). Note the catalog gap in the end-of-turn summary.
   - Multiple surfaces → load `sergeant-feature-delivery` as the umbrella, then the dominant surface skill.
4. **Match playbook.** Cross-reference the PR's intent against `docs/start/instructions/INDEX.md`. If a playbook fits the PR title (e.g. "add migration" → `add-sql-migration.md`, "add endpoint" → `add-api-endpoint.md`, "add hard rule" → `add-hard-rule.md`), load it as the canonical recipe.
...
2. **If this PR closes the whole phase** (all criteria for that phase now `- [x]`):
   ...
   - If this was the last phase and the whole initiative is done, update `> **Status:**` header to `Done`, add an `## Outcome` section, and follow the rename protocol in `docs/work/specs/initiatives/README.md` step 4 (`git mv NNNN-... _NNNN-...`, update all `.md` links, regenerate `follow-ups.md`).
3. **Sync README.md** row in `docs/work/specs/initiatives/README.md` if status text changed.
...
- Не дублюй контент з initiative-файлу — посилайся (`docs/work/specs/initiatives/NNNN-slug.md § Phase 1`).
