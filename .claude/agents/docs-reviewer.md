...
The whitelist is exactly four globs (canonical source: [`rules/26-pr-ledger-update-on-merge.md § Scope`](../../docs/governance/governance/rules/26-pr-ledger-update-on-merge.md), enforced by `scripts/ci/update-pr-backlinks.mjs`):

- `docs/governance/adr/*.md` (excl. `TEMPLATE.md`, `README.md`)
- `docs/work/specs/initiatives/*.md` (excl. `archive/`, `follow-ups.md`, `README.md`)
- `docs/start/instructions/*.md` (excl. `INDEX.md`, `README.md`, `_TEMPLATE-*`)
- `docs/governance/governance/rules/*.md` (excl. `README.md`)

If a diff touches one of those but `docs/governance/pr-ledger/index.json` is absent → **BLOCKER**. (Local check: `pnpm docs:check-pr-ledger`.)

> ⚠️ **Do NOT raise this BLOCKER for `docs/engineering/architecture/**`, `docs/work/specs/audits/**`, `AGENTS.md`, or `CLAUDE.md`** — the rule excludes them deliberately (audits are snapshot-natured; architecture is covered by drift-detectors). Flagging them is a false BLOCKER that stalls a correct PR.
