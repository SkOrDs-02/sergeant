...
- **Заміряний скоуп (2026-05-02 re-measure):** **447 помилок у 88 файлах**
  (повний `strict: true` + `allowJs: false` без жодних змін у коді,
  виміряно після `b5e47360 fix(web): eliminate implicit-any in modules/finyk`,
  `ca9d6f41 feat(web): intuitive good-practice improvements` та
  `fix(web): eliminate implicit-any in modules/fizruk/hooks/useWorkouts`).
  Розподіл за кодами: `TS7006` (parameter implicitly any) — 193, `TS7031`
  (binding element implicitly any) — 158, `TS7053` (element implicitly any)
  — 20, `TS2345` (argument type) — 17, `TS2339` (property does not exist)
  — 15, `TS7005` (var without type) — 12, `TS2322` (type assignability) — 9,
  `TS2307` (cannot find module) — 8, інші — 15.
- **Топ-блокери** (одиничний фікс розблокує найбільше):
  `modules/fizruk/components/workouts/WorkoutJournalSection.tsx` (31),
  ...
  `modules/fizruk/pages/Workouts.tsx` (17). Решта помилок розпорошені по
  `~78` файлах fizruk/onboarding/insights.
- **Закриті топ-блокери:**
  `modules/finyk/hooks/useStorage.ts` (було 71) — закрито у `b5e47360`;
  `modules/finyk/pages/AssetsTxPickerView.tsx` (було 30),
  `modules/finyk/pages/AssetsTable.tsx` (було 26),
  `modules/finyk/hooks/usePrivatbank.ts` (було 26) — закриті разом з рештою
  `modules/finyk/**` у тому ж PR; `modules/fizruk/hooks/useWorkouts.ts`
  (було 35) — закрито в поточному PR через явні типи `Workout`/`WorkoutItem`/
  `WorkoutGroup` на параметрах callback-ів. Загальне зниження з 768 до 447
  (-321) без жодних runtime-змін.
