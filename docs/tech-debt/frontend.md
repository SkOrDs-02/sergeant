...
- **Заміряний скоуп (2026-05-02 re-measure після `12fea1d5` + `e29b0ba4`):**
  **374 помилки у 46 файлах** (повний `strict: true` + `allowJs: false`
  без жодних змін у коді, через діагностичний `tsconfig.strict-full.json`).
  Розподіл за кодами: `TS7006` (parameter implicitly any) — 162, `TS7031`
  (binding element implicitly any) — 114, `TS7053` (element implicitly any)
...
- **Топ-блокери** (одиничний фікс розблокує найбільше):
  ...
  `core/insights/WeeklyDigestCard.tsx` (15). Решта помилок розпорошені
  по `~36` файлах fizruk/onboarding/insights.
- **Закриті топ-блокери:**
