...
На момент оновлення інвентарю — **32** файли міграцій (lex order, без `.down.sql`-компаньйонів): `001_noop.sql`, `002_ai_usage_daily.sql`, `003_baseline_schema.sql`, `004_ai_usage_daily_tool_bucket.sql`, `005_backend_hardening.sql`, `006_push_devices.sql`, `007_module_data_user_fk.sql`, `008_mono_integration.sql`, `009_waitlist.sql`, `010_mono_mcc_categorization.sql`, `011_webhook_events.sql`, `012_ai_usage_token_counters.sql`, `013_mono_ai_enrichment_queue.sql`, `014_mono_ai_category_fields.sql`, `015_n8n_failure_events.sql`, `016_constraints_and_query_plan_notes.sql`, `017_mono_webhook_secret_hash.sql`, `018_seo_tables.sql`, `019_growth_tables.sql`, `020_marketing_tables.sql`, `021_governance_audit.sql`, `022_n8n_*` family, `023_sync_audit_log.sql`, `024_module_check_and_soft_delete.sql`, `025_ai_memories_pgvector.sql`, `026_routine_tables.sql`, `027_sync_op_log.sql`, `028_openclaw.sql`, `029_fizruk_tables.sql`, `030_openclaw_write_audit.sql`, `031_tg_alert_acks.sql`, `032_query_plan_documentation.sql` (PR D closeout — query-plan COMMENT ON-и). Більшість має `.down.sql` companion для local rollback (production runner у `apps/server/migrate.mjs` ніколи не виконує down-міграції).
...
| **PR D** ✅         | Міграції: EXPLAIN ANALYZE inline-коментарі + CHECK-constraints + `idx_push_subs_user_active` як канонічний обʼєкт для `idx_push_subscriptions_user_id` | `apps/server/src/migrations/{005,016,024,032}*.sql`, оновлення `docs/tech-debt/backend.md`                                                                                                                        | —          | Ні (всі зміни — additive).                         |
