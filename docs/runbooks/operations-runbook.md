...
- `tools/console` (OpenClaw_sergeant_bot) → Railway service `sergeant-openclaw` (раніше `sergeant-hubchat` per ADR-0032 / Pain P10) — [`docs/deploy/console.md`](../deploy/console.md)
...
| **`tools/console`** | Railway UI → service `sergeant-openclaw` → Deployments → попередній build → «Redeploy».                      | ~1-2 хв     |
