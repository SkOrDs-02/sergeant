...
- **Spec execution:** `spec-executor` — виконує готову спеку з `docs/work/specs/`.
...
Спека — це самодостатній документ у [`docs/work/specs/`](../../work/specs/), написаний так, щоб сесія з нульовим контекстом могла його виконати: скоуп, що поза скоупом, план розкатки по PR і секція `§ Верифікація` з конкретними гейтами. Шаблон — [`TEMPLATE.md`](../../work/specs/TEMPLATE.md).
