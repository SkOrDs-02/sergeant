...
**Sergeant використовує мінімальний `.env.example` (≈20 рядків) у корені репо як шаблон для `pnpm dev`, а повний reference усіх ~100 змінних — [`docs/engineering/integrations/env-vars.md`](../../engineering/integrations/env-vars.md).** Ніколи не комить `.env`, `.env.local`, `.env.secrets`.

- **Локально:** копія `.env` поряд із `.env.example`. Мінімальний набір для `pnpm dev`: `DATABASE_URL`, `BETTER_AUTH_SECRET`, `ANTHROPIC_API_KEY` (для HubChat), `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` (для push). Опційні змінні (Sentry, PostHog, Voyage, Mono, OpenClaw, …) — дивись [`docs/engineering/integrations/env-vars.md`](../../engineering/integrations/env-vars.md) per-feature.
...
- `pnpm gen:adr` — створює `docs/governance/adr/NNNN-<slug>.md` із валідною шапкою (Status / Date / Reviewers / Supersedes / Related). Номер обчислюється через `nextAdrNumber()` у [`plopfile.mjs`](../../../plopfile.mjs), gaps пропускаються.
...
- [`docs/start/instructions/README.md`](../instructions/README.md) — execution recipes by trigger.
- [`docs/governance/governance/hard-rules-matrix.md`](../../governance/governance/hard-rules-matrix.md) — машино-читабельна enforcement-матриця.
