...
Для повторюваної перевірки продукту обирай сценарії з [комплектів верифікації](../../engineering/testing/verification/README.md). Перед запуском прочитай профілі, обмеження стенду та handoff попереднього сумісного прогону; створи новий run зі snapshot карток. Після запуску додай спроби з доказами, порівняння та handoff. Поточні статуси дефектів веди в [єдиному реєстрі](../../work/specs/audits/verification/README.md); історичний аудит залишається джерелом походження. Злитий PR сам по собі не підтверджує виправлення. Цей маршрут доповнює surface-скіли й наявні QA-лейни.
...
**Межа owner-агент ↔ deliver-squad** (рішення 11 спеки [agent-module-owners](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/90-work/planning/specs/archive/agent-module-owners.md)):
...
Службові агенти шару: `canon-drift-auditor` (read-only звіт канон↔код по модулю), `product-historian` (read-only «чому так вирішили» з журналів/ADR), `spec-executor` (виконує спеки з `docs/work/specs/` у worktree; диспатчиться з `sergeant-feature-delivery`). Топологія — [`.agents/agent-graph.json`](../../../.agents/agent-graph.json), гейт `pnpm lint:agent-graph`.
...
5. If change is product-facing or non-trivial, write/update spec in `docs/work/specs/`.
...
Use when the trigger is «check that docs aren't lagging behind code» / «reconcile drift and execute open doc tasks across the whole `docs/` tree», and the surface is broad (initiatives + planning + audits + launch + security). Canonical recipe: [`docs/start/instructions/reconcile-doc-drift.md`](../instructions/reconcile-doc-drift.md). This workflow adds the **parallel fan-out** layer on top of that single-document playbook.
...
3. **Split the inventory into disjoint surfaces** so parallel agents never touch the same file. One owner per tracker directory: `docs/work/specs/initiatives`, `docs/work/specs/planning`, `docs/work/specs/audits` + `docs/work/specs/security-hardening`, `docs/work/specs/launch`. **Never** hand an agent an `AUTO-GENERATED` file (`open-work.md`, `follow-ups.md`, `today.md`, `*.auto.json`) — those are regenerated in step 5, not edited.
4. **Fan out (parallel).** Spawn one read-only analysis agent per surface. Each agent: for every `Active`/`In progress`/`Draft` doc in its directory, (a) check whether all `#NNNN` PR-mentions are merged (`docs/governance/pr-ledger/index.json`); (b) grep `main` for evidence that `- [ ]` items are actually shipped; (c) return **precise, evidence-backed edits only** — which checkboxes to flip to `- [x]`, which `> **Status:**` headers to close, which `Next review` dates are stale. Conservative bias: when evidence is ambiguous, leave the doc unchanged and report it as "needs human". Do **not** archive in this sweep (archival is a separate, ≥90-day-gated pass — see playbook §5).
...
- **Write or debug a Playwright E2E test** → skill `sergeant-e2e-testing`, recipe [`docs/start/instructions/write-e2e-test.md`](../instructions/write-e2e-test.md). seedFTUX, web-first assertions, прогін проти `vite preview`, trace-дебаг.
- **Change an auth flow (Better Auth)** → skill `better-auth-best-practices`, recipe [`docs/start/instructions/change-auth-flow.md`](../instructions/change-auth-flow.md). Вузький обсяг, сервер+клієнт в одній зміні, верифікація кукі на парі Vercel ↔ Coolify. Якщо це governance привілейованого доступу — натомість `access-governance.md`.
- **Author or edit a SKILL.md** → skill `sergeant-writing-skills`, recipe [`docs/start/instructions/author-skill.md`](../instructions/author-skill.md). RED → GREEN → REFACTOR для інструкцій, далі `pnpm lint:skills && pnpm skills:lock` + рядок у `agent-skills-catalog.md`.
