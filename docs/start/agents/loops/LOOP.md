...
- **`active`** — запускається автоматично (scheduled або per-PR тригер) без ручного втручання.
- **`manual`** — увімкнено (`enabled: true`), але потребує явного trigger від власника; не самозапускається.
- **`blocked-on-infra`** — задизайновано і готово до вмикання, але блокується зовнішньою інфра-залежністю.
- **`designed`** — задизайновано, не увімкнено; очікує phased rollout рішення.

## Loop Status

| Loop                                            | Owner skill                         | Cadence                         | Phase | Enabled | Status           |
| ----------------------------------------------- | ----------------------------------- | ------------------------------- | ----- | ------- | ---------------- |
| [pr-review](#pr-review)                         | `sergeant-review-and-merge`         | двічі на тиждень (Пн+Чт ~10:00) | L1    | **Yes** | **active**       |
| [tech-debt-sweep](#tech-debt-sweep)             | `sergeant-tech-debt`                | 6h-1d                           | L2    | No      | designed         |
| [security-audit](#security-audit)               | `sergeant-security-audit`           | 1d                              | L1    | No      | designed         |
| [migration-guard](#migration-guard)             | `sergeant-data-and-migrations`      | per-PR                          | L2    | No      | designed         |
| [deploy-watch](#deploy-watch)                   | `sergeant-deploy-and-observability` | 5-15m                           | L2    | No      | blocked-on-infra |
| [e2e-flake-watch](#e2e-flake-watch)             | `sergeant-e2e-testing`              | per-PR                          | L1    | No      | designed         |
| [review-squad-parallel](#review-squad-parallel) | `sergeant-review-squad`             | per-PR                          | L1    | No      | designed         |
| [qa-squad-parallel](#qa-squad-parallel)         | `sergeant-qa-squad`                 | per-PR                          | L1    | No      | designed         |
| [council-advisory](#council-advisory)           | `sergeant-council`                  | per-decision                    | L1    | **Yes** | **manual**       |
| [planning-batch](#planning-batch)               | `sergeant-planning-batch`           | per-batch                       | L1    | **Yes** | **manual**       |
...
- **Status:** **blocked-on-infra** — Grafana Alloy працює під Coolify; n8n паузовано 2026-06-28 (`railway down`) і виведено з репо 2026-09-02 ([ADR-0090](../../../governance/adr/0090-n8n-decommissioned.md)); Sentry→Telegram alert chain треба перепідключити на server-side alert shipper — вмикати після цього.
- **Cost:** 500k tokens/day cap.
...
**Tooling limitation:** `loop-audit` шукає файли на repo root рівні (`LOOP.md`, `STATE.md`, `patterns/registry.yaml`, `loop-budget.md`). Ми свідомо тримаємо manifest у `docs/start/agents/loops/` щоб не дублювати sources of truth (Hard Rule #15). Через це `loop-audit . --suggest` показує ті самі false-positives незалежно від вмісту — це обмеження їх hardcoded path matching, не нашої schema. Source-of-truth для нашого loop readiness — внутрішня `pnpm lint:skills` + cross-ref validation (див. verification steps у PR description).

- Перед вмиканням будь-якого loop в L2/L3 — заповнити `enabled: true` у `registry.yaml`, оновити таблицю вище, додати entry до PR-ledger (`docs/governance/pr-ledger/index.json` per Hard Rule #26).
