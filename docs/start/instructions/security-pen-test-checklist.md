...
- Primary surface: `docs/work/specs/security-hardening/**` + `docs/governance/security/pen-tests/**`
- Coupled surface: `apps/server/src/{auth,http,modules}/**` (cards шипляться там)
- Governing skill: `sergeant-deploy-and-observability`

## Required context

- Перечитай [docs/work/specs/security-hardening/README.md](../../work/specs/security-hardening/README.md), щоб бачити поточний backlog і severity-розподіл.
...
- Відсортуй `docs/work/specs/security-hardening/*.md` за `Status: Closed` + `Severity: High|Critical`.
- Вибери щонайменше всі **High** карти, що ще не мають transcript-у в `docs/governance/security/pen-tests/` за поточний квартал.
...
- Шаблон transcript-у бери з [`docs/governance/security/pen-tests/2026-05-hardening-sweep.md`](../../governance/security/pen-tests/2026-05-hardening-sweep.md) — кожна H-карта в окремій секції з cURL-командами.
...
- Створи / онови `docs/governance/security/pen-tests/YYYY-MM-<sweep-name>.md` із результатами.
- Додай рядок у `## Verification checklist` для кожної атаки + експліцитний `Observed (YYYY-MM-DD)` маркер.
- Якщо знайдено follow-up gap (наприклад, нема automated CI асерту) — заведи `Follow-ups` секцію + open GH issue + закидай у `docs/work/specs/tech-debt/<surface>.md` якщо це довготривалий борг.
...
- Готовність до external pen-test → передай transcript у [`docs/work/specs/launch/business/04-launch-readiness.md`](../../work/specs/launch/business/04-launch-readiness.md) як evidence-документ.
...
- [ ] Transcript додано в `docs/governance/security/pen-tests/` і залінкований з owning-картами
...
- [`docs/governance/security/pen-tests/2026-05-hardening-sweep.md`](../../governance/security/pen-tests/2026-05-hardening-sweep.md) — приклад transcript-у (H5/H6/H8/H9)
