...
- Primary surface: `docs/governance/governance/hard-rules.json`
- Coupled surface: `AGENTS.md`, `CONTRIBUTING.md`, `docs/governance/governance/hard-rules-matrix.md`
...
CI-гейт `pnpm lint:hard-rules-registry` валить PR, якщо `AGENTS.md`, `CONTRIBUTING.md` і `docs/governance/governance/hard-rules.json` дрейфують один від одного — усі троє рухаються в одному PR (Hard Rule #15).
...
Додай новий запис у [`docs/governance/governance/hard-rules.json`](../../governance/governance/hard-rules.json) за канонічною схемою:
...
`kind` має бути одним із: `ci`, `eslint-rule`, `test`, `hook`, `branch-protection`, `codeowners`, `doc`, `convention`, `pr-template` (див. [`hard-rules.schema.json`](../../governance/governance/hard-rules.schema.json)).
...
Легенда лежить унизу [`hard-rules-matrix.md`](../../governance/governance/hard-rules-matrix.md) і у преамбулі `## Hard rules` в `AGENTS.md`. `pnpm lint:hard-rules-registry` і `loadRegistry()` (`scripts/docs/generate-hard-rules-matrix.mjs`) відкидають правила без валідного `category`.

Далі перегенеруй індекс:

```bash
pnpm hard-rules:generate         # перезаписує docs/governance/governance/hard-rules-matrix.md
...
- [ ] `docs/governance/governance/hard-rules.json` — запис із цілочисельним `id: N` присутній.
...
- [docs/governance/governance/hard-rules.json](../../governance/governance/hard-rules.json) — машино-читабельний реєстр.
- [docs/governance/governance/hard-rules.schema.json](../../governance/governance/hard-rules.schema.json) — JSON Schema.
- [docs/governance/governance/hard-rules-matrix.md](../../governance/governance/hard-rules-matrix.md) — авто-згенерована матриця-перехресні-посилання.
