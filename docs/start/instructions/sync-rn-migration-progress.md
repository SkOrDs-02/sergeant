...
**Trigger:** після merge порту web → mobile (див. `port-web-screen-to-mobile.md`) — оновити progress tracker `docs/engineering/mobile/react-native-migration.md`.

## Owner surface

- Primary surface: `docs/engineering/mobile/react-native-migration.md`
...
### 2. Прочитати `docs/engineering/mobile/react-native-migration.md`

Знайти **точне місце** (рядок таблиці, чекбокс, фазову секцію), яке відповідає кожному merged PR-у.

```bash
grep -n "<screen-or-section-keyword>" docs/engineering/mobile/react-native-migration.md
```
...
```bash
pnpm exec prettier --write docs/engineering/mobile/react-native-migration.md
git diff docs/engineering/mobile/react-native-migration.md
```
...
- PR description (`.github/PULL_REQUEST_TEMPLATE.md`):
  ...
  - Лінк на [`docs/start/instructions/port-web-screen-to-mobile.md`](./port-web-screen-to-mobile.md), якщо PR-и слідували йому.
...
- [ ] `pnpm format:check docs/engineering/mobile/react-native-migration.md` — green.
- [ ] PR-діф містить **тільки** `docs/engineering/mobile/react-native-migration.md` (нічого більше).
...
- [`docs/engineering/mobile/react-native-migration.md`](../../engineering/mobile/react-native-migration.md) — сам tracker.
