...
| Періодичний (раз на день/тиждень)      | Додай як HTTP endpoint, що тригериться зовнішнім cron-ом (GitHub Actions schedule) або серверним таймером ([ADR-0089](../../governance/adr/0089-job-substrates-outbox-broker-timer.md)), або повісь на добову гілку reminder-sweep-у. |
