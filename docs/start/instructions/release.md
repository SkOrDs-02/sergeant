...
- Перечитай [release-policy.md](../../governance/governance/release-policy.md) — таксономію merge-only / coordinated / high-risk.
- Перечитай [service-catalog.md](../../engineering/architecture/service-catalog.md) — щоб знати rollback-шлях і tier поверхні, яку зачіпаєш.
- Перечитай [platforms.md](../../engineering/architecture/platforms.md), якщо зміна перетинає Capacitor або Expo.
...
- Визнач, що це: merge-only, coordinated чи high-risk — за [release-policy.md](../../governance/governance/release-policy.md).
...
- Якщо реліз містить помітну для юзера зміну (нова фіча, видима поведінка, UX-поліпшення) — додай **один** запис у `whats-new` за процесом у [docs/product/whats-new/README.md](../../product/whats-new/README.md): markdown-файл + TS-запис у [`apps/web/src/core/whatsNew/releases.ts`](../../../apps/web/src/core/whatsNew/releases.ts), узгоджені (drift ловить `releases.test.ts`).
...
- Копія — мовою користувача про результат, не інженерний changelog (`feat(...)`-subject у модал не йде). Тон — за [style-guide.uk.md](../../product/copy/style-guide.uk.md) (`ти`-звертання).
