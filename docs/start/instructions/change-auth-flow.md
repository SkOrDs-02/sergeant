...
Better Auth — **high-risk integration-поверхня**. Помилка тут — це не баг одного екрана, а втрата сесій усіх юзерів або відкритий редирект. Тому два правила над усім: тримай зміну **вузькою** і верифікуй кукі на **парі Vercel ↔ бекенд (Coolify)**, бо cross-site cookie-поведінка — це deploy-concern, а не лише код.
...
- Лише UI-копія/верстка форми без зміни контракту → це робота `sergeant-web-ui`, не повний auth-флоу. Все одно прогони [§4](#4-верифікувати-на-парі-vercel--бекенд-coolify).
- Login / logout / session / redirect / cookie → [§1](#1-звузити-обсяг) → [§2](#2-сервер-і-клієнт-в-одній-зміні) → [§4](#4-верифікувати-на-парі-vercel--бекенд-coolify)
...
- Так (`sameSite`, домен куки, проксі `/api/*`, `ALLOWED_ORIGINS`) → перечитай [`docs/engineering/integrations/railway-vercel.md`](../../engineering/integrations/railway-vercel.md) **до** правки; це частина auth-контракту.
...
### 4. Верифікувати на парі Vercel ↔ бекенд (Coolify)

Compile success тут нічого не доводить. Перевір runtime-поведінку через Vercel-фронтенд (він проксює `/api/*` на бекенд під Coolify — це частина auth-контракту):
...
- [docs/engineering/integrations/railway-vercel.md](../../engineering/integrations/railway-vercel.md) — cross-site cookie контракт
