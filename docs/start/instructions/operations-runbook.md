...
[`docs/work/specs/planning/pr-plan-2026-05.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/planning/archive/pr-plan-2026-05.md).

> **Не дублює** інші runbook-и, а служить **routing-картою** до них.
> Канонічні «як саме виконати» команди живуть у:
>
> - [`docs/operations/observability/runbook.md`](../../operations/observability/runbook.md) — production incident-flow, alert-decoder
> - [`docs/start/instructions/database-backup-restore.md`](./database-backup-restore.md), [`./database-connection-pooling.md`](./database-connection-pooling.md), [`./postgres-read-replica.md`](./postgres-read-replica.md), [`./encryption-key-rotation.md`](./encryption-key-rotation.md) — конкретні DB-операції
> - [`docs/operations/deploy/`](../../operations/deploy/README.md) — Coolify / Vercel deploy walkthrough-и (OpenClaw doc — archived)
> - [`docs/start/instructions/`](./README.md) — repeatable процедури (incident, release, rotation, hotfix)
...
3. **Знайди відповідний runbook:** [`docs/operations/observability/runbook.md`](../../operations/observability/runbook.md) має алерт-decoder («Що робити, якщо ALERT-NAME триггериться»).
4. **Якщо runbook відсутній або не допомагає:** [`docs/start/instructions/declare-incident.md`](./declare-incident.md) — escalation flow + Telegram комунікація.
...
> reference. Rotation API-токенів — [`docs/start/instructions/rotate-secrets.md`](./rotate-secrets.md);
...
Канонічна service-таблиця з alerts/runbook/rollback per surface — [`docs/engineering/architecture/service-catalog.md`](../../engineering/architecture/service-catalog.md).
...
2. **PostHog dashboards** — [`docs/operations/observability/posthog-ftux-dashboards.md`](../../operations/observability/posthog-ftux-dashboards.md). Drop ≥ 30% у `signup_completed` = щось зламалось у auth flow.
...
Канонічний repeatable-recipe: [`docs/start/instructions/hotfix-prod-regression.md`](./hotfix-prod-regression.md).
...
n8n-шар виведено з репо ([ADR-0090](../../governance/adr/0090-n8n-decommissioned.md)); історичні workflow-JSON — у [permalink-снапшоті](https://github.com/SkOrDs-02/sergeant/blob/ffdf694cb60dcfeebc2c1de14887c5a8a1d71e6b/ops/n8n-workflows/).
...
```
                    ...
                    │
"щось не так"  ─────┼─ alert у Telegram? ────────► docs/operations/observability/runbook.md §alert-decoder
                    │
...
| Access review (хто має які доступи)            | Квартальна            | [`access-governance.md` § Periodic access review](./access-governance.md#3-periodic-access-review)                                  |

### 8.1. Migration `down.sql` drill

PR-32 з [`docs/work/specs/planning/pr-plan-2026-05.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/planning/archive/pr-plan-2026-05.md). Repo policy в Hard Rule #4 ([`docs/governance/governance/rules/04-sql-migrations-sequential-two-phase.md`](../../governance/governance/rules/04-sql-migrations-sequential-two-phase.md)): production **ніколи не запускає `.down.sql`** — production rollback завжди = compensating migration. Але `apps/server/src/migrations/NNN_*.down.sql` лишається обов'язковим інструментом для local rollback-у під час incident response / hotfix testing / DBA-recovery без backup-у.
...
Hard Rule #4 ([`docs/governance/governance/rules/04-sql-migrations-sequential-two-phase.md`](../../governance/governance/rules/04-sql-migrations-sequential-two-phase.md)): **destructive `DROP TABLE` / `ALTER TABLE … DROP COLUMN` мають проходити дві фази, розведені у часі мінімум на 14 днів.** Сенс правила — дати running app-у час перестати читати/писати в колонку/таблицю перед тим, як її фізично прибрати. Один-PR-DROP частіше = production incident.
...
Причина одна на всі три: перейменування вже застосованої міграції. Раннер трекає застосовані міграції за іменем файлу, тож під новим іменем той самий SQL виконався вдруге. Механіка й гейт, що це тепер блокує, — [Rule #4 § Перейменування вже застосованої міграції](../../governance/governance/rules/04-sql-migrations-sequential-two-phase.md).
...
2. **Runtime audit script** — `pnpm db:index-audit` (manual, поза CI). Опитує `pg_stat_user_indexes` + `pg_stat_user_tables` живої БД і генерує markdown report з трьома секціями:
   ...
   pnpm db:index-audit --write                     # docs/start/instructions/db-index-audit-YYYY-MM-DD.md
   ```
...
2. Файл: `docs/operations/postmortems/YYYY-MM-DD-<incident-name>.md` з template-ом з [`docs/start/instructions/write-postmortem.md`](./write-postmortem.md).
