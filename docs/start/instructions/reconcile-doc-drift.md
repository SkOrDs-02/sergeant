...
**Trigger:** «Актуалізуй документацію» / «Знайди дрифт і не позначені виконані таски» / «Прибери завершені плани/ініціативи/аудити» / періодична гігієна docs, щоб у кожному документі було видно стан і залишок роботи.

## Owner surface

- Primary surface: `docs/` (`initiatives/`, `planning/`, `audits/`, `launch/`, `security/hardening/`, `tech-debt/`, `superpowers/plans/`)
- Coupled surface: `docs/open-work.md`, `docs/governance/pr-ledger/index.json`, `scripts/docs/`, будь-який код під `apps/`/`packages/`, на який посилаються застарілі docs
- Governing skill: `sergeant-tech-debt`

### Prerequisites

- Прочитай [`AGENTS.md`](../../../AGENTS.md) → Hard Rule #10 (lifecycle markers), #15 (governance + UA docs), #23 (no local archive trees), #26 (PR ledger).
- Зрозумій формат дашборду відкритої роботи: [`docs/open-work.md`](../../open-work.md) (source = `> **Status:**` хедер кожного документа).
- Історична процедура до ADR-0081: [`initiatives/archive/README.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/initiatives/archive/README.md). Чинна процедура cleanup — §5 цього playbook.
...
- Передумови зникли, не починали → постав `Withdrawn` у тілі й лиши в активному списку для аудит-сліду

**Q4: Чи можна видалити frozen snapshot із checkout?**

- Статус `Closed/Done/Implemented`, Outcome та merge evidence заповнені, follow-up-ів немає → [§5 Прибрати завершене](#5-прибрати-завершене)
- Канонічні правила (Hard Rule / lint / ADR), породжені документом, ще **не** винесені в `AGENTS.md`/`docs/governance/governance/` → **STOP**: спочатку винеси їх
- Немає повного evidence або є відкриті follow-up-и → лиши на місці зі статусом `Closed`
...
flowchart TD
    ...
    Q4{"Q4: можна прибрати snapshot?"}
    Q4 -- "Outcome/evidence повні, правила винесені" --> S5["§5 Cleanup"]
    Q4 -- "правила ще в доку" --> STOP2["STOP: винести правила"]
    Q4 -- "бракує evidence / є follow-up" --> KEEP["лишити Closed"]
...
3. Завершені ініціативи з Outcome та merge evidence (кандидати на cleanup).
...
grep -E '"<NNNN>"' docs/governance/pr-ledger/index.json
```
...
grep -rn '^\s*-\s*\[ \]' docs/work/specs/initiatives docs/work/specs/planning docs/work/specs/launch docs/work/specs/audits docs/work/specs/security-hardening
```
...
Перед cleanup документ повинен мати статус `Closed`/`Done`/`Implemented`, заповнений Outcome, PR/commit evidence, без нових follow-up-ів; канонічні правила вже винесені (Q4). Спершу merge-ни цей стан. Потім окремим cleanup-комітом видали frozen tracker із `docs/work/{audits,initiatives,planning}` і переведи inbound references на immutable GitHub permalink попереднього commit. Локальні archive-дерева для цих трьох tracker-ів retired за ADR-0081.

### 6. Зафіксувати й верифікувати

Перегенеруй дашборди (тепер видалений snapshot зникне з open-work) і прожени гейти — див. [§ Verification](#verification).
...
- [ ] `pnpm docs:gen-open-work` перегенеровано; видалені завершені документи зникли з [`docs/open-work.md`](../../open-work.md), лічильники оновились
...
- [ ] Канонічні правила (Hard Rules / lint / ADR) живі в `AGENTS.md`/`docs/governance/governance/`, а не лише в Git snapshot
- [ ] `pnpm lint` — зелено
...
- **Триаж/виконання відкритих аудитів** як основна ціль — скіл `audits-runner` (`mode:"triage"|"execute"`); цей playbook прибирає лише вже закриті frozen snapshot-и.
...
- Git history — це історичний контекст, не живий контракт. Перед cleanup переконайся, що все канонічне винесено в `AGENTS.md`/`docs/governance/governance/` (Q4 / §5).
...
- Веди звірку й cleanup завершеної документації **окремим PR** від feature-роботи (soft rule).

## See also

- [AGENTS.md](../../../AGENTS.md) — Hard Rules #10, #15, #23, #26
- [`docs/open-work.md`](../../open-work.md) — згенерований single-pane всієї відкритої роботи
- [`docs/work/specs/initiatives/archive/README.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/initiatives/archive/README.md) — історична процедура до ADR-0081
