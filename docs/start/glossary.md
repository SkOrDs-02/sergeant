...
| **Finyk** (фінік)        | Модуль **фінансів** — витрати, бюджети. RQ-фабрика `finykKeys`. Володіє 15 таблицями `finyk_*` (міграція 039). **Баланси finyk не належать** — вони похідні від `mono_*`-таблиць (міграція 008); finyk лише читає й накладає оверлеї (категорії, спліти, приховані/виключені) поверх банківських даних. |
...
| Термін          | Що це                                                                                                                                                                                                              |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **HubChat**     | Web-**Асистент** для кінцевого користувача: tool-defs, executors, prompt cache, action cards. **HubChat ≠ OpenClaw.** Skill: `sergeant-module-ai`.                                                                 |
| **OpenClaw**    | _(Decommissioned 2026-07-20 — [ADR-0075](../governance/adr/0075-openclaw-gateway-decommissioned.md).)_ Історично — Telegram co-founder bot / external Gateway. **Не плутати з HubChat** (web-асистент для юзерів). |
| **Gateway**     | _(Historical.)_ Колишній OpenClaw Gateway runtime — повністю прибрано з репо.                                                                                                                                      |
| **Memory Bank** | Local-first сховище AI-фактів про користувача. ADR → [`0021-memory-bank.md`](../governance/adr/0021-memory-bank.md).                                                                                               |

## Дані, sync, час, гроші

| Термін                     | Що це                                                                                                                                                                                                                                    |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **syncV2 / CloudSync v2**  | Op-log writer-runtime (`getSyncEngineWriter()`), `core/syncEngine/`. v1 знятий — ADR → [`0047-cloudsync-v1-410-gone.md`](../governance/adr/0047-cloudsync-v1-410-gone.md).                                                               |
