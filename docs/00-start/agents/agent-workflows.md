...
Для повторюваної перевірки продукту обирай сценарії з [комплектів верифікації](../../02-engineering/testing/verification/README.md). Перед запуском прочитай профілі, обмеження стенду та handoff попереднього сумісного прогону; створи новий run зі snapshot карток. Після запуску додай спроби з доказами, порівняння та handoff. Поточні статуси дефектів веди в [єдиному реєстрі](../../90-work/audits/verification/README.md); історичний аудит залишається джерелом походження. Злитий PR сам по собі не підтверджує виправлення. Цей маршрут доповнює surface-скіли й наявні QA-лейни.
