...
3. **Split the inventory into disjoint surfaces** so parallel agents never touch the same file. One owner per tracker directory: `docs/90-work/initiatives`, `docs/90-work/planning`, `docs/90-work/audits` + `docs/security/hardening`, `docs/01-product/launch`. **Never** hand an agent an `AUTO-GENERATED` file (`open-work.md`, `follow-ups.md`, `today.md`, `*.auto.json`, `symbol-index.*`) — those are regenerated in step 5, not edited.
