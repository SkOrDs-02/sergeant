...
> **Last touched:** 2026-08-16 by @github-actions[bot]. **Next review:** 2026-11-18.
> **Status:** Active
...
- Coupled surface: `docs/open-work.md`, `docs/04-governance/pr-ledger/index.json`, `scripts/docs/`, будь-який код під `apps/`/`packages/`, на який посилаються застарілі docs
...
- Канонічні правила (Hard Rule / lint / ADR), породжені документом, ще **не** винесені в `AGENTS.md`/`docs/04-governance/governance/` → **STOP**: спочатку винеси їх, потім архівуй
...
pnpm docs:freshness-dashboard    # docs/04-governance/governance/freshness-dashboard.html — overdue Next review
...
grep -E '"<NNNN>"' docs/04-governance/pr-ledger/index.json
```
...
grep -rn '^\s*-\s*\[ \]' docs/90-work/initiatives docs/90-work/planning docs/01-product/launch docs/90-work/audits docs/04-governance/security/hardening
```
