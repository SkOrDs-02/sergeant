...
> **Last touched:** 2026-08-16 by @github-actions[bot]. **Next review:** 2026-12-09.
> **Status:** Active
...
- Primary surface: `docs/04-governance/governance/hard-rules.json`
- Coupled surface: `AGENTS.md`, `CONTRIBUTING.md`, `docs/04-governance/governance/hard-rules-matrix.md`
...
CI-гейт `pnpm lint:hard-rules-registry` валить PR, якщо `AGENTS.md`, `CONTRIBUTING.md` і `docs/04-governance/governance/hard-rules.json` дрейфують один від одного — усі троє рухаються в одному PR (Hard Rule #15).
...
Додай новий запис у [`docs/04-governance/governance/hard-rules.json`](../../04-governance/governance/hard-rules.json) за канонічною схемою:
...
`kind` має бути одним із: `ci`, `eslint-rule`, `test`, `hook`, `branch-protection`, `codeowners`, `doc`, `convention`, `pr-template` (див. [`hard-rules.schema.json`](../../04-governance/governance/hard-rules.schema.json)).
...
Легенда лежить унизу [`hard-rules-matrix.md`](../../04-governance/governance/hard-rules-matrix.md) і у преамбулі `## Hard rules` в `AGENTS.md`. `pnpm lint:hard-rules-registry` і `loadRegistry()` (`scripts/docs/generate-hard-rules-matrix.mjs`) відкидають правила без валідного `category`.

Далі перегенеруй індекс:

```bash
pnpm hard-rules:generate         # перезаписує docs/04-governance/governance/hard-rules-matrix.md
...
- [ ] `docs/04-governance/governance/hard-rules.json` — запис із цілочисельним `id: N` присутній.
...
- [docs/04-governance/governance/hard-rules.json](../../04-governance/governance/hard-rules.json) — машино-читабельний реєстр.
- [docs/04-governance/governance/hard-rules.schema.json](../../04-governance/governance/hard-rules.schema.json) — JSON Schema.
- [docs/04-governance/governance/hard-rules-matrix.md](../../04-governance/governance/hard-rules-matrix.md) — авто-згенерована матриця-перехресні-посилання.
