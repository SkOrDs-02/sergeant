...
> **Severity:** `blocker`
> **Last touched:** 2026-08-31 by @Skords-01. **Next review:** 2027-03-13.
> **Status:** Active
...
1. **`AGENTS.md`** — Hard Rules (#1–#26, з gap-ами по retired-правилах), Module ownership map for the path you're touching, AI-marker conventions, Domain invariants.
...
4. **The matching playbook** in `docs/00-start/playbooks/` — pick by trigger phrase. New API endpoint → `add-api-endpoint.md`. New HubChat tool → `add-hubchat-tool.md`. Removing code → `cleanup-dead-code.md`. Migrations → `add-sql-migration.md`.
...
| New design token, palette, or component           | `docs/05-design/design/design-system.md`, `docs/05-design/design/brandbook.md`, and the relevant audit (`docs/90-work/audits/*-audit-*.md`) if it changes status.      |
...
- `pnpm lint:governance-sync` — fails (error, not warning) on **concrete** dangling `apps/.../*.ts` / `packages/.../*.ts` / `scripts/...` refs in non-aspirational docs (anything outside `docs/01-product/launch/`, `docs/90-work/planning/`, `docs/02-engineering/integrations/*-roadmap.md`, `docs/90-work/audits/*-implementation-roadmap.md`, ADRs with `Status: proposed`). Refs containing glob/placeholder syntax (`*`, `?`, `<>`, `[]`, `{}`) are skipped — those are templates, not concrete claims.
