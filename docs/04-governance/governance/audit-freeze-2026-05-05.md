...
5. **Розширення AGENTS.md** новими правилами без обов'язкового lint-enforcement (нові rules → `docs/04-governance/governance/hard-rules.json` + ESLint plugin або CI-gate; pure-text rules відкладаються до post-freeze).
...
3. **Status registry update-и** в master tracker-ах:
   - [`docs/01-product/launch/product-os/ftux-master-tracker.md`](../../01-product/launch/product-os/ftux-master-tracker.md) — FTUX SSOT.
...
docs/04-governance/adr/00\d{2}-*.md         (any new ADR)
```

CI-warning виводиться як comment на PR з:

- Список файлів, що тригернули warning
- Лінк на цей файл (`docs/04-governance/governance/audit-freeze-2026-05-05.md`)
...
- **`docs/04-governance/governance/policy-review.md`** — каденс не міняється; freeze це point-in-time event, не permanent rule.

---

## Зв'язок з PR-планом 2026-05-05

Цей файл — **PR-01** з 22-PR плану ([master tracker §3](../../01-product/launch/product-os/ftux-master-tracker.md#3-pr-план)). Freeze-period (4 тижні) перетинається з Хвилями 1-2 з PR-плану:
...
- `docs/90-work/audits/`, `docs/90-work/initiatives/`, `docs/00-start/playbooks/`, `docs/04-governance/adr/` — це місця, куди легко додати «ще один аудит» / «ще одна ініціатива», і де темп зростання випередив темп viability.
...
- Питання щодо freeze-rule → review-comment у PR-1 (цей PR) або у [`docs/01-product/launch/product-os/ftux-master-tracker.md`](../../01-product/launch/product-os/ftux-master-tracker.md) §1 TL;DR.
