...
- **Related:**
  ...
  - [`docs/04-governance/adr/0017-better-auth-choice-and-session-model.md`](./0017-better-auth-choice-and-session-model.md) — auth-stack rationale.
...
| PII retention у Loki/Sentry | 30 days max (запит у Coolify/Sentry support — TBD)                                                  |
...
- Окремий worker-процес (або cron-job-ом всередині API-сервера). На
  Coolify single-instance — у `apps/server/src/cron/`-папці.
...
- Loki (self-hosted / Coolify stack) — 30 days default. Перевірити при production setup.
...
1. **Backup-policy і "right to be forgotten" trade-off.** Coolify/Postgres backups
