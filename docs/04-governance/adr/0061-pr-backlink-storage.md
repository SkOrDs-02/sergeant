...
> **Last touched:** 2026-07-29 by @github-actions[bot]. **Next review:** 2026-10-27.
> **Status:** Active
...
1. **Hybrid:** single JSON ledger (`docs/04-governance/pr-ledger/index.json`) як canonical source + автогенерований block `<!-- AUTO-GENERATED: PR-BACKLINKS-START -->` з ≤5 останніх PR у тілі кожного канонічного дока.
2. **Per-PR markdown files** (`docs/04-governance/pr-ledger/PR-NNNN.md`). Кожен merged PR отримує власний файл.
...
- **Canonical store:** `docs/04-governance/pr-ledger/index.json` — JSON-схема draft-07 у `docs/04-governance/governance/schemas/pr-ledger.schema.json`. Шейп:
  ```json
  {
    "version": 1,
...
- **Canonical doc paths** що отримують backlinks (whitelist у script):
  - `docs/04-governance/adr/*.md` (крім TEMPLATE.md, README.md)
  ...
  - `docs/04-governance/governance/rules/*.md` (крім README.md)
