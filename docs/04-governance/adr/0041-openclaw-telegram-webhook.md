...
- Set Railway env vars on `sergeant-openclaw` service (renamed from `sergeant-hubchat` per ADR-0032 / Pain P10; per `docs/03-operations/deploy/openclaw.md`).
...
Activated on Railway service `sergeant-openclaw` (project `humorous-eagerness`, environment `production`; service was renamed from `sergeant-hubchat` per ADR-0032 / Pain P10 — see `docs/03-operations/deploy/openclaw.md` rename runbook):
