...
1. **Створення.** Новий flag → `defaultValue: false`, `experimental: true`. PR має містити `Expires: YYYY-MM-DD` у `docs/04-governance/governance/feature-flags.md` (або в PR-description) — дата, до якої flag має graduate-нути або бути видалений.
...
- `Expires`-дата — best-effort; не enforced автоматично. Можна додати CI-job, що фейлить, якщо у `docs/04-governance/governance/feature-flags.md` є entrі з минулою датою — поки не зробили (low ROI; з 3 поточних flag-ів усі under control).
...
- ⏳ `docs/04-governance/governance/feature-flags.md` як централізована таблиця (Owner / Default / Expires / Rollout) — поки писалося ad-hoc у PR-описах.
