...
- Партиціювання по `server_ts` (найближчий кандидат на "created_at")
  ...
  `(user_id, idempotency_key, server_ts)`. Оскільки `server_ts` унікальний
  для кожного рядка за побудовою (`DEFAULT NOW()` при вставці), розширений
  констрейнт ніколи не спрацює як backstop — обидва конкурентні INSERT-и
  проходять, і apply-шлях виконується ДВІЧІ для одного ідемпотентного
  ключа. Це не гіпотетичний edge-case: це рівно сценарій, який
  ідемпотентність мала гарантувати (client retry, що обжинає у гонку з
  first-attempt).
