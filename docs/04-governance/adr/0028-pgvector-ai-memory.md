...
- **Integration tests**: `vectorStore.integration.test.ts` через testcontainers + `pgvector/pgvector:pg16` (тепер pg17 — image bump після ADR-у) — реальний pgvector behaviour, GDPR-cascade, atomic-rollback.
- **CI Postgres image**: усі workflow-и, які виконують `db:migrate:dev` (а отже й 025-міграцію) — `ci.yml`, `extended-e2e.yml`, `visual-regression.yml` — використовують `pgvector/pgvector:pg16` як `services.postgres.image` (SHA-pinned `sha256:7d400e34…`) на момент цього ADR-у; тепер `pg17` (SHA-pinned `sha256:feb68f4f…`), див. `docker-compose.yml`. Plain `postgres:17-alpine` падає з `extension "vector" is not available` ще на boot-і. Подробиці і pattern-копія блоку — у [`docs/02-engineering/integrations/voyage-pgvector.md` § CI Postgres image](../../02-engineering/integrations/voyage-pgvector.md#ci-postgres-image).
