...
- **CI Postgres image**: усі workflow-и, які виконують `db:migrate:dev` (а отже й 025-міграцію) — `ci.yml`, `extended-e2e.yml`, `visual-regression.yml` — використовують `pgvector/pgvector:pg16` як `services.postgres.image` (SHA-pinned `sha256:7d400e34…`). Plain `postgres:16-alpine` падає з `extension "vector" is not available` ще на boot-і. Подробиці і pattern-копія блоку — у [`docs/02-engineering/integrations/voyage-pgvector.md` § CI Postgres image](../../02-engineering/integrations/voyage-pgvector.md#ci-postgres-image).
