...
Розміщення: нейтральний до платформи пакет (робоча назва `packages/dualwrite-core`, `@sergeant/dualwrite-core`) — web `@shared/lib/sqliteWriter/core` **не підходить** як дім фреймворку, бо mobile не може імпортувати з `apps/web` (саме тому mobile сьогодні тримає копії). Точне ім'я/межі пакета — через `sergeant-monorepo-boundaries` (див. Open questions). Пакет без DOM/RN-залежностей: логер і `crypto.randomUUID` — ін'єктовані.
...
| PR                                                     | Title                                                  | Merged     |
| ------------------------------------------------------ | ------------------------------------------------------ | ---------- |
| [#209](https://github.com/Skords-01/Sergeant/pull/209) | fix(ci): stabilize CI failures and slim duplicate jobs | 2026-07-10 |
