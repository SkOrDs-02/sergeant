...
> **Last touched:** 2026-08-07 by Claude (додано виняток image-size — патча не існує; nanoid закрито override-ом, не винятком). **Next review:** 2026-11-05.
> **Status:** Active
...
### image-size infinite loops у ICNS/JXL/HEIF-парсерах (GHSA-W3RX-R6R6-PGPR, GHSA-5P2G-FCMC-QVQQ)

| Field      | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Advisory   | https://github.com/advisories/GHSA-w3rx-r6r6-pgpr (ICNS) + https://github.com/advisories/GHSA-5p2g-fcmc-qvqq (JXL/HEIF)                                                                                                                                                                                                                                                                                                                                    |
| Package    | `image-size@1.2.1`; вразливі `<=2.0.2`                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Severity   | high (CVSS 7.5 обидва) — DoS через нескінченний цикл у парсері (CWE-835)                                                                                                                                                                                                                                                                                                                                                                                   |
| Path       | `apps/mobile > expo@52.0.49 > react-native@0.76.9 > @react-native/community-cli-plugin@0.76.9 > metro@0.81.5 > image-size@1.2.1`. `metro` — єдиний споживач у дереві (перевірено по `pnpm-lock.yaml`).                                                                                                                                                                                                                                                     |
| Reason     | **Патча не існує в жодній опублікованій версії.** Вразливий діапазон `<=2.0.2`, а `2.0.2` — це поточний `latest` в npm (`legacy` — той самий `1.2.1`, що стоїть у нас), тож `pnpm audit` повертає `patched_versions: <0.0.0`. Override-нути нема на що; лишається чекати на upstream-реліз і на bump усередині `metro`.                                                                                                                                    |
| Mitigation | Build-time-only. `metro` — це JS-бандлер React Native: він виконується на машині розробника чи в CI під час білду, а в APK/IPA не потрапляє. У prod-дерево audit-у він входить лише тому, що `expo`/`react-native` — рантайм-залежності `apps/mobile`, і CLI-плагін тягнеться разом із ними. Вхід для парсера — власні закомічені ассети репо, а не дані з мережі чи від користувача, тож «зловмисний ICNS/JXL/HEIF» тут довелося б закомітити самим собі. |
| Due date   | 2026-10-31 — переоцінити при наступному bump-і Expo SDK; якщо до того вийде пропатчений `image-size`, закрити раніше через `pnpm.overrides`.                                                                                                                                                                                                                                                                                                               |
| Owner      | @SkOrDs-02                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `nanoid@>=3.0.0 <3.3.17`    | `>=3.3.17 <4.0.0` | GHSA-2v37-7h3g-55p8 — нескінченний цикл у кастомних генераторах при `size === 0`. Верхня межа `<4.0.0` обовʼязкова: єдиний споживач — `postcss` (CJS, вимагає `^3.3.x`), а `nanoid` 4+ — ESM-only, тож голий `>=3.3.17` затягував `6.0.1` і ламав би `postcss`. |
