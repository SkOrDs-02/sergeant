...
> **Last touched:** 2026-07-30 by Claude (два датовані винятки: brace-expansion, react-router). **Next review:** 2026-10-28.
> **Status:** Active
...
| Package    | `brace-expansion@1.1.16` і `@2.1.2` (вразливі `<=5.0.7`; патч у `5.0.8`)                                                                                                                                                                                                                                                                                                                                          |
| Severity   | high — DoS через необмежену довжину розкриття                                                                                                                                                                                                                                                                                                                                                                     |
| Path       | Усі шляхи — build-tooling `expo@52` у `apps/mobile`: `@expo/cli`, `@expo/metro-config`, `@expo/config-plugins`, `babel-preset-expo → @react-native/codegen`, кожен через `minimatch@3.1.5` або `minimatch@9.0.9`                                                                                                                                                                                                  |
| Reason     | **Патч застосовано там, де це можливо**: override `brace-expansion@>=3.0.0 <5.0.8 → 5.0.8` (у цьому ж PR, було `5.0.7`). Залишкові `1.1.16` і `2.1.2` не піднімаються: `5.0.8` — інший мажор, а їх споживають `minimatch@3` і `minimatch@9`, які розраховані на 1.x/2.x API. Підстановка мажора туди ламає резолвінг glob-патернів у Expo-білді, тобто лікування гірше за хворобу.                                |
| Mitigation | Це **білд-тайм tooling**, а не рантайм-код застосунку: `@expo/cli`, `metro-config` і `codegen` виконуються на машині розробника й у CI під час збірки, у бандл до користувача не потрапляють. Вектор DoS — зловмисний glob-патерн, а патерни тут приходять із конфігів репо, не з користувацького вводу. `apps/web` і `apps/server` цих шляхів не мають узагалі — вразливість живе лише під `apps/mobile → expo`. |
| Due date   | 2026-09-30 — вікно наступного Expo SDK bump-у (SDK 53/54), який має підтягнути свіжий `minimatch`. Якщо не закрито — переоцінити разом із запланованим підйомом Expo.                                                                                                                                                                                                                                             |
| Owner      | @SkOrDs-02                                                                                                                                                                                                                                                                                                                                                                                                        |
