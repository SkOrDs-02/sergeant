...
| ----------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Login + sessions (Better Auth)                        | S      | Better Auth password-hash; UA-drift detection; session revoke-on-password-change ([H3](hardening/archive/H3-session-revoke-and-binding.md), Closed)                                                                                                                               | Anonymous CGNAT abuse — partially covered by per-IP fallback rate-limit; secondary cap = [M9](hardening/archive/M9-per-ip-secondary-rate-limit.md) Closed |
...
| `transcribe` / `chat` AI endpoints                    | D/E    | USD-cap per user ([H9](hardening/archive/H9-transcribe-usd-cap.md), Closed); chat tool-iteration cap ([M7](hardening/archive/M7-chat-tool-iteration-cap.md), Closed); OpenClaw daily $5 cap with per-call cap ([M18](hardening/archive/M18-openclaw-per-call-usd-cap.md), Closed) | None                                                                                                                                                      |
...
| Surface                      | STRIDE | Existing controls                                                                                                                                               | Residual risk                                                        |
| ---------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| Telegram allowlist           | S/E    | `CONSOLE_BOT_TOKEN` allowlist fail-closed ([M15](hardening/archive/M15-console-allowlist-fail-closed.md), Closed); `/grant` requires founder                    | None                                                                 |
| Repo-root traversal          | T      | `OPENCLAW_REPO_ROOT` path-traversal guard ([L8](hardening/archive/L8-openclaw-repo-root-traversal.md), Closed); `safeJoin.ts` allowlist mapping                 | None                                                                 |
| Telegram message rendering   | I/T    | `parse_mode: "MarkdownV2"` ([M16](hardening/archive/M16-telegram-markdown-v2.md), Closed) — escapes user-supplied tokens                                        | None                                                                 |
| Per-user / global rate-limit | D      | Per-user bucket ([M17](hardening/archive/M17-console-global-rate-cap.md), Closed); global cap added                                                             | None                                                                 |
| Anthropic per-call USD cap   | D      | Per-call cap ([M18](hardening/archive/M18-openclaw-per-call-usd-cap.md), Closed); daily $5 budget guard                                                         | None                                                                 |
| Security-event push          | R      | Sentry / Pino metrics: `auth.session.ua_drift`, `prompt_injection_attempt`, `transcribe.usd_cap_hit`, `chat.tool_iteration_cap_hit`, `mono_webhook_bad_payload` | None — [I7](hardening/archive/I7-security-events-openclaw.md) Closed |
