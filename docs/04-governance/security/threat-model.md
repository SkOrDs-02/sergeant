...
| ----------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Login + sessions (Better Auth)                        | S      | Better Auth password-hash; UA-drift detection; session revoke-on-password-change ([H3](./hardening/H3-session-revoke-and-binding.md), Closed)                                                                                                                   | Anonymous CGNAT abuse — partially covered by per-IP fallback rate-limit; secondary cap = [M9](./hardening/M9-per-ip-secondary-rate-limit.md) Closed |
| `/api/mono/webhook`                                   | S/T    | HMAC signature verify; replay-window via timestamp; idempotency key; bad-payload metric; internal `/api/push/send` IP allowlist ([M14](./hardening/M14-internal-push-ip-allowlist.md), Closed)                                                                  | None                                                                                                                                                |
...
| Rate-limit (`apps/server/src/http/rateLimit.ts`)      | D      | Per-user bucket; per-IP fallback; structured `429` payload                                                                                                                                                                                                      | None — covered by [M9](./hardening/M9-per-ip-secondary-rate-limit.md) Closed                                                                        |
| `transcribe` / `chat` AI endpoints                    | D/E    | USD-cap per user ([H9](./hardening/H9-transcribe-usd-cap.md), Closed); chat tool-iteration cap ([M7](./hardening/M7-chat-tool-iteration-cap.md), Closed); OpenClaw daily $5 cap with per-call cap ([M18](./hardening/M18-openclaw-per-call-usd-cap.md), Closed) | None                                                                                                                                                |
...
| Logs (Pino)                                           | I/R    | `userId` hashed before log ([L10](./hardening/L10-user-id-hash-in-logs.md), Closed); structured Sentry forward                                                                                                                                                  | None — [I7](./hardening/I7-security-events-openclaw.md) Closed                                                                                      |
...
| Frontend CSP (`Content-Security-Policy-Report-Only`) | I/T    | C2 report-only baseline + Sentry/PostHog allowlist ([L11](./hardening/L11-csp-monitoring-allowlist.md), Closed); `cspMonitoringAllowlist.test.ts`       | Strict/enforce CSP remains a follow-up in [C2](./hardening/C2-frontend-csp.md); API CSP is enforced separately by Helmet. |
...
| Cleartext HTTP (`exp://` / `http://`) | I      | Capacitor `cleartext: false`; iOS `NSAppTransportSecurity = false`; Android `usesCleartextTraffic="false"`                                  | None — [L12](./hardening/L12-ios-app-transport-security.md) Closed |
...
| Security-event push          | R      | Sentry / Pino metrics: `auth.session.ua_drift`, `prompt_injection_attempt`, `transcribe.usd_cap_hit`, `chat.tool_iteration_cap_hit`, `mono_webhook_bad_payload` | None — [I7](./hardening/I7-security-events-openclaw.md) Closed |
