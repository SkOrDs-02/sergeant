...
| Ліміт       | `AI_QUOTA_PRESET_WEEKLY_LIMIT`, default `12` **на відро** (≤4 повідомлення × 2 запити + 1 повтор) |
| Cost        | `1`, як у `default`                                                                               |
| 429         | `code: "AI_QUOTA_PRESET"` — текст веде до безкоштовного ручного заповнення                        |

**Стеля зловживання.** Поле `preset` приходить від клієнта, і сервер не може
перевірити, що це справді заповнення профілю — будь-хто може причепити
`preset` до звичайного запиту й обслужитись із цього відра замість денного.
Захисту від цього немає в принципі, є лише обмеження масштабу: тижневе вікно
замість денного. При денному вікні стеля була б `+12` **щодня** (~84 на
тиждень) на кожен preset; при тижневому — `+12` на весь тиждень на кожен.
Сумарно сьогодні це `+24` на тиждень понад денні 5 (два preset-и × 12).
...
стеля множиться на кількість preset-ів; додаєш новий → перерахуй її.

Другий чинник, що робить зловживання малопривабливим: разом із preset-ом
сервер підмішує інструкцію режиму, тож зловмисник отримує не універсальний
LLM, а асистента, якому наказано ставити питання про профіль.
