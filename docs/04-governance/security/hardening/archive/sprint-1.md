...
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/04-governance/security/hardening/sprint-1.md`.
...
| ---------------------------------------- | -------------------------------------------------------------------------------- | -------- | ------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [C1](./C1-mono-webhook-secret-in-url.md) | Mono webhook secret з URL → header + sanitize logs + rotate                      | Critical | 1d     | backend  | **Closed** (2026-06-01, mitigated-final) — Phase 1 (log-redaction) + secret rotation (`rotateSecret.ts`, 90d) shipped. Monobank header rollout dropped: `/personal/webhook` is path-only `webHookUrl`, no custom-header/signature. Residual risk accepted. |
