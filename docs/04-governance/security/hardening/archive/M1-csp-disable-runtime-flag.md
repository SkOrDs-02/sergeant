...
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/04-governance/security/hardening/M1-csp-disable-runtime-flag.md`.
...
- Document the change in `docs/04-governance/security/access-policy.md` so operators know
...
- `docs/04-governance/security/access-policy.md` — note "no runtime CSP kill switch".
- Railway dashboard — remove `CSP_DISABLE` from production / staging projects
  (record removal in `docs/04-governance/security/secret-ownership-register.md`).
...
- `docs/04-governance/security/access-policy.md` — додано секцію `Runtime security knobs`, що формалізує "no runtime CSP kill switch" + загальний принцип щодо feature flags та audit trail.
...
# → no matches in code; only mentions у /docs/04-governance/security/hardening/* (історичний контекст)
```
...
- Записати видалення у `docs/04-governance/security/secret-ownership-register.md`.
