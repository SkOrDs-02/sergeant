...
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/04-governance/security/hardening/sprint-3.md`.
...
| ---------------------------------------------- | ---------------------------------------------------- | -------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [H4](./H4-encryption-key-rotation.md)          | No rotation procedure for AES-256-GCM data keys      | High     | backend  | 1.5 d _(closed 2026-06-01 — Phase 1 PR [#1679](https://github.com/Skords-01/Sergeant/pull/1679) (Better Auth); Phase 2 Mono `mono_connection.token_\*` versioned KeyRing, migration 074)\_                                                                                                                                    |
...
| [M9](./M9-per-ip-secondary-rate-limit.md)      | Mass-account abuse can scale rate-limit linearly     | Medium   | backend  | 0.5 d _(closed 2026-06-01 — `ipLimit` secondary per-IP bucket in `rateLimitExpress`; `RATE_LIMIT_IP_MAX` env var (default 200 r/min); distinguishable 429 codes `RATE_LIMIT_IP` / `RATE_LIMIT_USER`; no SQL migration needed)_                                                                                                |
...
| [M18](./M18-openclaw-per-call-usd-cap.md)      | OpenClaw daily $5 budget without per-call cap        | Medium   | console  | 0.1 d _(closed 2026-05-04 — PR [#1760](https://github.com/Skords-01/Sergeant/pull/1760); pre-flight cost estimator + guard in `tools/openclaw/src/openclaw/policy.ts`; metric `openclaw.per_call_cap_hit_total` wired)_                                                                                                       |
...
| [M21](./M21-coep-stripe-compatibility.md)      | `COEP: require-corp` may break Stripe / OAuth iframe | Medium   | frontend | 0.25 d _(closed 2026-05-05, doc-only — batched L4 + L5 + L6 + M21 hardening PR; canonical compatibility matrix + verification recipe added to `docs/03-operations/deploy/vercel.md` Third-party iframe section; no `vercel.json` change required because `require-corp` is load-bearing for SQLite-WASM crossOriginIsolated)_ |
| [I1](./I1-codeql-workflow.md)                  | Add CodeQL SAST workflow                             | Info     | platform | 0.5 d _(closed 2026-05-04 — `.github/workflows/codeql.yml` SHA-pinned with `security-extended` + `security-and-quality` suites; runbook + triage in `docs/04-governance/security/codeql.md`)_                                                                                                                                 |
...
- **H4 Phase 2:** Mono `mono_connection.token_*` migration
  `074_mono_token_key_version.sql` (additive nullable `token_key_version
SMALLINT`) + `mono/crypto.ts` ring-aware encrypt/decrypt + `mono/tokenStore.ts`
  read-with-lazy-reencrypt; dual-key ring honours `MONO_TOKEN_ENC_KEYS=v1:…,v2:…`
  - `MONO_TOKEN_ENC_KEY_CURRENT_VERSION`. Legacy unversioned ciphertext
    (`token_key_version=NULL`) decrypts transparently as v1; lazy re-encrypt
    observed via `mono_token_lazy_reencrypt_total{row_version,outcome}` and is
    best-effort (a failed write never breaks the request). _Closed 2026-06-01._
- **M11/I1:** CodeQL produces ≤ 5 findings on the first scheduled run; any
  remaining finding has a triage entry in `docs/04-governance/security/audit-exceptions.md`.
