...
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/04-governance/security/hardening/M15-console-allowlist-fail-closed.md`.
...
`tools/openclaw/src/security.ts` parses `CONSOLE_ALLOWED_TG_USER_IDS` and
...
- `tools/openclaw/src/security.test.ts` — add the table-driven test.
- `tools/openclaw/src/security.ts` — if needed, defensively short-circuit the
...
- `tools/openclaw/src/security.test.ts` — replaced the dev-only
