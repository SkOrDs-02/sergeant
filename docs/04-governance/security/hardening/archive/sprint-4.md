...
> **Last validated:** 2026-05-13 by Codex. **Next review:** ніколи (read-only архів).
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/04-governance/security/hardening/sprint-4.md`.
...
| [L2](./L2-permissions-policy-broader.md)   | Permissions-Policy could disable more APIs           | Low      | frontend | 0.1 d _(closed 2026-05-06 — header extended in `apps/web/vercel.json`; regression test `apps/web/src/test/permissionsPolicyHeader.test.ts` locks the directive set)_                                                                      |
...
| [I6](./I6-threat-model.md)                 | Document the STRIDE threat model per module          | Info     | platform | 1 d _(closed 2026-05-06 — `docs/04-governance/security/threat-model.md` shipped with per-module STRIDE tables + cross-cutting controls; linked from `docs/04-governance/security/README.md`)_                                             |
| [I7](./I7-security-events-openclaw.md)     | Push security events to OpenClaw                     | Info     | backend  | 1 d _(closed 2026-06-01 — typed emitter `securityEvents.ts` + call sites in `webhook.ts`, `auth.ts`, `chat.ts`, `transcribe.ts` + OpenClaw consumer `securityRoom.ts` + runbook `docs/03-operations/runbooks/security-events.md`)_        |
...
- I3–I7 land as enabled features with at least one verification artefact
  ...
  `docs/04-governance/security/threat-model.md` published, OpenClaw security topic with
