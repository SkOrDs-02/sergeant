...
> **Status:** Active.
> **Scope:** Server logs (Pino), Sentry payloads, Loki/Grafana retention,
> in-process error captures. Mobile/web log buffers — see
...
- `apps/server/src/obs/logger.ts` — `redactPaths` (Pino) + `redactKeyNames` (Sentry).
- `apps/server/src/sentry.ts` — `scrubPII()` (рекурсивний, case-insensitive, працює на `event.extra` / `event.contexts` / `event.breadcrumbs.data`).
...
`redactKeyNames` (для Sentry) і `redactPaths` (для Pino) зберігаються
у тому самому файлі (`logger.ts`) і експортуються як named bindings.
Якщо додаєш новий ключ-кандидат — додай його в обидва списки і
оновлюй цей документ у тому самому PR.
...
- [`apps/server/src/obs/logger.ts`](../../apps/server/src/obs/logger.ts) (Pino config)
- [`apps/server/src/sentry.ts`](../../apps/server/src/sentry.ts) (Sentry beforeSend)
