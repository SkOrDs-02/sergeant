...
| [M21](./M21-coep-stripe-compatibility.md)      | `COEP: require-corp` may break Stripe / OAuth iframe | Medium   | frontend | 0.25 d _(closed 2026-05-05, doc-only — batched L4 + L5 + L6 + M21 hardening PR; canonical compatibility matrix + verification recipe added to `docs/deploy/vercel.md` Third-party iframe section; no `vercel.json` change required because `require-corp` is load-bearing for SQLite-WASM crossOriginIsolated)_ |
