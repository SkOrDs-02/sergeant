...
| [I7](./I7-security-events-openclaw.md)     | Push security events to OpenClaw                     | Info     | backend  | 1 d _(closed 2026-06-01 — typed emitter `securityEvents.ts` + call sites in `webhook.ts`, `auth.ts`, `chat.ts`, `transcribe.ts` + OpenClaw consumer `securityRoom.ts` + runbook `docs/runbooks/security-events.md`)_                      |
