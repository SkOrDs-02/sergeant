...
| Пов'язані артефакти          | `.trivyignore`, `Dockerfile.api`, `Dockerfile.openclaw`, `docs/ops/docker-image-policy.md`, `docs/tech-debt/priority-1-executive.md` AP-03 |
...
3. **Немає CI-job-а, що б будував + сканував `Dockerfile.openclaw`** — Trivy зараз
...
4. **CI-прохідність** — `.github/workflows/container-scan.yml` білдить тільки
   `Dockerfile.api` і тільки linux/amd64. `Dockerfile.openclaw` без Trivy-гейта,
...
6. CI-гейт для `Dockerfile.openclaw` — окремий follow-up PR (вже
...
- [ ] CI-гейт для `Dockerfile.openclaw` — окремий PR (PR-30 follow-up).
...
> - [ ] Окремий follow-up: додати Trivy CI gate для `Dockerfile.openclaw`
