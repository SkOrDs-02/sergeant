...
Політика для runtime container-image-ів Sergeant: Hub API (`Dockerfile.api`) і Console / OpenClaw (`Dockerfile.openclaw`). Описує rationale за distroless multi-stage build (PR-30 — `docs/initiatives/stack-pulse-2026-05/pr-30-dockerfile-cleanup-cve.md`), CVE-бюджет, healthcheck-семантику, і rollout-послідовність.
