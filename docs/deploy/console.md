# Deploy `tools/console` (sergeant-openclaw)
...
**Що _не_ змінюється.** `tools/console` directory, `@sergeant/console` npm-package name, `Dockerfile.console`, `railway.console.toml` config-as-code path, healthcheck `GET /healthz`, secret-token. Усі endpoint paths (`/webhook/openclaw`, `/healthz`) ідентичні; зміна тільки у host-частині URL.
