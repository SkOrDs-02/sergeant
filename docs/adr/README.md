...
| 0027 | Політика OpenClaw, Console та MCP               | accepted | 2026-04-27 | `tools/console` як internal admin; allowlist по Telegram user-id; вивід агента — untrusted.                                                                                                                                                                                                   |
...
| 0035 | Distributed tracing — OpenTelemetry             | accepted | 2026-05-05 | OTel web→server (server-side shipped 2026-05-05). OTLP/HTTP exporter, vendor-agnostic; sample 10%/5%; Sentry browserTracing вимикається опційно через env.                                                                                                                                    |
...
| 0039 | Anthropic prompt-cache breakpoint policy        | accepted | 2026-05-04 | 2 `cache_control: ephemeral` breakpoints (`system[0]` + last `tool`) — кешуємо `SYSTEM_PREFIX` + tools registry; messages та per-user `system[1]` НЕ кешуються; 30%-floor для rollback. Закриває [Initiative 0005](../initiatives/_0005-ai-cost-and-prompt-cache.md).                         |
| 0041 | OpenClaw Telegram delivery via webhook          | accepted | 2026-05-03 | Feature-flag-gated `node:http` webhook server у `tools/console` (за замовчуванням off → long-poll); знижує latency approval-кнопок з 2-3с до <500мс; one-step backout через `OPENCLAW_USE_WEBHOOK=false`.                                                                                     |
...
| 0044 | Renovate vs Dependabot роль-дільниця            | accepted | 2026-05-04 | Renovate primary для regular weekly bumps; Dependabot security-only daily fallback. Видаляє ~12 duplicate-PR/тиждень. Закриває [Initiative 0008 Phase 3](../initiatives/_0008-platform-hardening.md).                                                                                         |
...
| 0050 | TypeScript major-version policy + `@types/node` pinning | accepted | 2026-05-06 | TS 6.x для server/web/packages, TS 5.9.x для mobile (Expo SDK 52), TS 5.7.x для tools/console; `@types/node` pinned до `^20` через `pnpm.overrides` + Renovate `allowedVersions: <21`. Closes stack-pulse PR-05.                          |
| 0051 | Pricing v3 — Free + Pro, single paid tier               | accepted | 2026-05-06 | Single paid tier $7/міс / $49/рік (₴/UA-only at launch), 7-day no-card trial, decoy/lifetime/pay-per-feature alternatives відкладені. Supersedes pricing sections (§2.2/§2.3/§3) в `docs/launch/business/01-monetization-and-pricing.md`. |
| 0052 | Mobile strategy — Capacitor primary                     | accepted | 2026-05-06 | `apps/mobile-shell` (Capacitor wrapper навколо `apps/web`) — primary mobile product; Expo `apps/mobile` під sunset (T₀/T₁/T₂ schedule).                                                                                                   |
...
> **Note on next ADR:** наступний номер — **`0055`** (`0029` і `0040` лишаються gap-ами).
