...
- **Related:**
  ...
  - [`docs/adr/0045-hard-rules-taxonomy.md`](./0045-hard-rules-taxonomy.md) — precedent for JSON registry as source of truth
...
1. **Single JSON aggregator** — один файл `docs/governance/knowledge-graph.json` зі схемою `{ nodes, edges }`, генерується з усіх існуючих silos. Schema у `docs/governance/schemas/knowledge-graph.schema.json`.
2. **SQLite on disk** — `docs/governance/knowledge-graph.sqlite` як binary артефакт; query через `better-sqlite3`. Дозволяє SQL-запити, складніша інтеграція в Git.
...
- **Schema:** `docs/governance/schemas/knowledge-graph.schema.json` (JSON Schema draft-07).
- **Storage:** `docs/governance/knowledge-graph.json` (auto-gen, committed) + `docs/governance/knowledge-graph.html` (HTML viewer з inline CSS і Mermaid sub-графами per node type).
