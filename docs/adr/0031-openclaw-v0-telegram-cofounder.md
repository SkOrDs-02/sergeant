...
`@Sergeant_alert_bot` + `apps/console`). Це **deep-dialogue** партнер для
...
- [ ] Server: `apps/server/src/routes/internal/openclaw.ts` — 5 routes
      ...
      `INTERNAL_API_KEY`.
- [ ] Console: `apps/console/src/agents/openclaw.ts` — agent definition
      with 7 tools (6 read + 1 narrow write).
- [ ] Console: `apps/console/src/openclaw/` — bot handler (DM-only
      enforcement, allowlist, multi-turn session, budget pre-check).
- [ ] Console: refactor `apps/console/src/index.ts` → run two bots in
...
- [ ] Update `docs/launch/openclaw-roadmap.md` §4.3 → Resolved decisions
      (link до цього ADR).
- [ ] Update `apps/console/README.md` — другий bot.
- [ ] CHANGELOG entry.
