...
`VALID_MODULES` явно whitelist-ує імена (`apps/server/src/modules/sync/syncV2.ts`),
...
`client_updated_at <= NOW()` завжди true ([syncV2.ts](../../apps/server/src/modules/sync/syncV2.ts) коментар).
