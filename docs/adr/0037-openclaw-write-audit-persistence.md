...
ADR-0036 (Phase 4) запустив 5 write-tools з founder-approval gate-ом. Кожен Approve / Reject / Executed transition зараз логуються одним `console.log("[openclaw] write-tool …")` рядком у `tools/console`. Це достатньо для smoke-test-у в перші тижні, але уже в Phase 4 acceptance-у виявилися два gap-и:
...
- `tools/console/src/openclaw/handler.ts` — callback handler wiring + `/audit` slash command.
