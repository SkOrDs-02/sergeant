...
> **PR-18** з [FTUX master tracker](../../work/specs/launch/product-os/ftux-master-tracker.md)
...
1. **Створи markdown** `docs/product/whats-new/YYYY-MM-DD-<slug>.md` за шаблоном
...
## Чернетка між релізами — `_draft-next.md`

Фікси накопичуються не по одному: між викатками бети їх збирає
[`_draft-next.md`](./_draft-next.md), і **запису в `releases.ts` для нього
немає навмисно**.

Причина в тому, що `releases.ts` — не беклог, а черга показу. Новий запис
миттєво стає `RELEASES[0]` і виїде користувачам на найближчому деплої, а
модалка одноразова: показавши недописаний список, переграти його не можна
(правити старий запис заборонено — п. 4 вище).

Drift-гейт це дозволяє: `releases.test.ts` вимагає markdown для **кожного
запису в `releases.ts`**, але не навпаки. Markdown без TS-запису — легальний
стан, і саме на ньому тримається чернетка.

Порядок випуску описаний у самому `_draft-next.md` § «Як випустити».
