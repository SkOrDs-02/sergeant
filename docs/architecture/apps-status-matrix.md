...
| `@sergeant/db-schema`           | `packages/db-schema`                     | `active`    | Drizzle ORM-схеми (Postgres + SQLite) і shared migration runner для `apps/server`. Зміни схеми завжди в парі з SQL-міграцією у `apps/server/src/migrations/`. | [`packages/db-schema/src`](../../packages/db-schema/src), [AGENTS.md rule #4](../../AGENTS.md)                               |
