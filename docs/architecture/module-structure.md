...
| **mobile/finyk**                | `hooks/` є, але мінімальний (3 файли boot-хуків)           | Mobile-finyk — переважно read-only view над web-станом; `hooks/` містить `useFinykDualWriteBoot.ts`, `useFinykMonoMirrorBoot.ts`, `useFinykSqliteReadBoot.ts` — boot/sync логіка при старті. Нові mutation-хуки додаються сюди за потреби.                                                                                                                      |
