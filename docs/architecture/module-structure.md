...
| ------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **web/finyk**                   | Немає `shell/`, є `hubRoutineSync.ts` поруч                | Cross-module sync (HubChat ↔ Routine via Mono webhooks) висить на одному файлі — поки рано виносити в окрему директорію. Routing — централізовано через `apps/web/src/core/lib/moduleRoutes.ts` + локальний router.      |
