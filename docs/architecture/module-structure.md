...
| **web/nutrition**               | Немає `pages/`, замість того `domain/`                     | Pages-mesh нативно в `components/NutritionBottomNav.tsx` + локальні `hooks/useNutritionRoute.ts` + `lib/nutritionRouter.ts` (react-router @7 path-router після initiative 0006 Phase 2.a, [#2104](https://github.com/Skords-01/Sergeant/pull/2104)); `domain/` тримає чисту бізнес-логіку backup/restore (не React). Доки 6 screens, окрема `pages/` — overkill. |
