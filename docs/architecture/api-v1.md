...
- `/api/auth/*` виключено — Better Auth-плагіни зашиті під `basePath: "/api/auth"`.
...
- Better Auth-плагіни (react client, Expo client) зашиті під фіксований `basePath: "/api/auth"` — змінити його без кастом-форків неможливо.
...
- `POST /api/v1/push/register` — див. [`docs/mobile/overview.md`](../mobile/overview.md).
- `GET /api/v1/me` — уніфікований "хто я" для web cookie і mobile bearer.

Усі решта endpoint-ів `v1` — це дзеркало існуючих `/api/*` без змін у контрактах. Додавати нові endpoint-и — **ЗАВЖДИ** одразу під `/api/...`; дзеркало під `/api/v1/...` вмикається автоматично.
...
Не раніше, ніж закінчиться rollout мобільного клієнта І жоден ендпоінт у фронтенді не ходить через `VITE_API_VERSION=none`. Зараз приблизний таргет — після тесту мобільного beta-релізу.

---

## 📌 Related docs

- [`docs/mobile/overview.md`](../mobile/overview.md) — API контракт для mobile client
- [`docs/architecture/service-catalog.md`](./service-catalog.md) — runtime inventory з `/api/v1/*` endpoints
