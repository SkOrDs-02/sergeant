...
>
> **2026-05-04 update (follow-up #1).** [#1671](https://github.com/Skords-01/Sergeant/pull/1671) виконав перший rollout — `apps/web/src/core/auth/AuthPage.tsx` повністю переписано: 426 LOC з 11×`useState` + 4×`validate*`-функції замінено на дві sub-форми `LoginForm` / `RegisterForm`, обидві на `useApiForm` + zod-схеми (`loginSchema`, `registerSchema`). Mode-switch робиться через `key={mode}` — re-mount гарантує clean state. Витягнуто `PasswordVisibilityToggle`, `PasswordStrengthBar`, `FieldError` як окремі компоненти. Додано `apps/web/src/core/auth/AuthPage.test.tsx` — 8 нових RTL-тестів (zod-валідація, happy path login/register, error handling, mode switch, name fallback).
>
> **2026-05-04 update (follow-up #2 — round 7).** [#1696](https://github.com/Skords-01/Sergeant/pull/1696) переніс на `useApiForm` третій (і останній) auth-екран — `apps/web/src/core/auth/ResetPasswordPage.tsx`. Видалено мікс `useFormValidation` + 5 локальних `useState` (password/confirm/status/serverError/passwordRef). Додано `resetPasswordSchema` з `superRefine` для cross-field перевірки `confirm === password`. Better Auth повертає помилки через `result.error` поле, тому `onSubmit` кидає `Error(message)` — `useApiForm.serverError` його одразу мапить на верхній банер з `role="alert"`. Native `<input>` замінено на shared `<Input>` компонент. Додано 7 нових RTL-тестів. **Auth-flow покриття `useApiForm` тепер 3/3 екрани** (login + register + reset). Залишилось мігрувати: finyk форми (Mono-token, transactions edit), fizruk (workout templates), nutrition (meal/recipe edit), routine (habit create/edit), і `ManualExpenseSheet` (єдиний реальний споживач `useFormValidation`, що залишився).
>
> **2026-05-04 update (follow-up #3 — round 8).** [#1724](https://github.com/Skords-01/Sergeant/pull/1724) виніс `useApiForm` rollout за межі auth-флоу — перший settings-form: `apps/web/src/core/profile/ChangePasswordSection.tsx`. Замінено 3 `useState` (current / next / confirm) + ручний onSubmit-flow на `useApiForm` + `changePasswordSchema` (zod з `superRefine` для cross-field `confirm === next`). Better Auth `result.error.message` мапиться через `throw new Error()` → `serverError` банер з `role="alert"`. Додано 6 нових RTL-тестів (`apps/web/src/core/profile/ChangePasswordSection.test.tsx`): порожні поля → zod errors + no API call; короткий новий пароль → "Мінімум 10 символів"; mismatch next/confirm → "Паролі не збігаються"; happy path → `changePassword` called + toast + fields cleared; server error (`result.error`) → alert банер; `online=false` → submit disabled. **`useApiForm` тепер у 4 екранах** (login + register + reset + change-password у профілі) — підтверджує, що шаблон масштабується від auth-форм до settings без додаткових абстракцій.
...
**Tracker hook.** Після того, як вибір зроблено, винести план у `docs/audits/2026-04-28-ux-improvement-plan.md` з KPI «N out of 30 forms migrated».
...
> **Tracker hook.** Коли з'явиться чек-лист реалізації, винеси у `docs/audits/2026-04-28-ux-improvement-plan.md` (Active) з KPI на квартал.
