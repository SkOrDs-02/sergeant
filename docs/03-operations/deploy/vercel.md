...
- Coolify** (image built by GitHub Actions → `ghcr.io`) — see
  [`../../04-governance/adr/0074-hosting-hetzner-coolify.md`](../../04-governance/adr/0074-hosting-hetzner-coolify.md)
  ...
  The Vercel edge-proxy (`/api/*` → `BACKEND_URL`) and same-origin cookie contract are
...
When tightening a header, follow [`../security/hardening/C2-frontend-csp.md`](../../04-governance/security/hardening/C2-frontend-csp.md)
...
4. If a block fires:
   - **CSP:** widen the relevant directive in `vercel.json` and re-test —
     ...
     rollout window per [C2](../../04-governance/security/hardening/C2-frontend-csp.md).
...
- [`../integrations/railway-vercel.md`](../../02-engineering/integrations/railway-vercel.md) — Vercel env + проксі-контракт (Railway-секції історичні)
- [`../adr/0009-hosting-split-railway-vercel.md`](../../04-governance/adr/0009-hosting-split-railway-vercel.md) — попередній Railway split (superseded ADR-0074)
