...
> ⚠️ **OpenClaw повністю decommissioned ([ADR-0075](../../04-governance/adr/0075-openclaw-gateway-decommissioned.md), 2026-07-20).** Після grammy-бота тепер прибрано й сам gateway (`Dockerfile.openclaw-gateway`, `packages/openclaw-plugin`, `ops/openclaw`, серверний `modules/openclaw`). Файл лишено як історичний запис (лінкується immutable-ADR 0032/0041).

> ⚠️ **REMOVED 2026-06-08.** Цей grammy-бот (`tools/openclaw` workspace + `Dockerfile.openclaw` + `railway.openclaw.toml`) і Railway-сервіс `sergeant-hubchat` **видалено** (Locked Decision #17 executed). Жива OpenClaw-поверхня тепер — **gateway** (`sergeant-openclaw-gateway`, `Dockerfile.openclaw-gateway`, `packages/openclaw-plugin`). Цей файл лишено як історичний record.
...
- **Dockerfile:** `Dockerfile.openclaw` (root монорепо).
- **Config-as-code:** `railway.openclaw.toml` — `builder=DOCKERFILE`, `restartPolicyType=ON_FAILURE`.
- **Deploy filter (`watchPatterns`):** запис у Railway DB (не у `railway.openclaw.toml` — Railway не дозволяє config-as-code для git-trigger полів). Поточний набір — `tools/openclaw/**`, `packages/config/**`, `Dockerfile.openclaw`, `railway.openclaw.toml`, root manifests (`package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `turbo.json`, `.npmrc`, `.nvmrc`), `patches/**`. Канонічна версія + причина — у [`./monorepo-deploy-filtering.md`](./monorepo-deploy-filtering.md). Long-poll grammy-боти не люблять зайві рестарти, тому patterns тримаємо вузькими.
...
mutation {
  serviceInstanceUpdate(
    ...
    input: { railwayConfigFile: "railway.openclaw.toml" }
  ) {
    id
  }
}
...
**Що _не_ змінюється.** `tools/openclaw` directory, `@sergeant/openclaw` npm-package name, `Dockerfile.openclaw`, `railway.openclaw.toml` config-as-code path, healthcheck `GET /healthz`, secret-token. Усі endpoint paths (`/webhook/openclaw`, `/healthz`) ідентичні; зміна тільки у host-частині URL.
