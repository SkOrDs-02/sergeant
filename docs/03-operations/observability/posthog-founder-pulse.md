...
- **Auto-import.** Manifest у `ops/posthog/dashboards/founder-pulse.json` — portable shape. Зараз — manual import via PostHog UI. Auto-import (CLI або n8n) — окремий PR під [PR-11 з pr-plan-2026-05](../../90-work/planning/archive/pr-plan-2026-05.md) (WF-16 розширення або новий планований скрипт `scripts/posthog/import-dashboard` (`.mjs`)).
...
- **AI-порада.** `ai_advice_shown` / `ai_advice_reacted` мають web-callsite-и
  ...
  **id**: він КЛІЄНТСЬКИЙ, тож одна й та сама денна порада, відкрита у вебі й
  на телефоні, дасть два різні `advice_id`. Придатно для «побачив →
  зреагував», НЕпридатно для «скільки унікальних порад згенеровано» — це
  вимагає серверного id (окрема стадія).
