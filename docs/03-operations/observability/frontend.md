...
> **Last touched:** 2026-08-27 by @claude. **Next review:** 2026-12-09.
> **Status:** Active
...
- `capture_exceptions` — **єдиний увімкнений автокаптур** попри
  ...
  Sentry.
- `enable_heatmaps: true` — другий і останній виняток із `autocapture: false`.
  Шле окремі `$$heatmap` події з **координатами** кліків і глибиною скролу,
  згрупованими по `$current_url`. DOM-вміст, текст елементів і значення полів
  НЕ збираються, тож PII-поверхня не зростає. Дивитись: PostHog → Heatmaps
  (або тулбар на самому сайті). Увімкнено, щоб бачити, куди тицяють ті ~81%,
  які заходять у застосунок і не починають онбординг, — подієва телеметрія
  каже «пішли», але не каже «куди тицяли».
- `session_recording` — replay увімкнено з **максимальним маскуванням**:
  `maskAllInputs: true` (жодне значення поля вводу не пишеться) і
  `maskTextSelector: "*"` (увесь текст замінюється заглушками). У записі
  лишається геометрія — розкладка, кліки, скрол, переходи, час на екрані.
  Тобто «де людина застрягла» видно, «що саме в неї написано» — ні.
  **Не послаблювати без рішення власника та оновлення політики
  приватності:** застосунок тримає фінанси, харчові щоденники й записи про
  травми. Останні — дані про здоровʼя, а це **особлива категорія** за
  GDPR Art. 9. Фінанси до Art. 9 НЕ належать (перелік там вичерпний), але
  лишаються чутливими за загальними ст. 5 і 32; підсумкову класифікацію
  для Sergeant визначає затверджена політика приватності. Точковий опт-аут
  для окремого вузла — клас `ph-no-capture` (дефолтний `blockClass`).
  Залежність: `worker-src blob:` у CSP (rrweb стискає запис у web worker)
  — директива вже є в `apps/web/vercel.json`, див.
  [`C2-frontend-csp.md`](../../04-governance/security/hardening/C2-frontend-csp.md).
  Замовляти запис клієнт може лише якщо в самому проєкті PostHog
  увімкнено **Session recording** (Settings → Project → Recordings) —
  без серверного тумблера SDK нічого не пише.
...
**Історично** (до v1 sunset у [ADR-0047](../../04-governance/adr/0047-cloudsync-v1-410-gone.md))
...
> **Статус (2026-08-10).** Ця секція до сьогодні описувала намір як факт:
> ні скрипта, ні воркфлоу не існувало — ані в робочому дереві, ані в історії
> git (`git log --all --diff-filter=D` порожній), і в проєкті `sergeant-prod`
> було рівно **0 анотацій**. Тепер обидва файли реально є.
>
> **Щоб анотації почали зʼявлятися, потрібен секрет** `POSTHOG_PERSONAL_API_KEY`
> (Settings → Secrets and variables → Actions). Опційні `vars`:
> `POSTHOG_PROJECT_ID` (дефолт `167740`), `POSTHOG_HOST` (дефолт
> `https://eu.posthog.com`). Джоб **fail-open**: без секрету він зелений і
> просто друкує причину — анотація це зручність для тріажу, а не гейт, і
> не повинна червонити `main` після кожного мерджу. Помилки API теж не
> валять збірку, а логуються як `::warning`.
...
Пропуск відбувається **лише** за відсутнього `POSTHOG_PERSONAL_API_KEY`:
скрипт друкує причину і виходить з exit 0 (graceful no-op), workflow не
червонітиме. `POSTHOG_PROJECT_ID` і `POSTHOG_HOST` — опційні: без них
беруться дефолти (`167740` / `https://eu.posthog.com`), і анотація **все
одно постить**. Тобто на форку з чужим ключем, але без `POSTHOG_PROJECT_ID`,
запис піде у прод-проєкт Sergeant — задавай `POSTHOG_PROJECT_ID` явно, якщо
середовище не sergeant-prod.
