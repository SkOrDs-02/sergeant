...
| **No SLA/SLO surface in chat**     | "Чи горить error budget?" — треба окремий dashboard. Telegram як alert hub не показує health, тільки точкові події.                                                                                 | Grafana покриває, посилання на дашборд є в `#ops` стрічці. Для phase 2 — Grafana Cloud, див. [`hosting-evolution.md`](../../02-engineering/architecture/hosting-evolution.md). |
| **Bot повна довіра**               | Якщо Telegram bot token компромет — атакер може писати від імені бота куди-завгодно у supergroup. На відміну від email — bot tokens в одному secrets-store.                                         | Rotate via [`docs/00-start/playbooks/rotate-secrets.md`](../../00-start/playbooks/rotate-secrets.md) → Telegram revoke + new BotFather token.                                  |
