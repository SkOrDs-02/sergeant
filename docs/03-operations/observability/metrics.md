...
**v2 op-log per-op (`sync_op_log_apply_total`)**: `table` ∈ whitelist `OP_LOG_TABLE_REGISTRY` + `__unknown__`; `status` ∈ `applied|rejected|duplicate`; `reason` — закритий allowlist (PR #043c + PR #042a/b, Stage 5): `none` (для applied) + 5 engine-level (`clock_skew` · `table_not_allowed` · `apply_failed` · `duplicate` · `op_not_supported`) + 48 apply-fn-level (`lww_conflict` · `tombstoned` · `not_found` · `delete_not_supported` · `user_id_mismatch` · `fk_violation` · `append_only_violation` + 12 `missing_*` + 29 `invalid_*`). Source-of-truth: `APPLY_REJECT_REASONS` / `ENGINE_REJECT_REASONS` у [`syncV2.ts`](../../../apps/server/src/modules/sync/syncV2.ts) — TS-union блокує emit невідомого літерала на compile-time, regression-тест у [`metrics.test.ts`](../../../apps/server/src/obs/metrics.test.ts) фіксує довжину allowlist-у. Cardinality cap: ~28 tables × 3 statuses × 60 reasons ≈ 5_040 series worst-case (phenomenologically <100 — більшість табл/reason пар не зустрічаються одночасно). PR #042a розширив engine-level allowlist `op_not_supported` як gate на `op='increment'` для таблиць поза `INCREMENT_OP_SUPPORTED_TABLES`; PR #042b опт-інив `routine_streaks` у whitelist + додав apply-level `missing_delta`/`invalid_delta` для PN-counter payload-валідації. W1-ROUTINE-APPEND (стадія 1) додав таблицю `routine_completion_events` у `OP_LOG_TABLE_REGISTRY` (43 таблиці) і apply-level `append_only_violation` — гейт на `op='update'|'delete'` для append-only журналу відміток: подія незмінна, виправлення історії = НОВА подія.
...
`provider` — фіксована множина: `hetzner` · `vercel` · `posthog` · `sentry` · `anthropic` · `voyage`. `plan` — стандартні tiers (`free` · `hobby` · `pro` · `team` · `business` · `enterprise` · `usage` · `budget`) або instance-label для VPS (`cx23`); конкретний рядок задається через `*_PLAN` env-vars. Для `hetzner`/`vercel`/`posthog`/`sentry` — реальна subscription cost; для `anthropic`/`voyage` — **budget envelope**, target для алертів run-rate-у.

Виставляється один раз на старті процесу з env-vars (`HETZNER_MONTHLY_COST_USD`, `VERCEL_MONTHLY_COST_USD`, `POSTHOG_MONTHLY_COST_USD`, `SENTRY_MONTHLY_COST_USD`, `ANTHROPIC_MONTHLY_BUDGET_USD`, `VOYAGE_MONTHLY_BUDGET_USD`) у [`index.ts`](../../../apps/server/src/index.ts). Невиставлене / нульове значення → серія НЕ зʼявляється у `/metrics` (gauge не пре-allocate-имо нулі, бо це б змусило в PromQL фільтрувати). Cardinality cap: 6 providers × ~7 plans = **~42** серій (реально 1–6, бо у поточному deployment-і кожен provider має один tier).
...
```promql
sum(infra_monthly_cost_usd{provider=~"hetzner|vercel|posthog|sentry"})       # фіксований monthly burn
...
sum(increase(ai_cost_estimate_usd_total[30d]))
  + sum(infra_monthly_cost_usd{provider=~"hetzner|vercel|posthog|sentry"})   # combined 30d run-rate
...
Сиріт і несумісних label-set-ів **не знайдено**. Всі 36 метрик інкрементуються щонайменше в одному emitter. Канонічний backlog — [`docs/90-work/planning/pr-plan-backend-perf-2026-05.md`](../../90-work/planning/archive/pr-plan-backend-perf-2026-05.md); тут — лише точки болю/розвитку observability-каталогу, які треба пам'ятати при дашборд-ревʼю.

### Нещодавно закрито

- **Per-model token + cost attribution для Anthropic / Voyage** — закрите [PR-04 з backend-perf плану](../../90-work/planning/archive/pr-plan-backend-perf-2026-05.md). До цього PromQL-приклади в [§6](#6-ai-anthropic--voyage) показували лише per-release split через `app_build_info` без single-model breakdown-у; тепер є concrete deep-dive (`model="claude-sonnet-4-5"`, `model="voyage-3.5-lite"`) + повний перелік типових значень `model`-лейблу для обох провайдерів із посиланнями на canonical emitter-и + drift-detector для нових Voyage-моделей без cost-counter серії.
- **Backend `/health` p95 SLO formalization** — закрите [PR-06 з backend-perf плану](../../90-work/planning/archive/pr-plan-backend-perf-2026-05.md). Informal AGENTS.md performance budget ("Backend `/health` p95 < 100 ms") тепер має recording-rule `job:health_p95_5m`, alert `BackendHealthP95High` (`severity=ticket`, `for: 5m`) і повноцінну SLO-секцію у [`SLO.md § 2.1`](./SLO.md#21-health-endpoint-p95) + runbook-anchor у [`runbook.md`](./runbook.md#backendhealthp95high). Раніше pure-AGENTS.md target не мав emitter-trace.
- **`/api/internal/*` Sentry sampling baseline і rule-ordering** — закрите [PR-07 з backend-perf плану](../../90-work/planning/archive/pr-plan-backend-perf-2026-05.md). Широке `match: "/api/internal/"` правило з `rate: 1.0` додано у `SENTRY_SAMPLING_RULES` ([`apps/server/src/sentry.ts:47-52`](../../../apps/server/src/sentry.ts#L47)); специфічніше `/api/internal/openclaw/write/` лишається перед ним за longest-prefix-first ordering. Тест [`__tests__/sentry-sampler.test.ts:87-89`](../../../apps/server/src/__tests__/sentry-sampler.test.ts#L87) фіксує поведінку. **Forward watch-out:** якщо 7-денний Sentry-baseline покаже quota-pressure — знизити rate до `0.5` per [`sentry-sampling.md`](./sentry-sampling.md).
...
2. **`/health/*` nested + short-alias probes** (`/health/liveness`, `/health/readiness`, `/health/startup`, `/livez`, `/readyz`, `/startupz`, `/healthz`) накриті спільним health-SLO §2.1 у `SLO.md`: p95 `< 100ms` через `job:health_p95_5m`. Якщо хоч одна probe-семантика виявиться окремо вартою власного бюджета (типу: `readiness` p95 < 250 ms бо ходить по worker fleet) — додати per-path recording-rules за патерном з PR-06.
