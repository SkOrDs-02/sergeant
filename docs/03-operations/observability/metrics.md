...
`upstream`. Перші пʼять — серійний ланцюг до виклику моделі; `pre_upstream`
накриває **все** наше від входу в handler, включно з парсингом і валідацією.
Тому `pre_upstream` мінус сума названих фаз — це неврахована робота, і саме ця
різниця ловить кроки, яких ми не назвали.
...
**Кардинальність**: 7 phase × (9 buckets + `_sum` + `_count` + `+Inf`) ≈ **84**
серії. Бакети навмисно рідші за сусідні гістограми (`5, 25, 100, 250, 500,
...
**SLO**: `apps/server/AGENTS.md` декларує «Anthropic `/api/chat` p95 first token
< 1.5 s». До першого ходу цей SLO **не застосовний** — першого токена там немає.
Чи стрімити перший хід (і чи переписати SLO під видиме очікування) — відкрите
рішення founder-а, AI-2 у
