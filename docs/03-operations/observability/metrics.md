...
> **Last touched:** 2026-05-14 by Codex. **Next review:** 2026-09-01.
> **Status:** Active
...
Сиріт і несумісних label-set-ів **не знайдено**. Всі 36 метрик інкрементуються щонайменше в одному emitter. Канонічний backlog — [`docs/90-work/planning/pr-plan-backend-perf-2026-05.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/planning/archive/pr-plan-backend-perf-2026-05.md); тут — лише точки болю/розвитку observability-каталогу, які треба пам'ятати при дашборд-ревʼю.

### Нещодавно закрито

- **Per-model token + cost attribution для Anthropic / Voyage** — закрите [PR-04 з backend-perf плану](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/planning/archive/pr-plan-backend-perf-2026-05.md). До цього PromQL-приклади в [§6](#6-ai-anthropic--voyage) показували лише per-release split через `app_build_info` без single-model breakdown-у; тепер є concrete deep-dive (`model="claude-sonnet-4-5"`, `model="voyage-3.5-lite"`) + повний перелік типових значень `model`-лейблу для обох провайдерів із посиланнями на canonical emitter-и + drift-detector для нових Voyage-моделей без cost-counter серії.
- **Backend `/health` p95 SLO formalization** — закрите [PR-06 з backend-perf плану](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/planning/archive/pr-plan-backend-perf-2026-05.md). Informal AGENTS.md performance budget ("Backend `/health` p95 < 100 ms") тепер має recording-rule `job:health_p95_5m`, alert `BackendHealthP95High` (`severity=ticket`, `for: 5m`) і повноцінну SLO-секцію у [`SLO.md § 2.1`](./SLO.md#21-health-endpoint-p95) + runbook-anchor у [`runbook.md`](./runbook.md#backendhealthp95high). Раніше pure-AGENTS.md target не мав emitter-trace.
- **`/api/internal/*` Sentry sampling baseline і rule-ordering** — закрите [PR-07 з backend-perf плану](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/planning/archive/pr-plan-backend-perf-2026-05.md). Широке `match: "/api/internal/"` правило з `rate: 1.0` додано у `SENTRY_SAMPLING_RULES` ([`apps/server/src/sentry.ts:47-52`](../../../apps/server/src/sentry.ts#L47)); специфічніше `/api/internal/openclaw/write/` лишається перед ним за longest-prefix-first ordering. Тест [`__tests__/sentry-sampler.test.ts:87-89`](../../../apps/server/src/__tests__/sentry-sampler.test.ts#L87) фіксує поведінку. **Forward watch-out:** якщо 7-денний Sentry-baseline покаже quota-pressure — знизити rate до `0.5` per [`sentry-sampling.md`](./sentry-sampling.md).
