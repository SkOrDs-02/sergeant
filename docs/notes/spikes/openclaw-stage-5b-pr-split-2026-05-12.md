...
- **Future strategic-modes у Stage 8+.** Voice/Canvas/WhatsApp персони можуть притягнути додаткові slash-команди (`/voice`, `/canvas`). Завдяки розбиттю кожна з них = ~80 LOC PR-а (один новий файл у `strategic-modes/`, plug-in у registry, drift-gate якщо є legacy primer, integration test).
