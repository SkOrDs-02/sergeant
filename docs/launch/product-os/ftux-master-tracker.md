...
| **PR-18** | feat(whats-new): in-product release notes modal          | ~200 | —    | `d7_returning_user_engagement_with_whats_new ≥ 30%` | 🚧 In flight: `apps/web/src/core/whatsNew/` (`<WhatsNewModal />` + `useWhatsNew` + `releases.ts`) + `docs/whats-new/` (markdown changelog) + 3 PostHog events. Auto-show гейт — `hasFirstRealEntry && !inFtuxSession`.                                                     |
