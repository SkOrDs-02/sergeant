...
> **Last validated:** 2026-05-04 by @Skords-01 (S1.3 + S1.4 + S1.5 + S2.4 + S3.4 + S3.5 + S3.1 + S3.2 + **S4.4 + S4.5 + S2.3 (web) + S4.1 + S2.3 (mobile parity)** виконані — 13 PR-ів злито з 22 рекомендацій). **Next review:** 2026-08-01.
> **Status:** Active
...
1. **Sprint 0 не пропускати.** Без PostHog (або еквівалента) усі наступні зміни — це сліпі гіпотези. _Update 2026-05-03 v2: S0.1 + S0.2 + **S0.4** + **S0.5** done (web-funnel закритий end-to-end). Лишилось тільки S0.3 (mobile parity). Деталі — у §2._
...
Перевірив код у `main` після перших спроб взяти S0.1 — частина S0 уже зроблена попередніми PR-ами. Після [PR #1570](https://github.com/Skords-01/Sergeant/pull/1570) (S0.5) і [PR #1582](https://github.com/Skords-01/Sergeant/pull/1582) (S0.4) у дереві лишилось тільки S0.3 (mobile parity). Рекалібрував статус нижче, щоб не дублювати роботу:
...
**Status update 2026-05-03 v2:** S0.5 ([PR #1570](https://github.com/Skords-01/Sergeant/pull/1570)) і S0.4 ([PR #1582](https://github.com/Skords-01/Sergeant/pull/1582)) shipped і змерджено в `main` у тому ж порядку (docs → events). Лишилось тільки **S0.3 (mobile parity)** і founder-tasks: вставити `VITE_POSTHOG_KEY` у Vercel + `apps/web/.env.local`, створити 5 insights у PostHog UI (HogQL запити готові у [`posthog-ftux-dashboards.md` §3](../observability/posthog-ftux-dashboards.md#3-the-five-saved-insights)) і вставити screenshot-лінки замість «TBD».
...
- **S2.1 / S2.2a / S2.2b / S2.3** — блоковані (S2.1 залежить від S1.1 copy-reviewer-а; S2.2 залежить від S2.1; S2.3 — логічно очікує S2.1).
- **S3.5 / S3.1 / S3.2** — виконані 2026-05-04 (PR #1623 / #1626 / #1630). S3 cleanup-ланка закрита: single-hero rule strengthened, CelebrationModal копія module-aware, SoftAuth копія gain-first з fear A/B fallback. Лишається **S3.3a / S3.3b** (value-progress bars, залежать від S2.1).
...
### Status check (2026-05-04 v3)

- **S3.4 + S3.5 + S3.1 + S3.2** — виконані 2026-05-04 (PR #1619 / #1623 / #1626 / #1630). 4/6 — більша частина reward-roadmap-у закрита.
- **S3.3a / S3.3b** — лишаються в черзі: обидва залежать від S2.1 (goal-aware first action), яка чекає copy-reviewer-а через S1.1.
...
**Сума:** 6 PR-ів, ~540 LOC. **Виконано:** 4/6 (S3.4, S3.5, S3.1, S3.2 — штамп 2026-05-04). Залишок (S3.3a, S3.3b) — гейтиться S2.1 (goal-aware first action) → S1.1 (copy-reviewer).
