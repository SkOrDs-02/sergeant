...
Stage 4a audit-hook (`before_agent_start` → `userMessage` read) має той самий issue (real event payload не має `userMessage`). Окремий PR #2469 (TODO): міграція на `session_start` або `agent_turn_prepare` з реальним event shape.

13. **Stage 4b fix-forward part 2 — `opts.name` required** (PR **#2470**, 2026-05-12): друге live smoke-test одразу після PR #2468 merge виявив, що `/runway` (та інші shortcuts) **все ще проходять повний agent-cycle** — short-circuit на `before_dispatch` не спрацював. Root-cause #2: реєстрація hook-у ніколи не відбувалась.
...
Stage 4a audit-hook (`before_agent_start` shape mismatch) лишається окремою задачею (PR #2469 TODO) — нова реєстрація з `opts.name` забезпечить що hook принаймні **запускається**, але `event.userMessage` все одно `undefined` поки не зробимо переїзд hook-у. Цей PR (#2470) — passive необхідна умова, не sufficient. Розблоковує: Stage 4b live (третя спроба smoke-test після redeploy), Stage 4a audit-hook fix forward (PR #2469).
...
| 2   | 5 hooks + Layer 0 shortcut router (17 shortcuts + canned templates, $0 LLM cost) + Layer 1 cheap-router          | 🟡 5/6: `before_dispatch` (Layer 0 shortcut router — Stage 4b fix-forward PR #2468; live після redeploy), `llm_input` (бюджет), `before_agent_start` (audit-open — `@deprecated` у real SDK, Stage 4a follow-up мігрує), `agent_end` (finalize), `before_tool_call` (approval) — усі 5 hooks реєструються. Layer 0 shortcut router з 17 shortcuts (`/metrics`, `/runway`, `/status`, … + UA-фрази) — live після PR #2468 redeploy. Layer 1 cheap-router (Haiku) — Stage 4c. | `src/legacy/budget.ts`, `audit.ts`, `shortcut-router.ts` + `shortcuts/`, `cheap-router.ts` |
