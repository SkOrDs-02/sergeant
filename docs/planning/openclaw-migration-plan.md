...
- **Per-persona `agents.<id>.tools.{alsoAllow,deny}`** у `ops/openclaw/openclaw.example.json` — кожна з 10 personas має власний subset (cofounder = 30 tools, eng = 9, devops = 10, pm = 7, growth = 7, finance = 4, data = 5, cs = 5, content = 5, seo = 7), `defaults.tools.alsoAllow = ["recall_memory"]` як baseline. Read-only personas (finance/data/seo) явно deny-ять всі 5 write-tools для defence-in-depth. Canonical mapping живе у `packages/openclaw-plugin/src/personas/allowlist.ts`; gate-test `config-gate.test.ts` ламається якщо SKILL.md ↛ allowlist.ts ↛ openclaw.example.json розходяться.
...
| ✅ 5a         | Per-persona tool allowlist + gate-test                                                                                                                       | merged [#2480](https://github.com/Skords-01/Sergeant/pull/2480) (`08802283`, 2026-05-12) — `agents.<id>.tools.{alsoAllow,deny}` per-persona, canonical mapping + gate-test | n/a                                       |
| 🚧 5b         | Strategic-modes wiring (`/plan` `/analyze` `/okr`) — розбито на 3 PR (PR-1 `/plan`, PR-2 `/analyze`, PR-3 `/okr` + tracker bump)                             | Stage 5a                                                                                                                                                                   | низький                                   |
| 🚧 5b/plan    | `/plan <topic>` strategic mode (`before_agent_start` hook + PLAN_PRIMER injection + drift gate з legacy console primer)                                      | Stage 5a — **PR open** ([#2482](https://github.com/Skords-01/Sergeant/pull/2482))                                                                                          | низький                                   |
| 🚧 5b/analyze | `/analyze <anomaly>` strategic mode (extends `ALL_STRATEGIC_MODES` catalogue + `ANALYZE_PRIMER` byte-for-byte drift gate, integration test reuses host hook) | Stage 5b/plan — **PR open** (Stage 5b PR-2)                                                                                                                                | низький                                   |
| ⬜ 5b/okr     | `/okr` strategic mode + parent Stage 5b ⬜→✅ bump                                                                                                           | Stage 5b/analyze                                                                                                                                                           | низький                                   |
