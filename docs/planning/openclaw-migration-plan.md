...
Після обох merge-ів `packages/openclaw-plugin` має 175/175 vitest, чистий lint/typecheck/build. Production redeploy відбувається через Railway auto-watch на `main`. Наступний крок — Stage 4c (Haiku JSON-classifier).
...
| 2   | 5 hooks + Layer 0 shortcut router (17 shortcuts + canned templates, $0 LLM cost) + Layer 1 cheap-router          | 🟡 5/6: `before_dispatch` (Layer 0 shortcut router, live; 2 шорткати фікс-forward у PR #2473 — `business_snapshot`/`ai_decisions` → `openclaw_invocations`/`openclaw_decisions`, регресія-тест запобігає повторам), `llm_input` (бюджет), `before_agent_start` (audit-open — `@deprecated` у real SDK; PR #2474 переключив hook з guessed `event.userMessage` на canonical `event.prompt`, лишається міграція на `session_start` як окремий cleanup), `agent_end` (finalize), `before_tool_call` (approval). Layer 0 shortcut router з 17 shortcuts (`/metrics`, `/runway`, `/status`, … + UA-фрази) — live. Layer 1 cheap-router (Haiku) — Stage 4c. | `src/legacy/budget.ts`, `audit.ts`, `shortcut-router.ts` + `shortcuts/`, `cheap-router.ts` |
...
### Stage 4c — як тестити (PR-pending checklist)
...
**Опціонально:** прокинути канонічний system prompt у Gateway env:

```
OPENCLAW_CHEAP_ROUTER_PROMPT_PATH=/app/ops/openclaw/cheap-router.system.md
```

Без змінної server використає embedded `DEFAULT_CHEAP_ROUTER_SYSTEM_PROMPT` (та сама копія з `ops/openclaw/cheap-router.system.md`, hard-coded у `apps/server/src/modules/openclaw/classify.ts` для prod-safety на випадок, якщо volume не змонтований).
