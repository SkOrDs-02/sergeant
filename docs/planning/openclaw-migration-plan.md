...
> **Last validated:** 2026-05-12 by Devin. **Next review:** після ручного Gateway redeploy (`railway up --service openclaw-gateway` з local `main` clone) та live smoke-test — див. handoff [`openclaw-stage-4b-debugging-handoff-2026-05-12.md`](../notes/spikes/openclaw-stage-4b-debugging-handoff-2026-05-12.md). Railway service не watch-ить GitHub repo, жоден з PR #2467/2468/2469 не задеплоєний.
...
14. **Real root-cause #3 — Railway service НЕ підключений до GitHub** (виявлено 2026-05-12 ~12:10 UTC): третій live smoke-test після PR #2469 merge також провалився — `/metrics` все ще повертав Opus prose. Розслідування Railway state виявило що production-Gateway все ще біжить **первісний deploy `74eab839` від `2026-05-12T10:06:43Z`** з `cliMessage: "Initial clean OpenClaw Gateway deploy"`. `service.serviceInstances.source.{repo,image}` обидва `null` — service **не watch-ить** GitHub repo для auto-deploy. PR #2467 (`bbed022e`), #2468 (`5cb31858`) та #2469 (`416eeba0`/`8458fa2a`) усі merged у `main`, але **жоден не задеплоєний**. Усі три "live smoke-test" діагноз-фази (root-cause #1: deprecated hook; root-cause #2: missing `opts.name`) **базувалися на ОДНОМУ і тому ж старому образі** = symptom "agent runs anyway" повторювався тривіально. Detailed handoff: [`docs/notes/spikes/openclaw-stage-4b-debugging-handoff-2026-05-12.md`](../notes/spikes/openclaw-stage-4b-debugging-handoff-2026-05-12.md). Next action — `railway up --service openclaw-gateway` з local `main` clone (швидкий шлях, ~5 хв), або підключити service до GitHub auto-deploy (permanent — окремий follow-up).
