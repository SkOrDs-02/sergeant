...
### Що сьогодні реально працює (Stage 2 production)

- **25 read-tools** у плагіні (`packages/openclaw-plugin/src/index.ts`): `recall_memory`, `read_strategy_docs`, `record_decision`, `query_app_db`, `get_server_stats`, `get_stripe_metrics`, `get_posthog_stats`, `get_sentry_issues`, `read_github`, `github_search`, `github_tree`, `github_diff`, `github_prs`, `get_github_releases`, `n8n_list`, `n8n_describe`, `n8n_trigger`, `n8n_activate`, `refresh_business_snapshot`, `read_workflow_logs`, `read_telegram_topic`, `seo_gsc_query`, `seo_psi_audit`, `seo_serp_lookup`, `set_reminder`. Усі — HTTP-проксі до `/api/internal/openclaw/<endpoint>` без змін у server API.
...
- **Плоский `tools.alsoAllow`** у `ops/openclaw/openclaw.example.json` — усі 10 personas мають доступ до всіх 25 tools (per-persona allowlist ще не wired, див. Stage 5a нижче).
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ | --- |
| 1   | 5 write-tools (`create_github_issue`, `commit_to_strategy_doc`, `post_to_topic`, `pause_workflow`, `mute_alert`)                        | ❌ 0/5 у плагіні (server endpoints ✅)                                                                                   | `src/legacy/write-tools/`                                                                  |
| 2   | 4 hooks: per-call budget gate (`/budget`), invocation finalize (`/invocations/finalize`), Layer 0 shortcut router, Layer 1 cheap-router | ❌ 0/4 зареєстровано                                                                                                     | `src/legacy/budget.ts`, `audit.ts`, `shortcut-router.ts` + `shortcuts/`, `cheap-router.ts` |
| 3   | Approval Variant B (custom hook) для write-tools                                                                                        | ❌ — Locked #5 фіксує B, але без hooks неможливо. Spike потрібен                                                         | `src/legacy/write-tools/approval-variants.ts`                                              |
| 4   | Council orchestration ($2.0 cap pre-gate, послідовність `devops → eng → pm → growth → finance → cofounder`)                             | SKILL `ops/openclaw/skills/council-roundtable/` є ✅, orchestrator ❌                                                    | `src/legacy/council.ts` + `council-config.test.ts`                                         |
| 5   | Strategic-modes wiring (slash-handlers `/plan` `/analyze` `/okr` + `strategicModes` блок у `openclaw.json`)                             | SKILL-и є ✅, wiring ❌                                                                                                  | `src/legacy/strategic-modes.test.ts` (13 tests)                                            |
| 6   | Per-persona tool allowlist (`agents.<persona>.tools` замість плоского `tools.alsoAllow`)                                                | ❌ — усі 10 personas мають всі 25 tools                                                                                  | `src/legacy/persona-allowlist.test.ts`                                                     |
...
### Що блокує наступний крок (Stage 4 spike)

Перш ніж писати Stage 3 (write-tools) або Stage 4 (hooks), треба **спайкнути реальний openclaw 5.7 SDK** і задокументувати:

1. **Hook API.** Чи існує `api.registerHook(name, handler)`? Які реальні `name`-и (`llm_input`? `tool_call_pre`? `agent_turn_end`?). Які payload shapes? Як hook повертає reject (`{ ok: false, status, reason }`) — це формат legacy-stubs, або справжній SDK хоче `throw`?
2. **Approval API.** Чи є native `tool.requiresApproval: true` у openclaw 5.7? Якщо так — Variant A (native) дешевший за Variant B (custom hook); Locked #5 потрібно переглянути.
3. **Per-persona allowlist.** Чи це `agents.<slug>.tools` у `openclaw.json`, чи `tools:` у SKILL.md frontmatter, чи окремий mechanism?
4. **Scheduler API.** Чи є `api.registerCronSkill` / `api.scheduleSkill` для morning-digest? Якщо немає — fallback на n8n Tier A.

Spike scope: ~1 година — заглянути в `node_modules/openclaw/plugin-sdk/*.d.ts` (поставлений у Gateway Dockerfile через `npm install openclaw typebox`) і виписати справжні type signatures у новий ADR-0056 (або в окремий `docs/notes/spikes/openclaw-sdk-5.7-real-api.md`).
...
| 🔬 ⬜ | SDK reality-check spike (hook / approval / allowlist / scheduler API)                           | Stage 2                                                                                                                                                   | низький — research-only                               |
| ⬜ 3a | Register `create_github_issue` write-tool                                                       | spike                                                                                                                                                     | низький — 1 tool, server endpoint ✅, легко відкотити |
| ⬜ 3b | Решта 4 write-tools (`commit_to_strategy_doc`, `post_to_topic`, `pause_workflow`, `mute_alert`) | Stage 3a                                                                                                                                                  | низький-середній — кожен tool ізольований             |
| ⬜ 4a | Budget + audit hooks (`llm_input` per-call + `agent_turn_end` finalize)                         | spike                                                                                                                                                     | середній — найбільший unknown                         |
| ⬜ 4b | Layer 0 shortcut router (17 shortcuts + canned templates)                                       | Stage 4a                                                                                                                                                  | середній — багато ребер, парсинг UA + EN + ISO        |
| ⬜ 4c | Layer 1 cheap router (Haiku JSON-classifier)                                                    | Stage 4b                                                                                                                                                  | середній — system prompt уже на volume                |
| ⬜ 5a | Per-persona tool allowlist + gate-test                                                          | Stage 3b                                                                                                                                                  | низький-середній — потрібен schema research           |
| ⬜ 5b | Strategic-modes wiring (`/plan` `/analyze` `/okr`)                                              | Stage 5a                                                                                                                                                  | низький                                               |
| ⬜ 5c | Council orchestration + $2.0 budget pre-gate                                                    | Stage 5b                                                                                                                                                  | середній — multi-turn, multi-persona                  |
| ⬜ 5d | Morning-digest cron (native scheduler або n8n fallback)                                         | Stage 4a + spike (4)                                                                                                                                      | середній — invariant API не підтверджений             |
| ⬜ 6a | Reactivate parity harness у CI                                                                  | Stage 4c + Stage 5c                                                                                                                                       | низький                                               |
...
| 🔬 Spike  | `pending` | —                                                                                                                                           | SDK reality-check: задокументувати реальний `api.registerHook`, approval mechanism, per-persona allowlist, scheduler API у `node_modules/openclaw/plugin-sdk/*.d.ts`. Output → `docs/notes/spikes/openclaw-sdk-5.7-real-api.md` або ADR-0056.                                                                            | Stage 2.5            |
| Stage 3a  | `pending` | —                                                                                                                                           | Register `create_github_issue` write-tool на real SDK (server endpoint `/api/internal/openclaw/write/github-issue` ✅). Найдешевший наступний крок — 1 tool, легко відкотити.                                                                                                                                            | Spike                |
| Stage 3b  | `pending` | —                                                                                                                                           | Решта 4 write-tools (`commit_to_strategy_doc`, `post_to_topic`, `pause_workflow`, `mute_alert`) переписані з `src/legacy/write-tools/` під real SDK.                                                                                                                                                                     | Stage 3a             |
| Stage 4a  | `pending` | —                                                                                                                                           | Budget + audit hooks (`llm_input` per-call → `/budget`, `agent_turn_end` → `/invocations/finalize`).                                                                                                                                                                                                                     | Spike                |
| Stage 4b  | `pending` | —                                                                                                                                           | Layer 0 shortcut router (17 shortcuts + canned templates). Cost saving uplift №1.                                                                                                                                                                                                                                        | Stage 4a             |
| Stage 4c  | `pending` | —                                                                                                                                           | Layer 1 cheap router (Haiku JSON-classifier). Cost saving uplift №2.                                                                                                                                                                                                                                                     | Stage 4b             |
| Stage 5a  | `pending` | —                                                                                                                                           | Per-persona tool allowlist (заміна плоского `tools.alsoAllow` на `agents.<persona>.tools` або еквівалент) + vitest gate-тест `persona-allowlist.test.ts`.                                                                                                                                                                | Stage 3b             |
| Stage 5b  | `pending` | —                                                                                                                                           | Strategic-modes wiring: `strategicModes` блок у `openclaw.json`, slash-handlers `/plan` `/analyze` `/okr`. Ports `legacy/strategic-modes.test.ts` (13 tests).                                                                                                                                                            | Stage 5a             |
| Stage 5c  | `pending` | —                                                                                                                                           | Council orchestration з `COUNCIL_DEFAULT_SEQUENCE` (Locked #8), $2.0 budget pre-gate, slash-handler `/council`. Ports `legacy/council.ts` + `council-config.test.ts`.                                                                                                                                                    | Stage 5b             |
| Stage 5d  | `pending` | —                                                                                                                                           | Morning-digest cron (`0 9 * * *` Kyiv) — native scheduler або n8n Tier A fallback (залежить від результатів спайку).                                                                                                                                                                                                     | Stage 4a + Spike     |
