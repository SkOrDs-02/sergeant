...
### Що сьогодні реально працює (Stage 4a production)
...
| 2   | 4 hooks: per-call budget gate (`/budget`), invocation finalize (`/invocations/finalize`), Layer 0 shortcut router, Layer 1 cheap-router | 🟡 3/4: `llm_input` (бюджет), `before_agent_start` (open invocation), `agent_end` (finalize invocation), `before_tool_call` (approval) — живі (цей PR, Stage 4a). Layer 0 shortcut router + Layer 1 cheap-router — Stage 4b/4c. | `src/legacy/budget.ts`, `audit.ts`, `shortcut-router.ts` + `shortcuts/`, `cheap-router.ts` |
...
| ✅ 4a | Budget + audit hooks (`llm_input` per-call + `before_agent_start` open + `agent_end` finalize) + write-approval (`before_tool_call.requireApproval`) | merged (цей PR — type-level зелений; live smoke-test після першого Gateway redeploy)                                                                      | n/a                                            |
