...
> **Last validated:** 2026-05-12 by Devin. **Next review:** після Stage 4b live smoke-test на Gateway після PR #2469 redeploy (перевірка, що hook реєструється з `opts.name` і `before_dispatch` короткозамикає агент; per-call cost = $0 для shortcut-матчів).
