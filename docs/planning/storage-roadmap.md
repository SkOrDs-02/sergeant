...
> **Last validated:** 2026-05-06 by Devin (Stage 7 entered: `module_data` column dropped + v1 server handlers видалено commit [`75dcdd5c`](https://github.com/Skords-01/Sergeant/commit/75dcdd5c) (PR #051+#052a в одному merge); web cloudSync engine tree drop in flight у PR #052b; mobile cleanup PR #052c наступний). **Next review:** 2026-08-04.
> **Status:** Active
>
...
> | 7 — cleanup                    | ⏳ in flight (3/4) | #051+#052a [`75dcdd5c`](https://github.com/Skords-01/Sergeant/commit/75dcdd5c) (drop `module_data` table + видалити v1 server `syncPush*`/`syncPull*` handler-и + `MODULE_DATA_MODULES` const); #052b — web cloudSync engine tree drop (in flight)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | #052c (mobile cleanup), #053 (KVStore deprecate), #054 (final localStorage burndown) |
>
...
#### **PR #052b — `chore(web): remove cloudSync v1 engine (storagePatch, dirty tracking, offline queue)`** ⏳ IN FLIGHT

- Видаляється весь dead-code engine tree під `apps/web/src/core/cloudSync/` —
  `engine/` (8 файлів: buildPayload, initialSync, pull, push, replay, retryAsync, upload),
...
- Total: ~7,500 LOC видалено (35 non-test + 23 test файлів + 2 integration test).
- App.tsx + useAppEffects.ts + OfflineBanner.tsx + MigrationPrompt UI **не чіпаємо в цьому PR** (rewire — окремий follow-up, бо це vertical в App.tsx, що окремо рев'ювиться).

#### **PR #052c — `chore(mobile): remove cloudSync v1 engine`** ⏳ ROADMAP

- Mirror того самого drop у `apps/mobile/src/sync/`.
  Mobile `useCloudSync` теж stub-нутий у попередньому Phase 5 client-cutover
  (Initiative 0003), engine код лежить dead-code.
- Total: ~24 non-test + 4 test файлів.
