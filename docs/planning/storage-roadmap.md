...
> **Last validated:** 2026-05-10 by @Skords-01 (Stage 9 COMPLETE 7/7 — PRs #060–#066 landed; PR #066 moved `createMemoryKVStore` to `@sergeant/shared/test-utils` and kept the web SSR/private-mode memory fallback app-local. Stage 9 hotfix tail (boot-path resilience for `sync_op_outbox`) landed post-canary — див. § Stage 9 нижче. Stage 7 9/9 — COMPLETE. Stage 8 dual-write default-on landed для всіх 4 модулів: Routine [#2133](https://github.com/Skords-01/Sergeant/pull/2133), Fizruk [#2135](https://github.com/Skords-01/Sergeant/pull/2135), Nutrition + Finyk + Finyk Mono mirror [#2178](https://github.com/Skords-01/Sergeant/pull/2178); plus Stage 8 dual-write telemetry sink (`ff92dbb4`) і PR #058 mobile sync-engine writer-runtime boot path landed у [#2118](https://github.com/Skords-01/Sergeant/pull/2118) alongside CloudSync v1 client cleanup. Read-default-on slice [#2179](https://github.com/Skords-01/Sergeant/pull/2179) was rolled back via [#2181](https://github.com/Skords-01/Sergeant/pull/2181) (`2735fa75`) after a PWA habit-input regression — re-rollout gated on stability re-verify. **Read-default-on quartet re-rolled out** після rollback #2181: Routine [#2244](https://github.com/Skords-01/Sergeant/pull/2244) (PR #055r2), Fizruk [#2247](https://github.com/Skords-01/Sergeant/pull/2247) (PR #055f2), Nutrition [#2251](https://github.com/Skords-01/Sergeant/pull/2251) (PR #055n2), Finyk (`24616449`, PR #055k2). **Stage 8 §3 parity probe quartet COMPLETE** на всіх 4 dual-write модулях: Routine ([#2243](https://github.com/Skords-01/Sergeant/pull/2243), `4ea2c952`), Fizruk ([#2257](https://github.com/Skords-01/Sergeant/pull/2257), PR #055f3), Nutrition ([#2259](https://github.com/Skords-01/Sergeant/pull/2259), PR #055n3), Finyk ([#2260](https://github.com/Skords-01/Sergeant/pull/2260), PR #055k3) — `<m>.sqlite.dualwrite.parity` decision-gate metric тепер populated на всіх 4 модулях. **Stage 8 dual-write feature-flag drop quartet COMPLETE** з revised scope (drop flag-gating only, не LS-write — schema-gap для Routine, parity з Routine для решти): Routine `#056r` (`ff852475`), Fizruk `#056f` (`abc575f0`), Finyk `#056k` ([#2265](https://github.com/Skords-01/Sergeant/pull/2265), `535a9984`), Nutrition `#056n` ([#2266](https://github.com/Skords-01/Sergeant/pull/2266), `65fe17fe`). **Stage 8 read-flag drop quartet COMPLETE** (registry + hook + boot cleanup, sets the canary for #057\*-tombstone): Nutrition `#057n-flag` ([#2269](https://github.com/Skords-01/Sergeant/pull/2269)), Finyk `#057k-flag` ([#2270](https://github.com/Skords-01/Sergeant/pull/2270)), Fizruk `#057f-flag` ([#2271](https://github.com/Skords-01/Sergeant/pull/2271)), Routine `#057r-flag` ([#2273](https://github.com/Skords-01/Sergeant/pull/2273)). **Stage 8 #057\*-tombstone quartet COMPLETE (4/4):** Nutrition `#057n-tombstone` ✅LANDED ([#2274](https://github.com/Skords-01/Sergeant/pull/2274), full LS-write + LS-read drop, residual-import, `STORAGE_KEYS.NUTRITION_*` `@deprecated`); Fizruk `#057f-tombstone` ✅LANDED ([#2275](https://github.com/Skords-01/Sergeant/pull/2275), mirror of Nutrition pattern + drift-fix — `triggerFizrukDualWrite` was declared in PR #028 but never invoked from any LS-write callsite, so the Fizruk SQLite mirror was effectively empty in production until this PR); Finyk `#057k-tombstone` ✅LANDED ([#2277](https://github.com/Skords-01/Sergeant/pull/2277), web 14 `usePersist`→`useReadonlyPersist` + `residualImport.ts`, mobile MMKV `safeWriteLS` removed from `budgetsStore`/`assetsStore`/`transactionsStore` + MMKV→SQLite `residualImport.ts`, 14+1 `STORAGE_KEYS.FINYK_*` `@deprecated`); Routine `#057r-tombstone` ✅LANDED (web LS-write drop in `routineStorage.ts` via SQLite cache write-through + `triggerRoutineDualWrite`, `residualImport.ts` LS→SQLite drain з stale LWW timestamp, `chatActions/routineActions.ts` мігровано на `loadRoutineState`/`saveRoutineState`, `STORAGE_KEYS.ROUTINE` `@deprecated`; mobile MMKV-write drop тримається окремим follow-up — поточний mobile dual-write поки completion-only, full-state mirror — Stage 10 mobile follow-up). **Stage 10 ✅COMPLETE (3/3) — web schema + web dual-write + mobile mirror landed; mobile tombstone landed теж ([#2288](https://github.com/Skords-01/Sergeant/pull/2288)). Stage 11 Nutrition full-state ✅ COMPLETE (4/4) — schema + web dual-write + mobile dual-write + mobile tombstone landed (`#057n-tombstone-mobile` drop MMKV writes for water-log + shopping-list + residual import). Див. § Stage 10 / § Stage 11 нижче.** PR #070r-schema ✅LANDED ([#2279](https://github.com/Skords-01/Sergeant/pull/2279)) — 7 new Routine SQLite/Pg tables + Drizzle schemas + client `004_routine_full_state.sql` + server `050_routine_full_state.sql` + `050_routine_full_state.down.sql` + 050 round-trip testcontainer harness + SQLite dialect re-exports brought into parity with Pg; PR #070r-dualwrite ✅LANDED ([#2281](https://github.com/Skords-01/Sergeant/pull/2281)) — `dualWrite/diff.ts` тепер emit-ить ops для habit/tag/category/pref/pushup/habit-order/completion-note + extended `parity.ts` (15 entity classes) + `sqliteReader.ts` повертає full-state warm cache + `adapter.ts` apply-paths покривають усі 7 нових таблиць, тобто нові Routine таблиці більше не порожні в production. **Outstanding:** Stage 8 web — COMPLETE (всі 4 module tombstone PR-и приземлились; #057r web shipped без canary per early-stage dev policy). **Stage 10 mobile mirror** ✅ LANDED [#2286](https://github.com/Skords-01/Sergeant/pull/2286) (mobile `dualWrite/{diff,adapter,parity}.ts` + `sqliteReader.ts` warm cache mirror усіх 7 нових Routine таблиць) + **mobile tombstone** ✅ LANDED [#2288](https://github.com/Skords-01/Sergeant/pull/2288) — Stage 10 quartet COMPLETE. **Stage 11 Nutrition full-state** ✅ COMPLETE (4/4): web schema [#2290](https://github.com/Skords-01/Sergeant/pull/2290), web dual-write [#2291](https://github.com/Skords-01/Sergeant/pull/2291), mobile dual-write [#2292](https://github.com/Skords-01/Sergeant/pull/2292), mobile tombstone `#057n-tombstone-mobile` (drop MMKV writes for water-log + shopping-list + residual-import drain on boot Stage 13 (Audit findings & cleanup) — 🔄 IN PROGRESS (4/9 LANDED) станом на 2026-05-10. Залендили: PR #071 mobile hubBackup delegation fix ([#2322](https://github.com/Skords-01/Sergeant/pull/2322)), PR #072 weekly-digest SQLite reader ([#2320](https://github.com/Skords-01/Sergeant/pull/2320)), PR #075 finyk excluded_stat_txs + dismissed_recurring SQLite cross-device sync ([#2321](https://github.com/Skords-01/Sergeant/pull/2321)), PR #076 drop dead syncedKV.ts + SYNC_EVENT/SYNC_STATUS_EVENT + lying OpenAPI registrations ([#2319](https://github.com/Skords-01/Sergeant/pull/2319)). Залишилось PROPOSED: PR #073 NUTRITION_SAVED_RECIPES mobile tombstone, PR #074 FINYK_SHOW_BALANCE SQLite reader, PR #077 drop dead sync-metadata STORAGE_KEYS + dirtyCount/queuedCount, PR #078 retire mono_mirror flag, PR #079 doc-drift refresh.) усі landed. Stage 9 KV store swap is complete. #045 Redis — opt-in optional Stage-6 follow-up. **Next review:** 2026-08-05.
> **Status:** Active
>
...
> | 8 — SQLite cut-over rollout      | ✅ COMPLETE (21/21)                                            | [#2118](https://github.com/Skords-01/Sergeant/pull/2118) (PR #058 mobile sync-engine writer-runtime boot path + CloudSync v1 client cleanup); Stage 8 dual-write default-on slice landed: Routine [#2133](https://github.com/Skords-01/Sergeant/pull/2133) (PR #055r1), Fizruk [#2135](https://github.com/Skords-01/Sergeant/pull/2135) (PR #055f1), Nutrition + Finyk + Finyk Mono mirror [#2178](https://github.com/Skords-01/Sergeant/pull/2178) (PR #055n1 + #055k1); Stage 8 dual-write telemetry sink ([`ff92dbb4`](https://github.com/Skords-01/Sergeant/commit/ff92dbb4)). Read-default-on slice [#2179](https://github.com/Skords-01/Sergeant/pull/2179) was reverted by [#2181](https://github.com/Skords-01/Sergeant/pull/2181) (`2735fa75`) after PWA habit-input regression, then re-rolled out per-module: Routine [#2244](https://github.com/Skords-01/Sergeant/pull/2244) (PR #055r2), Fizruk [#2247](https://github.com/Skords-01/Sergeant/pull/2247) (PR #055f2), Nutrition [#2251](https://github.com/Skords-01/Sergeant/pull/2251) (PR #055n2), Finyk (`24616449`, PR #055k2). **Stage 8 §3 parity probe quartet COMPLETE:** Routine ([#2243](https://github.com/Skords-01/Sergeant/pull/2243), `4ea2c952`), Fizruk ([#2257](https://github.com/Skords-01/Sergeant/pull/2257), PR #055f3), Nutrition ([#2259](https://github.com/Skords-01/Sergeant/pull/2259), PR #055n3), Finyk ([#2260](https://github.com/Skords-01/Sergeant/pull/2260), PR #055k3). **Stage 8 dual-write feature-flag drop quartet COMPLETE** (revised scope — drop flag-gating only, не LS-write): Routine `#056r` (`ff852475`), Fizruk `#056f` (`abc575f0`), Finyk `#056k` ([#2265](https://github.com/Skords-01/Sergeant/pull/2265), `535a9984`), Nutrition `#056n` ([#2266](https://github.com/Skords-01/Sergeant/pull/2266), `65fe17fe`). **Stage 8 read-sqlite flag-drop quartet COMPLETE:** Routine `#057r-flag`, Fizruk `#057f-flag`, Nutrition `#057n-flag`, Finyk `#057k-flag`. **Stage 8 tombstone PRs (4/4) COMPLETE:** Nutrition `#057n-tombstone` ([#2274](https://github.com/Skords-01/Sergeant/pull/2274)), Fizruk `#057f-tombstone` ([#2275](https://github.com/Skords-01/Sergeant/pull/2275)), Finyk `#057k-tombstone` ([#2277](https://github.com/Skords-01/Sergeant/pull/2277)), Routine `#057r-tombstone` ✅LANDED (web LS-write removal). | LS-reader removal (PR #057\*) quartet COMPLETE — early-stage dev shipped без canary вікон і decision-gate (per @Skords-01 § "ми ще в розробці"). Stage 10 dual-write extension landed ([#2281](https://github.com/Skords-01/Sergeant/pull/2281)) забезпечило full-state SQLite parity для Routine; web `#057r-tombstone` запушив LS-write drop для всіх 7 нових Routine таблиць. Mobile MMKV-write drop тримається окремим follow-up (Stage 10 mobile mirror needed first).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
> | 10 — Routine SQLite full-state   | ✅ COMPLETE (3/3) — web schema + web dualwrite + mobile mirror | PR #070r-schema landed: [#2279](https://github.com/Skords-01/Sergeant/pull/2279) — 7 new Routine SQLite/Pg tables (`routine_habits`, `routine_tags`, `routine_categories`, `routine_prefs`, `routine_pushups`, `routine_habit_order`, `routine_completion_notes`) + Drizzle schemas (SQLite + Pg) + client `004_routine_full_state.sql` + server `050_routine_full_state.sql` + `050_routine_full_state.down.sql` + 050 round-trip testcontainer harness + SQLite dialect re-exports brought into parity with Pg. PR #070r-dualwrite landed: [#2281](https://github.com/Skords-01/Sergeant/pull/2281) — `dualWrite/diff.ts` extended to emit ops for habit-create/update/archive/delete/restore + tag-create/update/delete + category-create/update/delete + pref-set + pushup-add + habit-order-set + completion-note-set; `parity.ts` extended до 15 entity classes; `sqliteReader.ts` повертає full-state warm cache; `adapter.ts` apply-paths покривають усі 7 нових таблиць; new Routine tables populated в production. PR #070r-mobile-dualwrite landed: [#2286](https://github.com/Skords-01/Sergeant/pull/2286) — mobile-сторона дзеркалить web full-state dual-write: `apps/mobile/src/modules/routine/lib/dualWrite/{diff,adapter,parity}.ts` + `sqliteReader.ts` warm cache + mobile telemetry sink (`apps/mobile/src/lib/observability/dualWriteTelemetry.ts`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Stage 10 web + mobile mirror landed. New Routine SQLite tables populated на обох платформах. Mobile MMKV-write для Routine залишається active — drop через `#057r-tombstone-mobile` (Stage 8 follow-up) unblocked, чекає bake-in `routine.sqlite.dualwrite.parity` decision-gate metric з #2286.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
> | 11 — Nutrition SQLite full-state | ✅ COMPLETE (4/4)                                              | [#2290](https://github.com/Skords-01/Sergeant/pull/2290) (PR #070n-schema), [#2291](https://github.com/Skords-01/Sergeant/pull/2291) (PR #070n-dualwrite), [#2292](https://github.com/Skords-01/Sergeant/pull/2292) (PR #070n-mobile-dualwrite), `#057n-tombstone-mobile` (drop MMKV writes for water-log + shopping-list + residual import on boot).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Stage 11 quartet COMPLETE. New Nutrition SQLite tables `nutrition_water_log` + `nutrition_shopping_list` populated на обох платформах через dual-write. `#057n-tombstone-mobile` дропнув mobile MMKV-write для water-log + shopping-list — SQLite warm cache єдине джерело істини; boot-time `residualImport.ts` drain-ить leftover MMKV payload зі stale LWW timestamp.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
> | 12 — Fizruk SQLite full-state    | ✅ LANDED (4/4)                                                | [#070f-schema](https://github.com/Skords-01/Sergeant/pull/2295) (`aa48c801`); PR #070f-dualwrite (web) [`a89ba326`](https://github.com/Skords-01/Sergeant/commit/a89ba326); [#070f-mobile-dualwrite](https://github.com/Skords-01/Sergeant/pull/2306) (`c27c6590`); [#057f-tombstone-mobile-stage12](https://github.com/Skords-01/Sergeant/pull/2310) (`fbbe3d52`); Stage 12.5: [#070f2-mobile-dualwrite](https://github.com/Skords-01/Sergeant/pull/2313) (`31817bab`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Mirror Stage 10 для Fizruk: ~5-6 нових SQLite/Pg таблиць (`fizruk_daily_log`, `fizruk_monthly_plan`, `fizruk_plan_templates`, `fizruk_programs`, `fizruk_wellbeing`, `fizruk_workout_templates`) + KV для active workout id → web dual-write extension → mobile dual-write extension → tombstone (web + mobile). Зараз 7 hooks (`useDailyLog`, `useMonthlyPlan`, `usePlanTemplate`, `usePrograms`, `useWellbeing`, `useWorkoutTemplates`, `useActiveFizrukWorkout`) пишуть напряму через `safeWriteLS` без dual-write на обох платформах. Web `#057f-tombstone` ([#2275](https://github.com/Skords-01/Sergeant/pull/2275)) не зачепив ці 7 entity classes. Найбільший залишковий schema gap у tail — пріоритет нижче за Stage 11.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
> | 13 — Audit findings & cleanup    | 🔄 IN PROGRESS (5/9) — див. § Stage 13                         | PR #071 ([#2322](https://github.com/Skords-01/Sergeant/pull/2322)), PR #072 ([#2320](https://github.com/Skords-01/Sergeant/pull/2320)), PR #075 ([#2321](https://github.com/Skords-01/Sergeant/pull/2321)), PR #076 ([#2319](https://github.com/Skords-01/Sergeant/pull/2319)), PR #079 (doc-drift refresh; commit ref filled in once PR lands)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Post-Stage-12.5 audit (read-only review всіх `safeWriteLS`/`safeReadLS` callsites + `STORAGE_KEYS` references + sync exports + OpenAPI registry + residual-import shapes на web + mobile). 2 справжні баги (mobile hubBackup MMKV-pass-through post-tombstone — silent functional break; weekly-digest reads dead `finyk_storage_v2` blob), 3 partial gaps (nutrition recipes mobile tombstone не закритий; finyk LS-only slots — `finyk_excluded_stat_txs` + `finyk_rec_dismissed` — без виділеного PR; misleading `@deprecated` tag на `STORAGE_KEYS.FINYK_SHOW_BALANCE`), 7 dead-code cleanup кандидатів (~600 LOC) — забуті exports, listeners, schemas, OpenAPI entries після CloudSync v1 sunset (T₀ = 2026-05-06, ADR-0047), 2 doc-drift items (Initiative 0003 Phase 6 status застарів; ADR-0047 без exit-criteria для sunset routes). 8 PR-ів декомпозовано: PR #071 (mobile hubBackup fix), #072 (weekly-digest reader swap), #073 (recipes mobile tombstone), #074 (FINYK_SHOW_BALANCE SQLite reader), #075 (finyk LS-only slots decision), #076 (drop syncedKV + SYNC_EVENT + lying OpenAPI — clear-win), #077 (drop dead sync-metadata STORAGE_KEYS + web `dirtyCount`/`queuedCount`), #078 (mono_mirror flag retire after burn-in), #079 (doc-drift refresh). |
>
...
> **Status:** 🔄 IN PROGRESS (5/9 LANDED, 4 PROPOSED) — last updated 2026-05-10.
>
> **Landed (5/9):** PR #071 ✅ ([#2322](https://github.com/Skords-01/Sergeant/pull/2322)
...
> — weekly-digest SQLite `finyk_prefs.monthly_plan_json` reader); PR #075
...
> OpenAPI registrations); PR #079 ✅ (doc drift refresh — Initiative
> 0003 Phase 2 PR placeholder resolved + Phase 6 bullets carry explicit
> commit/PR refs; ADR-0047 amendment + Phase 7 exit-criteria already
> in-tree from audit-prep commit `37bcba1c`).
>
> **Still proposed (4/9):** PR #073 (mobile `NUTRITION_SAVED_RECIPES`
> tombstone), PR #074 (finyk `show_balance` SQLite reader), PR #077 (drop
> dead sync-metadata STORAGE_KEYS + `dirtyCount/queuedCount` from
> `useSyncStatus`), PR #078 (retire `feature.finyk.sqlite_v2.mono_mirror`
> flag).
>
...
#### **PR #074 — `chore(finyk): SQLite reader for finyk_prefs.show_balance + tombstone FINYK_SHOW_BALANCE`** 📋 PROPOSED — LOW (closes A4)
...
- **A6 — Initiative 0003 Phase 6 stale.**
  [`docs/initiatives/0003-sync-v2-rollout-and-v1-sunset.md:237-244`](../initiatives/0003-sync-v2-rollout-and-v1-sunset.md)
