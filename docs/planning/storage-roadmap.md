...
> | 10 — Routine SQLite full-state | ✅ COMPLETE web (2/2); 📋 mobile mirror PROPOSED | PR #070r-schema landed: [#2279](https://github.com/Skords-01/Sergeant/pull/2279) — 7 new Routine SQLite/Pg tables (`routine_habits`, `routine_tags`, `routine_categories`, `routine_prefs`, `routine_pushups`, `routine_habit_order`, `routine_completion_notes`) + Drizzle schemas (SQLite + Pg) + client `004_routine_full_state.sql` + server `050_routine_full_state.sql` + `050_routine_full_state.down.sql` + 050 round-trip testcontainer harness + SQLite dialect re-exports brought into parity with Pg. PR #070r-dualwrite landed: [#2281](https://github.com/Skords-01/Sergeant/pull/2281) — `dualWrite/diff.ts` extended to emit ops for habit-create/update/archive/delete/restore + tag-create/update/delete + category-create/update/delete + pref-set + pushup-add + habit-order-set + completion-note-set; `parity.ts` extended до 15 entity classes; `sqliteReader.ts` повертає full-state warm cache; `adapter.ts` apply-paths покривають усі 7 нових таблиць; new Routine tables populated в production.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Schema-only — new tables sit empty in production until PR #070r-dualwrite (📋 PROPOSED) wires up the dual-write diff/apply paths for habit/tag/category/pref/pushup/habit-order/completion-note ops + parity probe + read-path adapter. Until #070r-dualwrite lands, Routine remains LS-primary / SQLite-mirror for non-completion fields, and Stage 8 `#057r-tombstone` (Routine LS-write removal) stays blocked.                                                          |
>
...
##### **Stage 10 — extend Routine SQLite schema to full LS coverage** ✅ COMPLETE web (2/2) · 📋 mobile mirror PROPOSED
...
##### **Stage 10 mobile mirror — Routine MMKV → SQLite full-state dual-write** 📋 PROPOSED
...
> mobile же залишився на старому completion-only dual-write. Тобто:
>
> - mobile dual-write пише лише `completion-add` / `completion-remove` /
>   `habit-rename` (історичний scope з PR #022 SPIKE) — ті самі 3 op-кінди,
>   що були до Stage 10;
> - mobile MMKV для Routine залишається source-of-truth для habits[],
>   tags[], categories[], prefs{}, pushupsByDate{}, habitOrder[],
>   completionNotes{}.
>
> Тому web `#057r-tombstone` ✅LANDED, а mobile аналог `#057r-tombstone-mobile`
> заблокований — drop MMKV-write без full-state dual-write = silent data loss.

**Scope (3 PRs, mirror of web Stage 10 + #057r):**

- **PR #070r-mobile-dualwrite** 📋 PROPOSED — extend `apps/mobile/src/modules/routine/lib/dualWrite/diff.ts`
  щоб emit-ити ops для habit-create/update/archive/delete/restore +
  tag-create/update/delete + category-create/update/delete + pref-set +
  pushup-add + habit-order-set + completion-note-set, дзеркалити
  `parity.ts` (15 entity classes), оновити mobile `sqliteReader.ts` щоб
  повертав full-state warm cache, mobile `adapter.ts` apply-paths
  покривають усі 7 нових таблиць. Web Drizzle schemas (SQLite + Pg)
  - serial migrations (`004_routine_full_state.sql` /
    `050_routine_full_state.sql`) уже landed у [#2279] — на mobile
    доскладається тільки клієнтська частина (Drizzle SQLite re-exports
  - dual-write/adapter/reader/parity).
