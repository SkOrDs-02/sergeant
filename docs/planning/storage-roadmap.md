...
> | 10 — Routine SQLite full-state | 🚧 IN PROGRESS (1/2)   | PR #070r-schema landed: [#2279](https://github.com/Skords-01/Sergeant/pull/2279) — 7 new Routine SQLite/Pg tables (`routine_habits`, `routine_tags`, `routine_categories`, `routine_prefs`, `routine_pushups`, `routine_habit_order`, `routine_completion_notes`) + Drizzle schemas (SQLite + Pg) + client `004_routine_full_state.sql` + server `050_routine_full_state.sql` + `050_routine_full_state.down.sql` + 050 round-trip testcontainer harness + SQLite dialect re-exports brought into parity with Pg.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Schema-only — new tables sit empty in production until PR #070r-dualwrite (📋 PROPOSED) wires up the dual-write diff/apply paths for habit/tag/category/pref/pushup/habit-order/completion-note ops + parity probe + read-path adapter. Until #070r-dualwrite lands, Routine remains LS-primary / SQLite-mirror for non-completion fields, and Stage 8 `#057r-tombstone` (Routine LS-write removal) stays blocked. |
>
...
##### **Stage 10 — extend Routine SQLite schema to full LS coverage** 🚧 IN PROGRESS (1/2)
...
- Until PR #070r-dualwrite lands, Routine remains LS-primary /
  SQLite-mirror for non-completion fields (the new tables sit
  empty in production until the dual-write diff/apply paths are
  wired).
...
- **PR #057r-flag** ✅ LANDED — drop the now-redundant
  ...
  Stage 10 PR #070r-dualwrite (schema landed in #2279, dual-write
  diff/apply paths still pending).
...
a dual-write context is registered. LS/MMKV-write залишається
source-of-truth до full-cut-over (Routine — gated на Stage 10 schema
extension; Fizruk/Nutrition/Finyk — gated на canary після #057\*).
Наступний rollout-крок: drop legacy readers + tombstone
