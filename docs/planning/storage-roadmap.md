...
>   weekly-digest reads dead `finyk_storage_v2` blob),
> - **3 partial gaps** (nutrition `NUTRITION_SAVED_RECIPES` mobile
>   tombstone не закритий; finyk LS-only slots без виділеного PR-у;
>   misleading `@deprecated` tag на `FINYK_SHOW_BALANCE`),
> - **7 dead-code cleanup кандидатів** (~600 LOC) post-Stage-7 / Stage-9
>   — забуті exports, listeners, schemas, OpenAPI entries, що не
>   звільнили після CloudSync v1 sunset (T₀ = 2026-05-06, ADR-0047),
> - **2 doc-drift items** (Initiative 0003 Phase 6 status застарів;
>   ADR-0047 без exit-criteria для sunset routes).
>
> Stage 13 декомпозує findings на 7 PR-ів, групованих за severity та
> ризиком. Жоден з них не блокер — Stages 0–12.5 покривають ~98%
> поверхні. Це cleanup-tail, який краще закрити окремою серією після
> Stage 12.5 burn-in.

**Group A — Bug fixes (HIGH severity).**

#### **PR #071 — `fix(mobile): hubBackup delegates to module-level apply functions, drops MMKV pass-through`** 📋 PROPOSED — HIGH (silent functional bug)
...
#### **PR #072 — `fix(insights): weekly-digest reads SQLite finyk_prefs.monthly_plan_json instead of dead finyk_storage_v2 blob`** 📋 PROPOSED — MEDIUM (silent feature deg)
...
#### **PR #075 — `chore(finyk): finyk LS-only slots → SQLite (excluded_stat_txs, rec_dismissed)`** 📋 PROPOSED — LOW (closes A5)
...
#### **PR #076 — `chore(shared): drop dead syncedKV.ts + SYNC_EVENT/SYNC_STATUS_EVENT + lying OpenAPI registrations`** 📋 PROPOSED — clear win, ~300 LOC
