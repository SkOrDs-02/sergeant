...
> | 11 — Nutrition SQLite full-state | 📋 PROPOSED (0/4)                                              | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Mirror Stage 10 для Nutrition: 2 нові SQLite/Pg таблиці (`nutrition_water_log`, `nutrition_shopping_list`) → web dual-write extension → mobile dual-write extension → tombstone (web + mobile). Зараз `nutritionStore.ts` water log + shopping list та частина `recipeBookStore.ts` пишуть напряму через `safeWriteLS` без dual-write на обох платформах (той самий schema-gap, що був у Routine до Stage 10). Web `#057n-tombstone` ([#2274](https://github.com/Skords-01/Sergeant/pull/2274)) не зачепив ці 2 entity classes. Stage 8 mobile MMKV-write drop для Nutrition заблокований на цей schema gap.                                                                                                                      |
> | 12 — Fizruk SQLite full-state    | 📋 PROPOSED (0/4)                                              | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Mirror Stage 10 для Fizruk: ~5-6 нових SQLite/Pg таблиць (`fizruk_daily_log`, `fizruk_monthly_plan`, `fizruk_plan_templates`, `fizruk_programs`, `fizruk_wellbeing`, `fizruk_workout_templates`) + KV для active workout id → web dual-write extension → mobile dual-write extension → tombstone (web + mobile). Зараз 7 hooks (`useDailyLog`, `useMonthlyPlan`, `usePlanTemplate`, `usePrograms`, `useWellbeing`, `useWorkoutTemplates`, `useActiveFizrukWorkout`) пишуть напряму через `safeWriteLS` без dual-write на обох платформах. Web `#057f-tombstone` ([#2275](https://github.com/Skords-01/Sergeant/pull/2275)) не зачепив ці 7 entity classes. Найбільший залишковий schema gap у tail — пріоритет нижче за Stage 11. |
>
...
> ([#2286]) на Nutrition залишається той самий schema-gap, що був на
> Routine до Stage 10 — `nutritionStore.ts` water log + shopping list та
> частина `recipeBookStore.ts` пишуть напряму через `safeWriteLS` без
> dual-write. На обох платформах (web + mobile). Поточний Nutrition
> dual-write covers лише meals / pantry / prefs / recipes (4 entity
> classes). Web `#057n-tombstone` ([#2274]) не зачепив water-log +
> shopping-list — вони лишаються LS-primary без SQLite mirror. Поки цей gap
> відкритий, mobile аналог `#057n-tombstone-mobile` блокується (drop без
> mirror = silent data loss). 2 нові entity classes — найменший залишковий
> schema gap у tail; пріоритет вище за Stage 12.
...
- **PR #070n-dualwrite** 📋 PROPOSED — extend `apps/web/src/modules/nutrition/lib/dualWrite/diff.ts`
  щоб emit-ити ops для water-log-set + shopping-list-set (одна-два op-кінди
  на entity class — water log і shopping list — це single-row state, не
  collection); web `parity.ts` extended до 6 entity classes;
  `sqliteReader.ts` повертає full-state warm cache для water log +
  shopping list; web `adapter.ts` apply-paths покривають 2 нові таблиці.
- **PR #070n-mobile-dualwrite** 📋 PROPOSED — mirror web Stage 11 на
  `apps/mobile/src/modules/nutrition/lib/dualWrite/{diff,adapter,parity}.ts`
  - sqliteReader (як зробили для Routine у [#2286]).
- **PR #057n-tombstone-mobile** 📋 PROPOSED — drop MMKV writes для water log +
  shopping list у `nutritionStore.ts` + recipe book write-through paths;
  add residual-import (MMKV → SQLite drain on boot з stale LWW timestamp,
  ідентичний pattern Routine `residualImport.ts`).
