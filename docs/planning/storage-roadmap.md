...
> **Status:** 🚧 IN PROGRESS (6/17). Stage 7 закрив **boundary** (eslint
...
>
> **Reverted:** read-default-on slice
> [#2179](https://github.com/Skords-01/Sergeant/pull/2179) (Routine + Fizruk
> `read_sqlite` default-on) was rolled back by
> [#2181](https://github.com/Skords-01/Sergeant/pull/2181) (`2735fa75`)
> after a PWA habit-input regression on installed PWA users —
> re-rollout requires a stability re-verify (див. risk register §5).
...
##### **PR #055r2 — `feat(routine): default-on feature.routine.sqlite_v2.read_sqlite`** ⏸ PAUSED (rolled back)
...
- **Status.** Initial roll-out shipped у
  ...
  Feature flags returned to `defaultValue: false` in both web and
  mobile registries — code-level state matches doc-narrative
  PAUSED. Re-rollout requires stability re-verify (див. §5 risk
  register).
...
- **PR #055n2** ⏸ PAUSED — default-on `feature.nutrition.sqlite_v2.read_sqlite`.
  Gated on PWA stability re-verify (read-default-on quartet was
  partially rolled back via [#2181](https://github.com/Skords-01/Sergeant/pull/2181)
  for Routine/Fizruk; Nutrition read-flag never flipped, but is
  pinned behind the same canary гата).
...
- **PR #055k2** ⏸ PAUSED — default-on `feature.finyk.sqlite_v2.read_sqlite`
  після canary для #055k1. Gated on PWA stability re-verify (див.
  PR #055r2 + risk register §5).
...
(квартет PR #055\*1 — landed via #2133/#2135/#2178; всі 4 read-flag-и
залишаються `defaultValue: false` після rollback #2181). Stage 8
dual-write telemetry sink (`ff92dbb4`) wired для Routine + Finyk +
Finyk Mono. Наступні rollout-кроки лишаються gated на canary:
`read_sqlite` re-rollout (PR #055\*2 quartet) → drop LS/MMKV write
safety net (PR #056\* quartet) → drop legacy readers + tombstone
`STORAGE_KEYS.*` (PR #057\* quartet). Re-rollout #055\*2 чекає на
PWA-canary stability re-verify (див. risk register §5 row
«Read-default-on PWA habit-input regression»).
