...
> **Last validated:** 2026-05-07 by Devin (Stage 9 IN PROGRESS 4/7 — PR #060 `kv_store` SQLite таблиця + bundled migration in flight ([#2155](https://github.com/Skords-01/Sergeant/pull/2155)); PR #061 `createSqliteKVStore` warm-cache adapter in flight ([#2157](https://github.com/Skords-01/Sergeant/pull/2157)); PR #062 `bootstrapKvStore()` web boot wiring + LS→`kv_store` one-time migration in flight ([#2159](https://github.com/Skords-01/Sergeant/pull/2159)); PR #063 `webKVStore` swap onto SQLite-backed `kv_store` in flight ([#2165](https://github.com/Skords-01/Sergeant/pull/2165)). Stage 7 9/9 — COMPLETE. Stage 8 PR #058 mobile sync-engine writer-runtime boot path landed in [#2118](https://github.com/Skords-01/Sergeant/pull/2118) alongside CloudSync v1 client cleanup. **Outstanding:** Stage 8 (SQLite cut-over rollout — 16 module rollout PR-ів: 4 модулі × 4 кроки) і Stage 9 (KV store swap — `webKVStore` LS-backed → SQLite-backed `kv_store`, 7 PR-ів, 4 in flight) — це operational rollout (cut-over default-on flag-ів), не нова інфра. PR plans у §3 нижче. #045 Redis — opt-in optional Stage-6 follow-up. **Next review:** 2026-08-04.
> **Status:** Active
>
...
> | 9 — KV store swap              | 🚧 IN PROGRESS (4/7)  | [#2155](https://github.com/Skords-01/Sergeant/pull/2155) (PR #060 `kv_store` SQLite table + bundled migration), [#2157](https://github.com/Skords-01/Sergeant/pull/2157) (PR #061 `createSqliteKVStore` warm-cache adapter), [#2159](https://github.com/Skords-01/Sergeant/pull/2159) (PR #062 `bootstrapKvStore()` web boot wiring + LS→`kv_store` one-time migration), [#2165](https://github.com/Skords-01/Sergeant/pull/2165) (PR #063 `webKVStore` swap onto SQLite-backed `kv_store`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | PRs #060–#063 in flight. Next: PR #064 drop LS mirror (4-week canary post-#063), PR #065 mobile mirror, PR #066 cleanup. Calendar: ~2 тижні coding + 1 місяць canary. |
>
...
> **Status:** 🚧 IN PROGRESS (4/7). PR #060 SQLite table + bundled
...
> PR #063 `webKVStore` swap onto SQLite-backed `kv_store` in flight
> ([#2165](https://github.com/Skords-01/Sergeant/pull/2165)).
