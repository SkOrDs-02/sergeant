...
- **Status:** ✅ Частково виконано / план скориговано 2026-05-14 у `codex/docs-open-work-drift-cleanup`: видалено фактично unused `idb-keyval` з `apps/web` і lockfile; `@fontsource-variable/dm-sans` залишено, бо він реально імпортується в `apps/web/src/index.css`; `@sergeant/shared` уже відсутній у `packages/openclaw-plugin/package.json`.
