...
> **Last validated:** 2026-06-03 (UA closeout — queue #3/#8 closed (verified-already-done, dev-only); earlier 2026-06-03 audits-runner closed #2/#4/#6; prev 2026-05-13 by Codex). All follow-up PR queue items now closed (0 outstanding). **Next review:** 2026-08-11.
> **Status:** Active
...
**Fix pass 2026-05-08 (continuation #3):** Закрито аудит-пункти #1–#4 з `docs/audits/archive/2026-05-07-app-audit.md` audit-comparison pass.

- **#1 P0 PROD `sync_op_outbox`** — закрито через [PR #2192](https://github.com/Skords-01/Sergeant/pull/2192) + [PR #2199](https://github.com/Skords-01/Sergeant/pull/2199). `apps/web/src/core/syncEngine/singleton.ts` тепер запускає `repairPartialOutboxMigration` (idempotent, відновлює corrupted post-002 стейт) + `runMigrations(ROUTINE_CLIENT_MIGRATIONS)` перед стартом sync-engine writer, з post-migration smoke check що кидає типовану помилку замість сирого `SQLITE_ERROR: no such table` 30 s пізніше. Verified: Sentry WEB-A pattern більше не відтворюється при чистому або post-002-partial SQLite стані.
- **#2 P1 `useAppLock` PBKDF2 salt** — закрито через [PR #2196](https://github.com/Skords-01/Sergeant/pull/2196). `apps/web/src/core/security/lockStorage.ts:43` вже містить `salt: salt as BufferSource`. Verification: `pnpm --filter @sergeant/web exec vitest run src/core/security/useAppLock.test.ts --reporter=verbose` — `9 passed` (включно з трьома раніше падаючими PBKDF2 тестами).
- **#3 P1 Full web Vitest gate** — закрито в цьому PR. `apps/web/src/test/setup.ts:52` `defineProperty` розширено з `writable: true`, що дозволяє `globalThis.localStorage = makeLS()` у тестах де потрібен ізольований стор. Verification: `pnpm --filter @sergeant/web exec vitest run --reporter=dot --maxWorkers=1` — **221 passed, 2197 tests, 0 failures** під Node 22.
- **#4 P2 Mobile umbrella imports** — вже закрито попередніми PR. Всі 5 мобільних файлів (`apps/mobile/src/core/db/kvStoreBoot.ts`, `finyk/lib/clientMigrate.ts`, `fizruk/lib/clientMigrate.ts`, `nutrition/lib/clientMigrate.ts`, `routine/lib/clientMigrate.ts`) вже використовують `@sergeant/db-schema/migrate/runner`. Verified: `grep -r "from \"@sergeant/db-schema/migrate\"" apps/mobile/src` — нічого, umbrella imports відсутні.

**Fix pass 2026-05-08 (continuation #2):** Закрито останні два пункти з queue — #9 і #10 — одним PR.

- **#9 P2/P3 Web performance** — `INEFFECTIVE_DYNAMIC_IMPORT` для `ProfilePage` усунено: `apps/web/src/core/app/HubMainContent.tsx` тепер lazy-load-ить `ProfilePage`, `HubReports` і `HubSettingsPage` через `lazyImport()` (helper із `apps/web/src/core/lib/lazyImport.ts`, що уже використовується для `AuthPage`/`PricingPage`/etc у `StandaloneRoutes.tsx`) і обгортає їх у `<Suspense fallback={<PageLoader />}>`. Шлях dynamic-import-у (`../profile/ProfilePage`) збігається з prefetch-loader-ом у `useRoutePrefetch.ts:52` верба́тім, тож Rollup dedupe-ить onto a single chunk. `apps/web/vite.config.js` `manualChunks` розширено: `vendor-markdown` тепер захоплює весь стек remark/mdast/hast/micromark/property-information/unified/vfile/decode-named-character-reference/character-entities/style-to-js/devlop/etc (раніше ці транзитиви react-markdown лишалися у головному `vendor`); додано окремі `vendor-react-query` (@tanstack/\*), `vendor-auth` (better-auth + @better-auth + @better-fetch + better-call + nanostores + defu) і `vendor-zod` (zod + @hookform/resolvers). Build verification (`pnpm --filter @sergeant/web build` під Node 20.20.2): **усі chunks тепер під 500 KB порогом, Vite не печатає `chunk size` warning**. Конкретні цифри (KB raw / gzip):
  - `index`: 654.91 / 191.77 → **430.47 / 127.78** (-34%)
  - `vendor`: 524.85 / 163.28 → **268.85 / 88.58** (-49%)
  - `vendor-zxing`: 428.35 / 113.95 (без змін, лишається <500 KB і завантажується тільки async із `useBarcodeScanner`)
  - нові chunks: `vendor-markdown` 113.87 / 34.39, `vendor-zod` 74.45 / 19.85, `HubSettingsPage` 65.17 / 19.15, `vendor-react-query` 43.31 / 12.77, `ProfilePage` 26.50 / 7.85 (раніше inlined у `index`/`vendor`), `vendor-auth` 26.26 / 10.01, `HubReports` 12.18 / 4.20.
  - `pnpm --filter @sergeant/web exec tsc --noEmit` зелений; `pnpm --filter @sergeant/web exec vitest run src/core/app/HubMainContent.test.tsx` — `3 passed` (мок `vi.mock("../profile/ProfilePage", …)` оновлено під новий import path).
- **#10 P3 Local lint reproducibility** — `pnpm lint` під Node 20.20.2 зелений: всі 17 lint-tasks (eslint-monorepo, lint:skills, lint:playbook-language, lint:tech-debt-freshness, lint:initiative-status-sync, lint:localstorage-allowlist, lint:env-single-source, lint:discoverability, lint:patches, api:check-openapi, api:check-openapi-types, etc.) пройшли без OOM/таймауту. ESLint worker heap OOM, зафіксований у audit, був специфічний для Node 25 — closure тут environment-only, без коду.
...
> **Closure note (2026-06-01, tail PR of "9 decisions", Decision #3 — Option A):** Verified-already-done in main. [`apps/web/src/core/auth/authClient.ts:37`](../../apps/web/src/core/auth/authClient.ts#L37) injects `"X-Requested-With": "XMLHttpRequest"` into Better Auth's `fetchOptions.headers` Proxy. Sign-up via the browser form now clears `requireCsrfHeader` and creates the account (`POST /api/auth/sign-up/email` → 200 instead of 403). M10 CSRF invariant preserved — the guard stays global; only the client now sends the canonical header. No code change needed in this tail PR; this anchor was the missing audit closure.
...
> **Closure note (2026-06-03, UA — queue #3 closeout):** Verified-already-done in main; lower-risk localized fix is in place, no ADR needed. (1) CSP: [`apps/web/index.html:67`](../../apps/web/index.html#L67) fallback `connect-src` вже містить `http://localhost:3000 http://127.0.0.1:3000` — dev API origin дозволений. Production CSP (canonical `vercel.json` headers) не послаблений: localhost-only origins у `<meta>` fallback ігноруються прод-доменом. (2) CORS allow-headers: глобальний `/api` middleware [`apps/server/src/http/apiCors.ts:31`](../../apps/server/src/http/apiCors.ts#L31) і `DEFAULT_ALLOW_HEADERS` у [`apps/server/src/http/cors.ts:33`](../../apps/server/src/http/cors.ts#L33) вже включають `X-Requested-With, traceparent, tracestate`; dev origins `127.0.0.1:5173/4173/5000` у `DEFAULT_ORIGINS`. Preflight для split-origin dev (`5173 → 3000`) проходить. Verification: `pnpm --filter @sergeant/server typecheck` green; `cors.test.ts + apiCors.test.ts + requireCsrfHeader.test.ts` → **45 passed**. No code change required in this pass.
...
> **Closure note (2026-06-03, UA — queue #8 closeout):** Verified-already-done in main. (1) CSRF 403: versioned path у CSRF exempt-list — [`apps/server/src/http/requireCsrfHeader.ts:73`](../../apps/server/src/http/requireCsrfHeader.ts#L73) `EXEMPT_PATH_PREFIXES` містить і `/api/metrics/web-vitals`, і `/api/v1/metrics/web-vitals`. Defense-in-depth: `apiVersionRewrite` ([`apps/server/src/app.ts:44`](../../apps/server/src/app.ts#L44)) і так мапить `/api/v1/* → /api/*` до того, як guard бачить шлях, тож обидва записи покривають endpoint. Legitimate web-vitals POST більше не дає 403. (2) Vercel insights MIME: `<Analytics />` гейтиться `shouldRenderVercelAnalytics()` ([`apps/web/src/main.tsx:120`](../../apps/web/src/main.tsx#L120)), що виключає `localhost / 127.0.0.1 / 0.0.0.0` — script `/_vercel/insights/script.js` не інжектиться у local server-static mode, тож strict-MIME error не виникає. Verification: `requireCsrfHeader.test.ts` exempt-paths suite покриває обидва шляхи (`/api/metrics/web-vitals`, `/api/v1/metrics/web-vitals`) → **45 passed** разом з CORS; server typecheck green. No code change required in this pass.
...
1. **P1 Web test gate:** ✅ закрито в claude/audit-comparison-fixes-DoLgC. `apps/web/src/test/setup.ts` `defineProperty` розширено з `writable: true`. Verification: `pnpm --filter @sergeant/web exec vitest run --reporter=dot --maxWorkers=1` — **221 passed, 2197 tests, 0 failures**.
2. **P1 Auth sign-up CSRF:** ✅ Closed 2026-06-03 — verified `apps/web/src/core/auth/authClient.ts:37` інжектить `"X-Requested-With": "XMLHttpRequest"` у Better Auth `fetchOptions.headers`. Browser sign-up проходить `requireCsrfHeader` (`POST /api/auth/sign-up/email` → 200 замість 403); M10 CSRF invariant збережено (guard лишається глобальним).
3. **P1 Dev/smoke CORS+CSP:** ✅ Closed 2026-06-03 (UA) — verified-already-done, localized dev-only fix in place (no ADR needed). CSP `connect-src` у [`apps/web/index.html:67`](../../apps/web/index.html#L67) дозволяє `http://localhost:3000` + `http://127.0.0.1:3000` (dev/fallback-only; прод CSP не послаблено). CORS allow-headers у [`apps/server/src/http/apiCors.ts:31`](../../apps/server/src/http/apiCors.ts#L31) + [`cors.ts:33`](../../apps/server/src/http/cors.ts#L33) включають `X-Requested-With, traceparent, tracestate`; dev origins (`127.0.0.1:5173/4173/5000`) у `DEFAULT_ORIGINS`. Acceptance виконано: `/api/v1/me` preflight проходить без CSP/CORS блокування. Verification: server typecheck green; `cors + apiCors + requireCsrfHeader` → **45 passed**.
4. **P1 Mobile Finyk test wrapper:** ✅ Closed 2026-06-03 — verified `apps/mobile/src/modules/finyk/pages/Transactions/TransactionsPage.test.tsx:124-128` загортає render у `ApiClientProvider` + `QueryClientProvider` + `ToastProvider` (test API client). Acceptance виконано: targeted `TransactionsPage.test.tsx` зелений.
...
6. **P2 Playwright smoke tooling:** ✅ Closed 2026-06-03 — verified `apps/web/playwright.smoke.config.ts:39` запускає `node ./tests/smoke/start-smoke-webserver.mjs`, а wrapper `apps/web/tests/smoke/start-smoke-webserver.mjs:8` Windows-safe (`process.platform === "win32" ? "pnpm.cmd" : "pnpm"`) — POSIX `sh -lc` усунено. Chromium install також уже відновлено локально.
...
8. **P2 Local telemetry cleanup:** ✅ Closed 2026-06-03 (UA) — verified-already-done. CSRF exempt-list у [`apps/server/src/http/requireCsrfHeader.ts:73`](../../apps/server/src/http/requireCsrfHeader.ts#L73) містить і `/api/metrics/web-vitals`, і `/api/v1/metrics/web-vitals` (плюс `apiVersionRewrite` мапить `v1 → unversioned` до guard-у) — legitimate web-vitals POST не дає 403. Vercel insights script гейтиться `shouldRenderVercelAnalytics()` у [`apps/web/src/main.tsx:120`](../../apps/web/src/main.tsx#L120) (виключає `localhost/127.0.0.1/0.0.0.0`), тож `/_vercel/insights/script.js` не вантажиться локально й MIME error не виникає. Verification: `requireCsrfHeader.test.ts` exempt-paths suite покриває обидва шляхи → **45 passed**; server typecheck green.
9. **P2/P3 Web performance:** ✅ закрито в [PR #2218](https://github.com/Skords-01/Sergeant/pull/2218). `HubMainContent.tsx` lazy-load-ить `ProfilePage`/`HubReports`/`HubSettingsPage` через `lazyImport()` + `<Suspense fallback={<PageLoader />}>` — `INEFFECTIVE_DYNAMIC_IMPORT` warning зник, dynamic-import path збігається з `useRoutePrefetch.ts:52`. `vite.config.js` `manualChunks` розширено: `vendor-markdown` тепер забирає весь react-markdown стек (mdast/hast/micromark/property-information/unified/vfile/etc), додано `vendor-react-query` (@tanstack/\*), `vendor-auth` (better-auth + supporting libs), `vendor-zod` (zod + @hookform). Build (`pnpm --filter @sergeant/web build`) — **жоден chunk не перевищує 500 KB**, `chunk size` warning зник: `index` 654.91 → **430.47 KB**, `vendor` 524.85 → **268.85 KB**, `vendor-zxing` 428.35 KB (async-only).
10. **P3 Local lint reproducibility:** ✅ закрито в [PR #2218](https://github.com/Skords-01/Sergeant/pull/2218). `pnpm lint` під Node 20.20.2 зелений: всі 17 lint-tasks (eslint-monorepo, lint:skills, lint:playbook-language, lint:tech-debt-freshness, lint:initiative-status-sync, lint:localstorage-allowlist, lint:env-single-source, lint:discoverability, lint:patches, api:check-openapi\*, etc.) пройшли без OOM/таймауту. ESLint worker heap OOM, зафіксований в audit, був специфічний для Node 25 — closure environment-only, без коду.
