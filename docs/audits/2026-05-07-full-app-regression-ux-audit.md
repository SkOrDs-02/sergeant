...
5. **P2 Mobile Jest reliability:** після provider fix повторити `pnpm --filter @sergeant/mobile exec jest --runInBand` під Node 20; закрити OOM/async `act()`/analytics leaks.
...
7. **P2 Manual UX pass hardening:** додати стабільне seeding/dismiss для welcome і module first-run sheets; повторити повну viewport/theme матрицю з screenshots для Auth, Hub, Finyk, Fizruk, Routine, Nutrition, HubChat.
