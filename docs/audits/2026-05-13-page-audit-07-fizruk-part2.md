...
if (!parsed.success) { /* show toast with messages.validation.measurement_outOfRange */ }
```
...
<div className="flex items-center gap-3 mb-2 text-2xs text-subtle">
  <span>Відпочинок: <span className="font-semibold text-text">{session.defaultRestSec}с</span></span>
  <span>Прогресія: <span className="font-semibold text-text">+{session.progressionKg} кг</span></span>
</div>
```
...
```tsx
<div role="img" aria-label={`Розклад програми: ${prog.schedule.map((s) => DAY_LABELS_FULL[s.day - 1]).join(", ")}; інші дні — відпочинок`}>
  {/* visual day strip */}
</div>
```
...
```ts
export function lastValidValue<T extends { value: number | null }>(data: readonly T[]): number | null {
...
```ts
it.each(BUILTIN_PROGRAMS)("%s: schedule keys all present in sessions", (prog) => {
...
function prLabel(n: number) {
  ...
  if (n % 10 >= 2 && n % 10 <= 4 && (n % 100 < 10 || n % 100 >= 20)) return `${n} рекорди`;
  return `${n} рекордів`;
}
```
