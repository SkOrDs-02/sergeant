...
- **`/api/internal/*` Sentry sampling baseline і rule-ordering** — закрите [PR-07 з backend-perf плану](../planning/pr-plan-backend-perf-2026-05.md). Поточні `SENTRY_SAMPLING_RULES` не мають широкого `/api/internal/` правила; 100% sampling застосований тільки до вузького `/api/internal/openclaw/write/`, решта internal/n8n/cron/read routes падають у fallback `0.05`. Test guardrail фіксує цей fallback і забороняє широке full-rate правило — деталі в [`sentry-sampling.md`](./sentry-sampling.md).
