...
- **Activation step:** виставити `MONO_AI_MEMORY_DIGEST_ENABLED=true` на n8n Railway env (Settings → Environment Variables), потім flip workflow toggle у self-hosted n8n UI. Без цього кроку workflow JSON залишається `active=false` у git per hard-rule [`validate-n8n-workflows.mjs`](../../tools/scripts/validate-n8n-workflows.mjs) («workflows in git must be inactive by default»).
...
- `VOYAGE_DAILY_BUDGET_USD_SOFT=1` (default `1`) — **in-process** soft daily cap (USD/day) для Voyage embeddings. На відміну від `VOYAGE_DAILY_BUDGET_USD` (Prometheus side), enforced серверним кодом ([`apps/server/src/modules/ai-memory/voyageBudget.ts`](../../apps/server/src/modules/ai-memory/voyageBudget.ts), PR-38): при перевищенні — idempotent Sentry warning (1× на (day, threshold)) і **skip non-critical embeddings** (background ingestion — digest, mono webhook, RAG-prep). User-facing recall лишається critical (alert fire-иться, але виклик пропускається — UX > soft cap). Set `0` щоб вимкнути soft-gate (тоді тільки Prometheus-side `VoyageDailyBudgetSoftBreach`). Лічильник resets at UTC midnight.
