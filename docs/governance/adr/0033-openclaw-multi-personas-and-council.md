...
- **Related:**
  ...
  - [`docs/work/specs/launch/tech/openclaw-roadmap.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/tech/openclaw-roadmap.md) — Phase 2.5 section.
...
### 1. Persona registry (`tools/openclaw/src/agents/personas.ts`)
...
1. ✅ Code: створити `tools/openclaw/src/agents/personas.ts` (registry + tool-filter + primer-builder).
2. ✅ Code: `tools/openclaw/src/agents/openclaw.ts` — `buildSystemPromptInline()` приймає optional `persona`, prepend-ить primer перед COMMON-prefix; `runOpenClawAgent()` приймає optional `persona`, фільтрує tools перед `runAgentLoop`.
3. ✅ Code: `tools/openclaw/src/openclaw/handler.ts` — додати `/ops`, `/growth`, `/eng`, `/finance`, `/cofounder` як persona-aware slash-команди + `/council` як sequential round-table; HELP_TEXT оновлений.
...
6. ✅ ADR-0033 (this).
7. ✅ Update `docs/work/specs/launch/tech/openclaw-roadmap.md`: Phase 2.5 → shipped.
