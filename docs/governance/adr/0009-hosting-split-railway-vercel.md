...
- **Supersedes:** —
- **Related:**
  - [`docs/engineering/integrations/railway-vercel.md`](../../engineering/integrations/railway-vercel.md) — operator-орієнтована how-to для обох платформ.
  - [`docs/engineering/architecture/platforms.md`](../../engineering/architecture/platforms.md) — стан web / native / capacitor-shell поверхонь.
...
- Додатковий hop: browser → Vercel Edge → Railway → Postgres. Latency зросла
  ...
  [`docs/operations/observability/SLO.md`](../../operations/observability/SLO.md#2-http-latency-slo-p95--1s-non-ai))
...
2. **Long-running AI requests** — Anthropic streaming до 30s (див. SLO § 5 у
   [`docs/operations/observability/SLO.md`](../../operations/observability/SLO.md#5-ai-anthropic-slo-970-)).
...
- `SERVER_MODE=railway` (або автодефолт) відрізняється від `SERVER_MODE=replit`
  ...
  [`docs/operations/observability/runbook.md`](../../operations/observability/runbook.md) assume-ує
  Railway.
