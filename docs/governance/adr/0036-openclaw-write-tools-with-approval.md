...
- **Related:**
  ...
  - [`docs/work/specs/launch/tech/openclaw-roadmap.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/tech/openclaw-roadmap.md) — Phase 4 section.
...
In-memory `Map<id, ApprovalRecord>`, TTL `10 min` (`OPENCLAW_APPROVAL_TTL_MS` нескриптовий, але overridable у constructor для тестів). Лежить у `tools/openclaw/src/openclaw/approval-store.ts`.
...
`createOpenClawToolExecutor()` (у `tools/openclaw/src/agents/openclaw.ts`) тепер містить gate:
...
- **Allowlist drift.** GitHub-PR на `docs/strategy/` тільки — а раптом founder захоче `docs/work/specs/launch/`? Доведеться оновлювати server-allowlist + redeploy. **Mitigation:** intentional friction; allowlist — explicit decision-point.
