...
- **Supersedes:** —
- **Related:**
  - [`docs/work/specs/initiatives/archive/_0014-knowledge-graph-and-catalogs.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/initiatives/archive/_0014-knowledge-graph-and-catalogs.md) §Phase 5
  - [`docs/governance/adr/0058-knowledge-graph-schema.md`](./0058-knowledge-graph-schema.md) — graph schema with `pr` node type
...
1. **Hybrid:** single JSON ledger (`docs/governance/pr-ledger/index.json`) як canonical source + автогенерований block `<!-- AUTO-GENERATED: PR-BACKLINKS-START -->` з ≤5 останніх PR у тілі кожного канонічного дока.
2. **Per-PR markdown files** (`docs/governance/pr-ledger/PR-NNNN.md`). Кожен merged PR отримує власний файл.
...
- **Canonical store:** `docs/governance/pr-ledger/index.json` — JSON-схема draft-07 у `docs/governance/governance/schemas/pr-ledger.schema.json`. Шейп:
  ```json
  {
    "version": 1,
    ...
    "prs": [
      {
        ...
        "author": "@Skords-01",
        "touchedDocs": [
          "docs/governance/adr/0058-...",
          "docs/work/specs/initiatives/0014-..."
        ]
      }
    ]
  }
...
- **Canonical doc paths** що отримують backlinks (whitelist у script):
  - `docs/governance/adr/*.md` (крім TEMPLATE.md, README.md)
  - `docs/work/specs/initiatives/*.md` (крім archive/, follow-ups.md, README.md)
  - `docs/start/instructions/*.md` (крім INDEX.md, README.md, \_TEMPLATE-\*)
  - `docs/governance/governance/rules/*.md` (крім README.md)
...
- **Path whitelist scopes thrashing.** Чотири класи canonical docs (ADR / initiative / playbook / rule) — стабільні, рідко перейменовуються. Не whitelist-уємо `docs/work/specs/audits/` бо audits є snapshot-документами; не whitelist-уємо `docs/engineering/architecture/` бо там auto-gen mirrors з Phase 3, які регулярно міняються від drift-detector.
...
- [`docs/work/specs/initiatives/archive/_0014-knowledge-graph-and-catalogs.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/initiatives/archive/_0014-knowledge-graph-and-catalogs.md) §Phase 5
- [`docs/governance/governance/schemas/pr-ledger.schema.json`](../governance/schemas/pr-ledger.schema.json)
