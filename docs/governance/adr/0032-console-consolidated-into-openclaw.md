...
- **Related:**
  ...
  - [`docs/work/specs/launch/tech/openclaw-roadmap.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/tech/openclaw-roadmap.md) — phasing.
  - [`docs/engineering/architecture/apps-status-matrix.md`](../../engineering/architecture/apps-status-matrix.md) — console deployment row.
...
- Railway service для bot-процесу: `sergeant-openclaw` (config-as-code path `railway.console.toml`, Dockerfile `Dockerfile.console`). Раніше сервіс називався `sergeant-hubchat` (epoch `tools/console`); перейменовано у PR-47 (Pain P10 з [`telegram-improvements-roadmap.md` §C.5](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/tech/telegram-improvements-roadmap.md#c-5)). Рунбук самого rename-у — [`docs/operations/deploy/openclaw.md` §«Railway service rename runbook»](../../operations/deploy/openclaw.md).
...
- **Менше Railway slot-ів.** Один service `sergeant-openclaw` (раніше `sergeant-hubchat`) замість двох.
...
2. ✅ ADR-0032 (this).
3. ✅ Update `docs/engineering/architecture/apps-status-matrix.md`: console row → `Status: dormant (consolidated into OpenClaw, ADR-0032)`.
4. ✅ Update `docs/operations/deploy/openclaw.md`: rename intent to `sergeant-openclaw` deployment (Railway service перейменовано у PR-47, раніше `sergeant-hubchat`), додати ENV-list для нових tool-ів.
5. ✅ Update `docs/work/specs/launch/tech/openclaw-roadmap.md`: Phase 1 scope включає `/status`, `/metrics`, `/digest`, `/logs`, `/review`.
6. ✅ Update `docs/start/instructions/openclaw-runbook.md`: команди + troubleshooting + persona-roadmap.
...
9. ✅ PR-47 (Pain P10): rename Railway service `sergeant-hubchat` → `sergeant-openclaw` (config-as-code посилання, runbook-и, webhook URL у ADR-0041). Набор команд для фактичної зміни на Railway side — у [`docs/operations/deploy/openclaw.md`](../../operations/deploy/openclaw.md) §«Railway service rename runbook».
