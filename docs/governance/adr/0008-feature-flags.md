...
- **Related:**
  ...
  - [`docs/start/instructions/add-feature-flag.md`](../../start/instructions/add-feature-flag.md) — операційний how-to.
  - [`docs/start/instructions/cleanup-dead-code.md`](../../start/instructions/cleanup-dead-code.md) — гартування flag-у при graduating.
...
1. **Створення.** Новий flag → `defaultValue: false`, `experimental: true`. PR має містити `Expires: YYYY-MM-DD` у `docs/governance/governance/feature-flags.md` (або в PR-description) — дата, до якої flag має graduate-нути або бути видалений.
...
- `Expires`-дата — best-effort; не enforced автоматично. Можна додати CI-job, що фейлить, якщо у `docs/governance/governance/feature-flags.md` є entrі з минулою датою — поки не зробили (low ROI; з 3 поточних flag-ів усі under control).
...
- ⏳ `docs/governance/governance/feature-flags.md` як централізована таблиця (Owner / Default / Expires / Rollout) — поки писалося ad-hoc у PR-описах.
