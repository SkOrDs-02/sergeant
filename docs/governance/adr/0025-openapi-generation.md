...
- **Related:**
  ...
  - [`docs/engineering/api/openapi.json`](../../engineering/api/openapi.json) — згенерований spec (committed).
  - [`scripts/api/generate-openapi.mjs`](../../../scripts/api/generate-openapi.mjs) — generator.
  - [`docs/governance/adr/0062-openapi-source-of-truth.md`](./0062-openapi-source-of-truth.md) — пізніший ADR, що підтверджує code-first (Zod→OpenAPI) як канонічне source-of-truth (доповнює, не суперечить).
  ...
  - PR-історія: PR-4.D (audit `docs/work/specs/audits/archive/2026-04-26-sergeant-audit-devin.md`).

---

## 0. TL;DR

Замість руками синхронізувати `apps/server` ↔ `packages/api-client` ↔ snapshot-тести, маємо одне джерело правди (`packages/shared/src/schemas/api.ts`) і автогенерований OpenAPI 3.1 у `docs/engineering/api/openapi.json`. CI gate (`openapi-freshness.yml`) падає, якщо коммітнутий spec відстає від zod-схем — drift у rule #3 ловиться автоматично.
...
Аудит-документ (`docs/work/specs/audits/archive/2026-04-26-sergeant-audit-devin.md`, рядок PR-4.D) вимагає codegen-рішення.
...
3. **Generator** — `scripts/api/generate-openapi.mjs` через `tsx/esm/api`. Пише `docs/engineering/api/openapi.json` (детермінований 2-space JSON + trailing newline).
4. **CI gate** — `.github/workflows/openapi-freshness.yml` запускає `pnpm api:check-openapi` (regenerates і `git diff`). Trigger: будь-яка зміна у `packages/shared/src/schemas/`, `packages/shared/src/openapi/`, `docs/engineering/api/openapi.json`, `scripts/api/`.
...
- **External docs**: `docs/engineering/api/openapi.json` придатний для імпорту в Postman/Insomnia/Swagger UI без додаткового build-step.
...
# Дивитись зміни:
git diff docs/engineering/api/openapi.json
```
...
on:
  pull_request:
    paths:
      ...
      - "docs/engineering/api/openapi.json"
      - "scripts/api/**"
...
jobs:
  check:
    name: docs/engineering/api/openapi.json ≡ zod-schemas
