...
- **Supersedes:** —
- **Related:**
  - [`docs/work/specs/initiatives/archive/_0014-knowledge-graph-and-catalogs.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/initiatives/archive/_0014-knowledge-graph-and-catalogs.md) §Phase 4
  - [`docs/engineering/architecture/diagrams/README.md`](../../engineering/architecture/diagrams/README.md) — C4 model policy
  - [`docs/governance/governance/symbol-index.json`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/04-governance/governance/symbol-index.json) — input source (Phase 2)
  - [`docs/governance/adr/0059-symbol-extraction-via-typescript-compiler-api.md`](./0059-symbol-extraction-via-typescript-compiler-api.md)
...
1. **Existing C3 diagrams ≠ component-per-service.** Файли [`c3-cloudsync.md`](../../engineering/architecture/diagrams/c3-cloudsync.md) і [`c3-chat-tool-use.md`](../../engineering/architecture/diagrams/c3-chat-tool-use.md) — це **feature-flow діаграми** з 50+ рядками editorial narrative (контракт tool_use ↔ tool_result, prompt-cache breakpoints, тестування, дані-залежності). Auto-gen стер би операційний контекст.
2. **C4 explicitly excluded.** `docs/engineering/architecture/diagrams/README.md` фіксує «Не додавайте C4 рівень (Code) — TS типи й тестові снепшоти його замінюють». Це усвідомлене rule, не gap.
...
1. **Auto-gen workspace dependency graph as a new C3 diagram.** Додати `docs/engineering/architecture/diagrams/c3-workspaces.md` (auto-gen Mermaid LR-граф із `@sergeant/*` import-edges), не торкатись existing C3 / C4.
...
- **New file:** `docs/engineering/architecture/diagrams/c3-workspaces.md` (auto-gen, AUTO-GENERATED marker).
- **Generator:** `scripts/docs/generate-architecture-diagrams.mjs`.
- **Input:** `docs/governance/governance/symbol-index.json` (Phase 2 output). Кожен `package.exports[].usedBy[]` запис → файл-шлях; групування за workspace prefix (`apps/*`, `packages/*`, `tools/*`) → cross-workspace edges.
- **Output content:**
