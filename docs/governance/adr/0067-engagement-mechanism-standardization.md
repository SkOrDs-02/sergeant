...
- **Related:**
  ...
  - [`docs/governance/governance/rules/02-rq-keys-via-centralized-factories.md`](../governance/rules/02-rq-keys-via-centralized-factories.md) — прецедент «один централізований фабричний шар замість inline-копій» (RQ keys)
  - [`docs/governance/adr/0011-local-first-storage.md`](./0011-local-first-storage.md) — клієнт як primary-стор; warm-cache канонічні читачі
  - [`docs/governance/adr/0021-memory-bank.md`](./0021-memory-bank.md) — local-first engagement-state прецедент
...
**Спостережуваний наслідок (не гіпотеза):** коли Stage-8 «tombstone» переніс canonical-істину у SQLite warm-cache і почав видаляти legacy `*_v1` LS-ключі на boot, ці п'ять копій довелося мігрувати **поштучно**. На момент написання ADR (2026-06-20) `insightsEngine.ts` досі читав тумбстоунлені ключі — **регресія, що мотивувала цей ADR**. **Update 2026-07-21:** `insightsEngine` мігровано на warm-cache; fizruk/nutrition reminders переведено на shared `useModuleReminder` (Kyiv-time). **Compute-шар (EngagementSignal aggregator) — досі pending** (Phase 4 цього ADR).

### Шар 2 — reminders (планування нотифікацій)

Три модулі мали **скопіпащені, але розбіжні** reminder-хуки (baseline на момент ADR):

| Хук                        | Файл                                               | Day-key / час (baseline ADR)                                   | Tick (baseline ADR)                       | Permission-tracking (baseline ADR)          |
| -------------------------- | -------------------------------------------------- | -------------------------------------------------------------- | ----------------------------------------- | ------------------------------------------- |
...
| `useFizrukWorkoutReminder` | `modules/fizruk/hooks/useFizrukWorkoutReminder.ts` | ~~host-local~~ → **Kyiv via `useModuleReminder`** ✅ (2026-07) | ~~`setInterval(30s)`~~ → shared scheduler | via `useModuleReminder`                     |
| `useNutritionReminders`    | `modules/nutrition/hooks/useNutritionReminders.ts` | ~~host-local~~ → **Kyiv via `useModuleReminder`** ✅ (2026-07) | ~~`setInterval(45s)`~~ → shared scheduler | via `useModuleReminder`                     |

**Update 2026-07-21:** Шар 2 (shared `useModuleReminder`) **shipped** для fizruk/nutrition/routine. Routine лишається окремим хуком-обгорткою, але scheduler уніфікований. **Compute-шар і dismiss-state — досі pending.**
...
- [`docs/engineering/architecture/domain-invariants.md`](../../engineering/architecture/domain-invariants.md)
- [`docs/governance/adr/0006-rq-keys-factory.md`](./0006-rq-keys-factory.md) — прецедент централізованого фабричного шару
- [`docs/governance/adr/0011-local-first-storage.md`](./0011-local-first-storage.md)
- [`docs/governance/adr/0019-push-notifications.md`](./0019-push-notifications.md) — server push fan-out (поза скоупом цього ADR)
