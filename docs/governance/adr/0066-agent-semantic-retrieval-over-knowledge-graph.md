...
- **Supersedes:** —
- **Related:**
  - [`docs/governance/adr/0058-knowledge-graph-schema.md`](./0058-knowledge-graph-schema.md) — джерело нод/ребер, що індексуються
  - [`docs/governance/adr/0059-symbol-extraction-via-typescript-compiler-api.md`](./0059-symbol-extraction-via-typescript-compiler-api.md) — `symbol-index.json` як ще одне джерело пойнтерів
  - [`docs/governance/adr/0028-pgvector-ai-memory.md`](./0028-pgvector-ai-memory.md) — наявний production-RAG-стек (Voyage + pgvector), який ми **не** перевикористовуємо як стор (див. Decision)
  - [`docs/engineering/architecture/ai-memory.md`](../../engineering/architecture/ai-memory.md), [`docs/engineering/architecture/rag-eval.md`](../../engineering/architecture/rag-eval.md) — embedding-конвеєр і eval-harness, який перевикористовуємо
  - [`docs/work/specs/initiatives/0018-agent-semantic-retrieval.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/initiatives/archive/_0018-agent-semantic-retrieval.md) — план виконання
...
- **Джерела індексу:** `knowledge-graph.json` (ноди core+extended), секції canonical-docs (`docs/governance/adr`, `docs/start/instructions`, `docs/governance/governance/rules`, `docs/engineering/architecture`, `.agents/skills/**/SKILL.md`), `symbol-index.json` (export → file:line + owning-package).
...
- **Сторідж (ключове):** **маніфест** `docs/governance/governance/retrieval-index.json` (чанки без векторів — diffable, queryable, у git) + **вектори** у `.cache/retrieval/<contentHash>.bin` (**gitignored**, регенерується лазі за content-hash). Жодних векторів у git → нема noisy diff (мітигація болю з ADR-0058).
...
- [`docs/work/specs/initiatives/0018-agent-semantic-retrieval.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/initiatives/archive/_0018-agent-semantic-retrieval.md)
- [`docs/governance/adr/0058-knowledge-graph-schema.md`](./0058-knowledge-graph-schema.md)
- [`docs/engineering/architecture/rag-eval.md`](../../engineering/architecture/rag-eval.md)
