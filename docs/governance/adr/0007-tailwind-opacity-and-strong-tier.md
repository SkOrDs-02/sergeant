...
- **Related:**
  ...
  - [`docs/design/design/brandbook.md`](../../design/design/brandbook.md) — секція WCAG-AA `-strong` Tier (per-family contrast table, decision matrix).
  - [`docs/design/design/archive/brand-palette-wcag-aa-proposal.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/05-design/design/archive/brand-palette-wcag-aa-proposal.md).
...
1. **Кожне saturated brand-family має `-strong` companion** (типово `-700`; для `nutrition` — `-800`, бо лаймовий лежить на жовтій частині спектру і `-700` все ще регресує до ~3.8 : 1):
   ...
   Повна таблиця — у `docs/design/design/brandbook.md` → "WCAG-AA `-strong` Tier".
