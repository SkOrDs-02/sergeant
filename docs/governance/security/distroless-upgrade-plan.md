...
> **Status:** Deprecated — completed point-in-time plan; current image policy lives in [`../../operations/ops/docker-image-policy.md`](../../operations/ops/docker-image-policy.md).
>
...
| Owner                        | @Skords-01 (platform / devops per `docs/work/specs/tech-debt/technical-assessment-2026-06-05.md` AP-03)                                                                  |
| Пов'язані артефакти          | `.trivyignore`, `Dockerfile.api`, `Dockerfile.openclaw-gateway`, `docs/operations/ops/docker-image-policy.md`, `docs/work/specs/tech-debt/priority-1-executive.md` AP-03 |
...
2. **Не можна локально перезібрати `Dockerfile.api` / `Dockerfile.openclaw`**,
...
3. **Окремий Trivy gap уже закрито:** job для `Dockerfile.openclaw`
   додано 2026-06-09 комітом [`120ec9d94`](https://github.com/Skords-01/Sergeant/commit/120ec9d944eebf8bbc15c5360c4f1e8dbd15ae1d), тож PR-time gate тепер покриває обидва distroless-образи.
...
1. **Node.js major bump** — з 20 → 22 змінюються V8 internals, deprecation API
   ...
   `apps/server` використовує **Node 22.x** (Volta 22.19.0, `Dockerfile.api` → `node:22.16.0-alpine`).
...
3. **nodejs24 / nodejs26 ще менш стабільні** — nodejs24 — Active LTS станом на
   ...
   maintainer.
4. **CI-прохідність** — `.github/workflows/container-scan.yml` тепер покриває
   і `Dockerfile.api`, і `Dockerfile.openclaw`, тож base-image зміну можна ловити
   PR-time на обох distroless-сервісах.
...
2. Оновити `package.json -> volta.node` + `engines.node >= 22` (якщо
   зазначено) + `Dockerfile.api:50` і `Dockerfile.openclaw:30`
   (`node:22.16.0-alpine` → distroless `nodejs22` when ready).
3. Оновити `Dockerfile.api:143` і `Dockerfile.openclaw:92`:
...
- [x] Створено цей документ (`docs/governance/security/distroless-upgrade-plan.md`).
...
- `docs/operations/ops/docker-image-policy.md` — канонічна політика distroless-вибору
- `docs/work/specs/tech-debt/priority-1-executive.md` — AP-03 action item ("Перезібрати
  базовий образ distroless до спливу CVE 2026-07-02")
- `docs/work/specs/tech-debt/technical-assessment-2026-06-05.md` — SEC-003 finding + theme
  "Вплив CVE у базовому образі distroless"
- `docs/work/specs/tech-debt/archive/technical-assessment-2026-06-05.json` — AP-03 JSON record
- `docs/governance/security/audit-exceptions.md` — `pnpm`-level audit exceptions
  (різний scope; `.trivyignore` — runtime container scan)
- `docs/work/specs/security-hardening/L13-docker-platform-pin.md` — platform pin policy
  (closes risk of `linux/arm64` drift на Apple Silicon)
- `docs/work/specs/initiatives/stack-pulse-2026-05/pr-30-dockerfile-cleanup-cve.md` —
...
> 2026-06-05 ([`docs/governance/security/distroless-upgrade-plan.md`](./distroless-upgrade-plan.md))
> виявив, що:
>
...
> - [ ] Оновити `Dockerfile.api:50` і `Dockerfile.openclaw:30`:
>       `node:20.20.2-alpine` → `node:22.x-alpine`.
> - [ ] Оновити `Dockerfile.api:143` і `Dockerfile.openclaw:92`:
...
> - [x] Follow-up із Trivy CI gate для `Dockerfile.openclaw` уже закрито
>       комітом [`120ec9d94`](https://github.com/Skords-01/Sergeant/commit/120ec9d944eebf8bbc15c5360c4f1e8dbd15ae1d).
...
>
> - Tracked as `AP-03` у
>   [`docs/work/specs/tech-debt/priority-1-executive.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/90-work/tech-debt/archive/priority-1-executive.md)
>   ("Перезібрати базовий образ distroless до спливу CVE 2026-07-02")
> - Тема "Вплив CVE у базовому образі distroless" у
>   [`docs/work/specs/tech-debt/technical-assessment-2026-06-05.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/90-work/tech-debt/archive/technical-assessment-2026-06-05.md)
> - SEC-003 finding у
>   [`docs/work/specs/tech-debt/archive/technical-assessment-2026-06-05.json`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/90-work/tech-debt/archive/technical-assessment-2026-06-05.json)
