...
> **Last touched:** 2026-08-27 by Claude (знято виняток react-router — upstream бекпортнув патч у 7.18.2, підняли залежність). **Next review:** 2027-11-25.
> **Status:** Active
...
| Field      | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Advisory   | https://github.com/advisories/GHSA-w3rx-r6r6-pgpr (ICNS), https://github.com/advisories/GHSA-5p2g-fcmc-qvqq (JXL/HEIF)                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Package    | `image-size@1.2.1`; вразливі `<=2.0.2` — тобто **всі опубліковані версії, включно з останньою**                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Severity   | high (обидві) — DoS через нескінченний цикл у парсерах ICNS / JXL / HEIF                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Path       | Лише транзитивно, жодного first-party залежника (`grep image-size` по всіх `package.json` — порожньо). Найкоротший шлях: `apps/mobile > … > expo@52.0.49 > react-native@0.76.9 > @react-native/community-cli-plugin@0.76.9 > metro@0.81.5 > image-size@1.2.1`.                                                                                                                                                                                                                                                                          |
| Reason     | **Патчу не існує.** `patched_versions` в advisory — `<0.0.0` (порожній діапазон), вразливий діапазон `<=2.0.2` покриває найсвіжіший реліз. Ні override, ні bump закрити це не можуть, доки upstream не випустить фікс.                                                                                                                                                                                                                                                                                                                  |
| Mitigation | Це білд-тулінг, а не рантайм. `metro` — бандлер React Native; у prod-дерево він потрапляє тільки тому, що `react-native` декларує CLI-плагін як runtime-залежність, а `image-size` там читає розміри **наших власних** ассетів під час збірки. У зібраному застосунку (`apps/mobile` release-bundle) пакета фізично немає; в `apps/web` і `apps/server` його немає взагалі. Шляху, яким користувацьке зображення дійшло б до цього парсера, у продукті не існує — тобто вразлива поверхня (парсинг недовіреного зображення) не досяжна. |
| Due date   | 2026-10-31 — переоцінити, коли upstream опублікує `image-size` з фіксом або коли `apps/mobile` підніме Expo SDK / react-native (тоді `metro` приїде з новішим транзитивом). Якщо ні того, ні того — підняти окремим `security:medium` issue, не продовжувати виняток мовчки.                                                                                                                                                                                                                                                            |
| Owner      | @SkOrDs-02                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

### extract-zip — дві окремі symlink-вразливості (GHSA-JMR9-QJV8-65GV / CVE-2026-56876 і GHSA-7PQW-9J4J-H8Q3 / CVE-2026-19693)

| Field      | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Advisory   | Два **різні** дефекти в одному пакеті, кожен зі своїм CVE — тому й тримаються одним записом лише через спільні пакет, шлях, причину відсутності патча й мітигацію: (1) https://github.com/advisories/GHSA-jmr9-qjv8-65gv (CVE-2026-56876) — `extract-zip` не валідує ціль symlink-а при розпакуванні, тож посилання з відносним шляхом (`../../../../etc/passwd`) витягується як є й дивиться поза цільову теку; (2) https://github.com/advisories/GHSA-7pqw-9j4j-h8q3 (CVE-2026-19693) — containment-перевірка накладається лише на батьківську теку запису, а не на його власний кінцевий компонент, тож архів із двома записами під однаковим імʼям (спершу symlink назовні, потім звичайний файл) пише крізь підкладене посилання |
| Package    | `extract-zip@2.0.1`; вразливі `<=2.0.1` — тобто **всі опубліковані версії, включно з останньою**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Severity   | high, обидва CVSS 3.1 = 8.1, але вектори різні й це не збіг: CVE-2026-56876 — `C:H/I:H/A:N` (читання І запис чужих файлів), CVE-2026-19693 — `C:N/I:H/A:H` (лише запис, зате з відмовою в обслуговуванні). Числа з `pnpm audit --json`, тобто з того самого джерела, що читає гейт                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Path       | Лише транзитивно, жодного first-party залежника. Шлях: `apps/web > @lhci/cli@0.15.1 > lighthouse@12.6.1 > puppeteer-core@24.43.1 > @puppeteer/browsers@2.13.2 > extract-zip@2.0.1`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Reason     | **Патчу не існує.** `patched_versions` в advisory — `<0.0.0` (порожній діапазон), вразливий діапазон `<=2.0.1` покриває найсвіжіший реліз. Ні override, ні bump закрити це не можуть, доки upstream не випустить фікс.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Mitigation | Dev-only тулінг: `pnpm audit --prod` цей advisory **не** показує — у prod-дереві пакета немає взагалі. `@puppeteer/browsers` використовує `extract-zip` рівно один раз — щоб розпакувати білд Chromium, завантажений із `storage.googleapis.com` за пінованою в `puppeteer-core` ревізією. Це не користувацький вхід: архів приходить не від людини й не з PR-а, а з фіксованого upstream-URL. Шляху, яким недовірений zip дійшов би до цього розпакування, у продукті не існує.                                                                                                                                                                                                                                                      |
| Due date   | 2026-11-30 — переоцінити обидва id разом (спільний пакет і спільна причина: патча немає для жодного), коли upstream опублікує `extract-zip` з фіксом або коли `@lhci/cli` перейде на новіший `puppeteer-core` без цього транзитива. Якщо ні того, ні того — підняти окремим `security:medium` issue, не продовжувати виняток мовчки.                                                                                                                                                                                                                                                                                                                                                                                                  |
| Owner      | @SkOrDs-02                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
> [`./hardening/I1-codeql-workflow.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/04-governance/security/hardening/archive/I1-codeql-workflow.md).
...
>
> Принцип триажу див. у [`./codeql.md`](./codeql.md) → "Триаж знахідок".
> Кожен запис нижче має бути **також** dismiss-нутий у GitHub UI
> (Security → Code scanning) з reason `Won't fix`, як вимагає протокол —
> цей git-tracked запис дублює UI-рішення, не замінює його.

### 2026-07-09 — `js/missing-rate-limiting` (~40 alerts) — internal M2M + signature-verified webhooks

| Field      | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Date       | 2026-07-09                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Query      | `js/missing-rate-limiting` — "This route handler performs authorization, but is not rate-limited" (`security-extended`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Path       | ~40 route handlers, three classes: (1) **`apps/server/src/routes/internal/**`** — усі за спільним bearer-guard; (2) **public webhooks** — `routes/billing.ts` (`/api/billing/stripe-webhook`), `routes/mono-webhook.ts` (`/api/mono/webhook[/:secret]`); (3) кілька session-auth user-роутів, що вже несуть `rateLimitExpress` на sensitive-шляхах.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Severity   | High (CodeQL default) → **accepted-by-design**, не реальна вразливість                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Reason     | False-positive-клас. **(1) Internal**: увесь `/api/internal/*` subtree за спільним `Authorization: Bearer <INTERNAL_API_KEY>` guard-ом (constant-time `safeStringEqual`, fail-closed 503 без ключа — див. `routes/internal/index.ts`). Це machine-to-machine (n8n → server), ніколи не публічне; rate-limit дає нуль безпеки (без токена туди не дійти) і зламав би легітимні n8n batch-виклики. **(2) Webhooks**: межа безпеки — HMAC/signature-verification (`verifyStripeSignature` / mono path-secret), не лімітер; per-IP DiD-лімітер свідомо відкладено (ризик дропнути легітимний Stripe/Mono burst на payment-шляху). **(3) User-роути**: session-auth + `rateLimitExpress` уже стоять де треба. Знахідки «спливли» як 40 нових на PR [#134](https://github.com/SkOrDs-02/sergeant/pull/134) через reflow рядків (asyncHandler-cleanup + prettier), не через новий код — умови pre-existing. |
| Mitigation | Bearer-guard (internal, constant-time + fail-closed); signature-verification (webhooks); session-auth + наявні `rateLimitExpress` (user-роути). Опційний follow-up: щедрий per-IP DiD-лімітер на публічні webhooks — окремий PR за sign-off власника (payment-path).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Alert      | https://github.com/SkOrDs-02/sergeant/security/code-scanning?query=pr%3A134+tool%3ACodeQL+is%3Aopen                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Owner      | @SkOrDs-02                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
> See [`./hardening/M11-eslint-plugin-security.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/04-governance/security/hardening/archive/M11-eslint-plugin-security.md).
...
| `security/detect-non-literal-fs-filename`       | ~~openclaw/tools.ts~~ (removed)                                      | **Historical** — OpenClaw doc-search helpers (decommissioned [ADR-0075](../adr/0075-openclaw-gateway-decommissioned.md)); baseline row retained for audit trail. |
...
| `security/detect-non-literal-regexp`            | ~~tools/openclaw router.ts~~ (removed)                               | **Historical** — OpenClaw persona slug regex (external gateway removed per ADR-0075).                                                                            |
...
| `no-restricted-syntax` (templated `pool.query`) | ~~openclaw/store.ts~~ (removed)                                      | **Historical** — OpenClaw dynamic WHERE (decommissioned ADR-0075); write-audit persistence moved to `ai-memory/invocation-audit`.                                |
...
| `no-restricted-syntax` (templated `pool.query`) | `apps/server/src/routes/internal/seo.ts` <!-- removed --> (7 hits)   | Internal SEO ingest with allowlisted column-set; values via `$N`.                                                                                                |
...
> See [`./hardening/L2-permissions-policy-broader.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/04-governance/security/hardening/archive/L2-permissions-policy-broader.md)
...
> See [`./hardening/L1-uuid-override.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/04-governance/security/hardening/archive/L1-uuid-override.md).
