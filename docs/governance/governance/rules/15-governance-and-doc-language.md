...
4. **The matching playbook** in `docs/start/instructions/` — pick by trigger phrase. New API endpoint → `add-api-endpoint.md`. New HubChat tool → `add-hubchat-tool.md`. Removing code → `cleanup-dead-code.md`. Migrations → `add-sql-migration.md`.
...
- `pnpm lint:governance-sync` — fails (error, not warning) on **concrete** dangling `apps/.../*.ts` / `packages/.../*.ts` / `scripts/...` refs in non-aspirational docs (anything outside `docs/work/specs/launch/`, `docs/work/specs/planning/`, `docs/engineering/integrations/*-roadmap.md`, `docs/work/specs/audits/*-implementation-roadmap.md`, ADRs with `Status: proposed`). Refs containing glob/placeholder syntax (`*`, `?`, `<>`, `[]`, `{}`) are skipped — those are templates, not concrete claims.
...
- OpenAPI / `docs/engineering/api/*` schema & description fields (consumed by tooling).
