...
- **Circular dependencies.** `pnpm lint` (`import/no-cycle`) ловить цикл як помилку
  ...
  факт, два споживачі.
- **PR-ledger entries.** Snapshot lists recent PRs from `docs/governance/pr-ledger/index.json`;
...
- **`docs/start/agents/agent-skills-catalog.md`** — catalog entry for the snapshot tooling
