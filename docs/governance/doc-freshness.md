...
Якщо ADR коли-небудь потребує операційних метаданих, які треба перевалідовувати за cadence-ом (наприклад, таблиця квот, прайс-лист), винесіть ці дані в окремий док під `docs/integrations/`, `docs/launch/` або `docs/03-operations/observability/` і додайте **його** заголовок — а сам ADR не чіпайте.
