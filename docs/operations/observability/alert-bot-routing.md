...
- and the `POST /api/internal/alerts/pending` endpoint (раніше його викликала
  ...
  разом з OpenClaw, [ADR-0075](../../governance/adr/0075-openclaw-gateway-decommissioned.md);
...
- **WF-15** (Railway deploy notifier) видалено разом із Railway-хостингом
  ([ADR-0074](../../governance/adr/0074-hosting-hetzner-coolify.md)) — файлу
...
- `INTERNAL_API_KEY` — Bearer token for `/api/internal/alerts/post`. Rotated
  via [`docs/start/instructions/rotate-secrets.md`](../../start/instructions/rotate-secrets.md).
...
(O5 / PR #2507). Команда-рантайм пішов разом з OpenClaw ([ADR-0075](../../governance/adr/0075-openclaw-gateway-decommissioned.md));
...
[`docs/operations/observability/runbook.md`](./runbook.md) § "Alert-bot escalation ladder".
