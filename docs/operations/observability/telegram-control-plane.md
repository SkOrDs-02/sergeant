...
формальне рішення — у [ADR-0030](../../governance/adr/0030-telegram-reporting-channel-structure.md).
...
| **No SLA/SLO surface in chat**     | "Чи горить error budget?" — треба окремий dashboard. Telegram як alert hub не показує health, тільки точкові події.                                                                                 | Grafana покриває, посилання на дашборд є в `#ops` стрічці. Для phase 2 — Grafana Cloud, див. [`hosting-evolution.md`](../../engineering/architecture/hosting-evolution.md). |
| **Bot повна довіра**               | Якщо Telegram bot token компромет — атакер може писати від імені бота куди-завгодно у supergroup. На відміну від email — bot tokens в одному secrets-store.                                         | Rotate via [`docs/start/instructions/rotate-secrets.md`](../../start/instructions/rotate-secrets.md) → Telegram revoke + new BotFather token.                               |
...
- [ADR-0030: Telegram reporting channel structure](../../governance/adr/0030-telegram-reporting-channel-structure.md)
- [ADR-0026: n8n workflow source of truth](../../governance/adr/0026-n8n-workflow-source-of-truth.md)
...
- [Hosting evolution (when to scale infra)](../../engineering/architecture/hosting-evolution.md)
- [HubChat orchestration playbook (бот-команди)](../../start/instructions/add-hubchat-tool.md)
