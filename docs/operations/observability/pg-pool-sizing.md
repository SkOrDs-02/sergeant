...
> Перетинається з [`docs/start/instructions/database-connection-pooling.md`](../../start/instructions/database-connection-pooling.md)
> (deploy-shape pgBouncer pooler) і [`docs/operations/observability/runbook.md`](./runbook.md)
...
- Алерти: `DbPoolWaitingSustained` (5m, ticket) і `DbPoolSaturated` (10m, page) —
  у [`docs/operations/observability/prometheus/alert_rules.yml`](./prometheus/alert_rules.yml).
...
Postgres-default `max_connections=100`, pgBouncer `DEFAULT_POOL_SIZE=20` (transaction-mode),
...
Coolify Postgres (`pgvector/pgvector:pg18`) має `max_connections=100` за
замовчанням, тому для single-replica dev `PG_POOL_SIZE=20` — комфортно.
...
1. Coolify → app → Environment Variables → `PG_POOL_SIZE` → новий integer.
...
- Read-replica pool sizing — у [`docs/start/instructions/postgres-read-replica.md`](../../start/instructions/postgres-read-replica.md).
