...
- Coolify\*_ (image built by GitHub Actions → `ghcr.io`) — see
  [`../../governance/adr/0074-hosting-hetzner-coolify.md`](../../governance/adr/0074-hosting-hetzner-coolify.md)
  for the current backend topology (supersedes the Railway split in
  [`../../governance/adr/0009-hosting-split-railway-vercel.md`](../../governance/adr/0009-hosting-split-railway-vercel.md)).
...
1. **The Vercel project's "Root Directory" is `apps/web`.** Vercel reads
   ...
   that silently ages out. The drift problem is [hardening card H7](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/04-governance/security/hardening/archive/H7-vercel-config-drift.md);
...
When tightening a header, follow [`../security/hardening/C2-frontend-csp.md`](../../work/specs/security-hardening/C2-frontend-csp.md)
...
| PostHog SDK + session replay | JS bundle from `*.posthog.com`; no iframe                    | Yes (`apps/web/src/core/observability/posthog.ts`)            | Yes                            | Same as Sentry — JS module, no iframe. CSP `connect-src` allowlists `https://*.posthog.com` (covered by [L11](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/04-governance/security/hardening/archive/L11-csp-monitoring-allowlist.md)).                                                                                                                                                                                                                            |
| Mono webhook surface         | Server-side only (Coolify)                                   | Yes                                                           | N/A                            | No browser iframe. Mono dashboards are accessed by the operator, not embedded.                                                                                                                                                                                                                                                                                                                                                                                                                                |
| OpenFoodFacts (OFF) lookup   | `fetch` over HTTPS                                           | Yes (nutrition module)                                        | Yes                            | `connect-src` only; not an iframe.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Stripe Checkout / Elements   | Cross-origin iframe from `js.stripe.com`                     | **No** (planned, see `apps/web/src/core/PricingPage.tsx:12`)  | **No without changes**         | Stripe.js loads `js.stripe.com` then injects a `<iframe>` from `https://hooks.stripe.com` / `https://m.stripe.com`. These origins do not advertise CORP — under `require-corp` the iframe fails with `ERR_BLOCKED_BY_RESPONSE`. Decision when Stripe ships: switch COEP to `credentialless` for `/(.*)` _or_ scope `unsafe-none` to billing routes via a `vercel.json` source-glob. Record the decision in this matrix and add an [audit-exceptions.md](../../governance/security/audit-exceptions.md) entry. |
...
--token=$VERCEL_TOKEN` gives the list. See
   [`../playbooks/hotfix-prod-regression.md`](../../start/instructions/hotfix-prod-regression.md)
   for the full rollback recipe.

## Cross-references

- [`../security/hardening/H7-vercel-config-drift.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/04-governance/security/hardening/archive/H7-vercel-config-drift.md)
- [`../security/hardening/C2-frontend-csp.md`](../../work/specs/security-hardening/C2-frontend-csp.md)
- [`../../governance/adr/0074-hosting-hetzner-coolify.md`](../../governance/adr/0074-hosting-hetzner-coolify.md) — чинна бекенд-топологія (Hetzner + Coolify)
- [`../integrations/railway-vercel.md`](../../engineering/integrations/railway-vercel.md) — Vercel env + проксі-контракт (Railway-секції історичні)
- [`../adr/0009-hosting-split-railway-vercel.md`](../../governance/adr/0009-hosting-split-railway-vercel.md) — попередній Railway split (superseded ADR-0074)
