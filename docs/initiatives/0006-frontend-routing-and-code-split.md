...
| `manualChunks` у vite.config.js | **Audited 2026-05-24 — no changes.** Кожна група має активний import: `vendor-react` (react/react-dom + transitives — runtime), `vendor-router` (react-router-dom — RouterProvider), `vendor-react-query` (@tanstack/* — RQ + persister), `vendor-virtuoso` (react-virtuoso — 8 callsite-ів у nutrition/routine/finyk/fizruk + shared PullToRefresh), `vendor-zxing` (@zxing/* — barcode scanner у nutrition), `vendor-auth` (better-auth + транзитиви — eager у `shared/lib/api`), `vendor-zod` (zod + @hookform/resolvers — dedupe між formed-модулями), `vendor-sentry` (@sentry/react — observability), `vendor-web-vitals` (dynamic import з idle-callback), `vendor-sqlite` (sqlite-wasm + drizzle — local DB), `@capacitor/*` → `undefined` (свідомо merge-иться у dynamic native chunks). Без цього split-а initial vendor лопнув би: поточний gzipped initial ~194 KB тримається саме на цьому розкладі. Initiative scope-у «видалити застарілі групи» немає — нічого не застаріло. Реальна Phase 5 cleanup-ціль (повернення per-route lazy entries замість `ActiveModuleView.tsx` switch) заблокована React Router 7 location-context bug-ом, описаним у `router.tsx` (Phase 2 history секція). |
| Route-loaders                   | PENDING — 0 з 4+ модулів мають route-loaders. Блок: `router.tsx` має один catch-all `path: "*"` через Phase-2 location-context bug → loader-и можна вішати тільки на per-route entries. Розблокування — спільний parent `RootLayout` з `<Outlet />`, винесення провайдерів вище за router (described in `router.tsx` Phase 5 paragraph). До цього loader-prefetch не дає виграшу: модулі і так підвантажуються через `lazyDefault` після того як FSM встановила `activeModule`, RQ-prefetch happens усередині `<App />`.                                                                                                                                                                                                                                                                                                                                                                                                                          |
