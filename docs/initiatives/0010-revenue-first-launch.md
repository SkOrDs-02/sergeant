...
> **Status:** In progress — Phases 0+1+2+3+4.1+4.2+4.3+5.1+5.2+6(partial) done; Apple Sign-In + PricingPage portal link landed 2026-05-24 (this branch). Pending: founder adds APPLE_* env vars in Railway/local, EN locale (6)
...
- [~] Лендинг `LandingPage.tsx` (260 рядків) виконує маршрут `/` для неавторизованих users. EN-локаль pending (uk.ts є, i18n config відсутній). Sitemap/robots.txt pending.
...
| **[CORRECTNESS]** Dual billing tables — `billing_subscriptions` (m047, Stripe-only, `plan IN ('plus','pro')`) ↔ `subscriptions` (m056, multi-provider, `plan IN ('free','pro')`)  | dev          | Source-of-truth для `getUserPlan()` — m056. Аудитувати всіх writer-ів m047 (`apps/server/src/modules/billing/**`) до Фази 4 paywall-gates; m070 enum patch у m056 (додати `'free'`) перед першим paywall PR. Pure-m047 writer = paid user виглядає free silently. Двофазовий DROP m047 — пост-launch ADR. |
| **[CONVERSION]** Stripe Checkout / email receipts за замовчуванням англійською, ADR-0051 фіксує UA-only MVP | dev          | Перевірити Stripe Dashboard → Settings → Branding → Localization до Фази 3.1: чи `locale: 'uk'` підтримується в Checkout Session (per [Stripe locales doc](https://docs.stripe.com/payments/checkout/customization/policies-and-branding#localization)). Якщо ні — або примусово EN на Checkout (плюс UA-помітка на /pricing що checkout англійською), або відкласти EN-locale launch і shipити з EN checkout. Гірше за все: випустити UA-only landing з EN-only checkout без явного попередження юзера. |
