...
| L3  | Dockerfile post-install cleanup для CVE-shrink — крихкий      | [PR-30](./pr-30-dockerfile-cleanup-cve.md)           | **Shipped** — PR pending (distroless multi-stage build для `Dockerfile.api` + `Dockerfile.console`; runtime `gcr.io/distroless/nodejs20-debian12:nonroot`)         |
