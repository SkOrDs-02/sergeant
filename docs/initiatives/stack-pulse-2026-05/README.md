...
| 39  | [`tools/console` Anthropic SDK 0.36 → 1.x](./pr-39-tools-console-anthropic-sdk.md)      | L12            | 0.5–1 день | Partially closed — merged [#2527](https://github.com/Skords-01/Sergeant/pull/2527) (SDK pin `^0.95.1 → ^0.95.2` + prompt caching opt-in `ANTHROPIC_PROMPT_CACHE=1` у `tools/console/src/agents/run-agent-loop.ts` + 10 unit-тестів + [ADR-0057](../../adr/0057-anthropic-sdk-v1-upgrade.md) § Prompt caching opt-in / § SDK v1 tracking). v1 bump блокований reality — Anthropic не GA-нула v1 (latest `0.95.2` на 2026-05-13); follow-up PR коли v1 з'явиться (трек у ADR-0057).                                                                            |
