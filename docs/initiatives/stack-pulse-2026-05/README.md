...
| 30  | [Dockerfile cleanup → distroless multi-stage](./pr-30-dockerfile-cleanup-cve.md)        | L3             | 1 день     | Shipped — PR pending (`Dockerfile.api` + `Dockerfile.console` → multi-stage `builder` + `deps` (alpine, has shell for cleanup) + `runtime` (`gcr.io/distroless/nodejs20-debian12:nonroot`). Fragile `find … -prune` cleanup перенесено у `deps` stage — у runtime його просто немає. NO HEALTHCHECK (distroless не має `wget`/`pgrep`; Railway external probe). `docs/ops/docker-image-policy.md` documented. Image sizes (cold build): api 750 MB / console 166 MB. `.dockerignore` fix `node_modules` → `**/node_modules` for nested-workspace exclusion.) |
