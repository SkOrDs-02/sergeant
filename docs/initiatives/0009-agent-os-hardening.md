...
> **Status:** In progress (Phase 1 — 4/5 PR merged 2026-05-04: [#1659](https://github.com/Skords-01/Sergeant/pull/1659) skills-lint, [#1660](https://github.com/Skords-01/Sergeant/pull/1660) Hard Rules categorization, [#1670](https://github.com/Skords-01/Sergeant/pull/1670) playbook-schema extension, [#1672](https://github.com/Skords-01/Sergeant/pull/1672) playbook-language warn-only linter. PR 1.2b — backfill EN-only playbook'ів (1-й batch: `add-hard-rule.md`) — відкрито [#1772](https://github.com/Skords-01/Sergeant/pull/1772). PR 1.3 (Husky `tsc-files` pre-commit gate) — `hold` → `ready` після зникнення pre-existing TS-помилок (`#1572`-fallout). Phase 2 — 2/4 merged 2026-05-04: [#1684](https://github.com/Skords-01/Sergeant/pull/1684) `docs/superpowers/` → `docs/agents/` rename, [#1705](https://github.com/Skords-01/Sergeant/pull/1705) PR 2.3 release-\* консолідація. PR 2.1 — specialists ↔ skills mapping — відкрито [#1687](https://github.com/Skords-01/Sergeant/pull/1687). PR 2.4 — access-\* консолідація — відкрито [#1707](https://github.com/Skords-01/Sergeant/pull/1707) (rebased on `main` after PR 2.3 merge). Phase 3 — PR 3.1 (demote design rules) — відкрито [#1725](https://github.com/Skords-01/Sergeant/pull/1725); PR 3.3 (slim `.env.example` + `docs/integrations/env-vars.md` reference) — відкрито [#1775](https://github.com/Skords-01/Sergeant/pull/1775). Phase 4 — 2/3 merged 2026-05-04: [#1785](https://github.com/Skords-01/Sergeant/pull/1785) PR 4.1 Renovate maintainer runbook (`docs/ops/renovate.md`), [#1788](https://github.com/Skords-01/Sergeant/pull/1788) PR 4.3 workflow audit + Owner block у 26 workflow-файлах (мерджнуто `67ba6b0b`). PR 4.2 — knip-only dead-code (видалено `ts-prune` + `depcheck`) — відкрито [#1795](https://github.com/Skords-01/Sergeant/pull/1795). Phase 5 — PR 5.1 (Plop `new-skill` + `new-playbook` generators) — відкрито [#1796](https://github.com/Skords-01/Sergeant/pull/1796); PR 5.2 (`docs/agents/onboarding.md`) — відкрито [#1728](https://github.com/Skords-01/Sergeant/pull/1728).)
...
| 1.2b | open   | Backfill EN-only playbook'ів і SKILL.md        | [#1772](https://github.com/Skords-01/Sergeant/pull/1772)                                                                | Відкрито 2026-05-04. 1-й batch: `docs/playbooks/add-hard-rule.md` перекладено на українську (Cyrillic ratio ~0.55) — приклад секцій: `## Trigger`, `## Goal`, `## Why this matters`, decision-tree з UA-glossary; технічні токени (CLI команди, JSON ключі, файлові шляхи, Hard Rule ID) збережено EN. `pnpm lint:playbook-language` ОК, `lang: en` allow-list скорочується на 1. Залишок warn-listу (15 EN playbook'ів `@Skords-01` + `@claude` + 12 SKILL.md) — наступні batch'і. Може йти паралельно з фазою 2.                                                                                                                                                                                                                                                                                                                                                                                                              |
| 1.2c | TBD    | Перемикання `lint:playbook-language` у gate-ON | —                                                                                                                       | Не розпочато. Прибрати `--warn-only` після завершення 1.2b.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 1.3  | ready  | Husky pre-commit `tsc-files`                   | —                                                                                                                       | Розблоковано 2026-05-04. Обидві pre-existing TS-помилки на `main` уже зафіксено `#1572`-fallout-коммітами: server-сторона — `8ae5caba` _fix(server): align rotateSecret mock with vitest@4 Mock<T>_; web-сторона — `7518734e` _fix(web): align vitest@4 spies + restore CelebrationModal moduleId_. Перевірено локально: `pnpm typecheck` (root, `turbo run typecheck` з `dependsOn: ["^build"]`) — 16/16 task-ів, 0 errors. Відсутній PR 1.3 hotfix не потрібен; PR 1.3 — реалізація pre-commit gate `tsc-files` — у запланованому слоті Sprint 2/3.                                                                                                                                                                                                                                                                                                                                                                           |
...
| ---------------------------------------------------------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| % playbook'ів, що відповідають Hard Rule #15 (UA prose або явний `lang: en`) | ~62% (26/42)                                        | ~64% (27/42 після злиття PR 1.2b — `add-hard-rule.md` перекладено)                                   | 100%                                                                       |
...
| EN-only playbook без `lang: en` allow-list                                   | 16                                                  | 15 після злиття PR 1.2b (#1772) — `add-hard-rule.md` UA                                              | 0                                                                          |
...
| Dead-code інструменти                                                        | 3 (knip + ts-prune + depcheck)                      | 1 (knip) після злиття PR 4.2 (#1795) — ts-prune + depcheck retired                                   | 1 (knip)                                                                   |
| Workflow-файлів з документованим owner                                       | 0/26                                                | 26/26 після злиття PR 4.3 (#1788) — `# Owner: @Skords-01` коментар-блок                              | 26/26                                                                      |
| Plop-генератори для governance-артефактів                                    | 5 (migration, rq-hook, hubchat-tool, endpoint, adr) | 7 після злиття PR 5.1 (#1796) — додано `new-skill` + `new-playbook` (UA-default; `lang: en` opt-out) | 8 (PR 5.1b: + `new-package`, `new-n8n-workflow`, `new-console-specialist`) |
