...
> ✅ **Closed 2026-05-31** — `consumeHubChatSse` тепер тримає `totalBytes` лічильник (cap 256 КБ) і обриває потік `reader.cancel()` + кидає `Error("Відповідь занадто довга")` при перевищенні. Окремо: якщо `buf` росте >8 КБ без `\n` (runaway-server без переносів) — той самий abort. `friendlyChatError` піднімає це на UI як `Помилка: Відповідь занадто довга`.
>
> — done 2026-06-15: додано **другий, in-hook** cap у `useChatSend.ts` поверх transport-level cap-у. Константа `MAX_STREAM_CHARS = 256 * 1024`; у follow-up SSE-loop (`acc += delta`) перед акумуляцією перевіряємо `acc.length + delta.length > MAX_STREAM_CHARS` → `ac.abort()` + `throw new Error("Відповідь занадто довга")`. Throw ловиться наявним `catch (e2)`, який рендерить friendly-tail на assistant-turn замість дампу мульти-МБ тіла. Це closeс саме той вектор, що флагнув audit (`acc += delta` unbounded у `useChatSend`), незалежно від consumer-level cap-у. Покрито тестом `useChatSend.test.tsx` («caps the accumulated SSE stream»).
...
> ✅ **Closed 2026-06-15** — зроблено **обидва** кроки рекомендації. (a) Новий `redactPii<T>` у `hubBackup.ts` — рекурсивний прохід по об'єктах/масивах, що видаляє identity-shaped ключі (`PII_KEY_RE = /^(_?)(user|owner|account|customer|client|device|session|auth)_?id$/i`) і застосовується до кожного module-payload-у (`finyk`/`fizruk`/`routine`/`nutrition`) у `buildHubBackupPayload` перед серіалізацією; domain record-id-и (`id`, `txId`, `habitId`) навмисно зберігаються для референційної цілісності round-trip-у. (b) `HubBackupPanel` копірайт доповнено явним PII-disclosure абзацом: попереджає, що файл містить особисті дані (суми, назви боргів, нотатки), які користувач вписував сам, і радить тримати його приватним (менеджер паролів / зашифрована хмара). Free-text PII (назви боргів) не редактиться навмисно — це власний контент користувача, потрібний для restore; disclosure його покриває. Покрито тестами `hubBackup.test.ts` (`redactPii` unit + payload smoke).
...
> ✅ **Closed 2026-06-15** — у `InlineAiRail.tsx` user-sourced `question` (raw search query, що може echo-ити localStorage-cached titles інших модулів) тепер проганяється через локальний `neutralizeMarkdown(text)` перед рендером — він замінює markdown control-символи (`* _ ` [ ] ( ) # > ~ | \`) на пробіли, тож crafted query не може фейкнути assistant-style emphasis/code/link усередині rail-а. `state.answer` (assistant-sourced) лишається на markdown-aware `AssistantMessageBody` — boundary тепер явна: user-text → plain, assistant-text → markdown. `AssistantMessageBody` (shared file) уже несе JSDoc-варнінг "User-supplied text must be rendered as plain text"; його не чіпав (поза file-ownership). Покрито тестом `InlineAiRail.test.tsx` («neutralises markdown control tokens»). NB: `SearchResultItem` рендерить `hit.title`/`hit.subtitle` як plain text у `<p>` — markdown туди вже не потрапляє.
...
> ✅ **Closed 2026-06-15** — додано три нові test-suite-и (поверх наявних `hubChatSessions.test.ts` + `ChatEmpty.test.tsx`):
>
> - **`chat/useChatSend.test.tsx`** (7 кейсів) — plain-reply path, validated tool-call → `executeActions` → follow-up stream, rejected tool-call (unknown name) → no mutation, broken envelope (missing `id`) → no mutation, loading-guard drops a concurrent send, empty-message no-op, **F15 SSE-cap** (oversized delta → friendly "занадто довга" tail). Mock-ає `chatApi.send/stream`, `executeActions`, `consumeHubChatSse`, `usePlan`, `useFinykHubPreview`, speech.
> - **`chat/useChatSessions.test.tsx`** (7 кейсів) — fresh-session mount, debounced flush re-derives auto title, **F14 guard** (user-owned title не steamroll-иться), create-session switch, delete + undo-toast restore, last-session delete mints a fresh one, `loadSessions` round-trip. Fake timers для `CHAT_HISTORY_WRITE_DEBOUNCE_MS`; mock-ає `useToast` + `showUndoToast` (capture `onUndo`).
> - **`search/searchSources.test.ts`** (7 кейсів) — empty-query landing (actions-only), Finyk tx token-match + non-match filter, title-prefix vs subtitle scoring, Routine/Nutrition name-match, ai-handoff fallback завжди є, LRU returns the same instance для repeated query.
>
> Плюс розширено `hubBackup.test.ts` (F20 redaction) і додано `search/InlineAiRail.test.tsx` (F21). Усі 45 кейсів зелені локально. `searchSources.test.ts` покриває scoring-вектор (F22 пункт), а timezone-guard для F1/F2/F8 уже стоїть у наявному `hubChatSessions.test.ts` (`Бесіда DD.MM`-формат) — окремий TZ-mock тест лишаю follow-up-ом, бо ці findings уже closed.
