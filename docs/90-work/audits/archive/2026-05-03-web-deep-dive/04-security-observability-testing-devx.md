...
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/90-work/audits/2026-05-03-web-deep-dive/04-security-observability-testing-devx.md`.
...
**Що бачу.** Файл великий і починає містити overlap з `AGENTS.md`, `docs/conventions/`, `docs/90-work/audits/*`.
...
**Що бачу.** Коли новий agent (Devin / Claude / etc.) клонує проєкт, що йому читати першим? Я бачу `AGENTS.md`, `WEB_FE_AGENT.md`, `CONTRIBUTING.md`, `docs/conventions/`, `docs/90-work/audits/` — це 30+ файлів. **Жодного «start-here» з структурою.**
...
1. `docs/00-start/agents/start-here.md` — onboarding-checklist:
   ...
   4. Skim `docs/90-work/audits/2026-04-28-comprehensive-audit.md` — context.
...
## 9.1 [Good] `docs/90-work/audits/*` структура

Періодичні аудити з timestamps — добре. Цей фрейм (поточний deep-dive) лежить у `docs/90-work/audits/` як `web-deep-dive` піджанр — окремо від генеральних аудитів, але в тій самій таксономії (раніше — окрема `docs/diagnostics/` секція; злито 2026-05-05). **Lifecycle:** Active → Superseded by next deep-dive on same topic.
...
> **2026-05-04 update.** `docs/90-work/audits/README.md` тепер має суцільну таблицю Status / Implemented / Outstanding / Tracker для всіх живих файлів + ad-hoc діагностик секцію. Додано «Як читати таблицю» + «Process» (CI freshness + quarterly recompilation). Більше не треба шукати по гіту, чи аудит ще актуальний.

**Що бачу.** `docs/90-work/audits/*` має 8+ файлів, але без status-індикатора («Active», «Implemented», «Superseded»). Reader не знає, чи це актуальна gap-list, чи історія.

**Recommendation.**

1. У `docs/90-work/audits/README.md` — таблиця:
...
| `docs/90-work/audits/README.md` status table (ROI 2.00) | §11                                      |
