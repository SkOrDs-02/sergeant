...
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/90-work/audits/2026-05-03-web-deep-dive/02-architecture-and-state.md`.
...
>
> **Update 2026-05-04 (follow-up #2 — round 7):** [#1689](https://github.com/Skords-01/Sergeant/pull/1689) поширив `noUncheckedIndexedAccess: true` на `@sergeant/insights`. 13 test-assertions у `finance.test.ts` + `registry.test.ts` полагоджено через optional-chaining (`?.`), pure src/ — 0 type-помилок (foundation-пакет уже жив у strict-режимі). 24/24 тестів пакету зелені, downstream typecheck чистий. **Strict-coverage 6/13 → 7/13 пакетів (54%)** (з урахуванням базового `@sergeant/config` + раніше мігрованих). Backlog: `@sergeant/finyk-domain`, `@sergeant/routine-domain`, `@sergeant/fizruk-domain`, `@sergeant/api-client`, apps.
>
...
**Tracker.** Винести у `docs/90-work/tech-debt/frontend.md` як окремий milestone `strict-migration` з KPI «N flags enabled».
...
>
> **2026-05-04 update (follow-up #2 — round 7).** [#1692](https://github.com/Skords-01/Sergeant/pull/1692) скоротив production-count 16 → 15: `apps/web/src/shared/hooks/useActiveFizrukWorkout.ts` (попередньо помічений як edge-case від follow-up #1) переведено на `safeReadStringLS` / `safeRemoveLS`. Семантика `readActiveId` перетворилася з "throw → optimistically повертати id" на "non-throw `null` від helper-ів → fallthrough до validate-against-workouts перевірки", поведінка ідентична — invalid id чиститься через `safeRemoveLS`. Бюджет жорстко зафіксовано у `.tech-debt/localstorage-allowlist-budget.json` на `15` (headroom 0): нові raw-LS сайти треба компенсувати міграцією існуючого. Лишилось 15 файлів — основна маса це storage primitives (`storage.ts`, `mmkvWeb.ts`, `mmkvWebMigrationV1.ts`) + cloudsync internals + module wrappers (`hubKvImpl.ts`, `pushPrefs.ts`).
>
> **2026-05-04 update (follow-up #3 — round 8).** [#1723](https://github.com/Skords-01/Sergeant/pull/1723) скоротив production-count 15 → 14: `apps/web/src/shared/hooks/usePushNotifications.ts` (287 LOC, наступний кандидат після round-7 audit-у) переведено на `safeReadStringLS` / `safeWriteLS` / `safeRemoveLS` для ключа `hub_push_subscribed`. 1 read у `useState` initializer + 2 writes (`subscribed=1` після successful subscribe, `=0` після `getSubscription()=null`) + 4 removes (catch-блоки `unsubscribe`, dual-fingerprint cleanup при міграції клієнтів, очищення на 401 server-rejection). Додано 3 нові RTL-hardening-тести: `getItem` кидає `SecurityError` (Safari Private Mode) → useState fallback на `false` без crash; `setItem` кидає `QuotaExceededError` → no-op silent → подальші ops у hook-у працюють; `removeItem` кидає → silent no-op. Бюджет фіксовано на `14` (headroom 0). Залишок (14 файлів): storage primitives (`storage.ts`, `mmkvWeb.ts`, `mmkvWebMigrationV1.ts`) + cloudsync internals (`cloudSyncQueue.ts`, `cloudSyncStorage.ts`, `cloudSyncDevices.ts`, `localFirstSyncManager.ts`) + module wrappers (`hubKvImpl.ts`, `pushPrefs.ts`, `i18n.ts`, `theme.ts`, `tipsCardsState.ts`, `kyivDay.ts`).

**Що бачу.** `docs/90-work/tech-debt/frontend.md:89-100` — є TODO-список з 17 файлами, які усе ще читають `localStorage` напряму через `eslint.config.js` allowlist. Допустима тимчасова фаза, але burn-down треба **запланувати**, а не «коли руки дійдуть».
...
> **Tracker hook.** Винести burn-down progress у `docs/90-work/tech-debt/frontend.md` як окремий KPI «localStorage allowlist size».

---

## 2.3 [Bad] CloudSync без E2E reproduction для split-brain

> **2026-05-04 update.** Закрито у foundation-формі: `apps/web/src/test/integration/cloudSync.splitBrain.test.ts` (видалено після рефакторингу — див. PR [#1607](https://github.com/Skords-01/Sergeant/pull/1607)) додає **10 split-brain сценаріїв** через in-memory `FakeServer` (репліка LWW SQL-логіки з `apps/server/src/modules/sync/sync.ts`) та два `FakeClient`-и. Покриває: idempotency, LWW ordering, conflict, tombstone wins, no resurrection (v1 поведінка), clock skew (+/− 5 min), network flap (3 5xx → 4-та проходить), pull respects dirty, multiple modules (50 ops × 5 modules з 10 fail). Suite runs <100ms у CI. PR [#1607](https://github.com/Skords-01/Sergeant/pull/1607) — merged. Наступні follow-up-и: real engine offline queue replay (jsdom + fake timers), Stryker mutation testing, `useCloudSync` hook integration (вже §2.4).
>
> **Update 2026-05-05 (round 17 — follow-up #1).** Закрито перший follow-up з round-7 списку: `apps/web/src/test/integration/cloudSync.replayEngine.test.ts` (видалено після рефакторингу — див. PR [#1968](https://github.com/Skords-01/Sergeant/pull/1968)) — 7 сценаріїв над **реальними** production-модулями (`engine/replay.ts`, `engine/push.ts`, `queue/offlineQueue.ts`, `queue/collectQueued.ts`, `queue/deadLetter.ts`) з jsdom + fake timers. Замокано тільки `syncApi` та IDB-шар (`sergeantDb`); `retryAsync` колапсовано до pass-through, щоб engine-level dead-letter поведінка не маскувалася його власним експоненційним backoff-ом. Покриває: offline→online round-trip pushDirty→replay, coalesce 3 offline writes у 1 replay payload, replay re-entry guard (concurrent calls fire only one network push), `recordReplayBatchFailure` attempt-counter, dead-letter promotion на `MAX_QUEUE_ATTEMPTS`, drop corrupted rows, pushDirty re-queue on 5xx (no data loss). Suite runs <30ms; інтеграційне покриття між seam-ами queue↔collectQueued↔replay↔pushDirty↔dead-letter, яке unit-тести у власних модулях не ловлять. PR [#1968](https://github.com/Skords-01/Sergeant/pull/1968). Залишилися Stryker mutation testing (вже частково — round 16 PR #1963 ramp-нув `cloudSync/queue/` до 75.54%) та `useCloudSync` hook integration (§2.4).
