...
> ✅ **Closed 2026-06-02** — введено same-tab signal: новий `useHubStorageBump()` (`apps/web/src/core/hub/useHubStorageBump.ts`) слухає `hubBus "storageUpdated"` (same-tab) + native `window "storage"` (cross-tab) і повертає bump-лічильник для `useMemo`-deps. Усі report-картки (`FitnessCard`/`ExpensesCard`/`NutritionCard`/`RoutineCard`, які рендеряться в HubReports) додали `useHubStorageBump()` у залежності агрегації. Signal емітиться з canonical write-точок кожного модуля: Routine `emitRoutineStorage()`, Fizruk `notifyFizrukSqliteCacheRefresh()`, Nutrition `persistNutritionLog()`, Finyk `useMonobankWebhook` (write `finyk_tx_cache`). Без петлі — hub-картки read-only, не пишуть у ключі-тригери (задокументовано в хуці). 21 тест (bump-хук, hubBus-подія, re-aggregation карток). tsc 0, eslint clean.
...
> ✅ **Closed 2026-06-02** — `MotivationalFooter` і `StreakIndicator` (`dashboard/dashboardCards.tsx`) тепер беруть `useHubStorageBump()` у залежності свого `useMemo` (раніше `useMemo([])` → застрягало на mount-значенні). Після першого внесення запису футер/стрік-лічильник оновлюються через той самий same-tab signal, що й F3 (`hubBus "storageUpdated"` + `window "storage"`). Покрито `dashboardCards.test.tsx` (re-aggregation на обидва сигнали).
