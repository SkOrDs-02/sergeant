...
- **#5 P2 Mobile Jest reliability** — закрито через [PR #2215](https://github.com/Skords-01/Sergeant/pull/2215). Module shell тести (`FinykApp.test.tsx`, `BudgetsPage.test.tsx`, `NutritionApp.test.tsx`, `RoutineApp.test.tsx`, `AnalyticsIdentityBridge.test.tsx`) обернуто в runtime-like `ApiClientProvider` + `QueryClientProvider` із pre-seeded `useUser` cache, hoisting violation у Jest mock factory виправлено через `mock`-prefix rename. Verification: `pnpm --filter @sergeant/mobile exec jest --runInBand` під Node 20.20.2 — `96 suites / 624 tests passed`, peak heap ~1 GB, без heap OOM і без `Cannot log after tests are done`/`Animated(View) ... act(...)` leaks. Generated docs (`docs/90-work/initiatives/follow-ups.md`, `docs/04-governance/governance/hard-rules-matrix.md`) перегенеровано в окремих commits.
...
**Fix pass 2026-05-08 (continuation #3):** Закрито аудит-пункти #1–#4 з `docs/90-work/audits/archive/2026-05-07-app-audit.md` audit-comparison pass.
