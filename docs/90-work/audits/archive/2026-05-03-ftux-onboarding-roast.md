...
>
> **Cross-refs:**
> [`docs/01-product/launch/product-os/ftux-sprint-plan.md`](../../../01-product/launch/archive/product-os/ftux-sprint-plan.md) — план реалізації рекомендацій ·
...
> - **S0.4 — 9 канонічних подій ✅** _shipped_ у [PR #1582](https://github.com/Skords-01/Sergeant/pull/1582). Wired call-sites на момент закриття S0.4: `celebration_shown` ([`FirstEntryCelebrationModal.tsx`](../../../../apps/web/src/core/onboarding/FirstEntryCelebrationModal.tsx)), `module_checklist_shown/_step_done/_dismissed` ([`ModuleChecklist.tsx`](../../../../apps/web/src/core/onboarding/ModuleChecklist.tsx)), `onboarding_step_viewed/_step_completed` ([`OnboardingWizard.tsx`](../../../../apps/web/src/core/onboarding/OnboardingWizard.tsx)), `hint_completed/_dismissed` (`HintsOrchestrator.tsx`, згодом видалений разом із web-підказками), `streak_milestone_reached` ([`dashboardCards.tsx`](../../../../apps/web/src/core/hub/dashboard/dashboardCards.tsx) — `<StreakIndicator/>` у hub, бо `<StreakCelebration>`-модалка ще не змонтована в дашборд). `first_real_entry` + `ftux_time_to_value` уже стріляли до S0.4 з `firstRealEntry.ts`; `budget_set` — з [`Budgets.tsx`](../../../../apps/web/src/modules/finyk/pages/budgets/Budgets.tsx). Funnel `started → step_viewed → step_completed → vibe_picked → first_action_picked → ftux_preset_picked → first_real_entry → celebration_shown` без gap-ів.
...
> **План реалізації:** [`docs/01-product/launch/product-os/ftux-sprint-plan.md`](../../../01-product/launch/archive/product-os/ftux-sprint-plan.md) — кожна рекомендація розписана як user-story + PR з AC і метриками.
...
7. **PostHog (або еквівалент).** Без funnel-метрик усе вищезазначене — це гіпотези. Це **передумова** до будь-якого A/B. _Web-частина закрита станом на 2026-05-03 — див. errata v2 угорі та [`ftux-sprint-plan.md` §2](../../../01-product/launch/archive/product-os/ftux-sprint-plan.md#2-sprint-0--analytics-live-1-тиждень) (S0.4 + S0.5 shipped). Mobile parity (S0.3) лишається._
...
> **Що далі:** [`docs/01-product/launch/product-os/ftux-sprint-plan.md`](../../../01-product/launch/archive/product-os/ftux-sprint-plan.md) розкладає всі 22 рекомендації по 5 спринтах із PR-розбивкою, AC, метриками і ризиками.
