...
> **Closure note (2026-06-15, fizruk-part2-audit-close):** - [x] Виправлено. Додано page-tests для всіх 4 сторінок:
> `Progress.test.tsx` (empty-state, PR-detection через Epley-1RM з mock-workouts, weight-delta, pushup cross-module card),
> `Measurements.test.tsx` (empty-form guard F4, out-of-range zod gate F3, decimal parse, delta, delete touch-target F8, label↔input F13),
> `Programs.test.tsx` (Kyiv `todayDayIndex` driving "Розпочати сьогодні" vs "Сьогодні відпочинок" F2, day-strip SR-summary F16, safe sessionKey resolution F6, activate/deactivate CTAs),
> `Body.test.tsx` (form inputs, energy/mood ARIA radiogroups F14, out-of-range weight zod boundary, valid submit, trends-collecting placeholder).
> Sub-component `Progress/PrBoard.test.tsx` covers the filter strip + empty states + 44px pill touch-target (F7). `Body/trendUtils.test.ts` already existed (firstValidValue/lastValidValue). 40 tests pass.
...
> **Closure note (2026-06-02, fizruk-part2-quick-wins):** - [x] Виправлено. `Programs.tsx` "Розпочати сьогодні" handler тепер робить `const session = prog.sessions[todaySession.sessionKey]; if (!session) { captureException(...); return; }` — non-null `!` прибрано (Hard Rule #19); silent schedule↔sessions drift логується у Sentry замість runtime-крешу в дочірньому компоненті.
...
> **Closure note (2026-06-02, fizruk-part2-quick-wins):** - [x] Виправлено. Обидві PR-filter pills у `Progress.tsx` (`Всі` + per-muscle) тепер `min-h-[44px]` замість `h-7` (28px) — WCAG 2.5.5 touch-target floor.
...
> **Closure note (2026-06-15, fizruk-part2-audit-close):** - [x] Виправлено (touch-target + semantic token). Кнопка "Видалити" у `Measurements.tsx` тепер `touch-target` (44×44 floor per `apps/web/AGENTS.md § Touch targets`) + `focus-ring` + `aria-label="Видалити замір"` + `type="button"`. Колір переведено з `text-danger/80` (opacity-on-saturated-fill) на семантичний `text-danger-strong`. `-mr-2 px-2` тримає візуальну позицію щільною, розширюючи hit-area. Двостадійний confirm НЕ додано — Undo-toast (`showUndoToast` → `restoreEntry`) лишається механізмом recovery (audit option pari з рештою модуля); explicit confirm-modal — окремий UX-follow-up якщо знадобиться.
...
> **Closure note (2026-06-15, fizruk-part2-audit-close):** - [x] Verified-already-done. Жодного raw `rgb(…)` у chart-props не лишилось. `Progress.tsx` тепер передає `color={statusColors.success}` / `statusColors.warning`; `Body.tsx` — `statusColors.success`, `chartSeries.fizruk.primary`, `statusColors.warning`, `chartPalette[8]`. Усі резолвляться з `@sergeant/design-tokens` (canonical token source, re-exported via `@shared/charts`) — dark-variant покривається token-системою, single source of truth. Закрито у попередньому token-pass; цей audit лише підтверджує.
...
> **Closure note (2026-06-02, fizruk-part2-quick-wins):** - [x] Виправлено. Day-strip контейнер у `Programs.tsx` тепер `role="img"` + `aria-label` зі списком тренувальних днів програми (напр. "Розклад програми …: Пн, Чт — тренування, інші дні відпочинок").
...
> **Closure note (2026-06-02, fizruk-part2-quick-wins):** - [x] Виправлено (стейл-group частина). Додано `useEffect` у `Progress.tsx`, який скидає `prFilter` → `"all"`, коли поточна група мʼязів зникла з `prs` (напр. видалили всі chest-сети у Workouts) — більше немає застряглого порожнього стану. URL-search-params persistence (back-nav share-link) лишається окремим follow-up.
...
> **Closure note (2026-06-15, fizruk-part2-audit-close):** - [x] Виправлено. PR Board (найбільший IIFE, ~130 рядків) винесено у `pages/Progress/PrBoard.tsx` — дзеркалить `Body/` split-precedent. `Progress.tsx` тепер 478 raw LOC (з 604). Примітка: файл і до цього **проходив** lint-gate `max-lines:[error,600]` бо правило має `skipBlankLines:true, skipComments:true` (eslint-counted < 600, тоді як `wc -l` показував 604) — eslint на `Progress.tsx` був зелений. Split дає реальний headroom + краще decomposition. Решта розділів (QuickStatsHeader, PushupActivityCard, WeightFatCards, MuscleVolumeBars) лишаються inline — extract за потреби.
...
> **Closure note (2026-06-15, fizruk-part2-audit-close):** - [x] Виправлено (explicit fields). `MeasurementEntry` у `hooks/useMeasurements.ts` тепер декларує всі 14 канонічних полів явно (`weightKg?`, `bodyFatPct?`, `neckCm?`, … `calfRCm?`) — IDE-autocomplete і typed-доступ для відомих ключів. Index-signature `[field: string]: number | string | undefined` **свідомо лишено** (з документуючим коментарем): web-тип мусить бути structurally-assignable до domain `@sergeant/fizruk-domain` `MeasurementEntry` (який сам має `[fieldId: string]: string | number | undefined`), бо `extractMeasurementSnapshots(entries: readonly MeasurementEntry[])` його споживає; плюс runtime-доступ `e[f.id]` де `f.id: string` з `MEASURE_FIELDS as const`. Повне прибирання signature (audit-варіант `& Partial<Record<MeasurementFieldId, number>>`) зламало б цю domain-compat — тому explicit-fields-over-retained-signature є коректним closed-станом. Stray-key drift тепер видно через явні поля; повна заборона stray keys — domain-package change (поза boundary), окремий follow-up якщо домен прибере свій index signature.
...
> **Closure note (2026-06-15, fizruk-part2-audit-close):** - [x] Verified-already-done. `packages/fizruk-domain/src/lib/trainingPrograms.test.ts` вже містить `describe("BUILTIN_PROGRAMS integrity")` з саме тим assertion-ом, що рекомендує audit: "every schedule sessionKey exists in the program sessions map" (`expect(program.sessions[slot.sessionKey]).toBeDefined()` для кожного program×slot), плюс "each program has at least one session definition", "program.days equals schedule length", "all schedule day values are 1..7". Integrity-invariant покрито на boot-рівні домену. Файл — у domain-package (поза fizruk-web-boundary цього прогону), змін не потрібно.
