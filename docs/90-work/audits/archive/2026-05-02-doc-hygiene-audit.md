...
- Основні проблеми — **дрейф у docs**, не в коді: ADR має пропуск номера (`0029`), README ADR має stale claim про «наступний номер», `docs/90-work/audits/` тримає поверх-перетину UX-документи без чіткого lifecycle, а `CLAUDE.md` / `DEVIN.md` майже клони.
...
- `apps/{web, server, mobile, mobile-shell, console}` + `packages/{shared, api-client, config, db-schema, design-tokens, finyk-domain, fizruk-domain, nutrition-domain, routine-domain, insights, eslint-plugin-sergeant-design}` — чисте розділення runtime-сервіси vs shared logic, точно як описано в [`docs/04-governance/adr/0024-monorepo-apps-packages-split.md`](../../../04-governance/adr/0024-monorepo-apps-packages-split.md).
...
- `.agents/skills/` (12 SKILL.md) + `docs/00-start/agents/agent-skills-catalog.md` мають deprecated→replacement-таблицю — добре читається.
...
| 1.2 | **`AGENTS.md` — 752 рядки / 70KB**                                                                                                    | Це найбільший repo-policy doc. Усе в ньому **варто** мати, але читачі-агенти не можуть його повністю утримати в контексті без фрагментації. Hard Rules (1–17) разом з шукаються найчастіше — їх можна винести в окремий `docs/04-governance/governance/hard-rules.md` (не вирішено в цьому PR; medium-task у §5). |
| 1.3 | **Дублікат «next review» дат**                                                                                                        | README.md, AGENTS.md, docs/README.md, docs/03-operations/postmortems/README.md, docs/04-governance/governance/README.md, docs/00-start/playbooks/README.md мають один і той самий `Last validated: 2026-05-02 by @claude` — не баг, але видно що це bulk-bump, не реальна ревалідація.                            |
...
**Проблема:** `docs/04-governance/adr/` має `0001…0028, 0030, 0031`. Між ними немає 0029.

- `docs/04-governance/adr/README.md:103` каже:
...
- Оновлено нотатку «Note on missing 0029» у `docs/04-governance/adr/README.md` з повним поясненням.
- Додано unit-тести у `scripts/docs/__tests__/check-adr-graph.test.mjs` для gap-rule та KNOWN_NUMBERING_GAPS allowlist.

### 2.2 Audit-перетин у `docs/90-work/audits/`
...
- `2026-04-28-ux-ui-audit.md` (30 KB) — UX/UI 2026
- `2026-04-28-ux-improvement-plan.md` (28 KB) — техплан UX
...
- Додано `Status` стовпчик у `docs/90-work/audits/README.md` (Active / Closed / Archived) разом із Lifecycle-секцією, що пояснює семантику.
- `ux-audit-2025.md` фізично перенесено в `docs/90-work/audits/archive/`, його `Status: Active` оновлено на `Status: Archived`, додано блок-нотатку про перенесення.
- Закриті аудити (`2026-04-26-*`, `2026-04-28-comprehensive-*`, `2026-04-28-ux-ui-audit`) позначено як `Closed`; активні трекери (`*-implementation-roadmap`, `2026-04-28-ux-improvement-plan`, `typography-2026-04-audit`) — `Active`.
- Шлях `docs/90-work/audits/ux-audit-2025.md` → `docs/90-work/audits/archive/ux-audit-2025.md` оновлено в `scripts/docs/freshness-config.json` і у прикладі в `docs/04-governance/governance/doc-freshness.md`.
...
- `docs/04-governance/adr/**` — свідомо виключені (immutability, описано в `doc-freshness.md`). OK.
- `docs/00-start/playbooks/_TEMPLATE-decision-tree.md` — це template, OK.
- `docs/03-operations/postmortems/TEMPLATE.md` — template, OK.
...
| ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docs/04-governance/adr/README.md:103`                  | «наступний номер `0031`» — реально 0032                                                                                                                                                   | Розв'язано                                                                                                                                                                                                 |
| `docs/90-work/audits/README.md`                         | каже `ux-audit-2025.md` «архівний», але файл не в archive/ і має `Status: Active`                                                                                                         | Розв'язано                                                                                                                                                                                                 |
| `docs/90-work/audits/archive/2026-04-28-ux-ui-audit.md` | "Дата аудиту: 28 квітня 2026" + "Дата оновлення: 2 травня 2026" + `Last validated: 2026-05-02` — три параметри дат у різних місцях шапки. Краще канонізувати під єдиний freshness-header. | ✅ Розв'язано / non-actionable (2026-06-03 — файл уже в `archive/`, має єдиний `Last validated:` freshness-header + явно позначений `Initial audit date: 2026-04-28 (історичний)`; дрейф дат канонізовано) |

### 2.6 Empty postmortems index

`docs/03-operations/postmortems/INDEX.md` має валідну структуру + freshness, але рядок «No published postmortems yet». Розглянуто і залишено без змін: empty-state-таблиця з посиланням на TEMPLATE — це інтенціональний onboarding-сигнал, не bloat. Якщо incidents справді не було — це bonus.
...
`strip-js-extensions` codemod (одноразовий міграційний інструмент, був у `scripts/`). Згадується тільки в `docs/90-work/tech-debt/frontend.md`. Не дзвонять з `package.json` чи CI.
...
- `docs/02-engineering/notes/spikes/routine-sqlite-v2.md` — лише один spike. Якщо це паттерн (плани мати кілька spike-ів), залишити. Якщо це one-off, можна перенести в `docs/90-work/planning/storage-roadmap.md` як inline-секцію.
...
2. ✓ **`docs/90-work/audits/README.md`**: додано `Status` стовпчик + Lifecycle-секція; `ux-audit-2025.md` перенесено в `archive/`.
...
5. ~~**Розбити `AGENTS.md`** + завести canonical full-text Hard Rules source~~ ✅ **Closed / non-actionable (2026-06-03):** семантичний gap уже закрито — canonical full-text Hard Rules source **існує**, але не як один монолітний `hard-rules.md`, а як **per-rule canonical bodies** у [`docs/04-governance/governance/rules/`](../../../04-governance/governance/rules/README.md) (по одному файлу на правило: повна проза + scope + enforced-by + BAD/GOOD приклади, напр. [`10-lifecycle-markers.md`](../../../04-governance/governance/rules/10-lifecycle-markers.md)). Архітектура single-source тепер три-рівнева й узгоджена:
   - **`docs/04-governance/governance/rules/*.md`** — authoritative full text (26 rule + `kyiv-time-helpers.md`), index у [`rules/README.md`](../../../04-governance/governance/rules/README.md).
   - **`AGENTS.md § Hard rules`** — компактна таблиця (number + 1-line summary + category + лінк на per-rule файл), а не дубльований full text.
   - **`docs/04-governance/governance/hard-rules-matrix.md`** (+ `hard-rules.json`) — auto-generated machine index; кожен рядок лінкує на відповідний `rules/NN-*.md`.
   ...
   Тобто рекомендований «single full-text source» реалізовано (краще, ніж один файл — per-rule файли + index), а матриця/AGENTS.md уже на нього посилаються. Створювати окремий `docs/04-governance/governance/hard-rules.md` означало б **дублювати** контент, у якого вже є single source — тому окремої дії не потрібно. Залишковий «AGENTS.md ще довгий» — це cosmetic-refactor, не doc-hygiene gap, і трекається окремо (initiative `0009-agent-os-hardening`, Phase 3.1).

6. ~~**`2026-04-28-ux-ui-audit.md` дати** — канонізувати під єдиний freshness-header (зараз 3 параметри в шапці).~~ ✅ Closed / non-actionable (2026-06-03): drift уже розв'язано. Файл переїхав у [`docs/90-work/audits/archive/2026-04-28-ux-ui-audit.md`](./2026-04-28-ux-ui-audit.md), його шапка тепер має єдиний канонічний `> **Last validated:** 2026-05-13` freshness-header + `Status: Closed` + явно позначений `> **Initial audit date:** 2026-04-28 (історичний)`. Три «конкуруючі» дати канонізовано в один freshness-header плюс explicitly-labeled historical date — окремої дії не потрібно.
...
- Hard Rules registry (`docs/04-governance/governance/hard-rules.json` + matrix + AGENTS.md) — найкраще, що я бачив у зрілому monorepo.
