...
> **Що це.** Це не postmortem справжнього incident-у. Це **retrospective audit / near-miss**, який закриває [`docs/90-work/initiatives/archive/_0011-foundation-adoption-and-process-discipline.md`](../../initiatives/archive/_0011-foundation-adoption-and-process-discipline.md) §Фаза 1 → PR 1.4. Питання, на яке цей документ відповідає: **«Чи був прапорець `CSP_DISABLE=1` коли-небудь увімкнений у production-Railway env-var-ах за 16 днів існування?»** Code-side cleanup завершений у [PR #1631](https://github.com/Skords-01/Sergeant/pull/1631); цей файл закриває operational boundary.
...
- `CSP_DISABLE=1` runtime kill-switch жив у `apps/server/src/http/security.ts` **16 днів** (2026-04-18 → 2026-05-04). Видалений у [PR #1631](https://github.com/Skords-01/Sergeant/pull/1631) разом з [M1 hardening card](../../../04-governance/security/hardening/archive/M1-csp-disable-runtime-flag.md).
...
| **Reviewers**         | @Skords-01                                                                                                                                                                                                                                                                                                                                                       |
| **Related artefacts** | [M1 hardening card](../../../04-governance/security/hardening/archive/M1-csp-disable-runtime-flag.md), [`access-policy.md`](../../../04-governance/security/access-policy.md), [PR #128](https://github.com/Skords-01/Sergeant/pull/128), [PR #345](https://github.com/Skords-01/Sergeant/pull/345), [PR #1631](https://github.com/Skords-01/Sergeant/pull/1631) |
...
- **Audit-trail gap:** Railway env-var changes не пишуться у `docs/04-governance/security/secret-ownership-register.md`, тому навіть legitimate-flip залишився б невидимим до моменту коли хтось би запустив `printenv | grep CSP` у production-shell.
...
| 2026-05-03              | Deep security review зафіксував M1: «`CSP_DISABLE=1` runtime fault-injection vector», CVSS 6.1.                                               | [`docs/04-governance/security/hardening/M1-csp-disable-runtime-flag.md`](../../../04-governance/security/hardening/archive/M1-csp-disable-runtime-flag.md)                          |
...
| 2026-05-04              | Initiative 0011 (vector-assessment 100-PR-roast) виявив, що audit на «чи був enabled у проді» НЕ зроблений у PR #1631 (operational boundary). | [`docs/90-work/initiatives/archive/_0011-foundation-adoption-and-process-discipline.md`](../../initiatives/archive/_0011-foundation-adoption-and-process-discipline.md) §Чому зараз |
...
2. **Реєстрації flag-а** у `docs/04-governance/security/access-policy.md` чи `docs/04-governance/security/secret-ownership-register.md` як «runtime security knob».
...
3. **Audit-trail:** `grep -r CSP_DISABLE apps/server/src/` повертає `no matches`. Згадки лишились у `/docs/04-governance/security/hardening/M1-csp-disable-runtime-flag.md` (closed-status-картка) і у цьому документі (історичний контекст).
...
| A4  | Оновити [`docs/04-governance/security/secret-ownership-register.md`](../../../04-governance/security/secret-ownership-register.md): додати retroactive-row для `CSP_DISABLE` із status `removed 2026-05-04` і lifetime `2026-04-18 → 2026-05-04`.                                                          | @Skords-01 | 2026-05-11 | closed |
...
4. **Cross-link з PR 1.3.** [PR 1.3 (#1697)](https://github.com/Skords-01/Sergeant/pull/1697) додає staging-verification gate на deploy-config-файли в репо. Цей audit показує, що runtime env-var changes у Railway dashboard — окремий клас ризику, поза boundary PR 1.3. Обидва треба у комплексі для повного coverage:
   ...
   - **Цей audit + A5** — фіксує що Railway env-var drift лишається unprotected; робота винесена у [`docs/90-work/tech-debt/backend.md` § Operational visibility — Railway env-var changes](../../tech-debt/backend.md#operational-visibility--railway-env-var-changes) як backlog-item (окрема ініціатива — пізніше).
...
>
> **Action artefacts.** Branch що закриває A1–A5 (цей PR) включає: (а) розширення `docs/04-governance/security/secret-ownership-register.md` retroactive-row для `CSP_DISABLE` із Status/Lifetime колонками (A4); (б) запис у `docs/90-work/tech-debt/backend.md` про Railway env-var change-tracking gap для майбутньої окремої ініціативи (A5 — без open initiative-файлу за рішенням @Skords-01); (в) оновлення `### Carry-over → successor` у [`0011-foundation-adoption-and-process-discipline.md`](../../initiatives/archive/_0011-foundation-adoption-and-process-discipline.md) — A1–A5 переведено у `[x]` checked, регенеровано `follow-ups.md`.
>
> **Tooling caveats.**
>
...
- [M1 hardening card](../../../04-governance/security/hardening/archive/M1-csp-disable-runtime-flag.md) — original closed card with technical detail, code-snippets, and tests.
- [`access-policy.md`](../../../04-governance/security/access-policy.md) — `Runtime security knobs` section formalising «no runtime CSP kill switch».
- [`docs/90-work/initiatives/archive/_0011-foundation-adoption-and-process-discipline.md`](../../initiatives/archive/_0011-foundation-adoption-and-process-discipline.md) §Фаза 1 → PR 1.4 (this audit).
