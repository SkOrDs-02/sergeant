...
**Фікс:** зважити вибір першого ходу ще й за латентністю. `flash-lite` дає
11/12 за 2,2 с, `gemini-3.1-flash-lite` — 10/12 за 1,25 с; різниця в точності
одна-дві пастки, різниця в часі — 5–13×.
...
- **B37 живцем:** `OPENROUTER_COACH_MODEL` резолвиться в `openai/gpt-5.1` —
  тобто стенд коуча тестує саме ту модель, яку tier-таблиця перевизначає на
  Sonnet.
