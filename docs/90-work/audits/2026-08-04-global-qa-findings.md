...
> **Last touched:** 2026-09-05 by Codex. **Next review:** 2026-12-07.
> **Status:** Active

## Повторна валідація 2026-09-05

Цей файл — журнал серпневого прогону, а не автоматично актуальний backlog.
Нижче зафіксовано перевірку `main` від 2026-09-05: код, регресійні тести та
новіший [product audit від 2026-09-01](./2026-09-01-product-audit/findings.md).
Старі докази лишаються історією, але не можуть самі по собі доводити поточний
стан.

| №   | Вердикт 2026-09-05 | Поточний доказ / дія                                                                                                                                                                                                             |
| --- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Відкрито**       | Симптом повторно зафіксовано 2026-09-01: 3 з 376 завантажень лишили порожній `#root`. Серпнева привʼязка до kvvfs/auth-переходів більше не доведена після закриття №2; потрібен контрольований repro і окрема root-cause задача. |
| 2   | Закрито            | `makeSqliteKvStoreClient` щоразу бере live SQLite handle; регресія покриває зміну partition без `DB has been closed`.                                                                                                            |
| 3   | Закрито            | `toJsonbParam()` пропускає валідний JSON-рядок без double-stringify; це зафіксовано unit-тестом.                                                                                                                                 |
| 4   | Закрито            | `AuthContext` має `signedOut` marker, що переважає застарілий React Query cache; є регресія для logout.                                                                                                                          |
| 5   | Закрито            | Онбординг більше не веде authenticated user на `/welcome`; вибір модулів одразу синхронізується з акаунтом.                                                                                                                      |
| 6   | Закрито            | Upstream 401 тепер перетворюється на 502, 429 — на 503, без хибного повідомлення про доступ користувача.                                                                                                                         |
| 7   | Закрито            | Refund робить атомарний `GREATEST(0, request_count - cost)`, тому не порушує check constraint.                                                                                                                                   |
| 8   | Закрито            | Додано legacy redirect `/finyk/cards` → `/finyk/assets` з unit-покриттям для path і hash.                                                                                                                                        |
| 9   | Закрито            | Завершене тренування на route-owned сторінці показує `WorkoutSummaryView`, не dead-end «Активне тренування не знайдено».                                                                                                         |
| 10  | Закрито            | `ManualExpenseSheet` показує «Максимальна сума: 10 000 000 ₴» та блокує save; це має регресійний тест.                                                                                                                           |
| 11  | Закрито            | Генератор повертає конкретну причину й наступний крок: повторити генерацію або додати продукти/змінити режим комори; покрито тестами `useNutritionRemoteActions`.                                                                |
| 12  | Закрито            | Старі англомовні literals більше не є user-facing: Quick Start лишився лише в історичному коментарі, а profile date використовує локалізований `DateField`.                                                                      |
| 13  | Закрито            | Поточна копія: «перевір свою поштову скриньку»; застарів лише коментар у тесті, його синхронізовано.                                                                                                                             |
| 14  | Закрито            | `/assistant` рахує сценарії з каталогу динамічно; константи «80» і «~60» прибрані.                                                                                                                                               |
| 15  | Закрито            | `ModuleBottomNav` ховає visual label від accessibility tree і лишає одне `sr-only` імʼя; коментар описує регресію №15.                                                                                                           |
| 16  | Закрито            | `POST /api/ai-memory/event-sync` видалено 2026-08-29; прибрано також застарілу заяву про живий producer у shared schema.                                                                                                         |
| 17  | Закрито            | Production startup тепер fail-fast без `RESEND_API_KEY`; сервер не може підтвердити запит на лист у конфігурації, де доставлення неможливе.                                                                                      |
| 18  | Закрито            | Date control починається порожнім (`""`), не з `0/0/0`; це покрито `BiometricsSection` test.                                                                                                                                     |

Отже, чинним дефектом із цього журналу лишається лише №1. Пункти 2–18 не
треба переносити як «відкриті» у нові задачі.
...
## Історична read-only код-діагностика (не поточний реєстр)

Цей розділ зберігає нотатки сесії 2026-08-04. Заявлений результат «47
верифікованих знахідок» не має у файлі відтворюваного переліку з
acceptance-критеріями, code revision і окремим доказом на кожен пункт.
Тому під час повторної валідації він **не є чинним backlog-ом і не є
підтвердженим числом**. Нижче — історичні гіпотези для окремого, evidence-first
аудиту, а не список дефектів, які можна переносити чи рахувати як відкриті.
