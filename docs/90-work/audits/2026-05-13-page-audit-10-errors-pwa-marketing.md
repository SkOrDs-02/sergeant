...
**Why it matters.**
The product promise is "Sergeant works offline — your data is queued, not lost" (per `OfflinePage` description copy and `docs/05-design/design/design-system.md § 15 Offline`). Users on flaky connections (subway, basement, EU-roaming) will see an unbranded browser chrome instead of the curated `<EmptyState>` + `<OfflineIllustration>` surface, and the "your data is safe locally" reassurance never reaches them. This silently regresses the PWA's offline-first value proposition.
