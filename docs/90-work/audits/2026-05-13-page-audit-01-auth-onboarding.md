...
>
> **Final closure (2026-06-03, explicit "finish all items now" request — deprecation window closed early):** Done. `PermissionsPrompt.tsx` і `PermissionsPrompt.test.tsx` більше не існують у дереві (`ls apps/web/src/core/onboarding/PermissionsPrompt.*` → "No such file or directory"). Repo-wide grep `grep -rn "PermissionsPrompt" apps/web/` → **0 збігів** (включно з `picksStorage.ts` — там немає JSDoc-посилання на назву компонента, лише прозовий коментар L13–L16 про «just-in-time prompt», який залишається коректним і самодостатнім). Єдині залишкові згадки — історичні/архівні: `docs/90-work/planning/dev-stack-roadmap.md:205` (changelog-запис, що knip-cleanup у [#716](https://github.com/Skords-01/Sergeant/pull/716) уже видалив файл) і `docs/90-work/audits/archive/ux-audit-2025.md` (архів, `Status: Archived` — контент не редагується per Rule #10). Жодного live-імпорту немає. Deprecation window (2026-07-15) закрито достроково на явний запит користувача; мертвий код повністю прибрано, F4 closed. Код не видалявся в рамках цього кроку (файли вже були відсутні), тож окремий typecheck-прогін не потрібен.
