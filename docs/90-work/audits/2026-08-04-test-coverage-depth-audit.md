...
> **Last touched:** 2026-08-25 by @claude. **Next review:** 2026-12-03.
...
**Побічна знахідка (окремий борг, не виправлено тут).** Числові поля вимірювань у
`applyMisc.ts` (`weight_kg`, `waist_cm`, `chest_cm`, `hips_cm`, `bicep_cm`,
`sleep_hours`, `energy_level`, `mood`) парсяться через **необмежені**
`parseOptionalNumber` / `parseOptionalInt` — без min/max guard. Сусідній
nutrition-апплаєр (`sync/nutrition/applySyncGoals.ts`) для аналогічних полів уже
використовує `parseOptionalBoundedNumber` з `syncV2-core.ts`, тобто хелпер існує і
патерн у репо є — fizruk просто його не застосував. Наслідок: клієнт може
записати `weight_kg: -500` чи `sleep_hours: 999`. Поточну поведінку **закріплено
тестом-пином** (назва явно каже «documents current behavior»), щоб мовчазна зміна
впала червоним. Виправлення — окремий PR під `sergeant-data-and-migrations`.
