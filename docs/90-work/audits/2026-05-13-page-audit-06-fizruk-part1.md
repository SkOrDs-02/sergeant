...
**Description.**
Hard Rule #10 (`lint-enforced-convention`) requires every file/doc to declare `> **Last validated:** YYYY-MM-DD by …` and `> **Status:**` (Active / Scaffolded / Deprecated / Archived). None of the ~13 scope files in this audit carry that header — neither the four route-shell pages nor the shell/router/header modules. Other audited surfaces in the repo (e.g. `docs/90-work/audits/archive/2026-05-07-app-audit.md`) do declare the header, so the omission is a discipline drift rather than a missing convention.
