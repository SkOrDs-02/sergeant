...
> **Owner:** `@Skords-01`
> **ETA:** 2 weeks
> **Sources:** Design Review 2026-05-03 §6, [`docs/90-work/tech-debt/frontend.md`](../tech-debt/frontend.md)
...
- [ ] Усі 8 top-level routes (Hub, Finyk, Fizruk, Nutrition, Routine, Insights, Settings, Onboarding) — окремі lazy-chunks. _Прогрес: 7/8 — `/nutrition/*` ([#2104](https://github.com/Skords-01/Sergeant/pull/2104)) + `/finyk/*` ([#2108](https://github.com/Skords-01/Sergeant/pull/2108)) + fizruk (PR #2541) + routine (Phase 2.d) + `/insights/*` + `/settings/*` + `/onboarding/*` (2026-06-15 — мігровано off the catch-all `path:"*"` HubPage). Lazy-chunk split тепер РЕАЛЬНИЙ: після RootLayout+Outlet (Phase 5, `bd5b06bde`) кожен route resolve-ить через `lazy: () => import(...)` і виходить окремим Rollup-чанком (`HubReports-*`, `HubSettingsPage-*`, `WelcomeScreen-*` для трьох нових; `<Name>App-*` для модулів) — старий «location-context bug блокує lazy» більше не діє. Лишається Hub (index + catch-all `HubPage`), який свідомо ділить один чанк (hub home + standalone routes + 404)._
...
- [x] Hash-URL compat shim реалізовано (HashRedirect.tsx + Vitest unit test у Providers). _Playwright e2e тест — DONE (2026-06-15): `apps/web/tests/smoke/hash-redirect-smoke.spec.ts` (`@critical`) перевіряє `/#fizruk/workouts` → `/fizruk/workouts` через URL-assertion (`toHaveURL`) + mount fizruk-shell._
- [x] Route-loaders використовуються щонайменше у 4 модулях (prefetch RQ-data). _Phase 5 done — `useModuleRouteLoader` у `ActiveModuleView.tsx` prefetch-ить `billingKeys.status`, `pushKeys.vapid` для всіх 4 модулів (finyk/fizruk/nutrition/routine) + `finykKeys.monoSyncState` для finyk при кожній навігації до модуля._
...
| `HashRedirect.test.tsx`                  | Vitest/RTL unit test. Playwright e2e тест (старий PWA-link `https://app/#fizruk/workouts` → новий `/fizruk/workouts`) — **DONE 2026-06-15**: `apps/web/tests/smoke/hash-redirect-smoke.spec.ts` (`@critical`, URL-assertion як deterministic signal). |
...
| Що | Як зафіксовано |
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| Per-route budgets | `scripts/check-bundle-size.mjs` — `FinykApp-`/150KB, `FizrukApp-`/180KB, `RoutineApp-`/120KB, `NutritionApp-`/100KB, total/500KB gzip. **2026-05-24:** prefix-и оновлено з `finyk-`/`fizruk-`/`routine-`/`nutrition-` на `<Name>App-` під реальні Rollup filename-и; до цього всі module-chunk-и попадали у "uncategorized" і budget-checker silently no-op-ив. Vendor-prefix-и теж перейменовано з `react-`/`router-`/`sentry-`/`virtuoso-`/`tanstack-` на `vendor-<name>-` (реальний chunkFileName patern із `manualChunks`). Додано budget-и для `vendor-auth-` (80), `vendor-zod-` (20), `vendor-zxing-` (250), `vendor-sqlite-` (280), `vendor-web-vitals-` (5) + catch-all `vendor-` (120) — раніше їх не moнitорили. Видалено orphan `recharts-` budget (dependency `recharts` не існує у `apps/web/package.json` — імовірно legacy від часів пре-T4-B chart-стеку). |
...
| `manualChunks` у vite.config.js | **Audited 2026-05-24 — no changes.** Кожна група має активний import: `vendor-react` (react/react-dom + transitives — runtime), `vendor-router` (react-router-dom — RouterProvider), `vendor-react-query` (@tanstack/_ — RQ + persister), `vendor-virtuoso` (react-virtuoso — 8 callsite-ів у nutrition/routine/finyk/fizruk + shared PullToRefresh), `vendor-zxing` (@zxing/_ — barcode scanner у nutrition), `vendor-auth` (better-auth + транзитиви — eager у `shared/lib/api`), `vendor-zod` (zod + @hookform/resolvers — dedupe між formed-модулями), `vendor-sentry` (@sentry/react — observability), `vendor-web-vitals` (dynamic import з idle-callback), `vendor-sqlite` (sqlite-wasm + drizzle — local DB), `@capacitor/*` → `undefined` (свідомо merge-иться у dynamic native chunks). Без цього split-а initial vendor лопнув би: поточний gzipped initial ~194 KB тримається саме на цьому розкладі. Initiative scope-у «видалити застарілі групи» немає — нічого не застаріло. **Reconciled 2026-06-15:** Phase 5 cleanup-ціль (per-route lazy entries замість `ActiveModuleView.tsx` switch) БІЛЬШЕ НЕ заблокована — RootLayout+Outlet landed (`bd5b06bde`), location-context bug resolved, і `router.tsx` тепер тримає справжні `lazy: () => import(...)` per-route children (4 модулі + insights/settings/onboarding). Стара примітка про «заблоковано RR7 location-context bug-ом» була stale. |
| Route-loaders | **Split into two mechanisms (reconcile 2026-05-29):** (1) **prefetch-via-hook — DONE.** `apps/web/src/core/lib/useModuleRouteLoader.ts` prefetch-ить `billingKeys.status` / `pushKeys.vapid` / `finykKeys.monoSyncState`, викликається в `apps/web/src/core/app/ActiveModuleView.tsx:70` (`useModuleRouteLoader(activeModule)`) — це й закриває DONE-критерій нижче + header «Phase 5 route-loaders closed 2026-05-23». (2) **native RR7 router-level loader entries — UNBLOCKED, ще не wired (reconcile 2026-06-15).** Стара примітка казала «STILL BLOCKED … router.tsx має один catch-all `path: "*"`» — це stale: RootLayout+Outlet вже landed (`bd5b06bde`), і `router.tsx` тепер має повноцінні per-route children (`finyk/*`, `fizruk/*`, `nutrition/*`, `routine/*`, `insights/*`, `settings/*`, `onboarding/*` + index + catch-all). Архітектурного блокера на `loader:`-поле більше немає — кожен child може отримати `loader` без винесення провайдерів. Прагматично prefetch-via-hook (`useModuleRouteLoader`) уже покриває DONE-критерій; перехід на native `loader:` — optional follow-up, не блокований. | |
...
| PR                                                       | Title                                                                         | Merged     |
| -------------------------------------------------------- | ----------------------------------------------------------------------------- | ---------- |
| [#308](https://github.com/Skords-01/Sergeant/pull/308)   | docs(docs): reconcile 90-work portfolio readiness                             | 2026-07-18 |
