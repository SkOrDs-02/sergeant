...
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/90-work/initiatives/stack-pulse-2026-05/pr-23-openapi-contract-tests.md`.
...
- **Full-route contract roundtrip:** representative `/api/me` success/401 coverage landed у `apps/server/src/__tests__/contracts/openapi-roundtrip.test.ts`; повне покриття всіх маршрутів ще не зроблено.
- **Schemathesis property-based testing:** не імплементовано. `.github/workflows/contract-tests.yml` не існує.
- **CI-job `lint:api-client` drift detector (per PR-18):** generated OpenAPI freshness checks уже виконуються у новому `.github/workflows/contract-tests.yml`; окремий client-consumer drift detector ще не зроблено.
...
- [ ] `apps/server/src/__tests__/contracts/openapi-roundtrip.test.ts` розширений з `/api/me` до ≥80% маршрутів сервера.
...
1. PR-1: ADR-0062 + code-first generation pipeline (no contract tests yet).
