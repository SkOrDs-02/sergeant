...
> **Status:** Phase 1 + Phase 2 scaffolding + Phase 2a + **Phase 2b shipped (2026-06-15).** Phase 1: baseline extracted into `eslint.baseline.js` (~180 рядків, byte-identical `--print-config`). Phase 2 scaffolding (2026-06-01): `scripts/eslint-print-config-diff.mjs` + surface snapshots під `scripts/__fixtures__/eslint-print-config/` + `pnpm lint:eslint-config-diff` gate. Phase 2a (2026-06-05): per-surface extraction — блоки у `eslint.web.js` / `eslint.server.js` / `eslint.mobile.js` / `eslint.shell.js` / `eslint.packages.js` / `eslint.cross-surface.js`; root manifest 1128 → 37 рядків. **Phase 2b (2026-06-15):** справжні standalone per-package `eslint.config.js` у 12 linted-пакетах (`apps/{web,server,mobile,mobile-shell}`, `packages/{shared,api-client,db-schema,finyk-domain,fizruk-domain,nutrition-domain,routine-domain,eslint-plugin-sergeant-design}`) — кожен re-export-ить root-manifest через `basePath`-wrapper (`eslint.per-package.js`), тож `turbo run lint` резолвить per-package config замість walk-up до root → unlocks per-package Turbo parallelism. Diff-gate переведено у cd-per-package режим; `normaliseConfig` пофікшено, щоб не корумпувати backslash у regex-селекторах на Windows. **Byte-neutral**: `lint:eslint-config-diff` — усі 7 fixtures match (5 снапшотів незмінні, 2 оновлені лише на pre-existing stale drift: server `max-lines` allowlist + 2 web i18n-allowlist записи). `turbo run lint` зелений: 15/15 tasks, 0 errors.
