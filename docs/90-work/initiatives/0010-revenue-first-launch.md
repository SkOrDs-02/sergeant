...
- [ ] Перший платний користувач: `subscriptions.plan = 'pro'` AND `subscriptions.status IN ('active','trialing')` AND `subscriptions.current_period_end > NOW()` AND `provider IN ('liqpay','plata')`. (Canonical table — m056; legacy `billing_subscriptions` (m047) — orphan, без writers, не використовуй у verification queries.)
