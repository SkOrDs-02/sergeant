...
> **✅ Completed:** Phases 0–6 (code) + m083 orphan DROP + provider-neutral billing copy + OpenAPI billing roundtrip
> **⏳ Pending (founder-блокери only):**
>
> - `APPLE_*` env vars у Coolify/prod
> - ФОП-реєстрація + `LIQPAY_ENABLED`/`LIQPAY_PUBLIC_KEY`/`LIQPAY_PRIVATE_KEY` (+ `PLATA_*` якщо потрібен) для live UA checkout
> - Перший live paid user + webhook evidence; A/B onboarding decision; launch smoke (CI + a11y + Lighthouse)
...
- `docs/90-work/audits/2026-05-04-revenue-and-marketing-roast.md` — аудит-сорс цієї ініціативи (~440 рядків).
- `docs/90-work/initiatives/0010-revenue-first-launch.md` — цей документ.
- Оновити `docs/90-work/initiatives/README.md` — додати рядок 0010 у таблицю активних ініціатив.
...
- [x] PR-и фаз 2–6 змерджені (код: billing multi-provider + paywall + auth + activation + landing/i18n). Live ops evidence — окремі пункти нижче.
- [ ] Перший UA-provider webhook у проді записаний (`provider IN ('liqpay','plata')` — callback/webhook route hit + row у billing event log). Stripe dormant для UA; не критерій launch.
- [ ] Перший платний користувач: `subscriptions.plan = 'pro'` AND `subscriptions.status IN ('active','trialing')` AND `subscriptions.current_period_end > NOW()` AND `provider IN ('liqpay','plata')`. (Canonical table — m056; legacy `billing_subscriptions` (m047) dropped in m083.)
- [ ] `/pricing` показує реальні CTA → LiqPay/Plata checkout (не waitlist), smoke-e2e зелений на enabled UA provider. **₴ UA-only.**
- [x] Google + Email + Apple sign-in реалізовані: `GoogleSignInButton.tsx` + `AppleSignInButton.tsx` + Better Auth providers. Apple provider **є** у `auth.ts` (`generateAppleClientSecret()` ES256 + `getSocialProviders()` реєструє `config.apple` за наявності `APPLE_CLIENT_ID/TEAM_ID/KEY_ID/PRIVATE_KEY`, `appleid.apple.com` у `trustedOrigins`). Лишається тільки **операційна** дія founder-а — APPLE\_\* env vars у Coolify/prod (не код).
...
- [ ] Production smoke: CI зелений на релізному коміті + a11y axe-core + Lighthouse budget (LHCI wired у `.github/workflows/lighthouse-ci.yml`; критерій — свіжий зелений прогін на launch-кандидаті).
...
| ------------------------------------------- | ---------------------- | ------ | --------------------------------- |
| Перший платний (`provider IN liqpay/plata`) | ≥ 1                    | ≥ 10   | SQL `subscriptions`               |
| Activation rate (v2)                        | ≥ 15 %                 | ≥ 30 % | PostHog funnel                    |
| Days to first paid                          | ≤ 28                   | ≤ 14   | calendar                          |
| `/pricing` → checkout open conversion       | ≥ 5 %                  | ≥ 12 % | PostHog                           |
| UA-provider webhook idempotency violations  | 0                      | 0      | LiqPay/Plata callback/event audit |
...
| **[HYGIENE]** Orphan billing table `billing_subscriptions` (m047)                                           | dev          | Audit-verified 2026-05-25 (#3109): m047 — orphan (0 writers, 0 readers). **Dropped 2026-07-20** у `083_drop_billing_subscriptions.sql` (TWO-PHASE-DROP: introduced 2026-05-25 / safe after 2026-06-08; founder confirmed no prod users). Canonical = m056 `subscriptions`.                                                                                                                                                                                         |
...
**Founder action — Apple env vars** (потрібні в Coolify + локально у `.env`):
...
4. Coolify → API service → Environment → додати 4 env-и (для multi-line `.p8`: paste з реальними newlines).
...
### Phase 6 — Landing page + EN locale + sitemap ✅ (closed 2026-07-20 reconcile)
...
- **Shipped:** EN-локаль (`uk.ts`/`en.ts` + `useLocale`), `apps/web/public/sitemap.xml` + `robots.txt`, PricingPage/PlanSection з multi-provider checkout/cancel (LiqPay/Plata; Stripe portal лише для legacy).
...
**Наступний крок (ops, не код):** live LiqPay/Plata — ФОП + env keys у Coolify; перший webhook + paid user. Код provider/checkout/callback уже landed (див. header 2026-07-20).

### Hygiene + contract pass ✅ (2026-07-20)
