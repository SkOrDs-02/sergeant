...
**Deny-list** (не потрапляє ніколи, незалежно від флагів): текст повідомлень і відповідей (`$ai_input`, `$ai_output_choices`), аргументи і результати tool-викликів (`$ai_input_state`/`$ai_output_state` лишаються порожніми у Фазі 2 — доки власник явно не затвердить санітизований піднабір), суми/баланси/валюти, назви контрагентів і мерчантів, назви страв і нутрієнти, OCR-текст і зображення чеків, email/імена. Канонічний перелік чутливих полів — той самий, що в pino-redaction (Hard Rule #21, `llmRedaction.ts`): розширення redaction-списку = перевірка цієї секції.
