...
> **Sources:** Vector assessment 2026-05-04 (внутрішній звіт-прожарка по 100 PR #1564–#1664), [`docs/90-work/initiatives/0010-revenue-first-launch.md`](../0010-revenue-first-launch.md) (governing freeze), [`docs/90-work/initiatives/archive/_0007-design-system-tooling.md`](./_0007-design-system-tooling.md), [`docs/90-work/initiatives/archive/_0008-platform-hardening.md`](./_0008-platform-hardening.md), [`docs/90-work/initiatives/archive/_0009-agent-os-hardening.md`](./_0009-agent-os-hardening.md), [`docs/04-governance/governance/hard-rules.json`](../../../04-governance/governance/hard-rules.json)

## TL;DR

За 100 PR з 2026-05-03/04 ми збудували **4 foundation-инструменти зі змінним adoption-рівнем** і допустили **3 типи процес-помилок** (порожнє тіло PR-ів, Vercel-конфіг flip-flop без staging-перевірки, ~4 тижні задвоєння Renovate × Dependabot до фіксу). [`0010-revenue-first-launch`](../0010-revenue-first-launch.md) фрізить product-scope на 4 тижні (до ~2026-06-01) для shipping білінгу — **0011 поважає цей freeze**: Phase 1 (process-discipline CI-guards) пройшла паралельно (інкрементальні зміни в `.github/workflows/`, не блокує білінг-PR-и), Phase 2 (foundation adoption) була завершена дешевше за заплановане вікно (2 дні замість 3 тижнів) — паралельні child-Devin-сесії з вузьким scope-ом, всі 9 PR-equivalents merged за 2026-05-04…05; Phase 4 закрита handoff-ом у 0007 round-10 без проміжного coverage-thermometer-PR. **Phase 3** (hardening verification — 4 PR-и) завершена достроково 2026-05-06…20 замість запланованого вікна 2026-06-23 → 2026-07-07. **Foundation snapshot 2026-05-05:** `useApiForm` — 6+ active consumers + 3/3 auth-екрани; `<DataState>` — 5/5 модулів migrated (finyk + fizruk + nutrition + routine + hub) + canary ESLint rule (`prefer-data-state` warn-only, 0 hits); типізований OpenAPI-клієнт — 0 raw `fetch()` у `apps/web/src/modules/`; Storybook — 37/37 `shared/ui` non-allowlisted + module-level stories (закрито 0007 round-10). Ця ініціатива закриває **adoption-розрив + три CI-guard-и + один retrospective audit** — у двох вікнах: in-freeze (Phase 1+2 done) та post-freeze (Phase 3).
...
**Status: 4/4 PR-ів merged станом на 2026-05-04; PR 1.4 audit operational follow-up A1–A5 закриті 2026-05-06.** Phase 1 завершено повністю — і code-side guards, і operational verification (Railway env / audit-log / Sentry) закриті. CSP_DISABLE retrospective переведено у Closed-state ([`docs/90-work/audits/archive/2026-05-04-csp-disable-retrospective.md`](../../audits/archive/2026-05-04-csp-disable-retrospective.md)).
...
- **Новий playbook:** `docs/00-start/playbooks/deploy-config-change.md` — decision tree (Mermaid) + per-surface verification steps (Vercel preview / Fly staging / Railway service) + emergency escape-hatch protocol. Зареєстровано у `playbook-catalog.md` + auto-regenerated INDEX.md.
...
- Файл: `docs/90-work/audits/archive/2026-05-04-csp-disable-retrospective.md` (а не `docs/incidents/...` як було у початковому плані — у репо вже існує `docs/90-work/audits/` як convention для retrospective-документів; `docs/03-operations/postmortems/` зарезервовано для real incidents з confirmed user-impact).
- Зареєстровано у `docs/90-work/audits/README.md` як Active / 0-of-5 implemented.
...
- `docs/05-design/design/design-system.md` — секцію `useFormValidation` видалено.
...
### Фаза 3 — Hardening verification (для launch readiness) — **DONE 2026-05-06…20** (4/4 PR-ів merged)

**Status:** Фаза завершена достроково — робота виконана 2026-05-06 (PR 3.1–3.3) і 2026-05-20 (PR 3.4), замість запланованого вікна 2026-06-23 → 2026-07-07. Реальний обсяг: 0011 doc-rationalisation підтягнула fact-state (файли вже є, лишилося відмітити + додати PR 3.4 decision-doc).

**PR 3.1 — `docs(docs): 0011 phase 3 pr 3.1 pen-test sweep for H5/H6/H8/H9`** (P0) — **MERGED [#2113](https://github.com/Skords-01/Sergeant/pull/2113) 2026-05-06**

- Файл: `docs/04-governance/security/pen-tests/2026-05-hardening-sweep.md` (504 LOC) + `docs/00-start/playbooks/security-pen-test-checklist.md` (76 LOC).
- Скоуп: для кожної з 4 carded vulns зроблено **manual e2e** + written reproduction:
  - **H5 (PR #1604):** OAuth callback з `exp://` redirect_uri у production-env → 400 / unauthorized підтверджено; dev-env: `exp://` дозволено (intentional dev-only allowance).
  - **H6 (PR #1608):** sign-up-flow з `REQUIRE_EMAIL_VERIFICATION=true` → `/api/mono/connect` без verification 403, з verified — 200; legacy-юзери з `email_verified=false` за `REQUIRE_EMAIL_VERIFICATION=false` default — не зачеплені.
  - **H8 (PR #1606):** attacking-origin → `/api/me`, `/api/mono/*`, `/api/chat/*` повертають `Cross-Origin-Resource-Policy: same-origin` навіть на 401-response.
  - **H9 (PR #1567 + PR #1613):** `AI_QUOTA_DISABLED=true` у production-env → startup fail; transcribe 9 МБ → USD-cap pre-charge 402 з friendly-error; happy-path → ledger-row у `ai_usage_daily.usd_micros`.
- **Follow-up з §Follow-ups transcript-у (не блокує цю PR):** production-Railway re-run (`@Skords-01`, target pre-launch Q3 2026) + external pen-tester engagement (I8 card). Це **launch-readiness** items, не 0011-scope.
- Owner: `@Skords-01`.
- **Risk:** низький, це підтвердження existing fixes.

**PR 3.2 — `test(server): session-protection coverage via Express router introspection`** (P1) — **MERGED [#2123](https://github.com/Skords-01/Sergeant/pull/2123) 2026-05-06**

- Файл: `apps/server/src/__tests__/session-protection.integration.test.ts` (369 LOC).
- Скоуп: програмний walk Express router-stack-у з `createApp()` enumerate-ить всі `(method, path)` пари; для кожної (за виключенням явного `EXEMPT_ROUTES` allowlist — `/api/health/*`, `/api/auth/*`, `/api/csp-report`, `/healthz`, `/api/metrics/web-vitals`) тест assert-ить що unauth-request повертає 401 з `Cross-Origin-Resource-Policy: same-origin` header (H8 invariant). Testcontainers `pgvector/pgvector:pg16` + повний migration set для real-DB integration (не моки).
- **Закрив:** мовчазний gap, на який покладався PR #1606. Майбутні route-и без `requireSession()` падають у CI.
- Owner: `@Skords-01`.
- **Risk:** середній — false-negatives якщо routing-introspection не покриває all dynamic mounts; митиґовано явним `EXEMPT_ROUTES` allowlist та auto-fail-on-new-routes тестом.

**PR 3.3 — `test(server): 0011 phase 3 pr 3.3 transcribe USD-cap e2e + fix bucket CHECK regression`** (P1) — **MERGED [#2126](https://github.com/Skords-01/Sergeant/pull/2126) 2026-05-06**

- Файл: `apps/server/src/modules/transcribe/__tests__/transcribe-usd-cap.e2e.test.ts` (354 LOC) + fixture `apps/server/src/modules/transcribe/__fixtures__/silence-5s.wav` (5-секундний WAV).
- Скоуп: real-Express app + real Postgres (Testcontainers) + real migrations 001-049 + real 5s WAV body; Groq Whisper мокнута на lib-boundary заради determinism + $0 CI-cost. 4 покриті case-и: (1) перший виклик у cap → 200 + Groq invoked + `ai_usage_daily` UPSERT; (2) другий виклик з cap=$0.01 → 402 + `code:"TRANSCRIBE_USD_CAP"` + friendly-error; (3) Groq-5xx → no charge (`ai_usage_daily` row не з'являється); (4) bucket CHECK regression fixed (модель name change → migration NNN).
- **Закрив:** PR #1613 mock-test gap; добавив bucket CHECK migration як побічний bonus-fix.
- Owner: `@Skords-01`.
- **Risk:** низький — Groq мокнута на library boundary, fixture deterministic.

**PR 3.4 — `docs(docs): 0011 phase 3 pr 3.4 email-verification sweep plan`** (P1) — **MERGED (цей PR)**

- Файл: `docs/01-product/launch/email-verification-sweep.md` (новий, decision-doc).
- Скоуп: для launch у Q3 2026 plan як підтягнути legacy-юзерів з `email_verified=false` без force-relogin. Три опції розписані з pros/cons:
  1. **Soft-gate з 14-денним warning-banner у hub + reminder cadence (day +1/+7/+13) + per-user `forceVerifyAt` sign-in soft-block (рекомендовано).**
  2. Gradual flag flip через `EMAIL_VERIFICATION_ROLLOUT_PERCENT` + deterministic per-user hash.
  3. Resend-CTA на login flow (status quo+).
- Рекомендована Опція 1 (закриває H6 residual risk + reversibility + 1-sprint cost). Implementation вийде окремою мікро-ініціативою (`0011a-email-verification-sweep` або incorporated у [`0010-revenue-first-launch`](../0010-revenue-first-launch.md) Phase 4 post-launch hardening — рішення `@Skords-01`).
- Decision gate: не пізніше 2026-08-31 (за 30 днів до Q3-launch-target 2026-09-30).
- Owner: `@Skords-01`.
- **Risk:** низький — це decision-doc, не implementation.
...
- [ADR-0046](../../../04-governance/adr/0046-storybook-vrt-scope.md) фіксує VRT scope.
...
> **Що це.** Open follow-up-и з Phase 1 (PR 1.4 audit operational items) і Phase 2 (severity-promote prefer-data-state). Парситься [`scripts/docs/generate-initiative-followups.mjs`](../../../../scripts/docs/generate-initiative-followups.mjs) у [`docs/90-work/initiatives/follow-ups.md`](../follow-ups.md). CI-гейт `Initiative follow-ups (in sync)` падає, якщо checked-in `follow-ups.md` розходиться з тим, що згенерує скрипт.

- [x] **2026-05-11:** A1 — підтвердити Railway env-cleanup (production + staging) і записати pre-existing-value у resolution log audit-у [`docs/90-work/audits/archive/2026-05-04-csp-disable-retrospective.md`](../../audits/archive/2026-05-04-csp-disable-retrospective.md). _Done 2026-05-06: Railway GraphQL `variables` query → 39 keys, 0 матчів `CSP|DISABLE|BYPASS|OVERRIDE`; staging environment не існує у проєкті; деталі в audit §Resolution log._
...
- [x] **CSP_DISABLE retrospective опубліковано і закрито:** [`docs/90-work/audits/archive/2026-05-04-csp-disable-retrospective.md`](../../audits/archive/2026-05-04-csp-disable-retrospective.md) (PR [#1699](https://github.com/Skords-01/Sergeant/pull/1699)) — code-side cleanup закрито у [#1631](https://github.com/Skords-01/Sergeant/pull/1631); operational-side A1–A5 закриті 2026-05-06 (audit Status = Closed, deltail у §Resolution log). Підтверджений висновок: `CSP_DISABLE` ніколи не був enabled у production за 16-day window — SEV4 near-miss закрита як zero-impact.
...
- [x] **Hardening verification (Phase 3):** 4 PR-и merged. PR 3.1 [#2113](https://github.com/Skords-01/Sergeant/pull/2113) — pen-test sweep для H5/H6/H8/H9 ([`docs/04-governance/security/pen-tests/2026-05-hardening-sweep.md`](../../../04-governance/security/pen-tests/2026-05-hardening-sweep.md) + [`docs/00-start/playbooks/security-pen-test-checklist.md`](../../../00-start/playbooks/security-pen-test-checklist.md)) з manual e2e + transcript-ом для кожної карти. PR 3.2 [#2123](https://github.com/Skords-01/Sergeant/pull/2123) — `apps/server/src/__tests__/session-protection.integration.test.ts` з Express router-introspection + Testcontainers `pgvector:pg16`. PR 3.3 [#2126](https://github.com/Skords-01/Sergeant/pull/2126) — `apps/server/src/modules/transcribe/__tests__/transcribe-usd-cap.e2e.test.ts` з real 5s WAV fixture (`__fixtures__/silence-5s.wav`), real-Postgres, Groq mocked at lib-boundary. PR 3.4 (цей PR) — [`docs/01-product/launch/email-verification-sweep.md`](../../../01-product/launch/email-verification-sweep.md) decision-doc з 3 опціями soft-gate + рекомендація. **Launch-readiness follow-ups** (production-Railway re-run + I8 external pen-test engagement) залишаються на `@Skords-01` як launch-window items — вони не блокують 0011 Phase 3 closure, бо хіля в пен-тест sweep transcript §Follow-ups і launch-readiness чеклісті 04, не в 0011-scope.
...
| -------------------------------------------------------------------------------------------------------------------------------------- | ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **PR-template guard блокує hot-fix-и в інциденті**                                                                                     | низька      | середній | `meta` лейбл як escape-hatch (потребує post-incident enrich body). Документ-flow в `docs/03-operations/runbooks/incident-response.md`.                                                     |
...
| **CSP_DISABLE-audit виявить, що прапорець був enabled у проді**                                                                        | низька      | високий  | Розширюємо у full security-disclosure процес з `docs/04-governance/security/disclosures/` записом + повідомленням постраждалих юзерів. Якщо impact = 0, закриваємо як zero-impact.         |
...
- **ETA загальна:** оригінально 7 тижнів від 0010-launch — фактично Phase 1+2+4 завершені за 2 дні (2026-05-04…05) через паралельні child-сесії; Phase 3 (4 PR-и) — за 2026-05-06…20 замість запланованого вікна 2026-06-23 → 2026-07-07 (випередження ~6 тижнів). Для closure 0011 залишаються лише операційні follow-up-и з § Carry-over (всі закриті 2026-05-06…10).
...
- **Phase 3** (hardening verification — пост 0010): **DONE 2026-05-06…20** (4/4 PR-ів: 3.1 [#2113](https://github.com/Skords-01/Sergeant/pull/2113), 3.2 [#2123](https://github.com/Skords-01/Sergeant/pull/2123), 3.3 [#2126](https://github.com/Skords-01/Sergeant/pull/2126), 3.4 — цей PR). Заплановане вікно 2026-06-23 → 2026-07-07 не використано — робота виявилася нерозпаралеленою від 0010 freeze і пройшла в бекграунді в травні child-Devin-сесіями. Launch-readiness follow-ups (production-Railway re-run пен-тестів + I8 external pen-test engagement) трекаються окремо в `docs/04-governance/security/pen-tests/2026-05-hardening-sweep.md` §Follow-ups і в `docs/01-product/launch/business/04-launch-readiness.md` — вони launch-window items, не 0011-scope.
...
- **Буфер до Q3-launch deadline 2026-09-30:** ≥ 19 тижнів (раніше 17; різниця — за рахунок Phase 3 раннього завершення 2026-05-06…20 замість 2026-06-23 → 2026-07-07).
