...
> **Owner:** `@Skords-01`
> **ETA:** 1 week
> **Sources:** Design Review 2026-05-03 §10 (Security/Platform), §12 (CI/Ops), [`docs/90-work/tech-debt/backend.md`](../../tech-debt/backend.md)
...
- README-замітка у [`docs/03-operations/ops/renovate.md`](../../../03-operations/ops) — як обробляти Renovate PR-и (auto-merge dev-deps; manual review для major / @anthropic / Better Auth).
...
- [`docs/03-operations/observability/runbook.md`](../../../03-operations/observability/runbook.md) — додати секції:
...
- README у [`docs/03-operations/ops/`](../../../03-operations/ops) — індекс операційних документів.
...
- [x] Renovate (primary) + Dependabot (security-only daily) активні за [ADR-0044](../../../04-governance/adr/0044-renovate-vs-dependabot.md); `anthropic`/`sentry`/`opentelemetry` groups додано.
...
- [x] [ADR-0044](../../../04-governance/adr/0044-renovate-vs-dependabot.md) (Renovate vs Dependabot) змерджено. Sigstore signing — паркнуто (опт-ін). Див. **Що НЕ увійшло**.
...
- [`docs/90-work/tech-debt/backend.md`](../../tech-debt/backend.md) — записи «no Retry-After», «no Renovate», «single /health»
...
- **`docs/03-operations/ops/` як новий silo** (Phase 5 spec) — проpущено. Existing `docs/03-operations/observability/`, `docs/04-governance/security/`, `docs/03-operations/deploy/`, `docs/02-engineering/integrations/` уже перекривають operational concerns. Натомість FAQ-секцію додано в runbook.md (там on-call team шукає when-things-break-info).
