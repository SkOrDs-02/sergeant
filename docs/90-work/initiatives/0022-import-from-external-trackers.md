...
- **Міграція (за потреби):** імпорт-журнал для ідемпотентності/дедуплікації — нова таблиця `import_batches` (`source`, `user_id`, `file_hash`, row counts, `status`). Наступний вільний номер — `084_*.sql` (поточний максимум — `083_drop_billing_subscriptions`; Hard Rule #4 — послідовно, без пропусків, two-phase для DROP).
...
- ~~**Privat24** — формат виписки звірити окремо до старту Фази 3 (research не підтвердив точну структуру).~~ **Закрито 2026-08-25** живим XLSX «Історія операцій за період» (мобільний застосунок). Реконструкція була НЕВІРНОЮ рівно там, де й попереджав `AI-DANGER`: колонки називаються «Сума в валюті картки» / «Валюта картки», а не «…рахунку». Три висновки, які видно лише на файлі: сума в валюті КАРТКИ несе знак (у «валюті транзакції» — модуль, напрям із неї не відновити); валютний фільтр теж має дивитись на картку, бо покупки Apple мають «Валюта транзакції = USD» при гривневій картці; десятковий роздільник — крапка, попри український локаль. Профіль і тести — `apps/server/src/modules/finyk/import/csvProfiles.ts`.
...
- [ ] Повторний (у т.ч. частковий, з іншого файлу) імпорт того ж періоду → 0 дублікатів **і 0 втрачених рядків** (ідемпотентність через **семантичний row-key** — рішення founder-а 2026-08-04, див. § Відкриті рішення №2; `file_hash` — лише журнальний атрибут батчу). Тест-кейс, що ловить надто грубий ключ: тренування з двома ідентичними сетами (та сама вага × повтори) має лишитись двома рядками.
...
| Дублікати при повторному/частковому імпорті                           | Ідемпотентний UPSERT по **семантичному row-key** (див. § Відкриті рішення №2 — ключ включає позицію рядка в сесії, тож легітимні повтори не злипаються); `file_hash` в `import_batches` — журнальний атрибут батчу, не ключ дедуплікації. |
...
2. **Dedup-стратегія** — ✅ **ВИРІШЕНО (founder, 2026-08-04): семантичний row-key** (варіант Б). Ключ рахується з нормалізованого вмісту рядка, а не з `file_hash` цілого файлу, тож повторний чи частковий пере-імпорт того самого періоду не дублює рядки навіть з іншого файлу. `file_hash` в `import_batches` лишається журнальним атрибутом батчу, але НЕ ключем дедуплікації.

   **Склад ключа (уточнено 2026-08-05).** Самих лише «дата + тип + значення» НЕ досить: два однакові робочі сети (та сама вага × повтори в одному тренуванні) — легітимні різні рядки, і надто грубий ключ склеїв би їх, тобто імпорт мовчки втрачав би дані. Тому ключ = `(source, user_id, session_identity, row_kind, normalized_values, occurrence_index)`, де:

   - `session_identity` — стабільний ідентифікатор сесії з експорту, якщо він там є (Strong/Hevy дають id або принаймні дату+час старту тренування); інакше — доменний сурогат: для фітнесу `(день + назва вправи)`, для харчування `(день + тип прийому)`, для фінансів `(день + рахунок)`.
   - `occurrence_index` — порядковий номер рядка **всередині** своєї `session_identity` після детермінованого сортування вихідного файлу. Саме він розводить легітимні повтори і лишається стабільним при частковому пере-імпорті того самого періоду, бо рахується від початку сесії, а не від початку файлу.

   Інваріант, який має тримати кожен адаптер: **той самий вхідний період → той самий набір ключів**, незалежно від того, скільки рядків до/після нього було у файлі.
...
| PR                                                     | Title                                                                   | Merged     |
| ------------------------------------------------------ | ----------------------------------------------------------------------- | ---------- |
| [#855](https://github.com/Skords-01/Sergeant/pull/855) | feat(server): виписки XLS/XLSX інших банків + звірений профіль Privat24 | 2026-08-25 |
