...
- [ ] Повторний (у т.ч. частковий, з іншого файлу) імпорт того ж періоду → 0 дублікатів (ідемпотентність через **семантичний row-key** — рішення founder-а 2026-08-04, див. § Відкриті рішення №2; `file_hash` — лише журнальний атрибут батчу).
...
| Дублікати при повторному/частковому імпорті                           | Ідемпотентний UPSERT по **семантичному row-key** (нормалізований вміст рядка, як `applySync`); `file_hash` в `import_batches` — журнальний атрибут батчу, не ключ дедуплікації. |
...
2. **Dedup-стратегія** — ✅ **ВИРІШЕНО (founder, 2026-08-04): семантичний row-key** (варіант Б). Ключ ідемпотентності рахується з нормалізованого вмісту рядка (дата + тип + значення + джерело), а не з `file_hash` цілого файлу — повторний або частковий пере-імпорт того самого періоду не дублює рядки навіть з іншого файлу. `file_hash` в `import_batches` лишається як журнальний атрибут батчу, але НЕ як ключ дедуплікації.
