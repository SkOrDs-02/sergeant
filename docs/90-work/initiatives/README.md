...
| 0010 | [Revenue-first launch (LiqPay/Plata + Apple/Google auth + Mono-wedge)](./0010-revenue-first-launch.md) | P0        | `@Skords-01` | Sprint 1–4 (4 wk)                      | **In progress** — Phases 0–6 **code-complete**: multi-provider billing (`liqpay`+`plata` for UA; Stripe dormant), `subscriptions` migrations (через m082), pricing v4 ([ADR-0068](../../04-governance/adr/0068-pricing-v4-uah-reverse-trial.md)), Google+Email+Apple sign-in, activation_v2, sitemap/robots, LandingPage + EN locale. **Pending — founder-ops only:** `APPLE_*` + `LIQPAY_*`/`PLATA_*` у Coolify, ФОП для LiqPay-мерчанта, перший live paid user + webhook evidence, A/B rollout decision. **Public metrics — deferred.** Canonical detail у doc header. |
