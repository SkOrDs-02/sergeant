...
function buildAudienceSurvey() {
  ...
  screening
    .setTitle(
      "Чи ведеш ти зараз або вів(-ла) раніше облік чогось у житті — гроші, їжа, тренування, звички?",
    )
    .setRequired(true);
  ...
  form
    .addMultipleChoiceItem()
    .setTitle(
      "Скільки різних інструментів сумарно використовуєш для всього обліку?",
    )
  ...
  const pageControl = form
    .addPageBreakItem()
    .setTitle("Кілька питань для тебе");
  ...
  form
    .addScaleItem()
    .setTitle(
      "Якби зʼявився інструмент, що веде все в одному місці й мінімізує ручний ввід — наскільки цікаво?",
    )
    .setBounds(1, 5)
  ...
  form
    ...
    .setHelpText(
      "Залишиш — потрапиш у список раннього доступу. Не залишиш — нічого страшного.",
    )
  ...
  Logger.log(
    "📊 Відповіді: File → залий у Google Sheet через кнопку Responses у формі.",
  );
}
