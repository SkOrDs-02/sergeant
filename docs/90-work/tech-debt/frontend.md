...
> **Оновлено 2026-07-01 (tech-debt reconcile).** §4: таблиця monitor-файлів ресинхронізована — `FinykApp.tsx` 647 raw / 586 eff (passes rule, headroom 14), `fizruk/lib/dualWrite/adapter.ts` **декомпозовано** до 139 raw / 120 eff і знято з monitor-листа; `ManualExpenseSheet.tsx` 683 raw / 547 eff — claim про декомпозицію 2026-05-29 підтверджений, не стейл. §8: лічильник no-eyebrow-drift 25 → 27 web (+2 нових сайти), mobile без змін (10). Каталог `apps-web-exhaustive-deps.md` дрейфнув: 33 файли з disable у коді проти ~12 задокументованих — sync-план у [tech-debt-assessment-2026-07-01.md](./tech-debt-assessment-2026-07-01.md).
...
**Статус:** Active — Sprint 0 PR shipped 2026-05-20 (RUM instrumentation live); чекаємо на ≥ 7 днів даних з прода щоб запустити Sprint 1.
...
| ------------------------ | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| 647 / 586                | `modules/finyk/FinykApp.tsx`              | Init. 0001 carry-over — passes rule, monitor (re-audit 2026-07-01; headroom 14 eff LOC — найтісніший monitor-кейс)                   |
| ~~642 / 567~~ 139 / 120  | `modules/fizruk/lib/dualWrite/adapter.ts` | ~~Новий leaker~~ **Декомпозовано** (re-audit 2026-07-01: orchestrator 139 raw / 120 eff — далеко під лімітом, знято з monitor-листа) |
...
Custom DS-rule пригнічується 27 разів у `apps/web/src` і 10 разів у `apps/mobile/src`
(re-audit 2026-07-01; на 2026-05-13 було 25 web — +2 нових сайти). Усі з обґрунтуваннями в коментарях (кастомні hero kickers,
...
patterns нижче приховували signal від real production drift).~~ **Виконано 2026-06-01.** `@typescript-eslint/no-explicit-any` підвищено до `error` для `apps/web/src/modules/**` і `apps/web/src/core/**` у `eslint.config.js` (виключення — `*.test.*`, `__tests__/`, `*.stories.*`). Три by-design сайти (`parseFizrukWorkouts.ts`, `searchCache.ts`, `lazyImport.ts`) вже мали `eslint-disable-next-line` з обґрунтуваннями — без змін. Нові `any` у production-коді тепер ламають CI.
