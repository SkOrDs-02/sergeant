...
**Що варто зробити в будь-якому разі, незалежно від вибору:** гейт має жити на **сервері**. Сьогодні `requirePlan` у репо існує (`apps/server/src/modules/billing/requirePlan.ts`), але **не використовується жодним маршрутом** — Pro-гейт по всьому застосунку суто клієнтський, тобто обходиться не лише другим екраном, а й звичайним `curl`. Це ширше за Monobank і тягне на окремий прохід.
