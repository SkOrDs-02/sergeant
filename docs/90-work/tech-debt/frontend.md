...
1. **Кікери — колір і риска замість `uppercase`** (92 вживання `uppercase` в
   `apps/web/src`). Це **не механічний прохід**: разом із ним вирішується, чи
   взагалі лишається роль `text-style-overline` (15 вживань), чиє призначення
   в `tailwind-preset.js` записано як «uppercase-кікери». Прибрати капс і
   лишити роль — суперечність; прибрати роль — зміна канону восьми ролей, а
   отже потрібне рішення власника, не правка. **Блокер: рішення про роль.**
