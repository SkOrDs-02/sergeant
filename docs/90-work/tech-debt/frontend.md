...
### Черга синку finyk не дренажиться (знайдено 2026-07-25)

**Симптом.** У локальному E2E-прогоні (Postgres 16 + pgvector 0.8.0,
`PW_SKIP_WEBSERVER=1`) finyk поставив **4 операції** в чергу синку, і при цьому
не вилетіло **жодного** запиту на `/api/sync`. Черга не спорожніла до кінця
прогону.

**Чому це не видно з CI.** Smoke-тест `deep-module-crud` при цьому **проходить**:
він перевіряє локальний стан (SQLite/LS), а не факт доставки в хмару. Тобто
зелений E2E тут нічого не доводить про синк — це саме той випадок, коли тест
дає хибне відчуття покриття.

**Свідомо НЕ зроблено:** не піднято таймаут очікування. Це сховало б симптом,
а не причину, і наступний, хто читатиме зелений прогін, отримав би ще менше
інформації, ніж є зараз.

**Наступний крок.** Відтворити локально й подивитись, на якому кроці рветься
ланцюг: постановка в чергу → тригер флашу → HTTP. Потрібно розрізнити «флаш
ніколи не викликається» і «викликається, але падає тихо».
