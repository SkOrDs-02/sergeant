...
1. **Кікери — колір і риска замість `uppercase`.** ✅ **Зроблено 2026-08-06**
   ...
   **Два нових борги, які створив цей прохід:**
   ...
   - **`SectionHeading` `2xs`/`xs`/`sm` стали синонімами.** Вони
     відрізнялися лише трекінгом, який існував заради капсу. Три імені для
     одного результату — наступний викликач обере `sm`, вважаючи його
     більшим. Зведення в одне ім'я — 136 call-site-ів, тож окремою правкою.
     Поки що поведінку тримає тест `2xs / xs / sm are synonyms after the
caps removal` і `AI-DANGER` біля `sizeTokens`: він забороняє
     «полагодити» це, вигадавши їм штучну різницю в кеглі (12px — підлога
     шкали, нижче не можна).
...
2. **Поширення правил 1–3 і 5 на решту екранів.** Механічно, але сотні
   ...
   **Пастка, на яку я наступив і яку треба знати наступному** — `text-xs` і
   ...
   `font-weight: 400`**, а сирий розмір успадковує вагу від контейнера. Тому
   заміна `text-xs` → `text-style-caption` НЕ нейтральна у двох випадках:
   поруч із `font-semibold` (два правила ваги на одному вузлі, результат
   залежить від порядку в CSS) і там, де вага успадковувалась навмисно. Так
