...
### Pro-гейт на підключенні Monobank стоїть лише на одному з двох входів
...
**Чому не полагоджено одразу.** Обидва боки виправні, і вибір — продуктовий, не технічний. Додати пейволл у Фінік = зробити суворішим FTUX-екран, тобто найчутливіше місце воронки. Прибрати з Налаштувань = віддати авто-синк у Free. Рішення за founder-ом.

**Що варто зробити в будь-якому разі, незалежно від вибору:** гейт має жити на **сервері**. `requirePlan` для цього вже є і вже працює — `routes/nutrition.ts` (`analyze-photo`, `refine-photo`) і `routes/ai-memory.ts` (два записувальні маршрути) стоять під `requirePlan(pool, "pro")`. Бракує його саме на `POST /api/mono/connect`: там Pro-гейт існує лише в UI Налаштувань, тобто обходиться і другим екраном, і звичайним `curl`. Тобто це не системна прогалина, а один незакритий маршрут — і закривається він тим самим middleware, що вже стоїть у сусідів.
...
### `text-subtle` у темній темі нижчий за AA на будь-якому 12px-тексті
...
### `text-{module}-strong` у темній темі — той самий дефект, що вже полагоджено для статусів

**Знайдено 2026-08-21** під час розбору «червоні літери в темній темі».
Статусна половина (`text-danger-strong` і три сусіди) полагоджена: тир
розведено на заливку (`statusStrongHex`, -800, статичний) і чорнило
(`--c-{status}-ink`, у «Чорнилі» -400). Модульна половина лишилась як була:
`text-finyk-strong` / `-fizruk-` / `-routine-` / `-nutrition-` віддають
статичний тир -800 у будь-якій темі, тобто на ink-поверхні це ~2:1.

Розмір: 189 використань, з них **46 без ручної `dark:`-пари** (brand 21,
fizruk 11, finyk 5, nutrition 5, routine 4) — саме вони й малюють темне по
темному просто зараз.

**Чому не в тому ж PR.** Механіка ідентична (`textColor` у пресеті +
`--c-{module}-ink` у `theme.css`), але наслідок ширший: модульні акценти
несуть ідентичність чотирьох модулів, і зміна тону тексту в темній темі
зачіпає кожен екран продукту, а не лише повідомлення про помилку. Це
окремий візуальний прохід із власним скріншот-ревʼю, а не хвіст фіксу
контрасту. Тестер про модульні тони не писав.

**Ціна відкладання** — рівно та сама, що була у статусів: ловиться лише
axe-ом і лише на тих екранах, які додані в `tests/a11y/axe.spec.ts`.
...
### Підняти `MIN_N` крос-модульних зв'язків з 5 до 10

**Рішення власника 2026-08-05.** На час бети лишаємо `MIN_N = 5`
(`apps/web/src/core/insights/digestCorrelations.ts`) — щоб на малих даних
продукт узагалі щось показував і ми побачили реакцію. **Підняти до 10** після
бети або раніше, якщо з'явиться бодай одна картка, у якій власник упізнає шум.
