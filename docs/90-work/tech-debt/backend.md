...
> **Last validated:** 2026-05-11 by @Skords-01 (doc-only refresh — § «Gradual TypeScript migration plan» формально закрито, P1-список синхронізовано з реальним кодом: `Assets.jsx (928)` / `Transactions.jsx (737)` стейлові посилання замінено на `.tsx`-файли з актуальним LOC, `WeeklyDigestStories.tsx` / `ActiveWorkoutPanel.tsx` декомпозовано і прибрано з топ-розпилу). PR D closeout (2026-05-03) — `032_query_plan_documentation.sql` дозакриває COMMENT ON expected-plan на гарячих обʼєктах синку/push/AI-квоти; всі CHECK / FK / partial-index requirements уже були в коді з 002–024. **Next review:** 2026-08-09.
...
>
> - **`routes/internal/ai-memory-dlq.ts`** (Major) — DLQ replay-цикл інкрементує `replayed` навіть якщо `enqueueMemoryIngest`/`markDlqRowReplayed` внутрішньо проковтнули помилку — немає strict-підтвердження успіху.
> - **`routes/internal/alerts.ts`** (Major) — `POST /api/internal/alerts/pending` приймає `notYetRepeated`/`notYetSentryWarned`/`notSnoozed` у `PendingBody`, але не прокидує їх у `listPendingAlerts()` — фільтри мовчки ігноруються.
> - **`routes/internal/openclaw/routes-write.ts`** (Major) — якщо `postToTopic()` уже повернув `status: "posted"` (Telegram-повідомлення реально відправлено), а наступний `recordTopicMessage()` кидає — роут віддає 500 на вже виконану неідемпотентну дію; клієнт може retry і задублювати пост.
> - **`routes/internal/seo.ts`** (Major, 2 місця) — `rank-snapshot` приймає дробовий `keywordId` (`Number.isFinite`, не `isInteger`) і `Math.trunc`-ить його при insert — може прив'язати rank-дані до чужого keyword; `sitemap`-snapshot вставляє `u.lastModified` без валідації формату дати, на відміну від backlink-полів поруч — malformed value може зламати весь snapshot-запит.
> - **`routes/internal/users.ts`** (Major — domain invariant) — cohort-запит (`GET /api/internal/users/cohort`) рахує «N днів тому» за **UTC**-датою, а не Europe/Kyiv (Hard Rule domain invariant); окремо — дробовий `days` (`1.9`) проходить валідацію (`Number.isFinite`) і мовчки truncate-иться замість 400.
> - **`routes/internal/growth.ts`** / **`routes/internal/marketing.ts`** (Minor) — `bigIntStr`-конверсія не clamp-ить від'ємні значення (`spendCents`/`cacCents`, `impressions`/`engagements`) — можна записати від'ємні cent/counter-метрики.
>
> Жодне з цього не блокує #134 (мовна/reformatting side-effect, не regression). Потребує окремого(-их) PR під `sergeant-bugfix-and-regression` — не заводжу тут повний план, лише реєструю знахідки, щоб не загубились.
>
