...
> **AI-NOTE — SPA-fallback.** Обидва варіанти (`/.*/` regex чи `/*splat`) валідні;
> `/.*/` ближчий до поточної поведінки «зловити все» без ризику інтерпретації
> сегментів. Вибрати один стиль і застосувати консистентно в `app.ts`.
...
- Точково:
  ...
  - `src/routes/frontend.ts` — smoke-перевірка роздачі `dist/assets` та
    SPA-fallback (curl глибокого шляху → має віддати `index.html`);
