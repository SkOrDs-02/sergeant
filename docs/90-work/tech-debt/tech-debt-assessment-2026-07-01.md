...
> **Last touched:** 2026-07-19 by Codex (Group 3 non-null/security reconciliation). **Next review:** 2026-08-19.
> **Status:** Active
...
**Прогрес 2026-07-19:** 4/4 недокументовані
`@typescript-eslint/no-non-null-assertion` прибрано з explicit guards і
regression coverage (`RoutineCard`, `NutritionCard`, `ExpensesCard`,
`WorkoutTemplatesSection`). Production suppressions 13 → 9. Єдиний згаданий
filesystem site у `apps/server/src/db.ts` уже має конкретний invariant:
`import.meta.url → __dirname → deployment-controlled migrationsDir`; HTTP/env/user
input у path відсутній, тому audit claim «без обґрунтування» був застарілим.
Пункти 3–4 нижче лишаються відкритими.
...
1. **Закрито 2026-07-19:** filesystem suppression перевірено dataflow-аудитом; path deployment-controlled, конкретне обґрунтування вже було в коді.
2. **Закрито 2026-07-19:** 4 недокументовані non-null assertions замінено explicit guards; targeted tests 25/25.
