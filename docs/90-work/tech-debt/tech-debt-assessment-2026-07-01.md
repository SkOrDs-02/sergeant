...
> **Last touched:** 2026-07-20 by @cursor (Group 2 + Lighthouse T5 marked done). **Next review:** 2026-10-18.
> **Status:** Active
...
Усі цільові `react-hooks/*` правила в `eslint.baseline.js` уже `"error"` (incl. `immutability`, `preserve-manual-memoization`, `purity`, `set-state-in-effect`, `refs`, `static-components`, `use-memo`). Історичний burndown-рецепт нижче лишається як архівний playbook; **не брати Групу 2 як відкритий backlog**.
...
| Lighthouse warn→error (T5)                      | ✅ shipped (2026-07)               | `apps/web/lighthouserc.json` — LCP `["error", { maxNumericValue: 3000 }]` (див. AGENTS.md perf budgets)                            |
