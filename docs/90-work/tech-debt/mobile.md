...
> **Last validated:** 2026-06-15 by @claude (docs-hygiene batch — `diff.ts` decomposition confirmed: large-files counter 1 → 0). **Next review:** 2026-08-13.
> **Status:** Active

> **Оновлено 2026-06-04.** PR [#3363](https://github.com/Skords-01/Sergeant/pull/3363) (wave-1 delegation fan-out) закрив 2 §5 «Великі файли»: `fizruk/lib/dualWrite/adapter.ts` 804 → orchestrator 188 + 9 family-файлів; `fizruk/pages/PlanCalendar.tsx` 661 → 406 (3 sub-components у `components/plan/`). Великі файли (>600 LOC) на mobile: 3 → 1. **2026-06-15:** `diff.ts` 633 декомпозовано у `lib/dualWrite/diff/` директорію → лічильник 1 → **0**. Деталі — § «Великі файли» + Status log.
>
...
| Великі файли (>600 LOC)                  | **Середній**            | **1** production-файл >600 LOC у mobile після [#3363](https://github.com/Skords-01/Sergeant/pull/3363): лишається тільки `fizruk/lib/dualWrite/diff.ts` 633. ✅ `fizruk/lib/dualWrite/adapter.ts` 804 → orchestrator 188 + 9 family-файлів і ✅ `PlanCalendar.tsx` 661 → 406 (3 sub-components) декомпозовані у #3363. `routine/pages/Calendar.tsx` 628 → decomposed into `pages/Calendar/` folder (P2.2b, #2780) — `index.tsx` 183 LOC. `OnboardingWizard.tsx` лишається у 390 LOC. |
...
| TypeScript-version drift                 | **Середній**            | `apps/mobile`: `typescript ~5.9.0`. `apps/web` + `apps/server` + `tools/openclaw`: `^6.0.3`. Mobile блокує bump через RN/Expo type compatibility — план: дочекатись Expo SDK 53.                                                                                                                                                                                                                                                                                                     |
...
скоротився до 5 файлів (усі — fizruk M3 + routine notifications).
...
**0** production-файлів >600 LOC у `apps/mobile/` після PR [#3363](https://github.com/Skords-01/Sergeant/pull/3363) (`adapter.ts` 804→188 + `PlanCalendar.tsx` 661→406 декомпозовані) та подальшої декомпозиції `diff.ts` 633 у `diff/` директорію. Re-count 2026-05-13 через `find apps/mobile/src apps/mobile/app -name '*.ts*' ! -name '*.test.*' | xargs wc -l | sort -nr`; `OnboardingWizard` лишається декомпозованим — див. нижче. Перші рядки зведено для історичного контексту (LOC «before → after»):

| Файл                                                          | LOC           | Пріоритет   | Нотатка                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------------------- | ------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~`apps/mobile/src/modules/fizruk/lib/dualWrite/adapter.ts`~~ | ~~804~~ → 188 | ✅ **DONE** | **✅ DONE — PR [#3363](https://github.com/Skords-01/Sergeant/pull/3363).** SQLite dual-write adapter декомпозовано на orchestrator (188 LOC) + 9 per-operation-family файлів (`workouts`, `dailyLog`, `templates`, `wellbeing`, `activeWorkout`, `customExercises`, `measurements`, `programs`, `parity`) у `lib/dualWrite/` без зміни SQL/LWW semantics. |
| ~~`apps/mobile/src/modules/fizruk/pages/PlanCalendar.tsx`~~   | ~~661~~ → 406 | ✅ **DONE** | **✅ DONE — PR [#3363](https://github.com/Skords-01/Sergeant/pull/3363).** Винесено 3 sub-components (`PlanCalendarHeader`, `CalendarGrid`, `DaySheet`) у `modules/fizruk/components/plan/`; сторінка стоншена 661 → 406 LOC, нижче порогу 600.                                                                                                           |
| ~~`apps/mobile/src/modules/fizruk/lib/dualWrite/diff.ts`~~    | ~~633~~ → 0   | ✅ **DONE** | **✅ DONE.** Diff-utilities декомпозовано у `lib/dualWrite/diff/` директорію (`diffArray.ts` + per-shape helpers: `workouts`, `dailyLog`, `activeWorkout`, `customExercises`, `measurements`, `monthlyPlan`, `planTemplate`, `programs`, `wellbeing`, `workoutTemplates` + `index.ts`). Файл `diff.ts` більше не існує.                                   |
...
**2026-05-13 update:** `core/ModuleErrorBoundary.tsx` TODO(phase-10) закрито у роасті `docs/90-work/audits/2026-05-13-mobile-reliability-ux-roast.md` — `componentDidCatch` тепер форвардить через `captureError`. `ExperimentalSection.tsx` markers щезли у попередньому settings sweep — реальний quick-grep видає тільки 2 живих TODO. `AI-LEGACY:` маркерів — 0. `FIXME` / `HACK` / `XXX` — 0.
...
| M9  | TS 6 bump для mobile + openclaw (після Expo SDK 53) | `apps/mobile`, `tools/openclaw` → `^6.x`.                                                              | M-L       | 🚫 Blocked-reason: dep-blocked — BLOCKED on Expo SDK 53                                                                                                                         |
...
- **2026-06-04:** PR [#3363](https://github.com/Skords-01/Sergeant/pull/3363)
  ...
  (188 LOC) + 9 per-operation-family файлів (`workouts`, `dailyLog`,
  `templates`, `wellbeing`, `activeWorkout`, `customExercises`,
  `measurements`, `programs`, `parity`) без зміни SQL/LWW semantics; ✅
  `fizruk/pages/PlanCalendar.tsx` (661 LOC) стоншено до 406 LOC через
  винесення `PlanCalendarHeader`, `CalendarGrid`, `DaySheet` у
  `modules/fizruk/components/plan/`. Великі файли (>600 LOC) на mobile: 3 → 1
  (лишається `fizruk/lib/dualWrite/diff.ts` 633).
