...
> **Оновлено 2026-06-04.** PR [#3363](https://github.com/Skords-01/Sergeant/pull/3363) (wave-1 delegation fan-out) закрив 2 §5 «Великі файли»: `fizruk/lib/dualWrite/adapter.ts` 804 → orchestrator 188 + 9 family-файлів; `fizruk/pages/PlanCalendar.tsx` 661 → 406 (3 sub-components у `components/plan/`). Великі файли (>600 LOC) на mobile: 3 → 1 (лишається `diff.ts` 633). Деталі — § «Великі файли» + Status log.
>
...
**1** production-файл >600 LOC у `apps/mobile/` після PR [#3363](https://github.com/Skords-01/Sergeant/pull/3363) (`adapter.ts` 804→188 + `PlanCalendar.tsx` 661→406 декомпозовані; лишається тільки `diff.ts` 633). Re-count 2026-05-13 через `find apps/mobile/src apps/mobile/app -name '*.ts*' ! -name '*.test.*' | xargs wc -l | sort -nr`; `OnboardingWizard` лишається декомпозованим — див. нижче. Перші два рядки зведено для історичного контексту (LOC «before → after»):
