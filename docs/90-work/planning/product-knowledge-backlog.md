...
| ------------------------------------------------------------------------------------------ | ---- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Черга синку finyk не дренажиться** — 4 операції в черзі, **нуль** запитів на `/api/sync` | code | крит  | локальний E2E-прогін (Postgres 16 + pgvector 0.8.0, `PW_SKIP_WEBSERVER=1`) під час Хвилі 2                                                                |
...
**Про чергу синку — головне.** Це єдина знахідка тут, яку я НЕ пояснив. У
локальному E2E finyk ставив операції в чергу, але жодного запиту на `/api/sync`
не вилетіло — при цьому сам тест `deep-module-crud` проходив, бо перевіряє
локальний стан. Тобто зелений E2E тут нічого не доводить про синк. Свідомо не
загашено підняттям таймауту: це сховало б симптом, а не причину.
