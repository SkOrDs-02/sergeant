...
| Задача                                                                                | Модуль    | Тип  | Насл. | Джерело                         | Стан                                                                     |
| ------------------------------------------------------------------------------------- | --------- | ---- | ----- | ------------------------------- | ------------------------------------------------------------------------ |
| ~10-15 подій «сигнал показано → дію зроблено» (по 2-3 на модуль) у `ANALYTICS_EVENTS` | крос      | code | крит  | брейншторм 1.3 · B1 усіх звітів | стадія 1 (реєстр): імена заморожені, callsite-ів ще нема                 |
...
> `VALUE LOOPS`. **Жодного callsite-у ще немає — жодна подія не емітиться**, тож
> твердження канонів («0 подій «порада→дія» в core», «телеметрії немає») поки
> лишаються правдою і навмисно НЕ переписані. Контракт заморожено першим, бо
> імена подій де-факто незворотні: ренейм ламає дашборди й губить історію
