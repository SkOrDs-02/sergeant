...
2. **`openclaw_invocations.user_message` = `"(empty user message)"` для кожного рядка з 2026-05-12.** Stage 4a audit-hook читав вгадане `event.userMessage`, але real openclaw 5.7 `before_agent_start` шле `{ prompt, runId?, messages? }` (див. [`openclaw-sdk-5.7-real-api.md`](../../02-engineering/notes/spikes/openclaw-sdk-5.7-real-api.md) § 1 — `before_agent_start` позначений `@deprecated` у real SDK саме тому, що його канонічні поля переплутані з `before_prompt_build`). **Fix-forward у [PR #2474](https://github.com/Skords-01/Sergeant/pull/2474) (merged 2026-05-12):**
...
| 9   | Phase 7 cutover (BotFather identity swap, видалення `tools/openclaw/src/openclaw/` через 28 днів)                | ❌ — обидва боти живуть паралельно                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | n/a                                                                                        |
...
| 11  | Voice toggle (`/voice on                                                                                         | off`), Canvas, ClawHub publishing, multi-channel                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | відкладено за планом — окремий micro-ADR після Phase 7                                     | n/a |

### Що блокує наступний крок (Stage 4a hook smoke-test)

SDK reality-check spike закрився 2026-05-12 — деталі у [`docs/02-engineering/notes/spikes/openclaw-sdk-5.7-real-api.md`](../../02-engineering/notes/spikes/openclaw-sdk-5.7-real-api.md). TL;DR результатів:
...
| ✅ 🔬         | SDK reality-check spike (hook / approval / allowlist / scheduler API)                                                                                                                                                                                                                                                                                                                    | merged ([`openclaw-sdk-5.7-real-api.md`](../../02-engineering/notes/spikes/openclaw-sdk-5.7-real-api.md))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | n/a                                     |
...
| ✅ 6b         | Manual parallel-run window closed 2026-05-13 за рішенням founder-а — cutover 2026-05-12 22:30 UTC пройшов без regression-ів, parity-harness (Stage 6a) у CI ловить drift автоматично, deletion legacy лишається на 2026-06-09 (Locked Decision #17)                                                                                                                                      | Stage 6a                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | високий — час                           |
| ✅ 7          | Phase 7 cutover executed 2026-05-12 22:30 UTC ([`cutover-openclaw-gateway.md`](../../00-start/playbooks/cutover-openclaw-gateway.md)): `OPENCLAW_BOT_TOKEN` cleared, grammy webhook deleted, Gateway live (long-poll). Deletion (Locked Decision #17): **CLOSED without deletion 2026-06-08 — grammy tree retained (still imported by `tools/openclaw/src/index.ts`); deadline lifted.** | Stage 6b                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | високий — deletion міркувань rollback-у |
| ⬜ 8+         | Voice toggle, Canvas, ClawHub, multi-channel                                                                                                                                                                                                                                                                                                                                             | Stage 7                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | n/a — окремі micro-ADR після cutover    |

---

## Мета

Замінити внутрішній OpenClaw co-founder бот (ADR-0031, `tools/openclaw/src/openclaw/`) зовнішнім [OpenClaw](https://github.com/openclaw/openclaw) — open-source персональним AI-асистентом (MIT, 370k+ зірок). Це дасть:
...
| 16  | _(descoped)_            | WhatsApp канал прибрано з roadmap-у 2026-05-13 за рішенням founder-а. Існуючі channel-stubs у backend reminders залишаються dormant до окремого reintroduction-ADR.                                                                                                                                                                                                                                                                        |
| 17  | Grammy deletion         | **CLOSED without deletion (2026-06-08, founder decision).** Початковий план: видалити `tools/openclaw/src/openclaw/` + `agents/{openclaw,personas,strategic-modes,dispatcher}.ts` на `cutover-day + 28`. На 2026-06-08 цей tree досі імпортується entrypoint-ом `tools/openclaw/src/index.ts` (live код), тож видалення скасовано — код **retained**, дедлайн знято. Якщо колись захочемо видалити — спершу від'єднати від `src/index.ts`. |
| 18  | Post-Gateway extensions | ClawHub publishing, спільні plugins, multi-channel (Slack/Discord/Signal/iMessage) — окремий **micro-ADR** після Phase 7 cutover. Не блокує цей план.                                                                                                                                                                                                                                                                                      |
...
- **Конфігурація:** template `ops/openclaw/openclaw.json` живе у репо (config-as-code), на старті контейнера копіюється у `~/.openclaw/openclaw.json` всередині mounted volume. Persistence: skills, canvas state, Telegram auth-state — на volume. Перезбірка контейнера auth не вбиває.
...
- **Що через dashboard / CLI один раз:** channel-pairing (Telegram webhook setup), OAuth flows для майбутніх каналів, live-операції (mute channel, restart agent).
...
- **Voice (default text reply, Locked decision #15):** OpenClaw native voice. Voice-нотатки з Telegram → STT → agent (вхід завжди приймається). **Reply-mode default — text**; voice-reply вмикається явно через `/voice on` (per-conversation toggle, persisted на volume); `/voice off` вимикає.
...
| PR-F   | `devin/<ts>-openclaw-cutover-and-cleanup`    | Phase 6.5 (parallel run на окремих bot-identity) → Phase 7 (нова bot-identity — primary; ADR superseded; reminder на grammy code deletion +28 днів). Grammy код **і runtime** залишаються як fallback (без feature-flag flip у `tools/openclaw`).                                                                                                                                                                                                          | PR-E + ≥1 тиждень parity |
...
| PR-F   | `merged`                | [#2420](https://github.com/Skords-01/Sergeant/pull/2420) | `claude/review-openclaw-migration-HSeEx`            | 2026-05-11        | Phase 0 Gateway infra + Phase 7 cutover docs. Додано: `Dockerfile.openclaw-gateway` (Node 24-alpine, single-stage — plugin runs as TypeScript source), `ops/openclaw/docker-entrypoint.sh`, `railway.openclaw-gateway.toml`, рядок у `docs/02-engineering/architecture/service-catalog.md` для `sergeant-openclaw-gateway`, ADR-0055 (Supersedes 0031/0036/0041), оновлено `docs/00-start/playbooks/rotate-openclaw-credentials.md` (§ Gateway bot token). Merged 2026-05-11 (`cfafd697`). Залишається ручним (поза цим PR): Railway service creation + 5 GB volume, BotFather `@OpenClaw_sergeant_v2_bot`, `openclaw plugin install`, Telegram webhook pairing, ≥1 тиждень Phase 6.5 parallel run. |
...
| Stage 7   | `pending` | —                                                                                                                                                                                                                                               | Cutover playbook (ADR-0056 Supersedes ADR-0055 § cutover). BotFather identity swap, webhook re-pairing, reminder на видалення `tools/openclaw/src/openclaw/` через 28 днів.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Stage 6b             |
| Stage 8+  | `pending` | —                                                                                                                                                                                                                                               | Voice toggle, Canvas, ClawHub publish, multi-channel — окремі micro-ADR-и після Phase 7 cutover.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Stage 7              |
...
| ~~`OPENCLAW_GATEWAY_ENABLED`~~       | ~~Новий feature flag для Phase 6.5~~          | **DEPRECATED 2026-05-11:** cutover тепер identity-based (окрема bot-identity), а не flag-based. Flag не додається у `tools/openclaw`.           |
...
| Шлях                                           | Файли             | Опис                                                                              |
| ---------------------------------------------- | ----------------- | --------------------------------------------------------------------------------- |
| `tools/openclaw/src/openclaw/`                 | 16 файлів (\*.ts) | Handler, session, approval, audit, security, bootstrap, webhook, commands, policy |
| `tools/openclaw/src/agents/openclaw.ts`        | 1                 | Agent loop + tool execution                                                       |
| `tools/openclaw/src/agents/personas.ts`        | 1                 | 5 personas + tool filters                                                         |
| `tools/openclaw/src/agents/strategic-modes.ts` | 1                 | /plan, /analyze, /okr modes                                                       |
| `tools/openclaw/src/agents/dispatcher.ts`      | 1                 | Agent network delegation                                                          |
| `tools/openclaw/src/index.ts`                  | часткове          | OpenClaw bootstrap code                                                           |

**Стратегія:** ~20 файлів + ~30 тестів **залишаються в репо як fallback** після cutover. У Phase 7 grammy bootstrap **залишається працювати** на старій bot-identity `@OpenClaw_sergeant_bot` (без змін у `tools/openclaw` / без unset `OPENCLAW_BOT_TOKEN`); cutover = founder переходить на нову Gateway-bot-identity. ADR-0031 маркиться superseded (architecture), але runtime не вимикається. Видалення коду — окрема ініціатива не раніше ніж через 4 тижні стабільної роботи Gateway, окремим PR з власним rollback-планом.
...
**Вихід Phase 0.5:** короткий note `docs/02-engineering/notes/spikes/openclaw-poc.md` з висновками + go/no-go для Phase 1. Якщо критичні gap-и — оновлюємо план перед стартом Phase 1.
...
- `cofounder` — full set; `ops/growth/eng/finance` — обмежений підсет (як у `tools/openclaw/src/agents/personas.ts`).
...
3. Існуючий grammy-бот `@OpenClaw_sergeant_bot` залишається працювати на старому Railway service `sergeant-openclaw` undisturbed — `OPENCLAW_BOT_TOKEN` env лишається на місці, bootstrap (`tools/openclaw/src/index.ts`) не змінюється.
...
1. **Founder перемикається** на нову Gateway-bot-identity як primary канал спілкування з Sergeant-co-founder. Жодного code-зміни у `tools/openclaw`, жодного env-flip.
...
3. **Документація:**
   ...
   - ADR-0031 (`docs/04-governance/adr/0031-openclaw-v0-telegram-cofounder.md`) → Status: Superseded by ADR-XXXX (але runtime лишається active — у статусі написати: «Architecture superseded; runtime kept as production fallback на старій bot-identity»).
   - ADR-0036 (`docs/04-governance/adr/0036-openclaw-write-tools-with-approval.md`) → Status: Superseded.
   - ADR-0037 (`docs/04-governance/adr/0037-openclaw-write-audit-persistence.md`) — лишається Active (server-side write-audit).
   - ADR-0041 (`docs/04-governance/adr/0041-openclaw-telegram-webhook.md`) → Status: Superseded (Gateway тепер сам обслуговує webhook на новій bot-identity; старий webhook лишається активним для fallback bot).
   - Новий ADR `docs/04-governance/adr/00XX-openclaw-external-gateway.md` — фіксує кінцеву архітектуру + identity-based cutover як design choice.
   - Hard Rule #20 — оновити «Why» секцію, що Gateway теж не зберігає PAT-и.
   - `docs/01-product/launch/tech/openclaw-roadmap.md` — позначити завершені віхи.
   - `docs/00-start/playbooks/rotate-openclaw-credentials.md` — оновити список secrets (додати Gateway-bot-token окремо від старого `OPENCLAW_BOT_TOKEN`).
4. **Залишається без змін:**
   ...
   - **Grammy bot runtime** — `tools/openclaw` + `OPENCLAW_BOT_TOKEN` env + Railway service `sergeant-openclaw` всі живуть як fallback (без будь-якої cutover-зміни)
5. **Планове видалення grammy (Locked decision #17, refined 2026-05-11):** на cutover-day Phase 7 (день переходу founder-а на нову bot-identity) автоматично викликається `set_reminder` на `cutover-day + 28 днів`, прив'язаний до `/Артем`: «оцінити, чи Gateway-bot стабільний — якщо так, переходимо до окремої ініціативи видалення `tools/openclaw/src/openclaw/` + agents-файлів + `OPENCLAW_BOT_TOKEN` з secret manager + suspend Railway service `sergeant-openclaw`». Сам PR-F цього видалення НЕ робить.

### Phase 8: Додаткові канали (опційно, поза цим планом)

> **2026-05-13 — WhatsApp descoped:** WhatsApp прибрано з in-scope roadmap-у за рішенням founder-а. Всі канали поза Telegram тепер опційні й кожен потребує окремого micro-ADR.

**Опційні канали (окремі ініціативи):**
...
- Railway service `sergeant-openclaw-gateway` **не повинен** мати у env жодного з `OPENCLAW_GITHUB_PAT`/`Git_PAT`/`GITHUB_TOKEN`. Це закріплюється у `docs/00-start/playbooks/rotate-openclaw-credentials.md` як обов'язковий чек.
...
- **Module ownership map** (`docs/02-engineering/architecture/module-ownership.md` + AGENTS.md) — додати рядок про новий пакет.
...
| **Загалом (engineering)**   |                                                                                                                                                 | **~25–34 дні**  |
| **Загалом з parity-window** |                                                                                                                                                 | **~32–41 днів** |
...
| **Витік PAT у Gateway env (Hard Rule #20)**                                                          | Середня                                                     | Pre-deploy чек у Railway (script у `docs/00-start/playbooks/rotate-openclaw-credentials.md`); smoke-test у Phase 0 ловить наявність PAT-змінних.                                                               |
...
1. **Швидкий rollback (ad hoc):** founder відновлює спілкування через `@OpenClaw_sergeant_bot`. Якщо Gateway service нестабільний — suspend його у Railway dashboard. Жодних змін у `tools/openclaw`, жодного env flip.
...
- `docs/90-work/planning/openclaw-migration-plan.md` (v3, поточний файл)
...
3. **Multi-channel** (Slack, Discord, Signal, iMessage) — кожен канал окремий ADR з security-review (allowlist, identity mapping, rate limits).
...
- **OK to reference (research-only) під час PoC:** approval-flow patterns, n8n wrappers, Telegram channel configs. Запис у `docs/02-engineering/notes/spikes/openclaw-poc.md` як baseline для Phase 4 design choice.
