...
> **Canonical current status (2026-06-08):** Stage 6b closed, Stage 7 cutover executed 2026-05-12 22:30 UTC. **Migration complete.** Locked Decision #17 (grammy internal-bot deletion) is **EXECUTED 2026-06-08 (founder reversed the earlier "retain" decision after confirming the new gateway works):** the entire `tools/openclaw` workspace + `Dockerfile.openclaw` + `railway.openclaw.toml` + the `sergeant-hubchat` Railway service are **deleted**. `sergeant-openclaw-gateway` (`Dockerfile.openclaw-gateway` + `packages/openclaw-plugin`) is now the **sole** OpenClaw surface. (The plugin's own `packages/openclaw-plugin/src/legacy/**` tree is separate and retained.) Any older "next step / retain" wording is superseded. This plan is now a candidate for `archive/`.
...
| 17  | Grammy deletion         | **EXECUTED 2026-06-08 (founder decision).** Весь `tools/openclaw` workspace + `Dockerfile.openclaw` + `railway.openclaw.toml` + Railway-сервіс `sergeant-hubchat` видалено; plugin-parity drift-gate тести знято. `sergeant-openclaw-gateway` — єдина OpenClaw-поверхня. (Раніше рішення було «retain», founder скасував його після підтвердження, що gateway працює.)                        |
