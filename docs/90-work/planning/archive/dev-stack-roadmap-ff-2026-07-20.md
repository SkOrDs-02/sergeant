...
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/90-work/planning/dev-stack-roadmap.md`.

**Статус:** весь топ-15 закритий + більшість Тиждень 1–4 / Місяць 2 беклогу теж залендено. Створено 2026-04-25. Останнє оновлення 2026-05-05: понад топ-15 додатково закрилися Storybook (44 stories у `apps/web`), Stryker mutation testing (`stryker.cloudSync.conf.json` — згодом retired 2026-05-06 разом із v1 engine drop, [#052b](https://github.com/Skords-01/Sergeant/pull/2046)), Argos visual regression (`@argos-ci/playwright`), Drizzle ORM POC (повноцінний `packages/db-schema` з PG + SQLite адаптерами), Helmet + CSP report-only middleware (`apps/server/src/http/security.ts`), gitleaks secret-scan + CodeQL + Trivy container-scan + release-SBOM, mobile Sentry SDK через `@sentry/react-native` (DSN-gated). Sentry mobile DSN provisioning у Expo EAS Secrets — все ще TODO, решта пунктів TL;DR — закриті. **Архів історичних сесій (2026-04-25 / 2026-04-25 evening):** [`archive/dev-stack-roadmap.md`](dev-stack-roadmap-ff-2026-07-20.md).
...
Залишковий backlog Q3 2026 — нижче в розділі «Next-up backlog» + у [`stack-pulse-2026-05/`](../../initiatives/archive/stack-pulse-2026-05/README.md) (16 PR-ів).
...
Окрема секція в `docs/90-work/planning/ai-coding-improvements.md` (#711) містить деталі по Vitest, Playwright, Argos, Storybook, snapshot tests. Тут — додаткові тулзи.
...
| **Detox**      | ✅ mobile e2e активний (`detox-android.yml` + `detox-ios.yml`); див. [`docs/90-work/planning/mobile-e2e-testing.md`](./mobile-e2e-testing.md) | $0                    |
...
- Беклог: PR #12 `sentry-traces-sampler.md` у [`stack-pulse-2026-05/`](../../initiatives/archive/stack-pulse-2026-05/archive/pr-12-sentry-traces-sampler.md) — dynamic traces sample-rate по route-pattern.
...
| **pgBouncer**          | Connection pooling              | $0             | ⏳ pending — PR #13 `postgres-pool-sizing.md` у [stack-pulse-2026-05](../../initiatives/archive/stack-pulse-2026-05/archive/pr-13-postgres-pool-sizing.md) (`pg.Pool.max` поки дефолт) |
...
| **Prisma**             | TS ORM з migrations             | $0             | ⏳ not used (Drizzle обраний, див. [`archive/orm-drizzle-vs-kysely.md`](./orm-drizzle-vs-kysely.md))                                                                                   |
...
- [x] **Monthly tech debt review** — проводиться через [`stack-pulse-2026-05/`](../../initiatives/archive/stack-pulse-2026-05/README.md) (16 PR-ів) + цей roadmap рефреш (2026-05-05).
...
Історичні журнали сесій «інфра-спринтів» (2026-04-25 day, 2026-04-25 evening) винесені в [`archive/dev-stack-roadmap.md`](dev-stack-roadmap-ff-2026-07-20.md), щоб роадмап залишався «forward-looking». Див. архів для хронології PR-ів #714 — #743.
...
- `docs/90-work/planning/ai-coding-improvements.md` (#711) — інфраструктура для AI-агентів.
- `docs/90-work/tech-debt/frontend.md` — існуючі borгs у web.
- `docs/90-work/tech-debt/backend.md` — існуючі borгs у server.
