...
> **Скоуп:** виконати outstanding items з [`2026-05-13-dead-code-hard-rules-roast.md`](../../audits/archive/2026-05-13-dead-code-hard-rules-roast.md) (§ P1 — `pnpm knip` deps sweep, 77 unused exports + 51 duplicate exports, mobile-shell unused exports, env-single-source Phase 2 burn-down, AuthPage re-decomposition) і tightening hard-rule контуру (новий lint-gate проти archive-move depth-drift, який зараз тільки watchlist у § P2). Усе закрите у самій прожарці (P0.1/P0.2/P0.3, P1.2, P1.6 markers, P1.4 partial budget restore) — поза скоупом цього плану.
>
> **Cross-refs:**
>
> - [`docs/90-work/audits/2026-05-13-dead-code-hard-rules-roast.md`](../../audits/archive/2026-05-13-dead-code-hard-rules-roast.md) — джерело open items (§ P1.1, P1.3, P1.4, P1.5, P1.6 і watchlist § P2).
> - [`docs/04-governance/governance/hard-rules.json`](../../../04-governance/governance/hard-rules.json) — 22-rule registry; HR-4 додає 23-й rule + canonical body.
> - [`docs/04-governance/governance/hard-rules-matrix.md`](../../../04-governance/governance/hard-rules-matrix.md) — машино-читабельна матриця (sync gate `pnpm lint:hard-rules-registry`).
...
- **Status:** ✅ Частково виконано 2026-05-14 через [#2835](https://github.com/Skords-01/Sergeant/pull/2835): видалено фактично unused `idb-keyval` з `apps/web` і lockfile (`apps/web/src/shared/lib/api/queryClientPersister.ts` уже мав fall-through localStorage path); `@fontsource-variable/dm-sans` залишено, бо він реально імпортується в `apps/web/src/index.css`; `@sergeant/shared` уже відсутній у `packages/openclaw-plugin/package.json`. Решту deps (`@fontsource-variable/dm-sans` / `@sergeant/shared`) перенесено у DC-1b follow-up — `pnpm knip` усе ще піднімає `@sergeant/shared` в openclaw-plugin без import-site, тож треба окремо обґрунтувати delete vs ignoreDependencies-rationale.
...
- **Status:** ⚠️ План скориговано 2026-05-14: перелік у картці застарів. Поточні `scanBarcodeNative`, `ensureCameraPermission`, `subscribeNativePush`, `getStoredNativePushToken`, `isCapacitor`, `getPlatform` покриті boundary/unit tests і частково використовуються web dynamic-import gates; без native smoke їх не видаляємо.
...
> **Status:** Shipped 2026-05-14 by Devin via [#2836](https://github.com/Skords-01/Sergeant/pull/2836). Migrated `apps/server/src/http/requireAnthropicKey.ts` (`process.env.ANTHROPIC_API_KEY` → `env.ANTHROPIC_API_KEY`). Refactored `apps/server/src/routes/coach.route.test.ts` (9 mutation sites collapsed to 2 `vi.stubEnv` calls + `loadCreateApp()` dynamic-import helper). Bumped `.tech-debt/env-single-source-budget.json` `98 → 95`. **Canonical pattern locked** for HR-2/HR-3: see the `loadCreateApp()` helper in `coach.route.test.ts` (`vi.resetModules()` + dynamic `import("./../app.js")` after `vi.stubEnv(...)` so the env-module sees stubbed values on re-parse). Test-passes: `coach.route.test.ts` 9/9; `pnpm lint:env-single-source` reports `95/95 reads (matches baseline)`.
...
- **Scope-файли:**
  - `scripts/check-archive-move-depth.mjs` — новий скрипт. Логіка: для кожного `docs/90-work/audits/archive/*.md` (і будь-якого `docs/**/archive/**.md`, якщо інший archive-pattern існує), парсить `[text](./path)` і fail-stop коли relative `path` resolv-ться у `docs/90-work/audits/X/`, який не існує (а існує `docs/X/`); пропонує fix (`bump-depth-by-one`).
  ...
  - `docs/04-governance/governance/hard-rules.json` — додати rule id `23`, title `Archive-move depth integrity — no broken ../X links in docs/90-work/audits/archive/**`, `category: "lint-enforced-convention"`, `enforced_by: [{kind: "ci", ref: "pnpm lint:archive-move-depth"}]`.
  - `docs/04-governance/governance/rules/23-archive-move-depth.md` — canonical body з BAD (`../initiatives/foo.md` у `docs/90-work/audits/archive/X.md`) / GOOD (`../../initiatives/foo.md`) прикладами.
...
- **Acceptance:**
  ...
  - Negative-smoke: створити `docs/90-work/audits/archive/_smoke.md` з `[x](../initiatives/foo.md)`, прогнати `pnpm lint:archive-move-depth` — exit 1 з actionable error; видалити smoke-файл.
...
- **HR-4 (lint gate):** false-positive ризик — gate може фейлити на валідних `../X` links, що ходять усередині `archive/`. Skript має explicit allowlist для links, target яких — теж у `docs/90-work/audits/archive/`.
...
- [`docs/90-work/audits/2026-05-13-dead-code-hard-rules-roast.md`](../../audits/archive/2026-05-13-dead-code-hard-rules-roast.md) — джерело open items.
- [`docs/90-work/audits/2026-05-05-dead-code-and-stale-links-audit.md`](../../audits/archive/2026-05-05-dead-code-and-stale-links-audit.md) — попередня dead-code прожарка (origin §3.2 і §3.4).
- [`docs/90-work/audits/archive/2026-05-07-app-audit.md`](../../audits/archive/2026-05-07-app-audit.md) — app-audit (web-blocker `db-schema/migrate` umbrella, mobile-shell §1.3).
- [`docs/04-governance/governance/hard-rules.json`](../../../04-governance/governance/hard-rules.json) — 22-rule registry; HR-4 додає 23-й rule.
...
- [`docs/90-work/initiatives/stack-pulse-2026-05/pr-01-unify-env-modules.md`](../../initiatives/archive/stack-pulse-2026-05/archive/pr-01-unify-env-modules.md) — origin для HR-1/HR-2/HR-3 budget ratchet.
