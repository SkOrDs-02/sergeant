...
> **Last validated:** 2026-06-07 by Codex (session docs ledger for web helper coverage PRs). **Next review:** ніколи (read-only архів).
> **Status:** Archived (read-only). Fast-forward archived 2026-07-20 (90-day gate skipped за рішенням founder-а). Source: `docs/90-work/planning/pr-plan-testing-devx-2026-05.md`.
...
- **Scope:** додати рядок «Для testing/devx surface — звіряйся з `docs/90-work/planning/pr-plan-testing-devx-2026-05.md` і починай із `.agents/skills/sergeant-start-here/SKILL.md`» (точний link-path формується відносно `CONTRIBUTING.md` як `./docs/90-work/planning/pr-plan-testing-devx-2026-05.md`).
...
- **Scope:** додати посилання на `docs/90-work/planning/pr-plan-testing-devx-2026-05.md` після згадки про initiative 0009. Заодно — оновити `Last validated` header (через `scripts/docs/bump-last-validated.mjs` під час pre-commit).
...
- **Status:** ✅ **Closed (2026-06-15)** — 9-file floor met. Earlier waves: PR [#3363](https://github.com/Skords-01/Sergeant/pull/3363) (wave-1 delegation fan-out) + PR [#3413](https://github.com/Skords-01/Sergeant/pull/3413) (focused helper coverage). #3363 додав module unit suites для тонких зрізів finyk/fizruk/nutrition (`apps/web/src/modules/{finyk/lib,fizruk/lib,nutrition/lib}/__tests__/*.test.ts` — pure date/stats/progress helpers; P1-6 closeout) + thresholds bumped +1pp (lines 38→39 / branches 31→32 / fns 28→29 / statements 37→38). #3413 закрив ще один helper-only batch без threshold ratchet: `activeWorkoutLib.test.ts`, `requestId.test.ts`, `finykSubscriptionCalendar.test.ts` покривають active-workout IDs/datetime/pace, sync request ids і fallback/cache поведінку subscription-calendar adapter-а. **Done 2026-06-15:** per-module selector tests (`finykKeysSelectors` / `fizrukQueryKeyContainment` / `nutritionKeysSelectors`) + integration/MSW suites (`monoBackfillProgress.msw` / `useMonoBackfillProgress` / `restSettingsScenario` / `useRestSettings` / `barcodeScan.msw` / `useBarcodeProductLookup`) — **9 files / 32 tests green**. Coverage floor `lines` is already 50 (root `coverage-thresholds.json`), ≥ the card's ≥45 target; further ratchet → separate follow-up per ADR-0020.
