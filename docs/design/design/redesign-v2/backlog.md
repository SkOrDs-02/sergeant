...
- [x] ~~NutritionDashboard `chartHex` → CSS var migration~~ **(D7 resolved 2026-05-18 as audit misclassification)**. `chartHex` (per `packages/design-tokens/tokens.js:294`) — це окремий **macro-nutrient palette** (kcal/protein/fat/carbs — 4 кольори по макросу), не bypass для `--c-chart-nutrition` (один-колір-на-модуль). Заміна втратила б differentiation між макросами. Якщо у dogfooding виявиться dark-mode saturation issue → створити нову задачу «macro contrast pivot» з `--c-macro-{kcal,protein,fat,carbs}` × 4 theme scopes. Деталі: [`alignment-audit-2026-05-18.md`](./alignment-audit-2026-05-18.md) §D7 resolved.
...
- [x] ~~**`Popover.tsx:242` v1 chrome** (Q6 #14b verified). `bg-panel border border-line rounded-2xl shadow-float` — real v1 surface, рендериться над v2 mesh hub. Migrate до `bg-surface-glass backdrop-blur-md`. **Розмір:** XS-S.~~ **Shipped 2026-05-20 у [#3058](https://github.com/Skords-01/Sergeant/pull/3058)** (Hub glass batch, merged як `2a2d62de`). Positioned overlay → raw glass utilities recipe (не Card primitive — absolute-positioned).
...
- **P-task / V-task / W-task (Phase 4):** primitive existing? | wiring file path | motion budget compliance (дизайн-конвенція)
...
- [ ] **`mockups/product/nudges/`** — anti-nag cross-module toast system з fatigue-pip counter. Найбільш elaborated novel surface. Refs: `docs/design/design/cross-module-prompts.md`.
- [ ] **`mockups/product/push/`** — push notification lock-screen templates (streak reminder / AI nudge / paywall re-engagement / weekly digest). Refs: `docs/work/specs/launch/tech/telegram-improvements-roadmap.md`.
- [ ] **`mockups/product/states/`** — empty/error/zero-data system, 5 surfaces (4 modules + hub). Refs: `docs/design/design/empty-states.md`.
...
- [x] ~~**AssistantAdviceCard + TodayFocusCard glass migration** — `apps/web/src/core/insights/AssistantAdviceCard.tsx:59`, `TodayFocusCard.tsx:183`. `bg-panel border border-line rounded-2xl shadow-card` → `<Card prominence="glass" radius="r-lg">`. Hub-level surfaces juxtapose'ують з v2 InsightCard/AIPill/WeeklyDigest. **Розмір:** S.~~ **Shipped 2026-05-20 у [#3058](https://github.com/Skords-01/Sergeant/pull/3058)** (Hub glass batch). Pattern drift вспіймана: AssistantAdviceCard evolved у gradient-border wrapper — inner surface мігрував на glass tokens, gradient-border visual identity збережений.
- [ ] **`core/hub/dashboard/dashboardCards.tsx` glass migration** — extension до C1 (`BentoCard`). Bundle з C1 у Phase 2.1. **Розмір:** S.
- [x] ~~**CrossModulePreview v1 chrome** — `apps/web/src/core/hub/CrossModulePreview.tsx:77`. Згадано в Phase 2.4 (Onboarding cards), але без verification recipe. Mapping `prominence="glass" radius="r-lg"`. **Розмір:** XS.~~ **Shipped 2026-05-20 у [#3058](https://github.com/Skords-01/Sergeant/pull/3058)** (Hub glass batch). `<section>` → `<Card as="section" prominence="glass" radius="r-lg" padding="md">`.
...
- [x] ~~**6.4 AI-source tag на tx/meal rows** (cherry-pick з handoff Phase 6 extension) — `<Badge size="xs" soft module="finyk">AI · Кав'ярні</Badge>` на auto-categorized Finyk tx-rows; `<Badge>AI · фото</Badge>` на photo-imported Nutrition meals. Pattern primitive — той самий `Badge soft module=` recipe (existing). Wiring-only. **Розмір:** XS-S.~~ **Shipped 2026-05-20 у [#3053](https://github.com/Skords-01/Sergeant/pull/3053)** (Phase 6.4). `TxRow` отримує новий Badge для auto-categorized tx (skip if `_manual` / `overrideCatId` / income / internal-transfer / `other`). `MealRow` existing macroSource Badge upgraded — sparkles Icon для `photoAI` / `recipeAI`; `productDb` keeps neutral tone.
- [x] ~~**W6 StreakFlame wiring (Routine hero)** (cherry-pick з handoff Phase 4 extension) — Coral radial-glow у правому верхньому куті Routine hero card + streak day counter. Motion-safe (дизайн-конвенція — займає AMBIENT slot замість mesh усередині hero). Hook: `useStreakFlame(streakDays)` повертає `{intensity, shouldAnimate}`. Primitive existing — потрібен hook + integration. **Розмір:** XS.~~ **Shipped 2026-05-19 у [#3047](https://github.com/Skords-01/Sergeant/pull/3047)** (Phase 6.5). Intensity tiers: 0=hidden, 1-6=low (yellow static), 7-29=medium (amber/orange pulse), 30-99=strong (red), 100+=violet milestone burst. Placement: `absolute top-3 right-3` inside Card з `min-h-[44px]` touch-target wrapper, `aria-hidden` (streak narrative covered у `HeroValueLine` + `KpiRowCompact`).

### Speculative gaps (verify first)

Потребують 5-min code-review для підтвердження. Повний перелік у [`handoff-package/hidden-tech-debt-audit.md`](./handoff-package/hidden-tech-debt-audit.md) §🤔.
...
- `docs/design/design/redesign-v2/governance.md` — adapter strategy + governance
