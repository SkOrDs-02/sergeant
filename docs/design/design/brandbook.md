...
> картці `#1f1a17` дає **2.07 : 1** — тобто текст помилки форми був там
...
>
> **2026-09-02: те саме розведення дістали бренд і чотири модулі.**
> `text-{accent}-strong` (`brand`, `finyk`, `fizruk`, `routine`,
> `nutrition`) резолвиться через `--c-{accent}-ink` — тир -400 для модулів
> і stone-300 для нейтрального бренду, той самий щабель, що вже несуть
> `--c-{module}-accent` і `--c-brand-soft-fg` у «Чорнилі». Джерело значень
> — `accentInkHex` / `accentStrongHex` у `tokens.js`.
>
> **Головна знахідка того проходу — не число, а метрика.** Борг був
> записаний як «46 місць без ручної `dark:`-пари». Виявилось, що рахувати
> відсутність пари недостатньо: 33 місця бренду пару **мали** —
> `text-brand-strong dark:text-brand` — але `dark:text-brand` віддає
> stone-700, тобто **1.75 : 1** на картці. Пара, яка сама нечитабельна,
> гірша за її відсутність: вона виглядає як полагоджене місце і переживає
> будь-який аудит «скільки лишилось без пари». Ті пари знято, а не
> доповнено — саме тому механізмом лікування є змінна, а не клас.
...
див. у [`docs/design/design/archive/brand-palette-wcag-aa-proposal.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/05-design/design/archive/brand-palette-wcag-aa-proposal.md).
...
tokens + review, ex-Hard Rule #16, retired [ADR-0081](../../governance/adr/0081-repository-simplification.md)).
...
> **Блок нижче — генерація ДО «Чорнила» (2026-08-05), лишений як історія.**
> Актуальні значення живуть у `.dark` у
> [`apps/web/src/styles/theme.css`](../../../apps/web/src/styles/theme.css) і
> дзеркалять `inkTheme.surface`: `#14100e` / `#1f1a17` / `#2a231f`. Коротка
...
> патчі див. у [`archive/dark-mode-audit.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/05-design/design/archive/dark-mode-audit.md)). Dark-mode border
...
> і фази лежать окремо в [`react-native-migration.md`](../../engineering/mobile/react-native-migration.md)
