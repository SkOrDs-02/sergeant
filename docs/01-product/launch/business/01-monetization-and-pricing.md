...
> **ЗАСТАРІЛО у частині цифр (2026-08-06).** Ця секція і § 9.6 рахують чат як
> Sonnet + Haiku з prompt-cache. **2026-08-04 чат переїхав на OpenRouter**
> (`CHAT_VIA_OPENROUTER`), і разом із транспортом змінилися і модель, і кеш, і
> розмір контексту — тож усі числа нижче описують конфігурацію, якої вже може
> не бути. Актуальний розрахунок, який перегенеровується з коду:
