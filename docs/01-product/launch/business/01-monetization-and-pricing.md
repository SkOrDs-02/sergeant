...
**Порядок величини на одне повідомлення з викликом інструмента:**
turn 1 (Haiku, здебільшого cache-read) ≈ $0.002–0.009; turn 2 (Sonnet, tool
result + до 2500 вихідних токенів) ≈ $0.01–0.05. Разом **≈ $0.015–0.06**.

**Стеля, яку дозволяє код** (не очікувана поведінка — саме максимум):

- **Free**, 15 одиниць/добу: або 15 простих запитів, або 5 з інструментом
  (3 одиниці кожен) → **$0.15–0.30/добу ≈ $4.5–9/міс**.
- **Pro**, 20 premium + 80 standard: ≈ 6 premium + 26 standard викликів з
  інструментами → **$0.75/добу ≈ $22/міс**, плюс необмежений floor-хвіст.

Проти виручки $4.6/міс це **у 1–5 разів під водою на стелі**. Рядок «AI API
marginal cost ≈ ₴5/міс» (≈ $0.12) занижений щонайменше на порядок.

**Структурна інверсія — головна знахідка.** Free і anon отримують **дорогу**
...
Наслідок: при зростанні саме **безкоштовні** користувачі домінують в
AI-собівартості, і кожен новий Free-юзер дорожчий за Pro на тій самій
кількості повідомлень. Єдиний важіль, що це згинає —
`ANTHROPIC_BUDGET_HARD_DEGRADE_ALL`, і він **за замовчуванням вимкнений**.

**Чого ця оцінка не знає** (і чому її не можна цитувати як факт):

1. Розмір спільного cache-префікса. `promptCache.ts` каже «≈6000 токенів
   tools+system», але реєстр — **77 інструментів**. Якщо префікс насправді
   15k, підлога вартості зростає втричі. Це треба **виміряти**
   (`count_tokens` або `pnpm eval:models`), а не припускати.
2. Скільки повідомлень на добу шле реальний користувач. Оцінка вище — стеля,
   а не середнє; при 20% використання квоти числа діляться на п'ять.
3. Курс ₴/$ у коді не зафіксований — узято ₴1 ≈ $0.023 з прози цього ж файлу.
