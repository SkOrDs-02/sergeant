...
- [ ] **Privacy Policy + Terms of Service опубліковані на public URL.** Без цього Apple/Google reject-нуть store-listing — див. [`docs/01-product/launch/business/04-launch-readiness.md` §1.1](../business/04-launch-readiness.md#11-обовʼязкові-документи).
...
- Прийнято рішення по §12: чи стартувати Phase 3 (Native Expo) як паралельний трек, чи відкласти до Expo feature-parity (Exit dashboard у [`docs/02-engineering/architecture/platforms.md`](../../../02-engineering/architecture/platforms.md)).
...
| HTTPS Universal Links (iOS) / App Links (Android)        | `assetlinks.json` + `apple-app-site-association` у `apps/web/public/.well-known/`, Vercel headers + rewrite                                                                                                                                                                                 | [`capacitor-deep-links.md`](../../../02-engineering/mobile/capacitor-deep-links.md) |
...
Джерело: [`apps/mobile-shell/README.md`](../../../../apps/mobile-shell/README.md) § «Що НЕ зроблено», [`docs/02-engineering/architecture/platforms.md` § 3 Capacitor shell](../../../02-engineering/architecture/platforms.md#-3-capacitor-shell--appsmobile-shell), [`docs/02-engineering/mobile/shell.md`](../../../02-engineering/mobile/shell.md).
...
- **Native APNs/FCM send-pipeline.** `src/pushNative.ts` реєструє токен і шле його у `push_devices`, але серверний `dispatch`-step через APNs/FCM ще не написаний — див. [`docs/02-engineering/mobile/overview.md` § Push notifications](../../../02-engineering/mobile/overview.md#push-notifications). На beta достатньо токен-registration smoke; на GA потрібен повний send.
- **App Store / Play Store метадані.** Іконки (1024×1024 iOS, всі density-bucket-и Android), screenshots (6.7" + 5.5" iPhone, 7" + 10" Android tablet), short/long description, keywords, category, age rating — все ще TODO. RN-апка теж не має цього (див. [`platforms.md` § 2](../../../02-engineering/architecture/platforms.md#-2-native-rn--appsmobile)), тож матеріали робляться раз і re-use-ються.
...
Джерело: [`docs/02-engineering/mobile/shell.md` § Передумови](../../../02-engineering/mobile/shell.md#передумови) + [`apps/mobile-shell/package.json`](../../../../apps/mobile-shell/package.json).
...
Джерело правди — [`docs/02-engineering/mobile/shell.md` § Release — iOS](../../../02-engineering/mobile/shell.md#release--ios). Сім обов'язкових secrets для [`mobile-shell-ios-release.yml`](../../../../.github/workflows/mobile-shell-ios-release.yml):
...
Одноразовий локальний крок (НЕ в CI, не в репо!). Джерело — [`docs/02-engineering/mobile/shell.md` § Release — Android](../../../02-engineering/mobile/shell.md#release--android).
...
- [ ] **Short + Long description** — UA + EN. Reuse positioning з [`README.md`](../../../../README.md) і [`docs/01-product/launch/business/02-go-to-market.md` § Headline формули](../business/02-go-to-market.md#41-product-hunt-playbook).
...
- [ ] **Native push send-pipeline** — щось на серверну сторону, навіть мінімальне (за `docs/02-engineering/mobile/overview.md` § Push notifications).
...
- **Sprint retrospective** — раз на 2 тижні, як з web (див. [`docs/01-product/launch/product-os/sprint-retros/`](../product-os)).
...
Джерело: [`docs/01-product/launch/business/04-launch-readiness.md` §1](../business/04-launch-readiness.md#1-юридичне-та-compliance) — повний legal checklist.
...
Джерело: [`docs/01-product/launch/business/01-monetization-and-pricing.md`](../business/01-monetization-and-pricing.md#5-payment-providers).
...
Для RN досягти 18/22 — це закрити: **RN-Nutrition full parity** (recipe/[id], photo-AI) + **RN-Voice (STT/TTS)** — обидва Phase 7 у [`docs/02-engineering/mobile/react-native-migration.md`](../../../02-engineering/mobile/react-native-migration.md).
...
- [`docs/90-work/initiatives/0010-revenue-first-launch.md`](https://github.com/SkOrDs-02/sergeant/blob/625921e85c7e961883d4cca64d9f6a177dbba823/docs/90-work/initiatives/0010-revenue-first-launch.md) — overarching launch ініціатива.
