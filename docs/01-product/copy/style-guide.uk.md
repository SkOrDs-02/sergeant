...
**Чому без `—`.** Довге тире між частинами речення стало візитівкою ШІ-тексту: користувач читає його як «це писала машина», і копія втрачає довіру ще до змісту. Замість нього: кома, двокрапка, дужки або окреме речення. Тире лишається тільки як плейсхолдер порожнього значення (`—` у клітинці таблиці) — там воно не текст, а символ. Те саме правило вшито у промпт асистента (`VOICE_RULE` у `apps/server/src/modules/chat/toolDefs/systemPrompt.ts`), щоб згенерований текст не повертав тире назад.
