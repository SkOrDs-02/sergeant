...
> [`sync-rn-migration-progress.md`](../../start/instructions/sync-rn-migration-progress.md).
...
| 3    | CloudSync v1 → Sync v2 (op-log + outbox) | 🔵 In progress | v1 client cut-over ([#2010](https://github.com/Skords-01/Sergeant/pull/2010)) + mobile engine drop ([`20793ad`](https://github.com/Skords-01/Sergeant/commit/20793adb)) + сервер `module_data` / v1 handlers видалено ([`75dcdd5`](https://github.com/Skords-01/Sergeant/commit/75dcdd5c)). Mobile writer-wiring трекає [plan 2026-05-06](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/90-work/superpowers/plans/archive/2026-05-06-sync-engine-writer-wiring.md). Деталі — Q13 + storage-roadmap Stage 5–7                                              |
...
| 7    | Модуль Харчування                        | 🔵 In progress | shell + Dashboard / Log / Water / Pantry / Shopping / RecipeDetail + barcode + photo-аналіз + AI shopping-list + DailyPlan AI-генерація денного плану. **In review:** AI recipe-generation (`RecipeRecommender`), photo edge-case error-handling                                                                                                                                                                                                                                                                                                                                                     |
| 8    | AI-шар (HubChat / Coach / Digest)        | 🔵 In progress | HubChat ([`apps/mobile/src/core/hub/HubChat.tsx`](../../../apps/mobile/src/core/hub/HubChat.tsx) + route `/hub-chat`), Coach-insight (`useCoachInsight`), WeeklyDigest (`useWeeklyDigest` + `useMondayAutoDigest`) — live. Залишок: voice у композері (STT mic + TTS-mute) — in review                                                                                                                                                                                                                                                                                                               |
| 9    | Hub-пошук + звіти                        | ✅ Done        | HubSearch ([`apps/mobile/src/core/hub/search/`](../../../apps/mobile/src/core/hub/search) + route `/hub-search`) + HubReports ([`apps/mobile/src/core/hub/HubReports.tsx`](../../../apps/mobile/src/core/hub/HubReports.tsx) + route `/hub-reports`). Pure-агрегатори з `@sergeant/insights`                                                                                                                                                                                                                                                                                                         |
...
- Push: `expo-notifications` + `PushRegistrar`-компонент, який після
  ...
  `POST /api/v1/push/register` (див. [`docs/engineering/mobile/overview.md`](./overview.md)).
...
- [`docs/engineering/mobile/overview.md`](./overview.md) +
  [`apps/mobile/docs/mobile.md`](../../../apps/mobile/docs/mobile.md) —
  API-контракт для мобілки: auth, deep links, push register,
  troubleshooting.
- [`docs/engineering/architecture/api-v1.md`](../architecture/api-v1.md) — опис
  `/api/v1/*` ендпоінтів.
- [`docs/design/design/brandbook.md`](../../design/design/brandbook.md) — вся візуальна
...
- **CloudSync v1 — викошений.** Сервер віддає `410 Gone`
  ...
  контекст.
- **Поточний стан sync на mobile** — Sync v2 op-log: per-module dual-write адаптери + MMKV/SQLite outbox + `SyncEnginePushScheduler` через `@sergeant/api-client`. Legacy `apps/mobile/src/sync/` stubs (`useSyncStatus` shape) — перехідний шар до повного writer-wiring (див. Stage 7 storage-roadmap). React Query warm-start через `PersistQueryClientProvider` + MMKV-персистер. Ключі префіксуються `mobile:` у `@sergeant/shared/storageKeys`.
- **Sync v2 op-log на mobile.** Per-module dual-write адаптери
  ...
  **змонтований у boot-pipeline**:
  ...
  - виклик `bootSyncEngineWriter({ captureException: captureError })` у
    ...
    [`docs/work/specs/superpowers/plans/2026-05-06-sync-engine-writer-wiring.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/90-work/superpowers/plans/archive/2026-05-06-sync-engine-writer-wiring.md)
...
- **Фінік** (`apps/mobile/src/modules/finyk/*`): усі 5 сторінок
  ...
  з period-filter — prev/next-month chevrons + custom date-range sheet —
  ...
  `apps/mobile/src/auth/e2eAuthMock.ts`). Android CI лейн
  ...
  `apps/mobile-shell/**`, mobile-shared packages): smoke-build зелений,
  full-сьюти follow-up.
...
| 2   | Hub-ядро                                   | ✅ Done        | 1                            | BRANDBOOK native-patterns (Q9). `ErrorBoundary` + `SyncStatusIndicator` + 6 HubSettings-секцій + HubDashboard (3 PR-серія) + оновлений `OnboardingWizard` + HubChat + HubSearch + HubReports. Залишок: voice у HubChat-композері — in review (онбординг ≥ web; пост-onboarding PresetSheet FTUX — окремий backlog).                                                |
| 3   | Sync v2 + офлайн outbox                    | 🔵 In progress | 1                            | MMKV/SQLite outbox + `SyncEnginePushScheduler` + NetInfo flush-on-reconnect. v1 `useCloudSync` знятий (ADR-0047). Writer-wiring per module — див. [`2026-05-06-sync-engine-writer-wiring.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/90-work/superpowers/plans/archive/2026-05-06-sync-engine-writer-wiring.md). |
...
| 7   | Порт модуля Харчування                     | 🔵 In progress | 1, 3, 6                      | shell + 8 сторінок + barcode + photo-аналіз + AI shopping-list + DailyPlan AI-генерація денного плану live. In review: AI recipe-generation + photo edge-case error-handling.                                                                                                                                                                                      |
| 8   | AI-шар (HubChat / Coach / Digest)          | 🔵 In progress | 2, 4–7                       | HubChat + Coach-insight + WeeklyDigest live (RN-стримінг через fetch ReadableStream). Залишок: voice у композері (`expo-speech-recognition` STT + `expo-speech` TTS) — in review.                                                                                                                                                                                  |
| 9   | Hub-пошук + звіти                          | ✅ Done        | 2, 4–7                       | `HubSearch` + `HubReports` live. Pure-агрегатори з `@sergeant/insights`.                                                                                                                                                                                                                                                                                           |
...
| `core/hub/HubReports.tsx`                                | `apps/mobile/src/core/hub/HubReports.tsx` + route `/hub-reports` | Done — реюз агрегаторів з `@sergeant/insights`; card-shell + 4 module-картки.                                                                                                                                                                                  |
| `core/HubChat.tsx` + `core/lib/hubChat*.ts`              | `apps/mobile/src/core/hub/HubChat.tsx` + route `/hub-chat`       | Done — streaming через ReadableStream. Залишок: voice у композері (STT mic + TTS-mute, `fromVoice`/`maybeSpeak`) — in review (swarm-гілка).                                                                                                                    |
...
| `core/cloudSync/hook/useSyncStatus.ts` (web)             | `apps/mobile/src/sync/*` + module boot hooks                     | Sync v2 status + MMKV outbox. v1 `useCloudSync` видалений. Writer-wiring in progress.                                                                                                                                                                          |
...
- Дизайн-патерни — секція «Native Patterns (iOS / Android)» у
  [`docs/design/design/brandbook.md`](../../design/design/brandbook.md) (PR
...
- Deep-links — таблиця у [`docs/engineering/mobile/overview.md`](./overview.md)
  § «Deep links».
...
- Payload — платформо-агностичний (один `PushPayload` shape, сервер
  ...
  [`docs/engineering/mobile/overview.md` § Push](./overview.md#push-notifications).
...
> треками; повний контекст — у [`docs/work/specs/planning/storage-roadmap.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/planning/archive/storage-roadmap.md).
...
- **Q13. Sync engine architecture.** ✅ **op-log + SSE pull + per-row apply-fns на сервері; PN-counter CRDT для streaks.**
  ...
  - SSE real-time. Старий CloudSync v1 (`apps/web/src/core/cloudSync/`)
    ...
    writer-wiring (план у `docs/work/specs/superpowers/plans/2026-05-06-sync-engine-writer-wiring.md`).
