...
> **Single source of truth → [`docs/engineering/architecture/api-contracts.md`](../architecture/api-contracts.md).** Той файл описує **що** живе у Pact-контракті і **як** працює consumer/provider pipeline. Цей runbook описує **що робити**, коли daily-cron детектує drift проти live staging.
...
Передай у `docs/work/specs/initiatives/` як окреме завдання — або контракт надто чутливий до даних на staging (треба матчери, `like()` / `term()`), або staging-state нестабільний (треба seed-стабілізація). Не tightening поза runbook-ом без обговорення.
      ...
      - name: Run drift checker
        ...
        run: |
          ...
          if [ -z "$STAGING_BASE_URL" ]; then
            echo "::error::STAGING_BASE_URL secret is not configured. See docs/engineering/testing/pact-drift-runbook.md § Setup."
            exit 2
          fi
        ...
        with:
          ...
          script: |
            ...
            const body = [
              ...
              '',
              'Follow [`docs/engineering/testing/pact-drift-runbook.md`](./pact-drift-runbook.md)',
              'for triage. TL;DR:',
              '',
      ...
      - name: Fail the job on drift or checker error
        ...
        run: |
          if [ "${{ steps.drift.outputs.drift_exit }}" = "1" ]; then
            ...
            echo "::error::pact-drift checker could not run (exit '${{ steps.drift.outputs.drift_exit }}'). Це помилка конфігурації, не drift — див. docs/engineering/testing/pact-drift-runbook.md § Setup."
          fi
          exit 1
```
...
- [`docs/engineering/architecture/api-contracts.md`](../architecture/api-contracts.md) — Pact pipeline overview, як додати новий endpoint.
