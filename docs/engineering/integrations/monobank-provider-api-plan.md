...
2. **🔑 Зберігання приватного ECDSA-ключа.** Новий секрет рівня `MONO_TOKEN_ENC_KEY`: лише в Coolify env (Railway retired — ADR-0074),
...
5. **⚖️ Юридично/безпека.** Гранульовані scopes + явна згода юзера; оновити
   [`pii-handling.md`](../../governance/security/pii-handling.md) і Privacy Policy, якщо запитуємо `p` (ПІБ).
