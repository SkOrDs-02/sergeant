...
| **Universal / App Links (HTTPS)** | ✅               | ✅              | ✅                 | RN — `app.config.ts` (`ios.associatedDomains` + Android `autoVerify: true` intent-filter), парсер у `apps/mobile/src/lib/deepLinks.ts` + AASA / assetlinks розширені на `com.sergeant.app`. Деталі — `docs/engineering/mobile/overview.md` § HTTPS Universal Links.                                   |
| **Offline / sync (Sync v2)**      | ✅               | ✅              | ✅                 | `core/syncEngine/` + `useSyncStatus`; shell — той самий web-бандл. v1 `useCloudSync` знятий (ADR-0047)                                                                                                                                                                                                |
| **Фінік** — Overview/Tx/Budgets   | ✅               | ✅              | 🟡                 | RN (`apps/mobile/src/modules/finyk/FinykApp.tsx`) рендерить лише Overview. Немає підключення банку, `NoBankBanner`, manual-only escape hatch, `RecurringSuggestions`, `NetworthChart`, AI-порад; `ManualExpenseSheet` існує в коді, але не змонтований у продуктовий екран.                           |
...
- `/api/*` поінтовує на бекенд `apps/server` (Hetzner CX23 + Coolify; Vercel env `BACKEND_URL` — див. [ADR-0074](../../governance/adr/0074-hosting-hetzner-coolify.md))
...
- HTTPS Universal Links / App Links теж підтримуються — див. `docs/engineering/mobile/capacitor-deep-links.md`
...
- **Feature parity інлання:** [`docs/work/specs/initiatives/archive/_0002-mobile-platform-decision.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/initiatives/archive/_0002-mobile-platform-decision.md)
- **API контракт:** [`docs/engineering/mobile/overview.md`](../mobile/overview.md)
- **RN migration roadmap:** [`docs/engineering/mobile/react-native-migration.md`](../mobile/react-native-migration.md)
- **Mobile strategy ADR:** [ADR-0052](../../governance/adr/0052-mobile-strategy-capacitor-primary.md) (supersedes ADR-0010 sunset)
- **Shell deep-links:** [`docs/engineering/mobile/capacitor-deep-links.md`](../mobile/capacitor-deep-links.md)
