...
- **Межа особистої доби — пристрій користувача** ([ADR-0078](../../governance/adr/0078-day-boundary-device-local.md)).
...
1. **bigint → string leaks ([#708](https://github.com/Skords-01/Sergeant/issues/708)).** Mono account balances suddenly went stringly-typed in the API; arithmetic in the dashboard silently produced `"123" + "456" = "123456"`. Fix: explicit `Number(r.id)` in serializers, snapshot tests on response shapes. ([Rule #1](../../governance/governance/rules/01-db-types-coerce-bigint-to-number.md).)
...
3. **Hardcoded RQ keys.** Several places had `["finyk", "transactions"]` inline; bulk-invalidate after a mutation missed half of them. Centralized factories make this impossible. ([Rule #2](../../governance/governance/rules/02-rq-keys-via-centralized-factories.md).)
4. **One-shot DB migration that dropped a column.** Pre-deploy ran the migration before the new image started serving, so the still-warm old version crashed on the missing column. Two-phase migration policy ([Rule #4](../../governance/governance/rules/04-sql-migrations-sequential-two-phase.md)) prevents this.
...
6. **Читання tombstone-нутого LS-ключа після міграції у SQLite (звіт тестерки 2026-08-18).** Кожен модуль дренує свій legacy-блоб у SQLite на буті й **видаляє** LS-ключ; канонічні читачі перевели на warm-caches, але FTUX-детекція «чи є справжні записи» (`packages/shared/src/lib/firstRealEntry.ts`) лишилася на старому скані пʼяти ключів. Для будь-якого post-migration профілю вона читала порожнечу — тобто **вічно бачила новачка**: FTUX-герой «З чого хочеш почати?» і `PresetSheet` не зникали, `hub_first_action_pending_v1` не знімався, вкладка «Звіти» лишалась замкненою, `first_real_entry` / `first_action_completed` / TTV не фаїрились. Урок ширший за один файл: **tombstone ключа — це зміна контракту читання**, і мігрувати треба ВСІХ читачів, включно з тими, що виглядають як «дешевий boolean-гейт». Гейт для нових читачів на хабі: не імпортуй модульні SQLite-читачі напряму (drizzle на eager-шляху вибиває бюджет 280 kB) — реєструй лічильник через `apps/web/src/core/onboarding/realEntryProbe.ts`.

7. **Direct `localStorage.setItem` in chat tool handlers.** A handler that writes to localStorage via `localStorage.setItem` (instead of the project's `lsSet` helper) bypasses quota fallbacks **and** the cloud-sync queue used by `cloudsync`. Under a concurrent request (e.g. user fires two tool calls fast, or background sync runs) the local write and the cloud-sync write race — the user sees the change in the UI but the next device boot pulls a stale value from cloud. Always go through `ls` / `lsSet` (or `safeReadLS` / `safeWriteLS` / `createModuleStorage`); the same wrappers are also enforced by the `sergeant-design/no-raw-local-storage` ESLint rule.
