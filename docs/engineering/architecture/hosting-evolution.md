...
- [`docs/work/specs/launch/business/03-services-and-toolstack.md`](../../work/specs/launch/business/03-services-and-toolstack.md) — повний каталог сервісів з цінами.
- [`docs/engineering/integrations/railway-vercel.md`](../integrations/railway-vercel.md) — як налаштувати поточний стек (Railway + Vercel).
- [`docs/operations/observability/`](../../operations/observability) — SLO, метрики, runbook.
- [`docs/work/specs/planning/dev-stack-roadmap.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/planning/archive/dev-stack-roadmap-ff-2026-07-20.md) — топ-15 dev-інструментів.
...
| **Logs**    | Текстові події (`user signed in`, `query failed`)           | Pino JSON у stdout, ALS-контекст `{requestId, userId, module}` — див. [`docs/operations/observability/logging.md`](../../operations/observability/logging.md) |
...
7. **Локальний n8n / OpenClaw** — якщо [docs/work/specs/launch/business/05-operations-and-automation.md](../../work/specs/launch/business/05-operations-and-automation.md) реально запускається і n8n cloud починає кусатись.
...
- **Grafana Cloud** (free tier) — скрейпимо `/metrics` через Grafana Agent (окремий маленький Railway-сервіс або локальний процес в apps/server контейнері). Імпортуємо `docs/operations/observability/dashboards/sync.json`. Заливаємо `alert_rules.yml`. Підключаємо Telegram contact point.
...
- **Stripe** — платежі. Див. [`docs/work/specs/launch/business/06-monetization-architecture.md`](../../work/specs/launch/business/06-monetization-architecture.md).
...
- Кожен alert у `alert_rules.yml` має `runbook:` annotation → лінк на `docs/operations/observability/runbook.md#секція`.
