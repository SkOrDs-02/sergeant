...
1. **Sync client wiring (наступна ініціатива).**
   ...
   - Деталі — [`sync-client-wiring.md`](../../work/specs/planning/sync-client-wiring.md) Phases 0–1.
...
Поточна архітектура — **v2-only** на сервері і **SQLite-only** для модульних даних на клієнті (dual-write teardown 2026-07-10). CloudSync v1 знятий; `sync_op_outbox` — єдиний sync-transport. **Найбільший ROI зараз:** wire client pull + outbox enqueue ([`sync-client-wiring.md`](../../work/specs/planning/sync-client-wiring.md)), потім retention/partition для `sync_op_log` і durable Nutrition backup.
