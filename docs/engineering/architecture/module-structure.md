...
| ------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **web/finyk**                   | Немає `shell/`, є `hubRoutineSync.ts` поруч                | Cross-module sync (HubChat ↔ Routine via Mono webhooks) висить на одному файлі — поки рано виносити в окрему директорію. Routing — react-router @7 через `/finyk/*` маршрут (initiative 0006 Phase 2.b, [#2108](https://github.com/Skords-01/Sergeant/pull/2108)): доменний `hooks/useFinykRoute.ts` + `lib/finykRouter.ts` з legacy hash-compat shim-ом.                                                                                                                                                                 |
...
| **web/nutrition**               | Є і `pages/`, і `domain/`                                  | `pages/` містить 4 screen-компоненти (Log, Menu, Pantry, Start) після initiative 0006 Phase 2.a ([#2104](https://github.com/Skords-01/Sergeant/pull/2104)); `domain/` тримає чисту бізнес-логіку backup/restore (не React, тестується ізольовано). Navigation-mesh через `hooks/useNutritionRoute.ts` + `lib/nutritionRouter.ts` (react-router @7 path-router).                                                                                                                                                           |
...
| **mobile/finyk**                | `hooks/` є, але мінімальний (3 файли boot-хуків)           | Mobile-finyk рендерить лише `pages/Overview` (`FinykApp.tsx`) — немає екранів підключення банку, manual-only escape hatch, `RecurringSuggestions`, `NetworthChart` чи AI-порад; `ManualExpenseSheet` існує в коді, але не змонтований. `hooks/` містить лише `useFinykDualWriteBoot.ts`, `useFinykMonoMirrorBoot.ts`, `useFinykSqliteReadBoot.ts` — boot/sync логіка при старті, без mutation-хуків. Деталі паритету: [`platforms.md` § Feature-parity матриця](./platforms.md#-0-feature-parity-матриця-web--shell--rn). |
...
| **mobile/{nutrition, routine}** | `pages/` присутній (на відміну від `web/routine`)          | Mobile використовує Expo Router (file-based routing) → `pages/` мапиться 1:1 на routes. `web/routine` тримає navigation в context без router; `web/nutrition` також має `pages/` (initiative 0006 Phase 2.a), тому контраст — лише з `web/routine`.                                                                                                                                                                                                                                                                       |
...
- **Modules не імпортують один одного напряму.** Cross-module комунікація — через `apps/web/src/shared/lib/modules/hubBus.ts` (event bus) або через спільні `packages/<X>-domain/` (тільки чиста логіка, без React).
- **`packages/*` не імпортує з `apps/*`** (Hard rule, [ADR-0024](../../governance/adr/0024-monorepo-apps-packages-split.md)).
...
- [`ADR-0024`](../../governance/adr/0024-monorepo-apps-packages-split.md) — `apps/*` vs `packages/*` boundary.
- [`ADR-0010`](../../governance/adr/0010-mobile-dual-track-capacitor-expo.md) — пояснює, чому `apps/mobile` ≠ `apps/mobile-shell`.
- [`ADR-0006`](../../governance/adr/0006-rq-keys-factory.md) — Hard rule #2 (RQ keys factory) — що зобов'язана експонувати кожна `<module>/lib/queryKeys.ts`-подібна точка.
- [`docs/engineering/architecture/frontend-overview.md`](./frontend-overview.md) — fronend-wide архітектурний overview.
- [`docs/engineering/architecture/apps-status-matrix.md`](./apps-status-matrix.md) — статус кожного app/package.
