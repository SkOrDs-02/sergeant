...
- `scripts/` — governance / docs / API / CI helpers. See [`docs/governance/governance/README.md`](../../governance/governance/README.md) for the full list.
...
- `apps/server` — Hetzner CX23 + Coolify via `Dockerfile.api` (образ `ghcr.io/.../sergeant-api`, GitHub Actions `deploy-api.yml`). Pre-deploy: `node dist-server/migrate.js` (Coolify `pre_deployment_command`). Health endpoint: `/health`. Migrations require `MIGRATE_DATABASE_URL` (= public DB URL). Rationale: [ADR-0074](../../governance/adr/0074-hosting-hetzner-coolify.md).
...
- Hard rules with full BAD/GOOD examples → [`docs/governance/governance/rules/`](../../governance/governance/rules).
