...
>
> **Оновлено 2026-07-20.** Wave 2 triage (Tasks 7–8): `NotificationsSection` wired to `useMonthlyPlan` (Phase 6 closed); `RoutineSection` already on `useRoutinePrefs` (closed — stale TODO removed earlier). Still blocked: `useChatSend` Phase 8 hub-context, HubReports billing/paywall, `exportReport` expo-print. Re-audit: **0** production files над 600 **effective** LOC. Raw >600 monitor: `routine/lib/sqliteWriter/adapter.ts` 735/~597, `nutrition/lib/sqliteWriter/adapter.ts` 636/~533, `core/dashboard/HubDashboard.tsx` 604/~481. Type-bypass allowlist **порожній**. Test-файлів: **148** (mobile) + **11** (mobile-shell). Coverage floor lines = **30**. Expo **52** / RN **0.76.9**; M9 SDK 53 — dep-blocked.
>
...
> **Оновлено 2026-05-15 (code-debt audit annex).** Monorepo-wide code-debt scan (Claude Opus 4.7 external session). `useChatSend.ts` Phase 8 hub-context TODO was newly tracked here; still open — see §6 TODO table (`🚫 Blocked-reason: owner-decision`). `NotificationsSection` Phase 6 and `RoutineSection` canonical-storage rows closed 2026-07-20 (Wave 2 triage).

> **Оновлено 2026-05-13 (roast #10).** Registry revalidated alongside `docs/work/specs/audits/2026-05-13-mobile-reliability-ux-roast.md`:
...
> `docs/work/specs/audits/archive/2026-04-28-sergeant-comprehensive-audit.md`. Цей файл — living
...
Live / closed markers у `apps/mobile/src` (не-test), triage 2026-07-20 (Wave 2 / Tasks 7–8):

| Файл                                     | Маркер                                                              | Статус                                                                                                                                                                  |
| ---------------------------------------- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `core/settings/NotificationsSection.tsx` | `TODO(mobile-migration, Phase 6): wire to useMonthlyPlan`           | ✅ Closed 2026-07-20 — wired to `useMonthlyPlan` (toggle + hour/minute; permission gate mirrors nutrition).                                                             |
| `core/settings/RoutineSection.tsx`       | `TODO(mobile-migration): swap for canonical routine-module storage` | ✅ Closed (pre-existing) — already on `useRoutinePrefs` / SQLite `routine_prefs`; orphan `@routine_prefs_v1` retired. Stale registry row removed.                       |
| `core/hub/useChatSend.ts`                | `TODO Phase 8` — mobile hub-context                                 | 🚫 Blocked-reason: owner-decision — `context = ""` until Phase 8 hub-context builder (web `buildContextMeasured` / shared lift); empty string is server-safe «no data». |
...
**T7 verification gate ([sprint-roadmap §1.1](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/planning/archive/sprint-roadmap-q2q3-2026.md#11-tech-борг)).**
