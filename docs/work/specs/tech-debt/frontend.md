...
>   це записано чесно і в `docs/design/design/README.md`, і в самому
...
> аудиту дизайн-системи (звіт `docs/work/specs/audits/2026-08-04-design-system-deep-audit.md`
...
>    module-accent записані як review-only і в `docs/design/design/README.md`,
...
1. **Sprint 0 (`feat/0017-hub-tab-perf-rum`) — shipped 2026-05-20**: PostHog `hub_tab_switch_perf` baseline, `PerformanceObserver({type:"longtask"})`. Runbook: [`docs/operations/observability/hub-perf-baseline.md`](../../../operations/observability/hub-perf-baseline.md).
...
1. **Доки дизайн-системи не знають про пʼяту родину.**
   `docs/design/design/README.md` і DesignShowcase описують палітру як
...
й обґрунтування — `docs/design/design/anti-slop-strategy.md` §4, підрозділ
...
### ~~Eager-бюджет пробито — весь UA-каталог їде першим екраном~~ — виконано 2026-09-12

**ЗАКРИТО 2026-09-12: 266.9 kB із 280.0, запас 13.1 kB.** Чотири модульні
каталоги (`uk.fizruk`, `uk.finyk`, `uk.nutrition`, `uk.routine`) більше не
спредяться в `messages` з `uk.ts`, а імпортуються прямо тими файлами, що їх
вживають (131 файл, ~380 місць виклику). Чанк `cn` упав **25.1 → 12.0 kB**,
критичний шлях — **286.8 → 266.9 kB (−19.9)**, preload-чанків 74 → 73.

**Виграш більший за оцінку (очікували ~10.6 kB, вийшло 19.9).** Прогноз рахував
лише вагу самих каталогів; насправді разом із ними з eager-графа вийшло й те,
що вони тягли за собою. Це той самий урок, що в закритому борзі `vendor-sqlite`
нижче, тільки з іншим знаком: оцінка «по вазі модуля» не описує складання
чанків ні вгору, ні вниз — має значення тільки замір після.

**Чого варто триматись надалі:** ліміт НЕ піднімали. Eager двічі ратчетнуто
вниз (470 → 430 → 280), і втретє важіль знайшовся так само — заміром сорсмапи,
а не здогадом. У eager лишається спільний кістяк (`uk.ts` 5.7 kB, навігація)
і чотири дрібні каталоги — `uk.privacy`, `uk.crossModuleLink`, `uk.pricing`,
`uk.dataExport` (разом ~2.5 kB). Їх не чіпали: виграш малий, а `uk.privacy` і
`uk.crossModuleLink` справді потрібні поза модулями.

**Як це виглядало до фіксу.** Заміри 2026-09-11:

| Дерево                          | eager        | проти ліміту 280.0 |
| ------------------------------- | ------------ | ------------------ |
| `dee22fa2`                      | 277.8 kB     | запас 2.2 kB       |
| `fb5a1a2f` (після #1098, #1103) | **286.0 kB** | **+6.0 kB**        |

Від закриття попереднього боргу (264.6 kB, 2026-08-07) критичний шлях набрав
**+21.4 kB** звичайною продуктовою роботою — жодного нового важкого vendor-а.

**Діагноз не гіпотеза, а розбір сорсмапи.** Чанк `cn` — 24.4 kB brotli, 14
модулів, і **23 із 24 kB — це UA-каталог інтерфейсу**:

```
 7.5 kB  shared/i18n/uk.ts
 5.1 kB  shared/i18n/uk.fizruk.ts
 2.5 kB  shared/i18n/uk.finyk.ts
 2.2 kB  shared/i18n/uk.nutrition.ts
 1.4 kB  core/hub/settingsSectionsCatalog.ts
 1.0 kB  shared/lib/modules/hubNav.ts
 …далі uk.privacy, uk.routine, uk.crossModuleLink, uk.pricing,
   uk.dataExport, uk.sergeant, uk.nutritionTdee
 0.2 kB  shared/lib/ui/cn.ts   ← хелпер, за яким названо чанк
```

Тобто рядки ВСІХ модулів завантажуються до першого екрана, хоча самі модулі
ліниві (`uk.fizruk` потрібен лише у Фізруку, `uk.pricing` — лише на сторінці
тарифів). Це той самий клас, що `posthog-js` і `vendor-sqlite` у закритих
боргах нижче: ліниве за задумом, eager за фактом складання чанків.

**Напрямок.** Розділити каталог по модулях так, щоб `uk.<module>.ts` тягнувся
разом зі своїм модульним чанком, а в eager лишився тільки спільний кістяк
(`uk.ts` + навігація). Верхня оцінка виграшу — ~12 kB (fizruk 5.1 + finyk 2.5

- nutrition 2.2 + routine 0.9 + pricing 0.7 + nutritionTdee 0.3), удвічі
  більше за перевищення. **Обовʼязково переміряй після кожного кроку:** у
  закритому борзі `vendor-sqlite` нижче двічі поспіль стояв хибний діагноз, і
  одна зі спроб дала −2.2 kB У МІНУС. `manualChunks` вирішує складання ПЕРШИМ,
  тож самого динамічного імпорту може не вистачити.

**Чого НЕ робити: піднімати ліміт.** Eager двічі ратчетнуто ВНИЗ (470 → 430 →
280), обидва рази важіль знаходився заміром. Підняти його — означає віддати
...
> [`fizruk-exercise-illustrations.md`](../fizruk-exercise-illustrations.md):
...
([`fizruk-catalog-programs-navigation.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/90-work/planning/specs/archive/fizruk-catalog-programs-navigation.md)),
