...
### Eager-бюджет вигорів до 2.2 kB запасу — знайти наступний важіль

**Статус: відкрито 2026-09-11.** Замір на `main` (`dee22fa2`):
**277.8 kB при ліміті 280.0**, 73 preload-чанки. Від закриття попереднього
боргу (264.6 kB, 2026-08-07) критичний шлях набрав **+13.2 kB** звичайною
продуктовою роботою — жодного нового важкого vendor-а, просто накопичення.

**Чому це терміново, попри зелений гейт.** Запасу лишилось 0.8%. Наступна
фіча, що зачепить критичний шлях, зробить гейт червоним — і тоді вибір буде
між ратчетом угору (третім поспіль для JS-метрики, і першим для eager після
двох ратчетів **вниз**) і терміновим пошуком важеля під тиском. Eager —
єдина з двох бандл-метрик, що корелює з відчутною швидкістю завантаження:
`size-limit` сумує всі 352 чанки, включно з тими, які більшість людей ніколи
не завантажить, тож lazy-split його не рухає, а eager — рухає.

**Де шукати.** Найбільші eager-чанки станом на 2026-09-11: `vendor-react`
37.5, `vendor-router` 27.2, `vendor` 24.6, `cn` 24.2, `vendor-react-query`
17.8 kB brotli. Перші три й п'ятий — каркас, їх не зрізати без зміни стека.
**Кандидат — `cn`:** назва від крихітного класнейм-хелпера, а важить 24.2 kB,
тобто це спільний чанк, який щось у себе увібрав. Перед будь-якою правкою
прочитай розбір нижче (закритий борг `vendor-sqlite`): там двічі стояв хибний
діагноз, і обидва рази лікували не те. **Спершу розбір сорсмапи чанка, потім
гіпотеза** — не навпаки.
