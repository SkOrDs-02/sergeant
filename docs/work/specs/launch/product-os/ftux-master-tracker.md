...
>
> - [`docs/work/specs/audits/archive/2026-05-03-ftux-onboarding-roast.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/audits/archive/2026-05-03-ftux-onboarding-roast.md) — оригінальна прожарка (історія, frozen).
> - [`docs/work/specs/launch/ftux-sprint-plan.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/product-os/ftux-sprint-plan.md) — sprint-roadmap (історія, frozen).
...
> [`docs/start/instructions/add-onboarding-step.md`](../../../../start/instructions/add-onboarding-step.md) — додавання кроку у `ONBOARDING_STEPS` ·
> [`docs/design/design/empty-states.md`](../../../../design/design/empty-states.md) — 3-tier empty states ·
> `docs/governance/governance/audit-freeze-2026-05-05.md` — freeze правила (4 тижні; з'явиться у PR-01).
...
>
> **Заборонено** під час freeze: нові аудиторські файли в `docs/work/specs/audits/`, нові ініціативи в `docs/work/specs/initiatives/`. Виняток — post-mortems завершених PR-серій (1 page max, в `docs/work/specs/launch/sprint-retros/`).
...
- **6 P0-проблем з 2026-05-03 roast:** 4 закрито (peek disclaimer, wizard-confetti remove, S3 reward-cluster, **demo-mode як first-class** — PR-05 #1986). 2 open у поточних PR-планах (hero copy benefit-driven, goal-aware first action для preset-cluster).
...
> Деталі sprint-items та status оригінально жили в [`ftux-sprint-plan.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/product-os/ftux-sprint-plan.md) §3-7a — там залишається **frozen reference**. Оновлення статусів — тут.
...
| Fizruk | Visual-cleanup precursor у feature-branch (інтегрованому через PR #19/#20): Measurements page тепер починається з єдиного hint-card («Як робити заміри») — видалено великий overview/header і дубльовані stat-картки; Progress page втратила дубльований `grid grid-cols-3` quick-stats блок (тепер summary-картки `Тренувань / PR вправ / Останнє` рендеряться лише раз). | [PR #17](https://github.com/Skords-01/Sergeant/pull/17) (Measurements header → single hint card) + [PR #18](https://github.com/Skords-01/Sergeant/pull/18) (Progress duplicate stats removal) | **PR-09 (outcome-card)** §5.3 / §5.4 Fizruk surface — менше visual-clutter на cold-start cards, які FTUX-обгортка teaser-итиме; **PR-12 (orchestrator)** — single-hero rule легше витримати після cleanup. |
...
> PR: **PR-14**. Декларативний `docs/operations/observability/ftux-slo.yml` + PostHog dashboard + Sentry/PostHog alert.
...
```yaml
# docs/operations/observability/ftux-slo.yml (proposed)
slos:
...
- **Архів стейл audit-tracker-ів:** `2026-04-28-ux-ui-audit.md`, `2026-04-28-ux-improvement-plan.md`, `2026-04-28-implementation-roadmap.md` → `docs/work/specs/audits/archive/` (PR-08).
- **`.replit`:** видалити (PR-08).
- **What's new:** ✅ `docs/product/whats-new/` (markdown changelog) + `apps/web/src/core/whatsNew/` (`<WhatsNewModal />` + `useWhatsNew` + `releases.ts` + tests; PR-18 shipped). Auto-show гейт — `hasFirstRealEntry && !inFtuxSession`, persist через `sergeant.whatsNew.lastSeenId.v1` LS. Drift-check: `releases.test.ts` валідовує що для кожного TS-запису є matching markdown у `docs/product/whats-new/<id>.md`.
...
| M-10 | FTUX SLO відсутні                                                        | ✅ Closed (declarative) | Declarative SLO landed у [`docs/operations/observability/ftux-slo.yml`](../../../../operations/observability/ftux-slo.yml) — 2026-05-13 roast PR. Alerting wiring (PostHog Alerts + Sentry webhook) лишається у PR-14 follow-up. |
| M-11 | A11y manual audit не проведений                                          | ⏳ Open                 | PR-16                                                                                                                                                                                                                            |
| M-12 | What's new modal                                                         | ✅ Closed               | PR-18 — `apps/web/src/core/whatsNew/` (`<WhatsNewModal />` + `useWhatsNew` + `releases.ts` + `storage.ts`, з тестами) + `docs/product/whats-new/`; інтегровано у `HubHomeView.tsx`                                               |
...
| ------ | -------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0      | PR-0 Telemetry                         | ✅ Closed  | Усі 9 PR-0 events landed в [`packages/shared/src/lib/analyticsEvents.ts:273-302`](../../../../../packages/shared/src/lib/analyticsEvents.ts) (`APP_LOCK_SETUP_*`, `BIOMETRIC_SETUP_COMPLETED`, `MODULE_SETTINGS_OPENED`, `MODULE_LANDING_TAB_CLICKED`, `ERROR_BOUNDARY_REQUEST_ID_COPIED`, `PERMISSIONS_SETTINGS_OPENED`). ADR-0054 розписує §1-§13 рішення. Анблок 11 downstream PR-ів (PR-1a/1b/2/3/6/7/14/16/22/42/X4). |
...
- [`docs/work/specs/audits/archive/2026-05-03-ftux-onboarding-roast.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/audits/archive/2026-05-03-ftux-onboarding-roast.md) — оригінальна прожарка з повним body. Цитати в §8.1-8.3 — з неї. Має redirect-banner на цей master tracker.
- [`docs/work/specs/launch/ftux-sprint-plan.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/product-os/ftux-sprint-plan.md) — оригінальний sprint-plan з повним PR-розписом (Sprint 0-5 + 6 cleanup batch). Деталі implementation per sprint-item — там. Має redirect-banner на цей master tracker.
...
2. **Не редагуй frozen sources** ([`2026-05-03-ftux-onboarding-roast.md`](https://github.com/Skords-01/Sergeant/blob/d068c73a2f21881d5c1305544fe99f3ea8be81f4/docs/90-work/audits/archive/2026-05-03-ftux-onboarding-roast.md), [`ftux-sprint-plan.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/product-os/ftux-sprint-plan.md)) — вони лишаються як history. Виняток — bump `Last validated:` (через `bump-last-validated.mjs`) і додання redirect-banner.
3. **Status legend:**
...
5. **Audit-freeze гарда:** під час freeze (до 2026-06-02) НЕ створюй нові файли в `docs/work/specs/audits/`. Якщо знаходиш нову проблему — додавай в §8.4 з ID `M-XX`.
