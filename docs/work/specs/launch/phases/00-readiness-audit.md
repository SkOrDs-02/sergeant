...
> [`docs/engineering/architecture/platforms.md`](../../../../engineering/architecture/platforms.md) ·
> [`docs/engineering/architecture/service-catalog.md`](../../../../engineering/architecture/service-catalog.md) ·
> [`docs/start/instructions/release.md`](../../../../start/instructions/release.md) ·
...
> [`docs/governance/adr/0052-mobile-strategy-capacitor-primary.md`](../../../../governance/adr/0052-mobile-strategy-capacitor-primary.md)
...
- **Стан:** `active` за [`apps-status-matrix.md`](../../../../engineering/architecture/apps-status-matrix.md). React 18 + Vite 8 PWA, Tailwind 4, TanStack Query, Better Auth cookie-сесії, Service Worker (`src/sw.ts`). Деплой — Vercel (статика + Edge Middleware-проксі на Coolify API), per-PR preview-середовища.
...
- **Auth secrets / Hard Rule #20:** OpenClaw PATs заборонені в проді, ротація через [`docs/start/instructions/rotate-secrets.md`](../../../../start/instructions/rotate-secrets.md). `BETTER_AUTH_TOKEN_ENC_KEY` + `NUTRITION_BACKUP_KEY_SECRET` — required у проді (з `.env.example`).
...
- **Push:** native APNs/FCM register endpoint живе, **fan-out у проді ще потребує credentials** ([`docs/work/specs/tech-debt/backend.md#push-credentials`](../../tech-debt/backend.md)).
...
- **Release pipeline:**
  ...
  - **iOS:** [`.github/workflows/mobile-shell-ios-release.yml`](../../../../../.github/workflows/mobile-shell-ios-release.yml) — **scaffold готовий**, але без Apple-секретів job логує `::warning::iOS release secrets not configured` і падає у unsigned-simulator-фолбек. Setup-контракт — [`docs/engineering/mobile/shell.md` § Release — iOS](../../../../engineering/mobile/shell.md#release--ios).
...
- **Sequence:** [`release.md § 1`](../../../../start/instructions/release.md#1-web--api) (Coolify pre-deploy migration → API image rollout → Vercel web).
...
- **Entry:** CP-4 exit + Expo feature parity ≥20/22 рядків у [`platforms.md`](../../../../engineering/architecture/platforms.md) (наразі 18/22), `eas build --profile production` зелений для iOS+Android, mobile-flaky-verify 20/20 baseline зелений.
