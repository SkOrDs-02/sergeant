...
> **Last touched:** 2026-09-11 by @claude. **Next review:** 2027-11-14.
...
> - [`product-os/paywall-implementation-plan.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/product-os/paywall-implementation-plan.md) — PR-20 gate, three paths
> - [`architecture/platforms.md`](../../../../engineering/architecture/platforms.md) — web ↔ shell ↔ RN feature-parity
> - [`playbooks/release.md`](../../../../start/instructions/release.md) — canonical release flow
> - [`governance/feature-flags.md`](../../../../governance/governance/feature-flags.md) — flag conventions
> - [`observability/posthog-ftux-dashboards.md`](../../../../operations/observability/posthog-ftux-dashboards.md) — funnel dashboards
...
- [x] **FTUX funnel працює:** 8 канонічних подій у PostHog (`onboarding_started → … → celebration_shown`); див. [`posthog-ftux-dashboards.md` §2](../../../../operations/observability/posthog-ftux-dashboards.md).
...
Аналітика — cookieless PostHog напряму в лендінгу. Операційна довідка — [`apps/landing/README.md`](../../../../../apps/landing/README.md). Останній редизайн — «redesign landing around connected insights»; скріншот стану — `docs/work/specs/audits/landing-redesign-2026-07-29.png`.
...
- [ ] **Запуск founder-pulse у PostHog:** [`docs/operations/observability/posthog-founder-pulse.md`](../../../../operations/observability/posthog-founder-pulse.md) — щоденний digest «новий signup / нова сесія / новий error».
...
- [ ] **Top-3 fix-PRs:** шипимо через стандартний release-flow ([`playbooks/release.md` §1`](../../../../start/instructions/release.md#1-web--api)).
- [ ] **Feedback loop close:** «Ось що ми виправили — спробуй ще раз».
- [ ] **NPS pulse:** PostHog Surveys, тригер `nps_survey_eligible` за віком акаунта — уже shipped, розсилати вручну не треба ([`feedback-loop.md`](../../../../operations/observability/feedback-loop.md)).
...
>
> - **Email** — єдиний транспорт це **Resend** (`RESEND_API_KEY`, `RESEND_FROM`). Код розсилок: `apps/server/src/email/authTransactionalMail.ts` (транзакційні) і `ftuxDripMail.ts` + `ftuxDripCopy.ts` + `ftuxUnsubscribeToken.ts` (FTUX-drip через BullMQ). **Блокер:** домен у Resend не верифіковано, тому масова розсилка сьогодні не піде — це і є причина, чому Telegram став головним каналом ([`telegram-waitlist.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/90-work/planning/specs/archive/telegram-waitlist.md)).
...
> - **NPS** — не Tally, а PostHog Surveys: клієнтський тригер за віком акаунта, разова настройка survey у дашборді ([`feedback-loop.md § 2`](../../../../operations/observability/feedback-loop.md)).
...
- Paywall PR-20 (див. [`paywall-implementation-plan.md`](https://github.com/Skords-01/Sergeant/blob/d1a37e0bed4e403477376eae9ee9a078e4179da8/docs/01-product/launch/archive/product-os/paywall-implementation-plan.md))
...
| **G-7** | **Capacitor readiness brief** | Phase 2 entry doc написаний; web↔shell parity-gap list documented          | `docs/work/specs/launch/phases/02-capacitor-launch.md` (Phase 2 deliverable) |
...
Коли всі hard gates пройшли — створи `docs/work/specs/launch/phases/02-capacitor-launch.md` як Phase 2 entry document. Включи:

- Phase 1 retrospective (1 секція): «що ми дізнались про юзерів, фідбек patterns»
- Web parity baseline (feature matrix): що в web є, що в shell поки немає (див. [`platforms.md` §0`](../../../../engineering/architecture/platforms.md#-0-feature-parity-матриця-web--shell--rn))
