...
- **Таймер зворотного відліку — не робити.** Fake countdown шкодить довірі UA-аудиторії; те саме правило в [`marketing/launch-plan.md`](../../../../product/marketing/launch-plan.md) § Urgency mechanic.
...
- **In-product:** NPS уже автоматизований через PostHog Surveys (тригер `nps_survey_eligible` за віком акаунта), фідбек-віджет — Settings → «Фідбек» (`feedback_submitted`). Деталі: [`feedback-loop.md`](../../../../operations/observability/feedback-loop.md).
...
- ✅ In-app feedback widget («Є ідея / Знайшов баг») — shipped: Settings →
  ...
  Ops-довідка: [`feedback-loop.md`](../../../../operations/observability/feedback-loop.md).
...
- ✅ NPS опитування після 7 календарних днів від створення акаунта (UTC) —
  ...
  [`feedback-loop.md § 2`](../../../../operations/observability/feedback-loop.md)).
...
6. МОНЕТИЗАЦІЯ (200 слів)
   Freemium модель, ціни, чому ₴199/міс — конкурентно (ADR-0068).
...
- **«Founder's Deal»:** перші 100 підписників — Lifetime Pro за ₴1 999
  (замість ₴1490/рік).
...
Вхідні дані:
  Pro ARPU           = ₴199/міс
  Avg. lifetime      = 6 міс → LTV = ₴1 194
  Free → Pro conv.   = 5 %
  LTV per install    = ₴1 194 × 5 % = ₴59.70

Вартість рефералу для маркетингу:
  Referrer reward    = 7 днів Pro = ₴199 ÷ 30 × 7 ≈ ₴46
  Referee reward     = 7 днів Pro trial = ₴0 (trial, не revenue loss)
  Ефективний CAC     = ₴46 за реферала

Висновок:
  CAC (₴46) < LTV per install (₴59.70) → юніт-економіка позитивна.
  Маркетинг готовий «платити» до ₴50 за рефералового юзера.
  При 10 друзів (Lifetime Pro = ₴1490 value) → CAC/реферал = ₴80.
...
| -------------------------- | ------------------------------------------------------------------------ | ---------------------------- |
| **Monobank**               | Інтеграція в маркетплейс Mono (партнерство не укладено, в коді відсутнє) | Трафік від 7M+ юзерів Mono   |
...
> **Unit economics check:** Pro = ₴199/міс, avg. lifetime ≈ 6 міс → LTV = ₴1 194.
> При конверсії free → Pro 5 % → LTV per install = ₴59.70.
> CPA повинен бути < ₴50 для здорової юніт-економіки (LTV/CPA ≥ 3:1 ідеально).
...
- **AI-коуч рівень 2** — проактивно пише: «Ти вже 3 дні не тренувався»
  (не реалізовано; сьогодні лише клієнтський авто-дайджест при відкритті
  застосунку — overview canon §1, B-6 diff-звіту).
...
>
> **Caveat — «база 500K+ продуктів» (`/blog/rahuvaty-kalorii` meta-description):**
> власної бази продуктів немає — це проксі до трьох зовнішніх джерел
> (OFF/USDA/UPCitemdb), USDA на `DEMO_KEY` (40 req/hr shared), UPCitemdb на
> hardcoded `trial`-endpoint (100 req/day). Деталі — `docs/product/modules/nutrition.md`
> §6. Формулювання лишається чернеткою — не копіювати цей рядок на живу
> сторінку без звірки з GTM-власником; точне публічне формулювання — його
> рішення (nutrition.md §6 сама це відкладає).
