...
function analyzeResponses() {
  ...
  if (!N) {
    Logger.log(
      "Ще немає жодної відповіді. Зачекай на перші сабміти й запусти знову.",
    );
    return;
  }
  ...
  line(
    "  → FRAGMENTATION RATE (3+ інструментів): " +
      frag +
      " з " +
      q5base +
      " (" +
  ...
  if (quitters.length) {
    line(
      "  → RETENTION-БЕНЧМАРК (медіана серед тих, хто кидав): " +
        quitters[Math.floor((quitters.length - 1) / 2)],
    );
  }
  line("  Причини кидання (Q8, вільний текст):");
  answered("Яка головна причина кидання").forEach(function (r) {
    const t = String(val(r, "Яка головна причина кидання"))
      .replace(/\s+/g, " ")
      .trim();
    if (t && t !== "-") line("    • " + t);
  });
  ...
  line(
    "  Роздратування (Q10), середнє: " +
      (mean("Наскільки тебе дратує") || 0).toFixed(2),
  );
  line(
    "  → PROBLEM PREVALENCE (оцінка 4–5): " +
      hot +
      " з " +
      annoyBase +
      " (" +
  ...
  line(
    "  Інтерес до рішення (Q11), середнє: " +
      (mean("наскільки цікаво") || 0).toFixed(2),
  );
  line(
    "  → інтерес 4–5: " +
      intHot +
  ...
  line(
    "## Pain score по модулях (середнє роздратування × найболючіший модуль)",
  );
  const byModule = {};
  rows.forEach(function (r) {
