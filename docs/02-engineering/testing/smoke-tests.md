...
> **Last touched:** 2026-08-23 by @claude. **Next review:** 2027-03-07.
> **Status:** Active
      ...
      - name: Run smoke checker
        ...
        run: |
          ...
          if [ -z "${TARGET_BASE_URL:-}" ]; then
            echo "::error::TARGET_BASE_URL is not set. Configure STAGING_BASE_URL secret or pass base_url input. See docs/02-engineering/testing/smoke-tests.md § Setup."
            exit 2
          fi
        ...
        with:
          ...
          script: |
            ...
            const body = [
              ...
              '',
              'Follow [`docs/02-engineering/testing/smoke-tests.md`](../../blob/main/docs/02-engineering/testing/smoke-tests.md)',
              'for triage. TL;DR:',
              '',
      ...
      - name: Fail the job on smoke failure or checker error
        ...
        run: |
          if [ "${{ steps.smoke.outputs.smoke_exit }}" = "1" ]; then
            ...
            echo "::error::post-deploy-smoke checker could not run (exit '${{ steps.smoke.outputs.smoke_exit }}'). Це помилка конфігурації, не liveness-регресія — див. docs/02-engineering/testing/smoke-tests.md § Setup."
          fi
          exit 1
```
