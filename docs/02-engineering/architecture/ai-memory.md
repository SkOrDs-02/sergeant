...
| `enqueueMemoryIngest` returns silently | `AI_MEMORY_ENABLED=false` або Redis incident | Перевірити env + Redis health у Coolify (ADR-0074). CLI продовжить counter-и, але queue лишиться порожньою — `ai_memories` rows не зʼявляться. |
