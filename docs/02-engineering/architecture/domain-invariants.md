...
1. **bigint → string leaks ([#708](https://github.com/Skords-01/Sergeant/issues/708)).** Mono account balances suddenly went stringly-typed in the API; arithmetic in the dashboard silently produced `"123" + "456" = "123456"`. Fix: explicit `Number(r.id)` in serializers, snapshot tests on response shapes. ([Rule #1](../../04-governance/governance/rules/01-db-types-coerce-bigint-to-number.md).)
...
3. **Hardcoded RQ keys.** Several places had `["finyk", "transactions"]` inline; bulk-invalidate after a mutation missed half of them. Centralized factories make this impossible. ([Rule #2](../../04-governance/governance/rules/02-rq-keys-via-centralized-factories.md).)
4. **One-shot DB migration that dropped a column.** Pre-deploy ran the migration before the new image started serving, so the still-warm old version crashed on the missing column. Two-phase migration policy ([Rule #4](../../04-governance/governance/rules/04-sql-migrations-sequential-two-phase.md)) prevents this.
