...
| **Logs**    | Текстові події (`user signed in`, `query failed`)           | Pino JSON у stdout, ALS-контекст `{requestId, userId, module}` — див. [`docs/03-operations/observability/logging.md`](../../03-operations/observability/logging.md) |
...
7. **Локальний n8n / OpenClaw** — якщо [docs/01-product/launch/business/05-operations-and-automation.md](../../01-product/launch/business/05-operations-and-automation.md) реально запускається і n8n cloud починає кусатись.
...
- **Grafana Cloud** (free tier) — скрейпимо `/metrics` через Grafana Agent (окремий маленький Railway-сервіс або локальний процес в apps/server контейнері). Імпортуємо `docs/03-operations/observability/dashboards/sync.json`. Заливаємо `alert_rules.yml`. Підключаємо Telegram contact point.
...
- **Stripe** — платежі. Див. [`docs/01-product/launch/business/06-monetization-architecture.md`](../../01-product/launch/business/06-monetization-architecture.md).
...
- Кожен alert у `alert_rules.yml` має `runbook:` annotation → лінк на `docs/03-operations/observability/runbook.md#секція`.
...
- Повний каталог сервісів + ціни: [`docs/01-product/launch/business/03-services-and-toolstack.md`](../../01-product/launch/business/03-services-and-toolstack.md).
- Monthly cost projection по фазах: [`03 §9`](../../01-product/launch/business/03-services-and-toolstack.md#9-повна-monthly-cost-projection).
- Pre-launch чеклист (де observability — requirement): [`docs/01-product/launch/business/04-launch-readiness.md#3-моніторинг-та-алерти`](../../01-product/launch/business/04-launch-readiness.md#3-моніторинг-та-алерти).
- Як налаштований поточний Railway + Vercel: [`docs/02-engineering/integrations/railway-vercel.md`](../integrations/railway-vercel.md).
- Observability reference: [`docs/03-operations/observability/`](../../03-operations/observability) (SLO, metrics, runbook, logging).
