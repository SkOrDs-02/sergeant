...
Деталі топології і проксі: [`docs/02-engineering/integrations/railway-vercel.md`](./railway-vercel.md).
...
- **`exp://` НЕ bound до конкретної аппки** — будь-який Expo Go застосунок на пристрої може її claim-ити, тому у production воно заборонене (закриває [hardening-карту H5](../../04-governance/security/hardening/H5-trusted-origins-exp-scheme.md)).
...
`AI_QUOTA_DISABLED=1` повністю вимикає квоту (no-op для `assertAiQuota`). **!!! ТІЛЬКИ для CI/test/dev.** У production `assertStartupEnv()` хард-блокує server-startup, якщо `AI_QUOTA_DISABLED=true` і `NODE_ENV=production` (або `RAILWAY_ENVIRONMENT`/`RAILWAY_SERVICE_NAME` виставлені). Без цього хард-блока випадковий copy-paste flag-а зі staging до prod дав би unlimited Anthropic budget burn. Дивись [`docs/04-governance/security/ai-quota-kill-switch.md`](../../04-governance/security/ai-quota-kill-switch.md).
...
### `ANTHROPIC_PROMPT_CACHE` _(optional, default off — OpenClaw gateway only)_

> **⚠️ Legacy-контекст.** `tools/openclaw` grammy-бот видалено з цього репо, а Railway виведено ([ADR-0074](../../04-governance/adr/0074-hosting-hetzner-coolify.md)); змінна стосується OpenClaw-gateway-процесу (тепер поза цим репо / не задеплоєний), **не** `apps/server`.

Опт-ін для prompt caching у OpenClaw agent-loop (PR-39, ADR-0057). Truthy values: `1`, `true`, `yes` (case-insensitive). Коли увімкнено, agent-loop додає `cache_control: { type: "ephemeral" }` на (a) system prompt і (b) останній tool у `tools[]`. Cache TTL — 5 хвилин; net-cost-win починається з ≥2 викликів у вікні (tool-use loop або кілька slash-команд підряд). Affects лише OpenClaw gateway-процес; не впливає на `apps/server` Anthropic-клієнт.
...
[Hardening карта H9](../../04-governance/security/hardening/H9-transcribe-usd-cap.md) — per-user-per-day USD cap на `/api/transcribe`, у _micros_ (1 USD = 1_000_000 micros).
...
Серверна episodic-memory (відмінна від Memory Bank — [ADR-0021](../../04-governance/adr/0021-memory-bank.md), local-first). Storage — `ai_memories` table з pgvector `HALFVEC(1024)` + HNSW + hash-partitioning по `user_id`.
...
Decision-point Day 30 — [`docs/03-operations/observability/runbook.md § AI memory activation & Day-30 decision-point`](../../03-operations/observability/runbook.md#ai-memory-activation--day-30-decision-point).
...
| Змінна                     | Default | Опис                                                                                                                                                             |
| -------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PG_POOL_SIZE`             | `20`    | Максимум клієнтів у пулі pg. Sizing rule + tuning — [`docs/03-operations/observability/pg-pool-sizing.md`](../../03-operations/observability/pg-pool-sizing.md). |
...
`CSP_DISABLE` видалено в M1 — дивись [`docs/04-governance/security/hardening/M1-csp-disable-runtime-flag.md`](../../04-governance/security/hardening/M1-csp-disable-runtime-flag.md). Для швидкого вимкнення CSP без блокувань — `CSP_REPORT_ONLY=1`.
...
> `@opentelemetry/*` пакетів). Причина й деталі — [ADR-0035 § Reversal](../../04-governance/adr/0035-distributed-tracing-opentelemetry.md).
...
- `OPENCLAW_GITHUB_APP_PRIVATE_KEY` — PEM-приватник App-у. Деякі secret-store-и (Vercel, Coolify, 1Password CLI) розплющують `\n` → `github-auth.ts` репарює `\\n → \n` перед `crypto.createSign('RSA-SHA256')`.
...
Дивись [`apps/mobile/src/lib/observability/posthog.ts`](../../../apps/mobile/src/lib/observability/posthog.ts) і [`docs/01-product/launch/product-os/ftux-sprint-plan.md`](../../01-product/launch/product-os/ftux-sprint-plan.md) §S0.3.

---

## 22. Cost monitoring (PR-33)

> Server-side env, читається у `apps/server/src/env.ts` і пушиться у Prometheus Gauge `infra_monthly_cost_usd` через `applyInfraMonthlyCosts()` під час bootstrap-у. Споживач — Grafana-дашборд [`docs/03-operations/observability/dashboards/cost-monitoring.json`](../../03-operations/observability/dashboards/cost-monitoring.json) (PR-33).
...
Дивись [`docs/03-operations/observability/metrics.md` §16 Cost monitoring](../../03-operations/observability/metrics.md#16-cost-monitoring-pr-33--pr-38) для PromQL-запитів і [`docs/03-operations/observability/dashboards/cost-monitoring.json`](../../03-operations/observability/dashboards/cost-monitoring.json) для імпорту в Grafana.
...
- [ADR-0028](../../04-governance/adr/0028-pgvector-ai-memory.md) — pgvector + Voyage AI memory.
- [ADR-0031](../../04-governance/adr/0031-openclaw-v0-telegram-cofounder.md) — OpenClaw v0.
- [ADR-0042](../../04-governance/adr/0042-password-hashing-strategy.md) — password hashing (scrypt у Better Auth, без 72-byte ліміту).
- [`docs/04-governance/security/hardening/`](../../04-governance/security/hardening) — карти H5, H6, H9, M1.
