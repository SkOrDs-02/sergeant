...
> **Last touched:** 2026-07-30 by @claude. **Next review:** 2027-01-27.
...
на сотнях тисяч rows-ів, без token-cost-а у Anthropic-context-і. На відміну від [Memory Bank](../../04-governance/adr/0021-memory-bank.md) (local-first, structured user-facts) — pgvector-store зберігає embedding-и довільного тексту і повертає top-K-семантично-найближчих.
...
| `embedding_model`    | `TEXT`                                     | `"voyage-3.5-lite"`                                                                    |
...
| `VOYAGE_EMBEDDING_MODEL`      | `voyage-3.5-lite` | Зміна → re-embed усіх row-ів. НЕ використовувати `voyage-3-lite` (512d, несумісно з HALFVEC(1024)). |
...
Threshold-и міграції на dedicated vector DB — у [ADR-0028 § scaling thresholds](../../04-governance/adr/0028-pgvector-ai-memory.md#scaling-thresholds).
...
- ADR-0028 — `docs/04-governance/adr/0028-pgvector-ai-memory.md`
