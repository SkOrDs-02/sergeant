...
- `lib/mono/sign.ts` *(new)* — `signRequest({ time, path, permissionsOrToken })`; читає приватний ключ з env.
- `modules/mono/authRequest.ts` *(new)* — handler `POST /api/mono/auth/request` (підпис → Mono → `{acceptUrl}`).
- `modules/mono/authCallback.ts` *(new)* — handler `X-Callback`: дістати `X-Request-Id`, викликати
