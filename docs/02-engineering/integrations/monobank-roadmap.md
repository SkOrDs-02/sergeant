...
| **C** | Auto-categorization по MCC           | плюс плюс             | середня    | середній | ✅ done — `apps/server/src/modules/mono/mccCategories.ts:32` викликається з `webhook.ts:296`. Пріоритет: ручний override > MCC/keywords > fallback `other` (`packages/finyk-domain/src/lib/categories.ts:86-98`). AI-категоризація пише лише в окремі `ai_category_*`-поля й **не** чіпає `category_slug` (`apps/server/src/modules/mono/enrichmentWorker.ts:33-34`). |
...
> **✅ Реалізовано (оновлено 2026-07-22, finyk-аудит D2).** MCC-мапінг живе в
> `apps/server/src/modules/mono/mccCategories.ts:32` і викликається з
> `webhook.ts:296` — це вже не план, а робочий код. Фактичний пріоритет
> категоризації: **ручний override > MCC/keywords > fallback `other`**
> (`packages/finyk-domain/src/lib/categories.ts:86-98`). AI-категоризація
> пише лише в окремі `ai_category_*`-поля й **не** перезаписує
> `category_slug` (`apps/server/src/modules/mono/enrichmentWorker.ts:33-34`).
> Секція нижче лишена як історія плану (структура шарів приблизно збігається
> з реалізацією, деталі колонок/міграції могли розійтись — не покладайся на
> код-фрагменти нижче, дивись живий код).
