...
- Drift gate: PLAN_PRIMER / ANALYZE_PRIMER порівнюється байт-у-байт з legacy console (`tools/openclaw/src/agents/strategic-modes.ts`). Якщо хтось зачепить legacy primer без bump-у в плагіні — CI падає. Це навмисний contract під час parallel-run з Gateway (Stage 6b).
...
OpenClaw 5.7 SDK реал API (verified у spike `docs/02-engineering/notes/spikes/openclaw-sdk-5.7-real-api.md` § 4):
...
PLAN_PRIMER + ANALYZE_PRIMER (і майбутній OKR_PRIMER) — це **byte-for-byte copy** з legacy console (`tools/openclaw/src/agents/strategic-modes.ts`). Drift gate тест читає legacy файл, reconstructs string literal через `Function()`, і порівнює:
...
**Чому не просто `import`?** Плагін `@sergeant/openclaw-plugin` має бути package-independent — окремий Docker image, deploy-ит-ся на Railway service `sergeant-openclaw-gateway`. Він НЕ може import-ити з `tools/openclaw/*` (workspace boundary). Замість прямого import — drift gate через file-read дає eventually-consistent гарантію.

**Lifetime контракту:** drift gate валідний поки legacy console живе (Stage 6b parallel-run). Як тільки Stage 7 cutover видаляє `tools/openclaw/src/agents/strategic-modes.ts` — drift gate треба видалити одним PR (з усіма трьома `*_PRIMER` тестами). Це vendoring trade-off: контракт є нудний, але defensive і catch-ить drift автоматично.
...
- Stage 7 — Cutover playbook (ADR-0056 supersedes ADR-0055). Деletion checklist включає: drift-gate тести × 3, `tools/openclaw/src/agents/strategic-modes.ts`, всі legacy primers.
...
| `docs/90-work/planning/openclaw-migration-plan.md`                                     | +4         | n/a (tracker)                                                                          |
| `docs/02-engineering/notes/spikes/openclaw-stage-5b-pr-split-2026-05-12.md` (цей файл) | +new       | n/a (knowledge handoff)                                                                |
