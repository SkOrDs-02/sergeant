...
1. **Status (2026-05-12 14:10 UTC):** **RESOLVED.** PR #2471 (`api.on` migration) змерджений у `main` як `3d4f9e1d` (2026-05-12 14:03:50 UTC) і auto-deploy на `sergeant-openclaw-gateway` пройшов SUCCESS. Live smoke о 17:08–17:09 EEST: `/new` → OK, `/metrics` → canned `📊 Метрики сьогодні` Markdown, `Дай метрики` (UA вільний текст) → той самий шорткат, `/runway` → canned `✈️ Runway` Markdown. Скріни — § 11 нижче.
2. **Symptom** (історично, до 14:03 UTC): `/metrics`, `/runway`, UA `Дай метрики` у Telegram DM з `@KENT_OPENCLAW_GATEWAY_BOT` повертають Opus-prose, не canned Markdown ≤2 сек. Усі команди йдуть через повний agent cycle = LLM cost ≠ $0.
3. **Real root-cause №4 (виявлено 2026-05-12 13:15 UTC, fix shipped — PR #2471)**: Sergeant plugin реєстрував 5 lifecycle hooks (`before_dispatch`, `llm_input`, `before_agent_start`, `agent_end`, `before_tool_call`) через **`api.registerHook(events, handler, opts)`**. У SDK 2026.5.7 цей метод призначений для **INTERNAL hook bus** (`type: "command" | "session" | "agent" | "gateway" | "message"`) — він пушить хуки у `registry.hooks` + `registerInternalHook()`. `hookRunner.runBeforeDispatch()` читає з **`registry.typedHooks`**, куди потрапляє лише `api.on(hookName, handler, opts?)`. Тому всі наші 5 хуків мовчки зареєстровані в "мертву" систему і ніколи не fires. Лог `sergeant.hooks.registered { ok: 5, failed: 0 }` обманював — реєстрація "успішна" з точки зору `registerHook` (бо `opts.name` валідний), але runtime ніколи їх не зачитує.
4. **Real root-cause №3 (старий, вже не актуальний)**: ~~Railway service не watch-ить GitHub~~ — застаріле. У поточному проекті `Sergeant` service `sergeant-openclaw-gateway` **підключений до GitHub** і auto-deploy працює. Усі попередні fix-forward (PR #2467/2468/2469) **були задеплоєні** — і всі вони били не ту проблему (фіксили `opts.name` для API, який runtime все одно ніколи не виконує для lifecycle hooks).
5. **Резюме циклу**: 4 ітерації fix-forward (PR #2467/2468/2469 + неpushed-діагностика від 12:10 UTC) пройшли локальний typecheck+test+lint+build, GitHub CI зелений, deploy ✓, але all hits the same "симптом не виправлений" mode. Причина: unit-тести мокали `api.registerHook` і нічого не знали про `api.on`. Реальний SDK contract (`api-builder.d.ts` + loader source) перевірений у § 4.5 нижче і виправлений у PR #2471 (live verified — див. § 11).
...
5. `docs/02-engineering/notes/spikes/openclaw-sdk-5.7-real-api.md` — переписати Row 1 (Hook API) — `api.on` canonical, `api.registerHook` лише для internal bus.
...
- `docs/02-engineering/notes/spikes/openclaw-sdk-5.7-real-api.md` — Row 5 (`before_dispatch` contract), Row 6 (`opts.name` loader validation).
- `docs/90-work/planning/openclaw-migration-plan.md` — § Reality update пункти 11, 12, 13.
...
| 2471 | refactor(openclaw-plugin): migrate lifecycle hooks from `registerHook` to `api.on` | `3d4f9e1d`               | **merged → deployed 14:03:50 UTC** ✓  | **Real root-cause #4 fix.** Replaces `for (...) registerHook(event, handler, { name })` з 5 явними `api.on(event, handler)`. + `allowConversationAccess: true` для conversation hooks. + Spike doc Row 1 переписаний. + tests. Live verification 17:08–17:09 EEST — § 11. |
...
> **Update 2026-05-12 14:10 UTC:** виконано. Усі smoke-команди показали shortcut поведінку (див. § 11). Stage 4b live. Stage 4a `before_agent_start` event shape mismatch і HTTP 400 у `query_app_db` (виявлено у `/runway` snapshot) — записані як окремі follow-up задачі у § 10.
...
- **(NEW 2026-05-12 14:10 UTC)** ~~`/runway` shortcut snapshot повертає `query_app_db failed: OpenClaw HTTP 400 for /query`. Шорткат сам fires миттєво і повертає Markdown — але всередині runway-handler-а виклик tool `query_app_db` йде через OpenClaw HTTP API і отримує 400. Окрема задача (швидше за все Stage 4c): з'ясувати чому `/query` endpoint у gateway відхиляє synthetic shortcut request. Не блокує Stage 4b. Workaround — runway шорткат і так повертає Stripe-секцію (теж noConfigured), тому користувач все одно бачить структуру.~~ **Resolved 2026-05-12 (Stage 4b follow-up):** root cause був не gateway proxy, а сам SQL у шорткатах: `runway.ts` запитував `business_snapshot` (таблиця ніколи не існувала в міграціях), `decisions.ts` запитував `ai_decisions` (теж не існує — канонічна таблиця `openclaw_decisions`). Обидва SQL не проходили `QUERY_APP_DB_TABLE_ALLOWLIST` (`apps/server/src/modules/openclaw/types.ts`) → server повертав 400 verbatim. Fix: `runway` тепер агрегує `openclaw_invocations` (Sergeant ops cost за 30 днів) + Stripe + явна позначка про відсутній business-snapshot pipeline; `decisions` мігровано на `openclaw_decisions(id, topic, decision, decided_at)`. Регресія-тест у `packages/openclaw-plugin/src/shortcuts/all-shortcuts.test.ts` ловить будь-який майбутній `FROM <not-in-allowlist>` у shortcut SQL.
- **(NEW 2026-05-12 14:10 UTC)** Усі external integrations (PostHog, Stripe, Sentry) повертають `notConfigured: true` бо ключів немає у Railway env. Це окрема задача (Stage 5 — connect-integrations); шорткат коректно віддає `notConfigured` payload з `note`-полем — це передбачена UX-поведінка, не bug у Layer 0.

---

## 11. Live verification 2026-05-12 17:08–17:09 EEST (14:08–14:09 UTC)

**Контекст:** через ~5 хв після auto-deploy `3d4f9e1d` (14:03:50 UTC) і `[gateway] http server listening (2 plugins: sergeant, telegram; 4.7s)` о 14:06:00 UTC, founder (`@dmytro.s.stakhov`, TG ID `319824665`) виконав smoke у DM з `@KENT_OPENCLAW_GATEWAY_BOT`.

| Команда (TG)             | Очікувана поведінка                          | Фактична поведінка (з TG скрінів)                                                                                                                        | Висновок                                                                      |
| ------------------------ | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `/new`                   | Миттєве `✅ New session started.`            | `✅ New session started.` миттєво.                                                                                                                       | OK (native command, не наш plugin)                                            |
| `/metrics`               | Canned `📊 Метрики сьогодні` Markdown ≤2 сек | `📊 Метрики сьогодні` Markdown з трьома секціями (PostHog/Stripe/Sentry). Усі `notConfigured: true` бо ключів немає. Без Opus prose. **Без "думаю...".** | **PASS** — Layer 0 shortcut fires                                             |
| `Дай метрики` (UA текст) | Той самий canned Markdown ≤2 сек             | Той самий `📊 Метрики сьогодні` Markdown. UA-text-matcher працює.                                                                                        | **PASS** — UA fallback router fires                                           |
| `/runway`                | Canned `✈️ Runway` Markdown ≤2 сек           | `✈️ Runway` Markdown зі Snapshot/Stripe/Sentry. Snapshot повертає `(query_app_db failed: OpenClaw HTTP 400 for /query)` — інше питання, не Layer 0.      | **PASS** — Layer 0 shortcut fires; внутрішній HTTP 400 — окремий issue (§ 10) |

**Що це означає:**

- `hookRunner.runBeforeDispatch()` тепер реально знаходить наш handler у `registry.typedHooks` і викликає його.
- Shortcut handler повертає `{ handled: true, response: { ... } }`, dispatcher його респектить, повний Opus-loop не стартує.
- Тобто і Stage 4b shortcut router, і Stage 4a `before_agent_start` audit-open — обидва тепер мають живі реєстрації.

**Що залишається невирифіковано в smoke (наступний крок — окрема сесія):**

- `/think <query>` — мало повернути повний agent cycle (Opus prose ≥15 сек). У смок не запускався, але це pass-through path, не торкається нашого hook fix.
- `agent_end` audit-finalize hook (Stage 3) — fires тільки коли agent loop йде. Перевіряти через PR-comment з `/think` тестом і grep `sergeant.audit.finalize` у Railway logs.
- `before_tool_call` write-tool freeze gate (Stage 4c) — fires тільки на approve_action. Окремо тестувати з `/audit <id> approve`.

---

> **Created by**: Devin session 4d68881f8cd142098f771876383d5639 (2026-05-12 ~12:15 UTC). **Live-verified** by Devin session f21cb069d8e240aaa35a275eeb29b59c (2026-05-12 14:10 UTC).
