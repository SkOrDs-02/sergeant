...
1. **Stage 5d — Morning-digest cron.** Не залежить від 5c. Спайк §4 у `docs/02-engineering/notes/spikes/openclaw-stage-4b-debugging-handoff-2026-05-12.md` фіксує що native scheduler (`cron.*` config-block) — supported, але invariant API не підтверджений (риск середній). Окремий код-PR; ~150–300 LOC, +~15 tests.
