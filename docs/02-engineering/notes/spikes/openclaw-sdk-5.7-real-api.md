...
> **Roadmap reference:** [`docs/90-work/planning/openclaw-migration-plan.md` § Reality update 2026-05-12](../../../90-work/planning/openclaw-migration-plan.md) — Stage спайк рядок (`🔬 Spike`).
...
| 3   | Per-persona allowlist                               | **Реально:** `AgentsConfig = { defaults?: AgentDefaultsConfig, list?: AgentConfig[] }` (`plugin-sdk/src/config/types.agents.d.ts:121`). Персони живуть у масиві **`agents.list[]`**, де кожен entry має `id: string` (required) + опціонально `tools: AgentToolsConfig = { profile?, allow?, alsoAllow?, deny?, byProvider?, … }` (`plugin-sdk/src/config/types.tools.d.ts`). **БГ! `AgentDefaultsConfig` НЕ має `tools` поля** — рунтайм відкине `agents.defaults.tools` з `Unrecognized key`. Shared baseline tools віддавати через **root-level `tools.alsoAllow`**, не через `agents.defaults`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Stage 5a — це чисто config-as-code change у `ops/openclaw/openclaw.example.json`: персони як `agents.list[]` масив (НЕ як `agents.<id>` keys), baseline `recall_memory` у root-level `tools.alsoAllow`. Жодних `register*Allowlist` API на стороні плагіна не треба. **Validation script** (`scripts/validate-openclaw-config.mjs`) проганяє `openclaw config validate --json` проти example — обов'язково локально перед PR який змінює конфіг. CI job wiring deferred у наступний PR з workflow-scope token (root cause crash-loop-у 16:40–18:00 UTC 2026-05-12 саме у відсутності такої валідації).                                                                                                                                                                                                                                                                                                                    |
...
### 6. Per-persona allowlist (`agents.list[].tools`)

> **2026-05-12 correction.** Раніше цей розділ описував `agents.<id>.tools` shape (читаючи `AgentToolsConfig`, без перевірки реального runtime schema). У production це **зруйнувало gateway** після merge Stage 5a (PR #2480): рунтайм-валідатор відкинув і `agents.defaults.tools`, і всі персона-ключі (`Unrecognized keys: cofounder, eng, …`). Реальна форма — `agents.list[]` array, де кожен entry має `id` + опціональний `tools`. Спайк-док тоді дивився лише на `.d.ts` types з SDK, не запускаючи `openclaw config validate` проти прикладу.

`plugin-sdk/src/config/types.agents.d.ts:121`:

```ts
export type AgentsConfig = {
  defaults?: AgentDefaultsConfig; // NB: NO `tools` field on AgentDefaultsConfig
  list?: AgentConfig[]; // ← per-persona configs go here, NOT as agents.<id> keys
};

// AgentConfig (same file)
...
Тобто Stage 5a виглядає так у `openclaw.example.json` (приклад для `seo` персони, яка не має write-tools):

```jsonc
{
  "agents": {
    "defaults": {
      /* workspace, model, models — NO `tools` here, рунтайм відкине */
    },
    "list": [
      {
        "id": "seo",
      ...
      // ... решта persona-entries
    ],
  },
  // Shared baseline tools — root-level, НЕ у agents.defaults.tools.
  "tools": {
    "alsoAllow": ["recall_memory"],
  },
}
```
...
**Validation script проти schema-drift-у:** `scripts/validate-openclaw-config.mjs` встановлює `openclaw@<version-з-Dockerfile>`, патчить plugin runtime config через `ops/openclaw/patch-sergeant-config.mjs`, проганяє `openclaw config validate --json`. Запускати локально (`node scripts/validate-openclaw-config.mjs`) перед PR який змінює `openclaw.example.json`. **TODO follow-up PR:** підняти цей скрипт як CI job у `.github/workflows/ci.yml` — потребує OAuth token з `workflow` scope, який цей fix-PR не має. Саме відсутності такої валідації було достатньо, щоб PR #2480 пройшов у production і зруйнував gateway.
