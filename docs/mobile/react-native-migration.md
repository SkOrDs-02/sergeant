...
| 2   | Hub-ядро                                   | ✅ Done        | 1                            | BRANDBOOK native-patterns (Q9). `ErrorBoundary` + `SyncStatusIndicator` + 6 HubSettings-секцій + HubDashboard (3 PR-серія) + оновлений `OnboardingWizard` + HubChat + HubSearch + HubReports. Залишок: voice у HubChat-композері + AI-customize крок onboarding — in review.                                                                          |
