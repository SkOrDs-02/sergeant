...
| **M-003** settings touch-targets      | `touch-target` на time/number-інпутах + label-рядках чекбоксів; push-toggle → `data-compact` + `before:` 44×44 hit-area (трек лишається 44×24, без розтягу в капсулу)                                                                 | settings `small = 0`; push-toggle трек 44×24, ::before 44×44, `data-compact` ✓                              |
| **M-004** StatusStrip 11px            | label `text-meta → text-caption` (12px), **локально** (не глобальний токен)                                                                                                                                                           | fizruk `10-11px = 0`                                                                                        |
| **M-005** FAB-overlap + обрізана таба | **ВИПРАВЛЕНО** (раніше won't-fix). (a) `page-tabbar-pad` 88→132px — контент скролиться вище плаваючого FAB (топ FAB=128px). (b) модуль-світчер: nutrition-таб «Харчування»→«Їжа» (короткий лейбл, aria повне) — більше не обрізається | `overflow:0`; усі 4 таби `truncated:false` («Фінік/Фізрук/Рутина/Їжа»), aria=«Перейти до модуля Харчування» |
