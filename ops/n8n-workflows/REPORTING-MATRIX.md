...
> **Last touched:** 2026-07-24 by @Skords-01. **Next review:** 2026-11-04.
...
⁽⁵⁾ Wave 3 §3.2 ADR-0038 — broadcast workflow спочатку постить idempotent
рядок у `tg_alert_acks` (`POST /api/internal/alerts/post`, `alertId =
...
/api/internal/alerts/ack`, далі `editMessageText` (стирає кнопки + додає футер
`✅ Прочитав by @user at HH:MM Z`) + `answerCallbackQuery` (toast). **WF-103**
— escalation cron: кожні 5хв тягне `POST /api/internal/alerts/pending`
(`olderThanMinutes=15, notYetEscalated=true`), фільтрує до P0/P1, шле `POST
/api/internal/alerts/escalate` і паралельно DM founder через
`@OpenClaw_sergeant_bot` (raw `sendMessage`, бо це окремий бот, не
`Sergeant_alert_bot` credential). Обидва inactive у проді поки не пройде
end-to-end test у staging. **WF-104 Sprint 6 update:** callback router тепер
має дві гілки — `ack:<r|i|m>:<alertId>` (T1 ack) і `snooze:<1h|4h>:<alertId>`
(T2 snooze, з WF-105 keyboard); snooze шле `POST /api/internal/alerts/snooze`
→ `editMessageText` із "Snoozed until HH:MM" футером.

⁽⁷⁾ Sprint 6 / alert-escalation-tiers ADR-0038 follow-on. **WF-105** —
Tier 2 repeat-ping cron: `*/15` тягне `POST /api/internal/alerts/pending`
(`olderThanMinutes=60, notYetRepeated=true, notSnoozed=true`), для кожного
шле `POST /api/internal/alerts/repeat` (stamps `repeated_at`) і
re-broadcast у original topic з prefix `⚠ REPEAT (Nхв без ack)` +
3-кнопкою (✅ Прочитав / 🕐 Snooze 1h / 🕓 Snooze 4h). **WF-106** — Tier 3
sentry-warn cron: `*/15` для 120-хв unacked-not-sentry-warned шле `POST
/api/internal/alerts/sentry-warn` → server-side `Sentry.captureMessage(
level=warning, tags.kind=unacked-alert-escalation)`; off-channel fallback
