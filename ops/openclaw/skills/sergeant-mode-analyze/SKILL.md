...
Audit trigger label (per `tools/openclaw/src/agents/strategic-modes.ts`): `strategic_analyze`.
...
Primer-джерело: `tools/openclaw/src/agents/strategic-modes.ts` `ANALYZE_PRIMER`. Текст у цьому SKILL.md — canonical у OpenClaw runtime.
