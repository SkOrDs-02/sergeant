...
Audit trigger label (per `tools/openclaw/src/agents/strategic-modes.ts`): `strategic_okr`.
...
Primer-джерело: `tools/openclaw/src/agents/strategic-modes.ts` `OKR_PRIMER`. Текст у цьому SKILL.md — canonical у OpenClaw runtime.
