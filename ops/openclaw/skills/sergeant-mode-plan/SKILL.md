...
Audit trigger label (per `tools/openclaw/src/agents/strategic-modes.ts`): `strategic_plan`.
...
Primer-джерело: `tools/openclaw/src/agents/strategic-modes.ts` `PLAN_PRIMER`. Текст у цьому SKILL.md — canonical у OpenClaw runtime; primer у внутрішньому боті залишається до Phase 7 cutover.
