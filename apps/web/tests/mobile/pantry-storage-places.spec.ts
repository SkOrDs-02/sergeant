...
test.describe("комора: місця зберігання", () => {
  ...
  test("зміна місця переживає перезавантаження", async ({ page }) => {
    ...
    // Доказ по UI, що переїзд ЗАСТОСУВАВСЯ, — і саме він робить тест
    // детермінованим (заміряно: без нього 1 прохід із 5, з ним 12 із 12).
    //
    // AI-DANGER: спокуса замінити це на «дочекатись сигналу, що запис у
    // SQLite доїхав» тут уже двічі не спрацювала, тож не повторюй наосліп.
    // `click()` повертається ДО коміту React, тому лічильник
    // `__sergeantSqliteRefreshCounts` міг стрибнути від попередньої пачки
    // (наповнення комори), яка дренажилась раніше, ніж клік узагалі
    // потрапив у чергу — 1 прохід із 5. А очікування «лічильник завмер»
    // (queue-idle) не завершується взагалі: він інкрементується безперервно.
    // Робочий інваріант простий — переїзд має бути ВИДИМИЙ до `reload()`.
    //
    // Побічно цей крок розрізняє два різні дефекти — «не застосувалось» і
    // «застосувалось, але не збереглось», які інакше виглядають однаково.
