          ...
          <MiniPhone dim>
            ...
            <div className="flex flex-1 min-h-0 flex-col gap-3 p-4 overflow-hidden">
              ...
              <div className="rounded-2xl bg-panel border border-line p-4">
                ...
                <p className="text-2xl font-semibold text-text leading-tight">₴ 1 240</p>
              </div>
              ...
              <div className="rounded-xl bg-panel border border-line px-3 py-2 whitespace-nowrap overflow-hidden">
                <span className="text-style-body text-text" style={{ fontSize: `${16 * scale}px` }}>
                  Категорія · Продукти
                </span>
              </div>
              <button type="button" className="mt-auto rounded-xl bg-accent px-4 py-3 text-style-caption font-medium text-bg">
                Додати витрату
              </button>
              <p className="text-2xs text-muted">Фіксований 16px — за XL текст обрізається.</p>
            </div>
          </MiniPhone>
        }
        after={
          <MiniPhone>
            <div className="flex flex-1 min-h-0 flex-col gap-3 p-4" style={{ fontSize: `calc(1rem * ${scale})` }}>
              <p className="text-muted" style={{ fontSize: "0.75em" }}>Сьогодні</p>
              <div className="rounded-2xl bg-panel border border-line p-4">
                <p className="text-muted" style={{ fontSize: "0.75em" }}>Витрачено</p>
                <p className="font-semibold text-text" style={{ fontSize: "1.6em", lineHeight: 1.2 }}>₴ 1 240</p>
              </div>
              <div className="rounded-xl bg-panel border border-line px-3 py-2">
                <span className="text-text" style={{ fontSize: "0.95em" }}>Категорія · Продукти</span>
              </div>
              <button
                type="button"
                className="mt-auto rounded-xl bg-accent font-medium text-bg"
                style={{ paddingBlock: "max(0.6em, 12px)", paddingInline: "1rem", fontSize: "0.95em" }}
              >
                Додати витрату
              </button>
              <p className="text-muted" style={{ fontSize: "0.7em" }}>Ритм і цілі зберігаються за будь-якого масштабу.</p>
            </div>
          </MiniPhone>
        }
      />
