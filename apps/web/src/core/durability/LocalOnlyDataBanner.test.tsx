...
describe("LocalOnlyDataBanner", () => {
  it.each([...NON_SYNCABLE_USER_IDS])(
    ...
    (id) => {
      ...
      // якщо з'явиться третій синтетичний id, він автоматично потрапить у
  ...
  it("копія називає саме те, що НЕ відновлюється з банку", () => {
    // Робить неможливим «пом'якшення» тексту до абстрактного «увійди, щоб
  ...
  it("кнопка бекапу з'являється лише коли є куди її повісити", () => {
    useLocalUserIdMock.mockReturnValue("local-anon");
