...
const CHUNK_ERROR_PATTERNS: ReadonlyArray<RegExp> = [
  ...
  // його не ловить. Саме це дало 18 користувачів на двох issue
  // (SERGEANT-API-M / SERGEANT-WEB-R) після деплою 2026-08-10, коли
  // `vendor-sqlite` виїхав з eager-графа й змінив хеш.
  //
  // Уточнення до діагнозу (заміряно на проді 2026-08-29): HTML приходив не
  // з кешу — його віддавав САМ Vercel. SPA-rewrite ловив усе, чого немає на
  // диску, тож запит відсутнього асета отримував `index.html` з кодом 200 і
  // типом `text/html`. Emscripten валив обидва шляхи (streaming і
  // ArrayBuffer), бо компілював сторінку застосунку як wasm.
  //
  // Корінь вилікувано двома правками, і ці патерни лишаються страхувальною
  // сіткою для клієнтів, які вже стейлі:
  //   1. `vercel.json` — `assets/` виключено з rewrite, відсутній асет
  //      тепер дає чесну 404;
  //   2. `vite.config.js` — `wasm` доданий у `globPatterns`, тож бінарник
  //      лежить у прекеші поруч зі своїм glue і в мережу не ходить.
  //
  // AI-NOTE: одного `reloadOnceForChunkError` тут МАЛО і завжди було мало.
  // Реліз-цикл — `prompt` без `skipWaiting` (див. AI-DANGER у `src/sw.ts`),
  // тож reload переасемблює ту саму стару версію: той самий воркер, той
  // самий прекеш, той самий запит. Проти стейл-прекешу лікує лише
  // оновлення воркера, а не перезавантаження сторінки.
