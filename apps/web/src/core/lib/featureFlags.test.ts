...
describe("featureFlags", () => {
  ...
  it("Stage 8 PR #057f drop: feature.fizruk.sqlite_v2.read_sqlite більше не існує у реєстрі", async () => {
    ...
    expect(
      FLAG_REGISTRY.find(
        (f) => f.id === "feature.fizruk.sqlite_v2.read_sqlite",
      ),
    ).toBeUndefined();
    expect(getFlag("feature.fizruk.sqlite_v2.read_sqlite")).toBe(false);
    expect(setFlag("feature.fizruk.sqlite_v2.read_sqlite", true)).toBe(false);
  });
  ...
  it("Stage 8 PR #057n drop: feature.nutrition.sqlite_v2.read_sqlite більше не існує у реєстрі", async () => {
    ...
    expect(
      FLAG_REGISTRY.find(
        (f) => f.id === "feature.nutrition.sqlite_v2.read_sqlite",
      ),
    ).toBeUndefined();
    expect(getFlag("feature.nutrition.sqlite_v2.read_sqlite")).toBe(false);
    expect(setFlag("feature.nutrition.sqlite_v2.read_sqlite", true)).toBe(
      false,
    );
  });
