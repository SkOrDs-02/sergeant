...
export function useDebtPaymentSplitSync(
  ...
  const reconcile = useCallback(
    (
      ...
      // Нове рішення скасовує попередній запит на підтвердження: той описував
      // розподіл, якого вже немає. Без цього людина, яка повернула частку
      // боргу замість відповіді на діалог, підтвердженням прибрала б щойно
      // відновлену привʼязку. Сьогодні діалог тримає тло інертним, тож шлях
      // закритий зверху — але коректність хука не має триматись на тому, що
      // саме його показує.
