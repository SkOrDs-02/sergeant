...
import type { FinykBlobTable, FinykIdTable } from "./diff.js";

/**
 * Declarative TableSpecs + prebuilt SQL for the finyk dual-write adapter
 * (ADR-0073 крок 5). Split out of `adapter.ts` so each table family stays
 * under the Hard Rule #18 `max-lines: 600` ceiling.
 *
...
export function blobUpsertSql(table: FinykBlobTable): string {
  return BLOB_UPSERT_SQL.get(table) ?? buildLwwUpsert(blobUpsertSpec(table));
}

export function blobDeleteSql(table: FinykBlobTable): string {
  return (
    BLOB_DELETE_SQL.get(table) ??
    buildDelete({
      table,
      deletePolicy: "soft",
      matchColumns: ["id", "user_id"],
