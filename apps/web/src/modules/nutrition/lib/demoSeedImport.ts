 ...
 * не з'являється — отже й нового місця, де дані можуть розійтися з
 * рештою модуля.
 *
 * AI-DANGER: виклик обов'язково гейтиться демо-режимом (див.
 ...
 *   - **pantryEvents** — ОБОВ'ЯЗКОВО `[]`. Це append-only журнал
 ...
 * сирий об'єкт — `prefsJson` збирається через `normalizeNutritionPrefs`,
 ...
 * самого київського дня, повторний `goal-period-insert` б'ється в той
