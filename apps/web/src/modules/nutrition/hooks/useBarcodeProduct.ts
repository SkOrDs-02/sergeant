 ...
 * (у комірці або в лотку) не бʼє сервер вдруге.
 *
 ...
 * користувачу.
 *
 * AI-DANGER: ковтати можна ЛИШЕ 404. Сервер віддає **503**, коли всі три
 * upstream-и каскаду не відповіли (аудит nutrition § G5) — це «бази лежать»,
 * а не «продукту немає», і перетворення його на `null` знову склеїло б два
 * різні стани в одне повідомлення «Продукт не знайдено». Розширення цього
 * `catch` на інші статуси поверне саме той баг, який G5 закриває.
 */

// Локальна TTL-політика: штрих-код вказує на конкретний товар, його
// `per100` не змінюється. 24 години свіжості, 7 днів у памʼяті/LRU — достатньо,
