 ...
 * план і § Ризики).
 *
 * Особливість fizruk: батч атомарний — адаптер обгортає всі ops у
 * `client.exec("BEGIN")` / `client.exec("COMMIT")`. Recording-клієнт
 * записує й `exec`-виклики (з `params: []`), тому транзакційні
 * statement-и — частина зафіксованої специфікації.
 *
