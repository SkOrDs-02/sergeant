  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <Button
        variant="ghost"
        onClick={() => {
          for (let i = 0; i < 4; i++) t.success("Збережено");
        }}
      >
        4× однакових (коалесинг)
      </Button>
    </div>
  );
}
...
/**
 * Одночасно видно щонайбільше `MAX_VISIBLE_TOASTS` (3) аркушів — решта
 * чекає у черзі провайдера й піднімається, щойно звільниться слот. Ніщо
 * не «виштовхується» мовчки: 6-й зʼявиться, просто пізніше.
 *
 * Друга кнопка показує коалесинг: чотири однакові `success` без action
 * зливаються в один аркуш із бейджем `×4` замість вежі з чотирьох.
 */
