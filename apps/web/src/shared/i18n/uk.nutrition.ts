 ...
 * Каталог модуля. Від 2026-09-12 НЕ спредиться в `uk.ts` — модульні файли
 * імпортують його прямо, щоб він не їхав першим екраном.
 */
export const nutritionPageMessages = {
  // Ключі, що раніше лежали інлайном у `uk.ts` під `messages.nutrition`.
  ...
  /**
   * Одна стрічка джерел у шапці «Додати продукти» (`PantrySourceTabs`,
   * рішення власника 2026-09-11). Group-label для a11y — самі сегменти
   * («По одному»/«Списком»/«Скан»/«З чека») лишаються короткими видимими
   * підписами прямо в компоненті, бо їх винесення в каталог не додає
   * перевикористання (кожен рядок — один call-site).
   */
  pantryCard: {
    sourceStripLabel: "Джерело для додавання продуктів",
  },
  ...
  /**
   * Список позицій, які фото-аналіз розпізнав на кадрі (ініціатива 0023).
   *
   * Ключі — plain-строки без інтерполяції (`MessageCatalog` не пускає
   * функції), тож рядок «300 ккал · 21/18/6 г» компонент складає сам із
   * `kcalUnit`/`gramsUnit` — той самий патерн, що `biometrics.ageLabel`.
   * `removePrefix` іде в aria-label разом із назвою страви: без назви
   * «Прибрати» звучало б однаково на кожному рядку, і людина зі
   * скрінрідером не знала б, який саме прибирає.
   */
  photoItems: {
  ...
  /** Додавання позиції у фото-список через каталог продуктів. */
  photoAddItem: {
    gramsLabel: "Вага порції, г",
    /** Рендериться як `${kcal} ккал ${forWord} ${grams} г`. */
    forWord: "за",
    addCta: "Додати",
    otherProductCta: "Інший продукт",
    cancelCta: "Скасувати",
  },
