 ...
 * `recall` / `event-sync`. Екран «Що ШІ про мене пам'ятає» робить дешеві
 * реляційні запити без жодного ембеддингу, і при спільному бакеті юзер,
 * який чистить пам'ять, впирався б у 429 приблизно на 25-му видаленні —
...
export function createAiMemoryRouter({ pool }: { pool: Pool }): Router {
   ...
   * прочистити велику пам'ять за один сеанс, і все ще зупиняє скрипт.
   */
  ...
  // Записує пам'ять лише Pro (ingest вище), тож у Free-юзера цей список
