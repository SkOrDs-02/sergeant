...
export function createPromptsInternalRouter(): Router {
  ...
  r.get("/api/internal/prompts/:namespace/:slug", async (req, res) => {
    ...
    try {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- `filePath` validated above: namespace/slug allowlist regex + normalize + PROMPTS_ROOT prefix guard.
