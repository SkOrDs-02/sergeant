...
function makeFakeDb(
  ...
  const query = (async (text: string, values: unknown[] = []) => {
    ...
    if (text.includes("FROM silpo_tx_receipt_link_rejections")) {
      // Пари, які користувач зняв руками (міграція 127) — негативний
      // фільтр matcher-а.
...
describe("pullAndSyncReceipts", () => {
  ...
  it("НЕ відновлює пару, яку користувач розлінкував руками", async () => {
    // Головний тест міграції 127. Без негативного фільтра розлінк був би
    ...
    // або фоновий poller) мовчки повернув би те, що людина щойно прибрала.
    const db = makeFakeDb({
      monoTransactions: [
        {
          id: "mono-1",
