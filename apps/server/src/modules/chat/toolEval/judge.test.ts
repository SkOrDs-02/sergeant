...
describe("розбір вердикту", () => {
  ...
  it("приймає латинське OK нарівні з кириличним", () => {
    // Дві однакові на вигляд літери з різних абеток коштували цілого кейса:
    // суддя відповів `OK: …`, а звіт показав «без вердикту».
    expect(parseVerdict("OK: розвідка перед дією").ok).toBe(true);
    expect(parseVerdict("BAD: зробив не те").ok).toBe(false);
  });
