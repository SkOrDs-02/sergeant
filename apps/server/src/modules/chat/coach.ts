 ...
 * у `apps/web/src/sw.ts`). Ріжемо на запису, а не на відправці: інакше в БД
 * лежав би текст, довший за все, що взагалі може долетіти до юзера.
 */
const NUDGE_BODY_MAX = 200;

/**
 * Кладе останній згенерований текст поради у `sergeant_nudge_cache` для
 * серверного шедулера (міграція 094).
 *
 ...
 * шедулер читає лише найсвіжіший, історія не потрібна.
 */
...
Відповідай ТІЛЬКИ текстом повідомлення, без вітань, без підписів, без лапок.`;
  ...
  // можна — але саме тут народжується єдиний текст, який шедулер потім зможе
  // переслати. Сервер не вміє згенерувати пораду сам: снапшот приходить із
  // клієнтського SQLite. Тому кладемо «консерву» — шедулер її переюзає, якщо
  // юзер завтра не зайде.
  const userId = (req as WithSessionUser).user?.id;
  if (userId && text && text.trim()) {
    void saveNudgeCache(userId, text.trim());
  }
