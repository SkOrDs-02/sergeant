 ...
 *   огорожа + парний параграф + сканер ін'єкцій. Обидва блоки одним бампом
 *   навмисно — cache-key прив'язаний побайтно, два бампи = дві інвалідації.
 *   Знахідки A2/A4 в `docs/90-work/audits/ai-abuse-2026-08-05.md`.
 * v18 (2026-08-05): `remember` став `strict: true`, `category` — обов'язковою
 ...
 *   групування секції «Пам'ять ШІ» по категоріях (з іконками в
 ...
 *   перший рядок tool_result, а той несе UUID запису пам'яті
 *   (`Запам'ятав: … (Уподобання, id:84920a0a-…)`) — id потрібен їй самій, щоб
 ...
 *   Обидві половини одним бампом навмисно — cache-key прив'язаний побайтно.
 *   Cache-prefix invalidation.
 */
 ...
 * пам'яті; id потрібен їй для `forget`, але людині це шум, який читається
 * як збій.
 *
