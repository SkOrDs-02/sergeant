...
export const QUERY_FIZRUK_TOOLS: AnthropicTool[] = [
  {
    name: "query_workouts",
    description:
      "Пошук і вибірка завершених тренувань Фізрука за період, з опційним фільтром за вправою або мʼязовою групою. Read-only. Для 'покажи мої тренування за останній тиждень'. Повертає кількість, сумарний обʼєм і список.",
    input_schema: {
      type: "object",
      properties: {
        period_days: {
        ...
        muscle: {
          type: "string",
          description: "Фільтр за мʼязовою групою (опційно)",
        },
        limit: {
          type: "number",
          description:
  ...
  },
  {
    ...
    description:
      "Динаміка прогресу в конкретній вправі (макс. вага, обʼєм, повтори) за період у Фізруці. Read-only. Для 'як змінилась моя жим лежачи за місяць?'. Повертає зміну від першої до останньої сесії та найкращі показники.",
    input_schema: {
      type: "object",
      properties: {
        exercise_name: {
  ...
  },
  {
    name: "training_stats",
    description:
      "Агрегована статистика тренувань Фізрука за період: частота на тиждень, улюблені вправи, розподіл по мʼязових групах. Read-only. Для 'які мʼязи я треную найчастіше?'.",
    input_schema: {
      type: "object",
      properties: {
        period_days: {
        ...
        top: {
          type: "number",
          description:
            "Скільки топ-вправ і топ-мʼязів повернути, 1-20 (опційно, default 8)",
        },
      },
    },
  },
