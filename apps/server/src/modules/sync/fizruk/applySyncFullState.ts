...
/**
 * Applies the append-preserving injury row contract.
 *
 * The five-column schema intentionally has no generic updated/deleted audit
 * columns. Its LWW clock is therefore the latest business transition:
 * `cleared_at` for a cleared injury, otherwise `noted_at`. Clearing is the only
 * permitted mutation; delete and reactivation preserve the injury history.
 */
export async function applyFizrukInjuries(
  ...
  const muscleGroup =
    typeof row["muscle_group"] === "string" && row["muscle_group"].length > 0
      ? row["muscle_group"]
      : null;
  if (!muscleGroup) {
    return { status: "rejected", reason: "missing_muscle_group" };
  }
  const notedAt = parseRequiredDate(row["noted_at"]);
  if (notedAt === "invalid") {
    return { status: "rejected", reason: "invalid_noted_at" };
  }
  const clearedAt = parseOptionalDate(row["cleared_at"]);
  if (clearedAt === "invalid" || (clearedAt && clearedAt < notedAt)) {
    return { status: "rejected", reason: "invalid_cleared_at" };
  }

  const existing = await queryOne<ExistingFizrukInjury>(
    client,
    `SELECT user_id, muscle_group, noted_at, cleared_at
  ...
  if (existing) {
    ...
    const existingClock = existing.cleared_at ?? existing.noted_at;
    if (existingClock.getTime() >= clientTs.getTime()) {
      return { status: "rejected", reason: "lww_conflict" };
    }
    if (
      existing.muscle_group !== muscleGroup ||
      existing.noted_at.getTime() !== notedAt.getTime()
    ) {
      return { status: "rejected", reason: "append_only_violation" };
    }
    if (existing.cleared_at !== null && clearedAt === null) {
      return { status: "rejected", reason: "tombstoned" };
    }
    if (clearedAt === null) {
      return { status: "rejected", reason: "lww_conflict" };
  ...
  await client.query(
    `INSERT INTO fizruk_injuries
     ...
     VALUES ($1, $2, $3, $4, $5)`,
    [id, userId, muscleGroup, notedAt, clearedAt],
  );
  return { status: "applied" };
}
