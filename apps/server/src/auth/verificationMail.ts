...
/**
 * Екранування для значень усередині лапок атрибута (`href="…"`). Перенесено
 * з `auth.ts` без зміни поведінки.
 */
...
/**
 * Екранування для текстового вузла (`<strong>…</strong>`). Відрізняється від
 * атрибутного тим, що додатково закриває `>`: у тілі елемента незакритий `>`
 * не створює вразливості (тег без `<` не почнеться), але лишає у листі
 * сміття виду `&lt;script>` замість читабельного тексту.
 */
...
export function verificationMail(url: string): AuthMailContent {
  ...
  return {
    ...
    html: `<p>Підтверди email:</p><p><a href="${escapeHtmlAttr(link)}">Підтвердити</a></p><p>Якщо ти не реєструвався — просто проігноруй цей лист.</p>`,
  };
}
...
export function passwordResetMail(url: string): AuthMailContent {
  return {
    ...
    html: `<p>Перейди за посиланням, щоб задати новий пароль:</p><p><a href="${escapeHtmlAttr(url)}">Скинути пароль</a></p><p>Якщо ти не запитував скидання — просто проігноруй цей лист.</p>`,
  };
}
...
export function changeEmailConfirmationMail(
  ...
  return {
    ...
    html: `<p>Ти попросив змінити адресу акаунта на <strong>${escapeHtmlText(newEmail)}</strong>.</p><p><a href="${escapeHtmlAttr(link)}">Підтвердити зміну</a></p><p>Якщо це був не ти — НЕ переходь за посиланням і зміни пароль: адресу акаунта намагаються перепривʼязати.</p>`,
  };
}
