...
function makeDb(rows: Record<string, unknown>[]) {
  ...
  const query = vi.fn(async (sql: string, params?: unknown[]) => {
    if (sql.includes("INSERT INTO sergeant_push_log")) {
...
describe("runSergeantNudgeSweep", () => {
  ...
  it("у тихі години не шле нічого і не чіпає БД", async () => {
    ...
    const send = vi.fn(async () => {});
    const summary = await runSergeantNudgeSweep({
      db,
...
describe("startSergeantNudgeWorker", () => {
  it("не стартує без REDIS_URL і не кидає", async () => {
    vi.resetModules();
    vi.doMock("./connection.js", () => ({
      BULLMQ_QUEUE_PREFIX: "sergeant",
      createBullConnection: () => null,
    }));
    const mod = await import("./sergeantNudge.js");
    expect(mod.startSergeantNudgeWorker()).toBeNull();
  });
});
