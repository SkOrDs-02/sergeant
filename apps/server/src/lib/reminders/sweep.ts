 ...
 *
 * Решта фонових задач (`ftuxDrip`, `authMail`) — BullMQ, тобто вимагають
 * `REDIS_URL`, якого у проді немає (див. `lib/jobs/connection.ts`). Черга,
 * що не стартує, тут гірша за просту петлю: канал мовчав би далі. Хвилинний
 * `setInterval` + дедуп у Postgres дає ту саму гарантію «не більше одного
 * пушу на подію» навіть при кількох репліках — виграє той процес, чий
 * `INSERT` застовпив рядок першим.
 *
 * ── Порядок claim → send ────────────────────────────────────────────────
 *
