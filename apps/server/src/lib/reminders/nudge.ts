 ...
 * пропонував повторити патерн `ftuxDrip`. Він же чесно зазначав: «без
 * `REDIS_URL` шедулер вимикається цілком». У проді `REDIS_URL` не заданий,
 * тобто той код задеплоївся б і мовчав — канал лишився б зламаним рівно так
 * само, як був. Тут та сама форма роботи, що й у нагадувань: добовий прохід
 * із дедупом у Postgres. Черга не додає гарантії, яку вже дає `INSERT ...
 * ON CONFLICT`, тож вона тут зайвий рухомий елемент.
 *
