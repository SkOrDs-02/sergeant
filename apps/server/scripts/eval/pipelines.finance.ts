...
const classifyPipeline: Pipeline = {
  ...
  cases: [
    ...
    classifyCase(
      ...
      "НЕПРАВИЛЬНО: впевнено приписати ветклініку до health (це категорія здоров'я ЛЮДИНИ) чи до groceries. Прийнятно: other, або health із невисокою впевненістю.",
...
const coachPipeline: Pipeline = {
  ...
  cases: [
    ...
    coachTurn(
      ...
      "НЕПРАВИЛЬНО: вигадати числа, яких немає («ти витратив 4200 грн»). Пам'яті немає, знімка немає — єдина чесна відповідь не містить конкретних сум.",
      {
        dateContext: {
          todayKey: "2026-07-23",
          weekDayUk: "четвер",
    ...
    coachTurn(
      "регрес проти пам'яті",
      "НЕПРАВИЛЬНО: похвалити прогрес. Пам'ять каже 4 тренування й 86% звичок минулого тижня, зараз 1 і 29% — це падіння, і коуч має його назвати, а не привітати.",
      {
        dateContext: {
          todayKey: "2026-07-26",
          weekDayUk: "неділя",
...
const chatPipeline: Pipeline = {
  ...
  cases: [
    ...
    },
    {
      ...
      trap: "НЕПРАВИЛЬНО: переказати всі п'ять чисел підряд. Відповідь має назвати, куди пішла основна частина.",
      user: synthesisTurn(
    ...
    },
    {
      ...
      judge: mentions(/трену|достав|їж/i, "зв'язок між модулями"),
    },
  ],
  candidates: [
    {
...
const analysisPipeline: Pipeline = {
  ...
  cases: [
    ...
    },
    {
      ...
      // назване поєднання, лише без кореня «зв'яз». GLM-5.2 двічі отримав ❌
      // за відповідь, яка описувала звʼязок краще за ту, що пройшла.
      judge: mentions(
        /звʼяз|зв'яз|пов'яз|повʼяз|разом|одноча|коли|саме ті|ті самі|збіга|патерн|кореляц|компенс|тягне за собою|через це|призвод/i,
