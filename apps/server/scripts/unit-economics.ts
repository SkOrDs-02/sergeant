...
**1. Під шлюзом у контекст ПЕРШОГО туру досі їде весь реєстр.** Вимір цього
...
(${FACTS.gatewayPrefix.tokens.toLocaleString("uk-UA")} токенів). Різниця **${prefixRatio.toFixed(1)}×**.

Тур СИНТЕЗУ це вже не платить: [#669](https://github.com/SkOrDs-02/sergeant/pull/669)
звузив його до визначень, згаданих у реплеї — на типове повідомлення це
≈${FACTS.gatewaySynthesisPrefix.tokens.toLocaleString("uk-UA")} замість ${FACTS.gatewayPrefix.tokens.toLocaleString("uk-UA")} токенів. Безпечно, бо \`tool_use\` із того туру
однаково відкидається обома транспортами. **Перший тур лишається
невиправленим** — і саме він тепер найбільша окрема стаття.
...
**4. Структурну інверсію знято.** Free та anon ішли на **premium**-модель
синтезу без деградації, тоді як Pro після 20 викликів доби падає на standard —
неплатник мав кращу модель за платника на його 21-му повідомленні.
[#668](https://github.com/SkOrDs-02/sergeant/pull/668) перевів неоплачений чат
на standard: повідомлення premium — ${money(msgPremium, 4)}, standard —
${money(msgStandard, 4)}, тобто **${(msgPremium / msgStandard).toFixed(1)}×**.

Виняток, зроблений [#671](https://github.com/SkOrDs-02/sergeant/pull/671):
**коуч лишився на premium.** Там деградація це \`gpt-5.1\` → \`gemini-2.5-flash-lite\`,
тобто найбільший розрив у якості за найменшу економію (≈$0.05 на Free-юзера в
місяць), а сам ендпоінт не має plan-gate — Free бачить його на дашборді щодня.
Це і є причина, чому Free нижче коштує більше за самий лише чат.
...
2. **Ціна \`glm-5.2\` залежить від провайдера.** Модель хостять кілька, і
   OpenRouter маршрутизує між ними: каталог на 2026-08-06 дає від
   $0.5446/$1.712 у знижених до $0.90/$3.15 при листовій $1.40/$4.40. Узято
   $0.70/$2.20 — типовий рядок без знижки; під Nitro/Exacto маршрут піде
   дорожчим. \`deepseek-v4-flash\` однозначний: $0.09/$0.18.

   > Попередня редакція виводила обидві ціни назад із вартості прогону стенду,
   > бо прайс-листа не було. Для deepseek вхід збігся ($0.08 проти $0.09), а
   > вихід був завищений удвічі; \`glm-5.2\` було занижено ВТРИЧІ, і саме він
   > стоїть на premium-синтезі. Числа нижче — після виправлення.
...
1. **Розвантажити ПЕРШИЙ тур.** Найбільше, що лишилось: сам префікс — **${prefixShare.toFixed(0)} %**
   вартості першого туру, тобто більшість грошей витрачається ще до того, як
   модель прочитала питання. Прийом #669 тут не працює: на першому турі модель
   ще НЕ знає, який інструмент їй потрібен, тож підмножину не побудувати.
   Варіанти — додати моделі шлюзу в allowlist tool search (якщо він їх узагалі
   підтримує) або модульний підбір за контекстом запиту.
