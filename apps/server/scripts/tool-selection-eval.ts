 ...
 * «запам'ятай». Підсумок друкується рядком `implicit remember: N/M` — решта
...
/**
 * Неявні факти — сказані мимохідь, без прямого «запам'ятай».
 *
 * AI-CONTEXT: у `CASES` факт про себе представлений однією реплікою, яка
 * починається дослівно з «Запам'ятай:». Це найлегший різновид: намір
 ...
 * `preference`, `allergy`), бо саме по них групується секція «Пам'ять ШІ».
...
const IMPLICIT_FACT_CASES: ToolCase[] = [
  ...
  },
  {
    ...
    user: "Хочу скинути 5 кг до вересня. Скільки калорій я вже з'їв сьогодні?",
    accept: ["remember"],
  },
  {
  ...
  },
  {
    ...
    user: "Мене висипає від морепродуктів, тому суші пропускаю. Що з'їсти після тренування?",
    accept: ["remember"],
  },
  {
...
async function main(): Promise<void> {
  ...
  console.log(
    "\nНеявні факти (сказані мимохідь, без «запам'ятай») — чи викликано `remember`:",
  );
  for (const model of models) {
