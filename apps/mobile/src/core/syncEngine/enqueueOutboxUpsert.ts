 ...
 * sync_op_outbox (mobile).
 *
 * Mirrors `apps/web/src/core/syncEngine/enqueueOutboxUpsert.ts` — same
 * interface, same semantics, same idempotency contract. Lives here
 * (not in a shared package) so mobile dualWrite adapters can import it
 * without a circular-dependency issue.
 *
 * Error policy: this helper propagates SQL errors to the caller. The
 * caller (dualWrite adapter) wraps every op in try/catch and swallows
 * errors so a sync-enqueue failure never breaks the local write.
 */
