...
const mockGetTtv = jest.fn();
const mockGetModule = jest.fn();

jest.mock("@sergeant/shared", () => ({
  ...jest.requireActual("@sergeant/shared"),
  getTimeToValueMs: (...args: unknown[]) => mockGetTtv(...args),
  getFirstRealEntryModule: (...args: unknown[]) => mockGetModule(...args),
}));

import { useFirstEntryCelebration } from "./useFirstEntryCelebration";

describe("useFirstEntryCelebration (mobile)", () => {
  beforeEach(() => {
    mockGetTtv.mockReset().mockReturnValue(12345);
    mockGetModule.mockReset().mockReturnValue("routine");
  });
  ...
  it("does not open when the user already had real data on mount", () => {
    ...
    expect(mockGetTtv).not.toHaveBeenCalled();
  });
  ...
  it("opens once when the flag flips from false to true", () => {
    const { result, rerender } = renderHook(
      ({ has }: { has: boolean }) => useFirstEntryCelebration(has),
