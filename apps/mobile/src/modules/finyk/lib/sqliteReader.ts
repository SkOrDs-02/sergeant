...
export interface SqliteFinykCache {
  ...
  /**
   * Mono transaction ids excluded from statistics (parsed from
   * `excluded_stat_tx_ids_json`). Cross-device prefs slice —
   * Stage 13 / PR #075.
   */
  excludedStatTxIds: string[] | null;
  /**
   * Recurring-banner ids the user dismissed (parsed from
   * `dismissed_recurring_json`). Cross-device prefs slice —
   * Stage 13 / PR #075.
   */
