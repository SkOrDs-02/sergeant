...
/**
 * Async SQLite-side adapter for the mobile Finyk dual-write layer.
 *
 * Stage 4 PR #036 of `docs/planning/storage-roadmap.md`. Migrated onto
 * `@sergeant/dualwrite-core` in ADR-0073 крок 6 (mobile mirror of web крок 5):
 * the op-loop is now `createApplyOps` (best-effort) and every table's SQL is
 * emitted by the shared `buildLwwUpsert` / `buildDelete` builders via the
 * TableSpecs in `specs.ts` (split out to stay under Hard Rule #18). Behaviour
 * and emitted SQL are byte-identical to the previous hand-written adapter —
 * pinned by `adapter.snapshot.test.ts` (the snapshot must NOT be updated).
 *
 * Both web and mobile copies use the same `SqliteMigrationClient`
 * (`{exec, run, all}`) shape, so a single SQL surface serves both web
 * (sqlite-wasm) and mobile (expo-sqlite), and unit-tests run unchanged on
 * `better-sqlite3`.
 *
 * Design notes:
 *
