...
describe("manualTaxonomy — цілісність таблиці", () => {
  ...
  // Регресія з browser-QA 2026-09-02. Палітру цей файл уже стеріг (тест
  // вище), а другий споживач канонічного id — ні: пікер бюджетного ліміту й
  // розбивка витрат по категоріях будуються з `MCC_CATEGORIES` через
  // `mergeExpenseCategoryDefinitions`. `utilities` і `other` існували лише в
  // ручній таксономії, тож витрату завести можна було, а ліміт на неї — ні, і
  // в аналітиці ці гроші не з'являлись зовсім.
  //
  // Перевіряємо КЛАС, а не дві категорії: будь-яка нова ручна категорія
  // витрат, чий канонічний id не має відповідника в базовому списку, впаде
  // тут — саме цього гейта бракувало, коли 2026-08-25 лагодили `cafe` й
  // `tech`, а `utilities` лишили.
  it("канонічний id кожної ручної категорії ВИТРАТ існує в MCC_CATEGORIES", () => {
    const baseIds = new Set(MCC_CATEGORIES.map((c) => c.id));
    for (const d of MANUAL_EXPENSE_TAXONOMY) {
      expect(
        baseIds.has(d.canonicalId),
        `канонічної категорії "${d.canonicalId}" (з "${d.id}") немає в MCC_CATEGORIES — ` +
          `ліміт на неї виставити неможливо, а її витрати не потраплять у категорійну розбивку`,
      ).toBe(true);
    }
  });
