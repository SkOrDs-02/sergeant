 ...
 *    (канон `food`), `health`, `shopping`, `smoking`, `alcohol`.
 *    Цигарки й алкоголь дістали власні кошики 2026-08-25 (рішення
 *    founder-а: «цигарки та алкоголь різні категорії») — до того обидва
 *    падали в `groceries`, і чек із цигарками чесно повідомляв, що
 *    спліту немає, бо «все їжа»;
...
const SLUG_PREFIX_MAP: ReadonlyArray<readonly [RegExp, string]> = [
  [/^(tobacco|cigar|smok|tyutyun)/i, "smoking"],
  [/^(alco|alko|wine|beer|spirit|vino|pyvo)/i, "alcohol"],
...
const NAME_KEYWORD_MAP: ReadonlyArray<readonly [RegExp, string]> = [
  [
    // Без голого «стік»: у чеку founder-а він трапився в «Ковбаски …
    // Салямі Стік яловичі» і робив із ковбаси цигарки. Нагрівані стіки
    // однаково ловляться за брендом (heets / terea) або за «тютюнов».
    /(цигарк|сигарет|тютюн|цигар|iqos|heets?|terea|(?<!\p{L})(glo|veev|lyft)(?!\p{L})|снюс)/iu,
    "smoking",
  ],
  [
    // Межа слова тут — НЕ `\b`: під прапорцем `u` кирилиця не входить у
    // `\w`, тож `\bвино\b` не збігся б ніколи. Лукараунди на `\p{L}`
    // тримають «Вино» і відкидають «ромашк», «пивон», «джинс».
    //
    // `(?<!без)` перед «алкогол» — інакше «Напій безалкогольний» і «Пиво
    // безалкогольне» лягли б саме в ту категорію, яку їхня назва явно
    // заперечує.
    /((?<!без)алкогол|(?<!\p{L})(вино|вина|пиво|пива|джин|ром|сидр|бренді|вермут)(?!\p{L})|ігрист|шампанськ|віскі|коньяк|горілк|лікер|текіл|наливк|настоянк)/iu,
    "alcohol",
  ],
  [
  ...
  [
    /(порош(ок|ку)\s*праль|праль|засіб\s*для\s*(миття|прання|чищення)|мийн|відбілювач|кондиціонер\s*для\s*білизн|туалетн(ий|ого)\s*папір|папір\s*туалетн|рушник\s*паперов|фольг|пергамент|^\s*пакет|пакет[иі]?\s*(для\s*сміття|фасув|майк)|сміттєв|губк[аи]|ганчірк|батарейк|лампочк|свічк|корм\s*для|наповнювач\s*для)/iu,
    "shopping",
  ],
];
