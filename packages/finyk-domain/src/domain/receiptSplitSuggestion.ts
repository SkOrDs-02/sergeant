...
const NAME_KEYWORD_MAP: ReadonlyArray<readonly [RegExp, string]> = [
  [
    /(зубн|шампун|гел[ьл]\s*для\s*душ|дезодорант|прокладк|тампон|підгузк|памперс|бритв|лез[ао]|вітамін|парацетамол|ібупрофен|пластир|бинт|антисепт|серветк.*вологі|крем\s*для)/iu,
    "health",
  ],
  [
