 ...
 *
 * ## Що показав перший вимір (2026-08-25)
 *
 * 83 унікальні назви зі 105 позицій, `category_slug` порожній У ВСІХ —
 * тобто `SLUG_PREFIX_MAP` на живих даних не спрацьовує жодного разу, і
 * мапер тримається виключно на назвах. Точність — 100%: 71 `groceries`,
 * 8 `shopping` (пакети, батарейка, туалетний папір), 4 `smoking`.
 * Алкоголю в цій вибірці НЕМАЄ ЖОДНОГО, тож alcohol-правила живими
 * даними поки не перевірені — це і є наступна дірка в покритті.
 *
 * ## Як наповнити
 *
 ...
 * реальним покращенням мапера.
 *
 * `1` — бо перший прогін на живих даних (83 назви з чеків founder-а,
 * 2026-08-25) дав рівно 100% ПІСЛЯ двох виправлень, які ці ж дані й
 * виявили: голе «стік» робило цигарки з «Ковбаски … Салямі Стік
 * яловичі», а «алкогол» без `(?<!без)` зловило б «безалкогольне».
 * Опускати поріг заради зручності не можна — тоді фікстура перестає
 * бути гейтом і стає декорацією.
 */
const MIN_ACCURACY: number | null = 1;
