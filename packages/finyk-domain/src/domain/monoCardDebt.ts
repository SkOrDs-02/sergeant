...
/**
 * Чому привʼязана операція рахується (або не рахується) погашенням.
 *
 * AI-CONTEXT: привʼязати можна будь-яку операцію, але в суму погашеного
 * потрапляють не всі. Без цього класифікатора UI ставив зелену галочку
 * навіть там, де внесок нульовий — покупка по самій картці борг збільшує,
 * а надходження на інший рахунок картки взагалі не торкалось. Галочка
 * обіцяла «зараховано», а насправді був мовчазний no-op.
 */
export type MonoCardLinkKind =
  /** Гасить борг — потрапляє в суму погашеного. */
  | "repayment"
  /** Покупка по самій картці — борг збільшує, у погашене не йде. */
  | "card-purchase"
  /** Рух на іншому рахунку, який картки не торкався. */
  | "other-income";

export function classifyMonoCardLink(
  tx: MonoCardTx,
  cardAccountId: string,
): MonoCardLinkKind {
  if (isMonoCardRepayment(tx, cardAccountId)) return "repayment";
  return tx?._accountId === cardAccountId ? "card-purchase" : "other-income";
}
