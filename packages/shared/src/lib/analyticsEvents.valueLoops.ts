...
export const VALUE_LOOP_ANALYTICS_EVENTS = Object.freeze({
  ...
  //
  // `helpful` / `not_helpful` додані 2026-09-01 (батч «телеметрія дії»).
  // Це ЄДИНА подія в реєстрі, яка несе оцінку якості поради, і потрібна
  // вона рівно для того, чого не вміє жодна інша: відрізнити корисний
  // інсайт від правдоподібного шуму. Без неї «AI щось написав, юзер щось
  // натиснув» — уся наша картина, і kill-критерій AI-шару
  // (`product-overview.md` §10) лишається нефальсифікованим.
  //
  // Це РЕАКЦІЯ, а не окрема подія: та сама вісь, що `ask_ai` і `refresh`
  // (що людина зробила з побаченою порадою), той самий знаменник
  // `ai_advice_shown`, той самий `seconds_since_shown`. Окрема подія
  // розірвала б воронку надвоє на рівному місці.
  //
  // Текст поради сюди НЕ їде — так само як у `ai_advice_shown`. Оцінка
  // без тексту не пояснює ЧОМУ порада погана; це свідома ціна, бо тіло
  // поради — вільний український текст про фінанси й тіло конкретної
  // людини (Hard Rule #21). Питання «чому» ставиться в UI, а не в
  // payload — і зараз воно не ставиться взагалі.
  //
  ...
  // ── 6) Тертя запису: «від кнопки до збереженого» ────────────────────
  //
  //   ENTRY_COMPOSE_FINISHED { module: "nutrition" | "fizruk",
  //                            entry_kind: string,
  //                            surface: string,
  ...
  //                            instrumentation_version: number }
  //
  // Навіщо окрема подія, коли `nutrition_meal_logged` уже є. Та подія
  // відповідає «скільки записів» — цей знаменник у нас є. Вона НЕ
  // відповідає «скільки це коштувало людині», а саме цим міряють себе
  // конкуренти: Dr. Muscle публічно оптимізує time-to-first-set
  // (10.2 с → 4.1 с після rebuild), MacroFactor продає «на 50 % менше
  // тапів». У нас це не міряється ніяк, тож будь-яке спрощення форми
  // сьогодні неможливо ні довести, ні спростувати.
  //
  // ОДНА подія з дискримінатором `module`, а не пара іменованих (на
  // відміну від §2, де кожна подія несе власні доменні поля). Причина:
  // поля тут ідентичні для всіх поверхонь — це вимір тертя інтерфейсу,
  // не доменний факт. Той самий прецедент, що `value_signal_*` §1.
  //
  // `outcome: "abandoned"` обовʼязковий, інакше метрика бреше в
  // найгірший бік: форма, яку кинули на півдорозі, виглядала б просто
  // як відсутність запису, і «швидко» не відрізнялось би від «здався».
  // Кинуті композиції — знаменник, без якого медіана саму себе хвалить.
  //
  // `ms` — СИРИЙ інтервал, без порогів і без обчислених булеанів (те
  // саме правило, що `ms_since_signal` §2): вікно «швидко» обирається у
  // PostHog-запиті й переглядається заднім числом.
  //
  // `backgrounded` — чи ховалася вкладка між відкриттям і результатом.
  // Без цього прапорця розподіл часу запису визначають не форми, а люди,
  // які пішли по каву з відкритим шитом, і медіана стає шумом. Це
  // дискримінатор для дашборда, а НЕ поріг у коді: подія летить у будь-
  // якому разі, включно з інтервалом у півгодини.
  //
  // Вмісту запису тут немає: ні назви страви, ні ваги, ні вправ —
  // лише enum-и, прапорці й тривалість (Hard Rule #21).
