...
export interface PushPayload {
  /** Заголовок нотифікації. Обовʼязковий — всі три канали його вимагають. */
  title: string;
   ...
   * шторку другим рядком. Канали, що шлють одну й ту саму подію більш ніж
   * одним шляхом, зобовʼязані задавати стабільний tag.
   *
   * Найважливіший випадок — нагадування. Серверний sweep
   * (`apps/server/src/lib/reminders/`) і локальний таймер відкритої вкладки
   * (`useModuleReminder`) можуть спрацювати на одну хвилину обидва. Вони
   * використовують ОДИН І ТОЙ САМИЙ рядок tag-у (`routine_notify_<habitId>_
   * <HH:MM>_<dayKey>`), тож другий банер заміщує перший замість лягти під
   * ним. Це і є механізм, який робить два незалежні шляхи доставки
   * безпечними.
   *
