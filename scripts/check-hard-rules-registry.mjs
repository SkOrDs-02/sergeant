...
//
// Validates the Hard Rules registry at `docs/governance/governance/hard-rules.json`:
//
//   1. Conforms to the JSON Schema at `docs/governance/governance/hard-rules.schema.json`
...
//      `docs/governance/governance/rules/NN-<slug>.md` whose H1 reads
...
const SCHEMA_PATH = resolve(
  ROOT,
  "docs/governance/governance/hard-rules.schema.json",
);
const AGENTS_PATH = resolve(ROOT, "AGENTS.md");
const CONTRIB_PATH = resolve(ROOT, "CONTRIBUTING.md");
const RULES_DIR = resolve(ROOT, "docs/governance/governance/rules");
...
function main() {
  ...
  // `docs/governance/governance/rules/NN-<slug>.md` whose H1 is `# Rule <id> — <title>`.
  ...
  if (existsSync(RULES_DIR) && Array.isArray(registry.rules)) {
    ...
    for (const r of registry.rules) {
      ...
      if (matches.length === 0) {
        errors.push(
          `rule-file-sync: rule #${r.id} ('${r.title}') has no matching file in docs/governance/governance/rules/ (expected NN-<slug>.md)`,
        );
        continue;
      }
      if (matches.length > 1) {
        errors.push(
          `rule-file-sync: rule #${r.id} has multiple files with the same NN prefix in docs/governance/governance/rules/: ${matches.join(", ")}`,
        );
        continue;
      }
      ...
      try {
        ...
        errors.push(
          `rule-file-sync: cannot read docs/governance/governance/rules/${matches[0]} — ${err.message}`,
        );
        continue;
      }
      ...
      if (!h1) {
        errors.push(
          `rule-file-sync: docs/governance/governance/rules/${matches[0]} has no '# Rule N — <title>' H1`,
        );
        continue;
      }
      ...
      if (fileId !== r.id) {
        errors.push(
          `rule-file-sync: docs/governance/governance/rules/${matches[0]} H1 says rule #${fileId} but file is named ${matches[0]} (expected #${r.id})`,
        );
      }
      if (fileTitle !== r.title) {
        errors.push(
          `rule-file-sync: rule #${r.id} title drift — registry='${r.title}' vs docs/governance/governance/rules/${matches[0]} H1='${fileTitle}'`,
        );
      }
    }
    ...
    for (const [id, names] of filesByPrefix) {
      if (!knownIds.has(id)) {
        errors.push(
          `rule-file-sync: docs/governance/governance/rules/${names[0]} numbered #${id} but registry has no such rule`,
        );
      }
    }
  }
...
function report(errors, jsonMode, registry) {
  ...
  console.error(
    "\nFix: update docs/governance/governance/hard-rules.json so it agrees with AGENTS.md " +
