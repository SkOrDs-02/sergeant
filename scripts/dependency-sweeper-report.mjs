...
const rows = Object.entries(outdatedMap).map(([name, info]) => {
  ...
  const kind =
    wanted && latest && wanted === latest
      ? "in-range"
      : classifyBump(wanted, latest);
...
advisories.sort(
  (x, y) => (sevRank[x.severity] ?? 9) - (sevRank[y.severity] ?? 9),
);
const actionableAdvisories = advisories.filter((a) => !a.waived);
const highSev = actionableAdvisories.filter(
...
out.push("");
out.push(
  "> Режим **L1: report-only**. Цей звіт нічого не комітить і не бампить — лише читає дерево залежностей. Рішення про апдейт ухвалює власник.",
);
out.push("");
out.push("## TL;DR");
out.push("");
out.push(
  `- 📦 Застарілих пакетів: **${rows.length}** (safe: ${safeRows.length}, risky/major: ${riskyRows.length})`,
);
out.push(
...
if (highSev.length)
  gates.push(
    `**${highSev.length} high/critical CVE** — ескалювати чіпом, НЕ автофіксити`,
  );
if (riskyRows.length)
...
out.push(
  gates.length
    ? gates.map((g) => `- ${g}`).join(nl)
    : "- (немає) — цього циклу все в межах safe-зони",
);
out.push("");
...
if (safeRows.length) {
  ...
  for (const r of safeRows.slice(0, 60)) {
    out.push(
      `| \`${r.name}\` | ${r.latest} | ${r.kind} | ${r.isDev ? "dev" : "prod"}${r.isTypes ? " · @types" : ""} |`,
    );
  }
...
if (riskyRows.length) {
  ...
  for (const r of riskyRows.slice(0, 60)) {
    out.push(
      `| \`${r.name}\` | ${r.wanted} | ${r.latest} | ${r.kind} | ${r.isDev ? "dev" : "prod"} |`,
    );
  }
  if (riskyRows.length > 60)
    out.push(`| … | | | | +${riskyRows.length - 60} ще |`);
} else {
...
if (actionableAdvisories.length) {
  ...
  for (const a of actionableAdvisories.slice(0, 40)) {
    out.push(
      `| ${a.severity} | \`${a.id}\` | \`${a.module}\` | ${a.title.slice(0, 80)} |`,
    );
  }
  out.push("");
  out.push(
