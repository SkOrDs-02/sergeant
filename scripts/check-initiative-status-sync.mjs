...
//
// CI guard for `docs/work/specs/initiatives/README.md` ↔ `docs/work/specs/initiatives/[_]?[0-9]{4}-*.md`
// status synchronization.
//
// Validates three invariants:
//   1. Every `NNNN-*.md` (or `_NNNN-*.md` for completed-prefix files) in
//      `docs/work/specs/initiatives/` is referenced by a row in README.md (active table
...
const INITIATIVES_DIR = resolve(REPO_ROOT, "docs/work/specs/initiatives");
...
/** List initiative files (NNNN-*.md) in `docs/work/specs/initiatives/`. */
...
export function run({
  ...
  for (const [id, info] of fileStatuses.entries()) {
    if (!readmeStatuses.has(id)) {
      errors.push(
        `${info.file}: no row in docs/work/specs/initiatives/README.md for #${id}. ` +
  ...
  for (const id of readmeStatuses.keys()) {
    ...
    if (!fileStatuses.has(id) && stat !== "Archived") {
      errors.push(
        `docs/work/specs/initiatives/README.md: row for #${id} but no matching ${id}-*.md file in docs/work/specs/initiatives/.`,
      );
    }
  }
  ...
  for (const [id, info] of fileStatuses.entries()) {
    ...
    if (readmeStatus === null) {
      errors.push(
        `docs/work/specs/initiatives/README.md: row for #${id} (${info.file}) — no recognised status keyword in cell.`,
      );
      continue;
    }
    if (readmeStatus !== info.status) {
      errors.push(
        `docs/work/specs/initiatives/README.md: status drift for #${id} — README says "${readmeStatus}", ${info.file} header says "${info.status}".`,
      );
    }
  }
...
function formatReport(result) {
  ...
  lines.push(
    "Fix by either:",
    "  • Updating the offending row in docs/work/specs/initiatives/README.md to match the file header, or",
