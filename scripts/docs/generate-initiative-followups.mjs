...
//
// Scan `docs/work/specs/initiatives/[0-9]*.md`, extract each initiative's
// `### Carry-over → successor` block, and generate
// `docs/work/specs/initiatives/follow-ups.md` — a consolidated index of all open
...
// `Done` / `Closed` so `ls docs/work/specs/initiatives/` separates active from done
// — see `docs/work/specs/initiatives/README.md` § Гайдлайн → completed-prefix.
...
/**
 * Read every `*-*.md` file in `dir` (defaults to docs/work/specs/initiatives/), parse
...
export function renderFollowUps(items, { today = todayISO() } = {}) {
  ...
  lines.push("");
  lines.push(
    "Зведений календар відкритих follow-up-ів з усіх ініціатив у [`docs/work/specs/initiatives/`](./README.md). Source = `### Carry-over → successor` блок у кожному файлі (тільки `- [ ]`-пункти; checked-off — історія, в індекс не йдуть).",
  );
  lines.push("");
  lines.push(
