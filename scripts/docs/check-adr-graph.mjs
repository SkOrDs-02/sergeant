...
//
// Validate the integrity of the ADR collection in `docs/governance/adr/`:
//
...
//      ADR replaced them.
//   4. Every ADR file is listed in `docs/governance/adr/README.md` (the «Поточні
...
//      `docs/governance/adr/README.md`'s «Note on missing NNNN» blockquotes).
...
const ADR_DIR = resolve(REPO_ROOT, "docs/governance/adr");
 ...
 * in `docs/governance/adr/README.md` under a «Note on missing NNNN» blockquote so
...
export const KNOWN_NUMBERING_GAPS = new Set([
  ...
  // See `docs/governance/adr/README.md` § «Note on missing 0029».
  "0029",
  ...
  // standalone ADR was never written. See `docs/governance/adr/README.md`
  ...
  // retroactively, so `0056` stays a known gap. See `docs/governance/adr/README.md`
...
export function validateGraph(adrs, readmeIndexed) {
  ...
  for (const missing of findNumberingGaps(adrs)) {
    errors.push(
      `numbering gap: ADR-${missing} is missing and not whitelisted in KNOWN_NUMBERING_GAPS (docs/governance/adr/README.md must also document it under «Note on missing ${missing}»)`,
    );
  }
  ...
  for (const a of adrs) {
    ...
    if (!readmeIndexed.has(a.number)) {
      errors.push(
        `${basename(a.file)}: ADR-${a.number} is not listed in docs/governance/adr/README.md`,
      );
    }
...
if (isMain) {
  ...
  if (errors.length > 0) {
    console.error(
      `[check-adr-graph] ${errors.length} problem(s) found in docs/governance/adr/:`,
    );
    for (const e of errors) console.error(`  ✘ ${e}`);
    console.error(
      `\nFix the metadata in the offending ADR files (Status, Supersedes) or update docs/governance/adr/README.md to list missing entries.`,
    );
    process.exit(1);
  }
