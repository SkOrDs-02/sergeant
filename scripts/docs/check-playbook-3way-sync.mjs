...
//
// Ensures every concrete docs/start/instructions/*.md playbook is visible in both:
...
const DEFAULT_PLAYBOOKS_DIR = resolve(DEFAULT_ROOT, "docs/start/instructions");
...
export function checkPlaybook3WaySync({
  ...
  if (!existsSync(indexPath)) {
    failures.push(`docs/start/instructions/INDEX.md is missing.`);
  }
  if (!existsSync(catalogPath)) {
    failures.push(`docs/start/instructions/playbook-catalog.md is missing.`);
  }
  ...
  for (const file of files) {
    if (!indexLinks.has(file)) {
      failures.push(`${file}: missing from docs/start/instructions/INDEX.md.`);
    }
    if (!catalogLinks.has(file)) {
      failures.push(
        `${file}: missing from docs/start/instructions/playbook-catalog.md.`,
      );
    }
  }
