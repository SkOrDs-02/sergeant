 ...
 *
 *
 * **Ратчет 2026-09-12 (третій униз): 280 → 268 kB.** UA-каталог інтерфейсу
 * ПІШОВ з критичного шляху: 286.7 → **260.8 kB**. Гейт до цього був
 * червоний на `main` (перевищення 6.7 kB), і спокуса була підняти стелю —
 * замір показав, що підіймати нічого не треба.
 *
 * Розбір сорсмапи чанку `cn` (25.0 kB brotli): 15 модулів, 147.1 kB сирих
 * джерел, і **128.4 kB з них — UA-каталог**. Сам хелпер `cn.ts`, за яким
 * названо чанк, важить 1.3 kB. Причина не в тому, що каталог великий, а в
 * тому, що він ОДИН об'єкт: `uk.ts` статично зшиває десять модульних
 * файлів (`uk.fizruk` 29.1 kB, `uk.finyk` 14.8, `uk.nutrition` 12.6, …),
 * тож будь-який eager-споживач `messages` тягнув усі.
 *
 * Таких споживачів було дев'ять, і кожному потрібно від однієї до двох
 * груп: `settingsSectionsCatalog.ts` брав РІВНО один рядок. Лікування —
 * вузький `uk.core.ts` (вісім груп + `auth`), у який ті дев'ять і
 * переведені; `messages` розкладає ядро назад, тож решта застосунку не
 * змінилась.
 *
 * **Той самий урок про ОСТАННЄ ребро, тільки дорожчий на один прогін.**
 * Після переводу восьми поверхонь замір дав 280.6 kB — тобто гейт лишався
 * червоним, а чанк `uk` (21.6 kB) лишався в preload. Дев'ятим виявився
 * `AuthContext.tsx`, і знайшовся він не відразу: шлях там ВІДНОСНИЙ
 * (`../../shared/i18n/uk`), тож перший скан по `@shared/i18n` його не
 * побачив. Знявши його, число впало 280.6 → 260.8, а `uk.ts`, `en.ts` і
 * `index.ts` пішли з preload-графа зовсім — en-копія (20.3 kB) доти їхала
 * до першого екрана українським користувачам, які мову не перемикають.
 *
 * Нове число має ~2.7% запасу над фактом, тобто 7.2 kB. Тісно навмисно:
 * лишити стелю на 280 означало б 19.2 kB слаку — запрошення до
 * «комфортної зони», проти якої стоїть увесь цей файл.
 * Запас саме 2.7%, бо різниця CI↔локально для ЦЬОГО заміру мала: на `main`
 * CI дав 286.9, локально 286.7 (для `size-limit` розбіжність інша — див.
 * `AGENTS.md § Performance budgets`).
 *
 * Актуальний розбір і напрямки — `docs/work/specs/tech-debt/frontend.md`.
 */
const DEFAULT_LIMIT_BYTES = 268_000;
