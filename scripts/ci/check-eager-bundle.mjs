 ...
 *
 * **Це не списання боргу, а відновлення сигналу.** Ратчет назад до ≤450 kB —
 * окрема задача в `docs/90-work/tech-debt/frontend.md`: винести `vendor-sqlite`
 * (~69 kB) з критичного шляху. Він там через статичний
 * `@sergeant/db-schema/sqlite` у `apps/web/src/core/db/kvStoreBoot.ts` —
 * KV-store піднімається на буті, тож винесення потребує async-boot гейта, а не
 * простої заміни імпорту на `import()`.
 */
const DEFAULT_LIMIT_BYTES = 470_000;
