 ...
 * не перевірили решту.
 *
 * **Борг лишається.** `vendor-sqlite` (~69 kB) досі найбільший eager-чанк.
 * Замір сорсмапи (2026-08-07) спростував ОБИДВА попередні діагнози. У чанку
 ...
 *
 * Тобто вагу дає РАНТАЙМ drizzle, а табличні визначення — лише міст: вони
 * імпортують `drizzle-orm/sqlite-core`, а `manualChunks` склеює весь
 * `drizzle-orm` в один чанк. Через це ОДИН eager-імпорт будь-якого символу з
 * барелю робить eager усі 69 kB — і саме тому спроби «зрізати частину мостів»
 * давали мінус: платили зайвими чанками, не прибравши жодного кілобайта.
 * Ділити `db-schema` заради розміру defs сенсу не має.
 *
 * Актуальний розбір і напрямки — `docs/90-work/tech-debt/frontend.md`.
 */
const DEFAULT_LIMIT_BYTES = 430_000;
