...
| `pnpm docs:check-links`           | Сканує всі `*.md` на биті `[text](target)` лінки. CI запускає з `--strict-external` + `docs/external-link-allowlist.json` |
...
15. **Читай governance перед написанням коду; оновлюй документацію разом із кодом; внутрішні docs — українською** — читай `AGENTS.md`, `CONTRIBUTING.md`, `CLAUDE.md` й відповідний playbook перед написанням коду. Документація — частина change-set: коли код / контракти рухаються, оновлюйте відповідні документи в тому ж PR (api-client types, design-system, playbooks, freshness headers). `pnpm lint:governance-sync` тепер фейлить CI на dangling-refs у non-aspirational docs (ADR/governance/playbooks); placeholder/glob-патерни (`*`, `<>`, `[]`, `{}`) скіпаються. Внутрішня прозова документація (ADR-и, playbooks, audits, runbooks, governance) — **українською** (з 2026-04-30 promoted з soft → hard); англійська — лише для README, ADR titles/Status, OpenAPI, commit/PR descriptions, code identifiers. Повна must-update таблиця та language-exemption список — `AGENTS.md` § Hard Rule #15.
16. **Типографічна шкала — семантичні стилі + 12 px floor** — для будь-якого слота з документованою роллю використовуй одну з `.text-style-*` утиліт (`hero` 26 / `title` 20 / `body` 16 / `label` 14 / `caption` 12 / `overline` 12 UPPER), визначених у `packages/design-tokens/tailwind-preset.js → plugins.semanticTypography` — вони фіксують font-size / line-height / weight / letter-spacing разом, щоб layout не дрейфував по одній з осей. Floor — **12 px** (`text-style-caption` / `text-xs`). `text-3xs` (9 px) видалено зі шкали. `text-2xs` (10 px) — лише для chart axis ticks і декоративних метаданих (timestamps, badge counts), ніколи на primary content / button labels. Повний контракт — `AGENTS.md` § Hard Rule #16.
17. **Animation budget — max 2 concurrent, 3 tiers** — кожна анімація належить рівно до одного tier-а: **AMBIENT** (`shimmer`, `pulse-soft`, `wiggle` — looped, завжди за `motion-safe:`), **RESPONSE** (`fade-in`, `slide-up`, `scale-in`, `press-scale`, `hover-lift` — one-shot, 150–300 ms, `ease-out`), **CELEBRATE** (`check-pop`, `bounce-in`, `success-pulse`, confetti — лише на milestones: streak 7/30/100/365, weekly goal hit; **не** на кожному checkbox-tick). Одночасно на екрані: max **1 AMBIENT + 1 RESPONSE**; stagger-група = 1 RESPONSE (max 30 ms між дітьми, total cap ≤ 150 ms — `Math.min(index * 30, 150)`). Не загортай у `StaggerChild` компонент, що вже має внутрішню entry-анімацію (double-animation). `showConfetti` на `AnimatedCheckbox` / `HabitCheckbox` — лише на streak milestones. Повна таблиця прикладів — `AGENTS.md` § Hard Rule #17.
