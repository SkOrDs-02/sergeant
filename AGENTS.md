...
No harness stores tool config in the checkout — no per-harness agent files, permissions, or MCP wiring. (The one `.kilo/` entry, [`.kilo/harness-versions.json`](.kilo/harness-versions.json), is harness-neutral version metadata — see [§ Harness version](#harness-version) — not Kilo config.) Every harness is an **equal peer**: it reads `AGENTS.md` + `.agents/skills/` from the repo for shared policy, then keeps its own models, permissions, MCP wiring, custom agents and commands in its own global config home. **None of them is "the" driver of this repo.**
...
| `apps/web` LCP (median, 4 LHCI routes) | **`error` at > 3000 ms** (enforced hard-gate у `lighthouserc.json`; 2000 ms — початковий warn-таргет, тепер superseded) | `apps/web/lighthouserc.json` + `.github/workflows/lighthouse-ci.yml` (status `Lighthouse CI`); local: `pnpm --filter @sergeant/web lighthouse`                                                                                |
