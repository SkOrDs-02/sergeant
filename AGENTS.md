...
**Ратчет 2026-08-05 (eager 450 → 470 kB).** Заміри цього дня: `origin/main` — **467.7 kB** у 111 preload-чанках, гілка pre-beta-аудиту — 464.6 kB у 109. Тобто 450 kB було пробито ще до PR #627: між 2026-08-02 і 2026-08-05 main виріс на ~38 kB, і гейт падав на кожному PR незалежно від змісту — той самий стан «червоний завжди = вимкнений», яким обґрунтовано ратчет нижче. Нове число має ~0.5% запасу над фактом main. Ратчет **назад до ≤450 kB** — окремий борг у [`frontend.md`](./docs/90-work/tech-debt/frontend.md): винести `vendor-sqlite` (~69 kB) з критичного шляху; він там через статичний `@sergeant/db-schema/sqlite` у `apps/web/src/core/db/kvStoreBoot.ts`, тож потрібен async-boot гейт, а не проста заміна на `import()`.
