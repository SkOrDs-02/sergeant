...
>
> Поточний розподіл (17 rules): 8 `blocker-invariant`, 9 `lint-enforced-convention`, 0 `active-initiative`. Правила #8, #9, #11–#14, #16, #17 та #24 retired рішенням [ADR-0081](./docs/governance/adr/0081-repository-simplification.md): візуальні конвенції лишаються у design tokens/Storybook/review, а committed agent-каталоги прибрані. Машино-читабельна матриця: [`docs/governance/governance/hard-rules-matrix.md`](./docs/governance/governance/hard-rules-matrix.md). Семантика категорій — у [`docs/governance/adr/0045-hard-rules-taxonomy.md`](./docs/governance/adr/0045-hard-rules-taxonomy.md). Per-rule canonical bodies: [`docs/governance/governance/rules/`](./docs/governance/governance/rules/). 3-way sync gate (AGENTS.md ↔ JSON ↔ per-rule files): `pnpm lint:hard-rules-registry`.
...
**Винесення зроблено того ж дня (PR #1105) — і дало три знахідки, яких із `check` не було видно.**

Обидва бандл-гейти живуть тепер у власній джобі `bundle-budgets` із власним прод-білдом; форму конструкції стереже парсерний тест [`ci-bundle-budget-gates.test.mjs`](./scripts/__tests__/ci-bundle-budget-gates.test.mjs) поруч із наявним `ci-dedupe-gate`. Ціна — ~5 хв дубльованого білда на прогін; плата — число на КОЖНОМУ прогоні незалежно від стану тестів і лінту.

**Знахідка перша: гейти ховали ще й один одного.** `size-limit` падав — і eager, що стояв кроком НИЖЧЕ, ставав `skipped`. Тобто критичний шлях не міряли **жодного разу** за весь час мовчання, і в жодному логові це не виглядало як проблема. Тепер у eager `always()`, а тест вимагає його наявності окремо.

**Знахідка друга, і вона тривожніша за стелю: eager пробито.** На `dee22fa2` замір дав 277.8 kB при ліміті 280 — запасу 0.8%, і тут же було записано «наступна фіча на критичному шляху зупинить PR-и». Наступна фіча приїхала того ж вечора: після злиття #1098 і #1103 eager став **286.0 kB, тобто +6.0 kB понад ліміт**. Саме eager корелює з відчутною швидкістю завантаження (`size-limit` сумує всі 350 чанків, включно з тими, які більшість людей ніколи не завантажить, тож lazy-split його не рухає, а eager — рухає).

**Діагноз є, і він не там, де здавався.** Розбір сорсмапи чанка `cn` (24.4 kB, 14 модулів): **23 із 24 kB — це UA-каталог інтерфейсу**, весь одразу. `uk.ts` 7.5, `uk.fizruk.ts` 5.1, `uk.finyk.ts` 2.5, `uk.nutrition.ts` 2.2, далі routine, pricing, privacy, dataExport, nutritionTdee. Сам хелпер `cn.ts`, за яким названо чанк, важить 0.2 kB. Тобто рядки ВСІХ модулів їдуть у першому екрані, хоча самі модулі ліниві. Розділення каталогу по модулях звільняє ~12 kB — удвічі більше за перевищення. **Не лікуй це підняттям ліміту:** eager двічі ратчетнуто ВНИЗ (470 → 430 → 280), і кожен раз важіль знаходився саме так — заміром, а не здогадом. Борг із цим розбором — у [`tech-debt/frontend.md`](./docs/work/specs/tech-debt/frontend.md).

**Знахідка третя: розбіжність CI ↔ локально НЕСТАБІЛЬНА — то є, то немає.** Запис нижче («Чому ратчет униз відкладено») фіксує 2.3% розходження 2026-09-02: CI 1376.6 проти локальних 1408.8. Три заміри 2026-09-11 дали таке:

| Дерево                          | CI          | Локально (Linux-контейнер) | Розбіжність |
| ------------------------------- | ----------- | -------------------------- | ----------- |
| `a4fb7ca6` (2026-09-02)         | 1376.6      | **1409.1**                 | 32.5 kB     |
| `dee22fa2`                      | **1429.75** | **1429.4**                 | 0.35 kB     |
| `fb5a1a2f` (після #1098, #1103) | **1399.7**  | **1433.0**                 | 33.3 kB     |

Тобто на одному дереві числа зійшлися впритул, а на сусідньому розійшлись на ті самі ~2.3%. **Одноразовий збіг на `dee22fa2` не був поясненням** — це видно лише тому, що замір повторили втретє. Перевірено очевидного підозрюваного: коміт `b3c7287c` («не давати заміру тихо недорахувати») стосується скрипта підрахунку UA-рядків, не бандла.

Практичний наслідок незмінний і навіть твердіший: **ратчет униз лишається відкладеним**, бо ліміт, зелений у CI, може бути червоним локально на тому ж коміті — а `pnpm --filter @sergeant/web size` документований тут як pre-PR перевірка. Поки причина невідома, стелю тримає ЛОКАЛЬНЕ (більше) число: воно консервативніше і не робить локальну перевірку завжди червоною.

**І ще один замір того ж дня, уже на дереві після #1098 і #1103:** `dee22fa2` проти `a4fb7ca6` (2026-09-02) по чанках — **1409.1 → 1429.4 kB, +20.3 kB, 347 → 352 чанки**. Приріст розмазаний по продуктовій роботі: `PlanningSubscriptions` +5.2 (новий), `useHubDashboardState` +7.6 (увібрав `WeeklyDigestCard` −8.7), `Workouts` +3.8, `NutritionApp` +3.3, `ProfilePage` +2.3; `Assets` −9.6 / `AssetsForm` +8.8 — просто розділився. Це підтверджує діагноз вище з іншого боку: жодного важкого новачка, тільки накопичення.
...
Нове число має ~2.7% запасу над фактом — навмисно тісно, бо 19 kB слаку це запрошення до «комфортної зони». Запас саме такий, бо різниця CI↔локально для ЦЬОГО заміру мала: на `main` CI дав 286.9, локально 286.7 (для `size-limit` розбіжність інша — див. запис 2026-09-11 вище).
...
**Ратчет 2026-08-05 (eager 450 → 470 kB).** Заміри цього дня: `origin/main` — **467.7 kB** у 111 preload-чанках, гілка pre-beta-аудиту — 464.6 kB у 109. Тобто 450 kB було пробито ще до PR #627: між 2026-08-02 і 2026-08-05 main виріс на ~38 kB, і гейт падав на кожному PR незалежно від змісту — той самий стан «червоний завжди = вимкнений», яким обґрунтовано ратчет нижче. Нове число має ~0.5% запасу над фактом main. Ратчет **назад до ≤450 kB** — окремий борг у [`frontend.md`](./docs/work/specs/tech-debt/frontend.md): винести `vendor-sqlite` (~69 kB) з критичного шляху. **Тут двічі стояв хибний діагноз.** Спершу «через `kvStoreBoot.ts`, потрібен async-boot гейт», потім «вагу дають табличні визначення `@sergeant/db-schema/sqlite`». Замір сорсмапи 2026-08-07: у чанку рівно 53 модулі — 52 з `drizzle-orm` і один tree-shaken стаб `sqlite-wasm`, модулів `db-schema` там **нуль** (усі `sqliteTable()`-визначення важать ~6.5 kB і лежать окремо). Вагу дає рантайм drizzle; табличні визначення — лише **міст**: вони імпортують `drizzle-orm/sqlite-core`, а `manualChunks` склеює весь `drizzle-orm` в один чанк, тож ОДИН eager-імпорт із барелю робить eager усі 69 kB. Спроба лікувати це реєстром і `import()` дала −2.2 kB у мінус і відкочена. Актуальний розбір і два робочі напрямки — у [`frontend.md`](./docs/work/specs/tech-debt/frontend.md).

**Повернення 1.43 → 1.39 MB (2026-09-01) — без ратчету, і це головне.** Гейт був червоний (перевищення 6.7 kB), і спокуса була підняти стелю. Причина виявилась іншою: у прод-бандлі лежав чанк `DesignShowcase` на 192 kB сирих, хоча код навмисно його звідти прибирає. `StandaloneRoutes.tsx` гейтує сторінку через `import.meta.env.DEV`, але `useRoutePrefetch.ts` тягнув той самий модуль **безумовним** `import()` у мапі префетчу. Одного такого ребра досить, щоб гейт в іншому файлі нічого не означав. Після гейтування мапи префетчу чанк зник, і факт упав з 1.43 до **1.39 MB** при незмінній стелі 1.42.

Урок ширший за цей випадок і вже третій за формою (див. `posthog-js` і `vendor-sqlite` нижче): **гейт лінивості діє лише доти, доки жоден інший модуль не імпортує ту саму ціль безумовно.** Перед тим як піднімати стелю, треба переконатись, що перевищення не є сміттям, яке код і так вважає виключеним: `ls apps/server/dist/assets/` проти очікуваного набору чанків дешевший за ратчет, а ратчет незворотний на практиці.
...
Нове число дає ~4% запасу над фактом — достатньо, щоб звичайна продуктова робота не червонила гейт щотижня, і мало, щоб новий важкий vendor усе одно розбудив. **Це не дозвіл рости далі:** перший кандидат на скорочення — `DesignShowcase` (196 kB сирих) — внутрішня демо-сторінка дизайн-системи, яку продакшн-користувач не відкриває ніколи; винести її з прод-білда і повернути ліміт униз — окремий борг. **Половину цього боргу закрито 2026-09-01** (див. запис «Повернення 1.43 → 1.39 MB» нижче): сторінки в прод-бандлі більше немає. Ліміт лишався 1.42 MB до 2026-09-11 — чому саме, і чому ратчет відкладено попри вільний запас, розібрано там же; чинне число тепер 1.44 MB (див. ратчет 2026-09-11 вище).

**Чому ратчет униз відкладено (замір 2026-09-02).** Після виносу `DesignShowcase` запас над стелею зʼявився, і ратчет 1.42 → 1.40 виглядав очевидним. Заміри його зупинили: **та сама голова дає різні числа локально й у CI**.

| Метрика | CI (`Bundle size guard`) | Локально (`pnpm --filter @sergeant/web exec size-limit`) |
| ------- | ------------------------ | -------------------------------------------------------- |
| JS      | 1376.6 kB                | **1408.8 kB**                                            |
| CSS     | 36.8 kB                  | **37.69 kB**                                             |

Розбіжність ~2.3% на ОБОХ метриках, тобто це не вміст бандла, а середовище (версія brotli/Node), — вміст дав би розбіжність в одній метриці, не в обох однаково. Перевірено, що це не сміття від старих збірок: усі 378 чанків з одного прогону, і `pnpm build` (як у CI) дає той самий локальний результат, що й `--filter @sergeant/web build`.

Наслідок практичний: ліміт 1.40 MB був би зеленим у CI (запас 23 kB) і **червоним у кожного локально** (1408.8 > 1400). А `pnpm --filter @sergeant/web size` документований тут як pre-PR перевірка — зробити її завжди червоною означає повторити стан «червоний завжди = вимкнений», яким обґрунтовано два ратчети нижче. Тому спершу треба зрозуміти, звідки 2.3%, і лише потім рухати стелю.
...
`pnpm format:check && pnpm lint && pnpm check:typecheck-and-test && pnpm build` (= `pnpm check`; `check:typecheck-and-test` = `turbo run typecheck test --concurrency=2`, який запускає обидва pipelines паралельно без перепідписування вкладених test worker-ів). When changing UI: attach a screenshot. When shipping a heavy import: `pnpm --filter @sergeant/web size` (blocking). Full CI matrix + non-blocking workflows: [`docs/governance/governance/release-policy.md`](./docs/governance/governance/release-policy.md), `.github/workflows/`. Markdown link checker (`docs-automation.yml`) runs `--strict-external` against [`docs/governance/governance/external-link-allowlist.json`](./docs/governance/governance/external-link-allowlist.json).
