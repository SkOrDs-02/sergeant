...
**Знахідка друга, і вона тривожніша за стелю: eager — 277.8 kB при ліміті 280**, запасу 0.8%. Саме eager корелює з відчутною швидкістю завантаження (`size-limit` сумує всі 352 чанки, включно з тими, які більшість людей ніколи не завантажить, тож lazy-split його не рухає, а eager — рухає). Наступна фіча на критичному шляху зупинить PR-и. Борг заведено окремо в [`tech-debt/frontend.md`](./docs/work/specs/tech-debt/frontend.md) з першим підозрюваним — чанк `cn` на 24.2 kB, тобто спільний чанк, що щось у себе увібрав.

**Знахідка третя: розбіжність CI ↔ локально не відтворилась.** Запис нижче («Чому ратчет униз відкладено») фіксує 2.3% розходження на обох метриках 2026-09-02: CI 1376.6 проти локальних 1408.8. Білд того самого дерева (`a4fb7ca6`) 2026-09-11 дав **1409.1 kB** — локальна гілка таблиці відтворюється точно, а CI-гілка ні: того ж дня CI дав 1429.75 проти локальних 1429.4, тобто розбіжність 0.35 kB. Перевірено очевидного підозрюваного — коміт `b3c7287c` («не давати заміру тихо недорахувати») стосується скрипта підрахунку UA-рядків, не бандла. **Причина лишається невідомою, але станом на 2026-09-11 розбіжності немає**, тож підстава відкладати ратчет униз («спершу зрозуміти, звідки 2.3%») більше не спирається на свіжий замір. Перш ніж рухати стелю вниз — переміряти обидві сторони ще раз: один збіг не тренд.
