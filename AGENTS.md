...
| ------------------------------------------------ | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/web` JS total (brotli)                     | **≤ 1.45 MB**                       | `pnpm --filter @sergeant/web exec size-limit` (CI job `bundle-budgets`)                                                                                                                                                 |
...
**Ратчет 2026-09-11 (JS 1.42 → 1.45 MB) — і кінець мовчання гейта, яке тривало тиждень.** Гейт не міг спрацювати фізично: крок `size-limit` стоїть після «Format, lint, test, build», а той падав на трьох застарілих тестах. Полагодили тести (#1102) — і першим же прогоном гейт дав **1429.75 kB** проти стелі 1420, тобто пробито на 9.75 kB. **Це не регрес того PR, а перший чесний замір після тижня тиші** — рівно той самий сценарій, що описаний у ратчеті 2026-08-18 нижче, і вже вдруге. Тому цим же PR `size-limit` і eager винесені в окрему джобу `bundle-budgets`: доки вони кроки всередині `check`, кожен червоний крок вище вимикає їх мовчки.

**Чому підняли, а не зрізали.** Спершу перевірили те, чого не перевірили 2026-08-19: чи не є перевищення сміттям, яке код і так вважає виключеним (урок `DesignShowcase`). Зібрали прод-білд на поточному `main` і на `a4fb7ca6` (2026-09-02) і звели brotli по кожному чанку: **1409.1 → 1429.4 kB, +20.3 kB, 347 → 352 чанки**. Приріст розмазаний по продуктовій роботі — `PlanningSubscriptions` +5.2 (новий), `useHubDashboardState` +7.6 (увібрав `WeeklyDigestCard` −8.7), `Workouts` +3.8, `NutritionApp` +3.3, `ProfilePage` +2.3; `Assets` −9.6 / `AssetsForm` +8.8 — просто розділився. Аналога `DesignShowcase` немає: найважчі чанки (`vendor-sentry` 98.4, `vendor-zxing` 84.7, `vendor-sqlite` 70.6, `vendor-posthog` 61.6 kB brotli) навмисні й уже ізольовані. Нове число дає ~20 kB (1.4%) запасу — навмисно тісніше за 4% ратчету 2026-08-19, бо тут накопичення, а не новий vendor.

**Головне з цього заміру — не стеля, а два факти під нею.** Перший: на 2026-09-02 факт був 1409.1 при стелі 1420, тобто запасу лишалось 11 kB, а не 43, як показувало записане CI-число 1376.6. Стеля була фактично пробита звичайною продуктовою роботою ще тоді. Другий і важливіший: **eager — 277.8 kB при ліміті 280**, запасу 2.2 kB. Саме eager стереже відчутну швидкість завантаження, і він за крок від червоного — наступна фіча на критичному шляху зупинить PR-и, тож шукати важіль (за зразком `posthog-js` / `vendor-sqlite`) треба вже зараз, не після падіння.

**Розбіжність CI ↔ локально не відтворилась.** Запис нижче («Чому ратчет униз відкладено») фіксує 2.3% розходження на обох метриках 2026-09-02: CI 1376.6 проти локальних 1408.8. Білд того самого дерева (`a4fb7ca6`) 2026-09-11 дав **1409.1 kB** — локальна гілка таблиці відтворюється точно. А сьогоднішні числа зійшлись: CI 1429.75 проти локальних 1429.4, розбіжність 0.35 kB. Перевірено очевидного підозрюваного — коміт `b3c7287c` («не давати заміру тихо недорахувати») стосується скрипта підрахунку UA-рядків, не бандла. **Причина розбіжності лишається невідомою, але станом на 2026-09-11 її немає**, тож підстава відкладати ратчет униз («спершу зрозуміти, звідки 2.3%») більше не спирається на свіжий замір. Перш ніж рухати стелю вниз — переміряти обидві сторони ще раз, бо один збіг ще не тренд.
