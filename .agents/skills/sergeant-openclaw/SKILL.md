...
OpenClaw Gateway — зовнішній Telegram-шлюз (ADR-0055). Він **не є** internal bot на Grammy. Це окремий service із власним Dockerfile, пакетом-адаптером і config-as-code. ⚠️ Historically hosted on Railway; після декомісії Railway (ADR-0074, config-файли видалено 2026-07-19) Gateway не задеплоєний — міграція на Coolify або deprecation TBD.
...
| Service (ex-Railway, не задеплоєний) | `sergeant-openclaw-gateway` | Node 24-alpine, `Dockerfile.openclaw-gateway` |
...
3. Push. ⚠️ Auto-redeploy наразі відсутній — Railway декомісовано; після відновлення хостингу (Coolify TBD) верифікуй healthcheck Gateway + Telegram `@OpenClaw_sergeant_v2_bot`.
